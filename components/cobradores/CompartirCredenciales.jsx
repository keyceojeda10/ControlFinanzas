...
export default function CompartirCredenciales({ nombreCobrador, email, password, nombreOwner, telefono }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-[10px] text-[var(--color-text-muted)] text-left">Enviar credenciales al cobrador</p>

      {/* WhatsApp (primario - es lo que la mayoria usa) */}
      <button
        onClick={handleWhatsApp}
        className="w-full h-11 rounded-[12px] bg-[#25D366] hover:bg-[#1eb855] text-[var(--color-text-primary)] text-sm font-semibold transition-colors flex items-center justify-center gap-2"
      >
      ...
      <div className="flex gap-2">
        {hasNativeShare && (
          <button
            ...
            className="flex-1 h-11 rounded-[12px] bg-[#1f1f1f] hover:bg-[var(--color-bg-hover)] border border-[var(--color-border)] text-[var(--color-text-primary)] text-sm font-medium transition-colors flex items-center justify-center gap-1.5"
          >
        ...
        <button
          ...
          className={`${hasNativeShare ? 'flex-1' : 'w-full'} h-11 rounded-[12px] bg-[#1f1f1f] hover:bg-[var(--color-bg-hover)] border border-[var(--color-border)] text-[var(--color-text-primary)] text-sm font-medium transition-colors flex items-center justify-center gap-1.5`}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
