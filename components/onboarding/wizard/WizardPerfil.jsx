      ...
      <div className="max-w-md mx-auto">
        <div className="text-center mb-6">
          <div className="w-14 h-14 rounded-[14px] flex items-center justify-center mx-auto mb-4"
            style={{ background: 'rgba(245,158,11,0.12)', color: 'var(--cf-gold-dark)' }}>
          ...
          <p className="text-[13px] max-w-[300px] mx-auto leading-relaxed" style={{ color: 'var(--cf-ink-3)' }}>
            Estas en el plan <strong style={{ color: 'var(--cf-ink)' }}>{config?.nombre ?? plan}</strong> que
          ...
          {PLANES_EQUIPO.map((key) => {
            ...
            return (
              <button
                ...
                style={{
                  background: key === 'growth' ? 'rgba(139,92,246,0.06)' : 'var(--cf-card)',
                  border: `1px solid ${key === 'growth' ? 'rgba(139,92,246,0.25)' : 'var(--cf-border)'}`,
                }}
              >
                <div className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-[15px] font-bold" style={{ color: 'var(--cf-ink)' }}>
                        {p.nombre}
                      </p>
                      {key === 'growth' && (
                        <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold"
                          style={{ background: 'rgba(139,92,246,0.15)', color: 'var(--cf-ink-2)' }}>
                          Recomendado
                        </span>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-[11px]" style={{ color: 'var(--cf-ink-3)' }}>
                      <span>{p.maxUsuarios} usuarios</span>
                      <span>{p.maxRutas} rutas</span>
                      <span>{p.maxClientes.toLocaleString()} clientes</span>
                    </div>
                    <p className="text-[13px] font-bold mt-1 font-mono-display" style={{ color: key === 'growth' ? 'var(--cf-ink-2)' : 'var(--cf-ink-2)' }}>
                      {formatMoney(p.precio)}<span className="text-[10px] font-normal" style={{ color: 'var(--cf-ink-3)' }}>/mes</span>
                    </p>
                  </div>
                  <svg className="w-5 h-5 shrink-0 transition-transform group-hover:translate-x-0.5" fill="none" stroke={key === 'growth' ? 'var(--cf-ink-2)' : 'var(--cf-ink-3)'} viewBox="0 0 24 24">
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="text-center mb-7">
        <div className="w-14 h-14 rounded-[14px] flex items-center justify-center mx-auto mb-4"
          style={{ background: 'rgba(245,197,24,0.12)', color: 'var(--cf-gold)' }}>
          <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.6}
        ...
        <p className="text-[13px] max-w-[280px] mx-auto leading-relaxed" style={{ color: 'var(--cf-ink-3)' }}>
          Esto nos ayuda a personalizar los pasos iniciales para ti.
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[16px] font-bold mb-0.5" style={{ color: 'var(--cf-ink)' }}>
                Cobro solo
              </p>
            ...
            <svg className="w-5 h-5 shrink-0 transition-transform group-hover:translate-x-0.5" fill="none" stroke="var(--cf-gold)" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          </div>
        </button>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <svg className="w-5 h-5 shrink-0 transition-transform group-hover:translate-x-0.5" fill="none" stroke="var(--cf-ink-2)" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          </div>
        </button>
