    ...
    <div className="w-full bg-gradient-to-r from-[rgba(245,197,24,0.12)] to-[rgba(245,197,24,0.04)] border border-[rgba(245,197,24,0.2)] rounded-[12px] px-4 py-3 mb-4">
      ...
      <div className="flex items-center gap-1.5 mb-2">
        {[1, 2, 3, 4].map(i => (
          <div
            ...
            style={{
              background: i <= progreso ? 'var(--cf-gold)' : 'rgba(245,197,24,0.15)',
            }}
          />
        ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-[var(--cf-gold)] mb-0.5">{actual.titulo}</p>
          <p className="text-xs text-[var(--cf-ink-3)] leading-relaxed">{actual.descripcion}</p>
        </div>

        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={handleOmitir}
            className="text-[10px] text-[var(--cf-ink-3)] hover:text-[var(--cf-ink-3)] transition-colors whitespace-nowrap"
          >
            Omitir guía
          </button>
          {actual.accion && (
