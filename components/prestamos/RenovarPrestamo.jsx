      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl p-3" style={{ background: 'rgba(245,197,24,0.06)', border: '1px solid rgba(245,197,24,0.2)' }}>
          <div className="flex items-center gap-2 mb-2">
            <svg className="w-4 h-4 text-[var(--cf-gold)]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
            </svg>
            <span className="text-xs font-medium" style={{ color: 'var(--cf-ink-3)' }}>Préstamo actual</span>
          </div>
          {clienteNombre && <p className="text-[11px] mb-1.5" style={{ color: 'var(--cf-ink-3)' }}>{clienteNombre}</p>}
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            <div className="flex items-center justify-between">
              <span className="text-[11px]" style={{ color: 'var(--cf-ink-3)' }}>Presto</span>
              <span className="text-sm font-semibold font-mono-display" style={{ color: 'var(--cf-ink-2)' }}>{formatMoney(montoAnterior)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[11px]" style={{ color: 'var(--cf-ink-3)' }}>Cuota</span>
              <span className="text-sm font-semibold font-mono-display" style={{ color: 'var(--cf-ink-2)' }}>{formatMoney(cuotaAnterior)}</span>
            </div>
            <div className="flex items-center justify-between col-span-2">
              <span className="text-[11px] font-medium" style={{ color: 'var(--cf-gold)' }}>{modoUsaTabla ? 'Capital adeudado' : 'Saldo pendiente'}</span>
              <span className="text-lg font-bold font-mono-display" style={{ color: 'var(--cf-gold)' }}>{formatMoney(saldo)}</span>
            </div>
          </div>
        </div>
        ...
        {/* Input principal */}
        <div>
          <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--cf-ink)' }}>
            Total del nuevo prestamo
          </label>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium" style={{ color: 'var(--cf-ink-3)' }}>$</span>
            <input
              type="number"
              inputMode="numeric"
        ...
        {montoNum > 0 && (
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--cf-border)' }}>
            <div className="px-3 py-2 flex items-center justify-between" style={{ background: 'var(--cf-surface)' }}>
              <div className="flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5" style={{ color: 'var(--cf-ink-3)' }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
                <span className="text-xs font-medium" style={{ color: 'var(--cf-ink-3)' }}>Saldo absorbido</span>
              </div>
              <span className="text-sm font-semibold font-mono-display" style={{ color: 'var(--cf-ink-3)' }}>
                {formatMoney(Math.min(saldo, montoNum))}
              </span>
            </div>
            <div className="px-3 py-2.5 flex items-center justify-between" style={{ borderTop: '1px solid var(--cf-border)' }}>
              <div className="flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5" style={{ color: 'var(--cf-green-dark)' }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                </svg>
                <span className="text-xs font-semibold" style={{ color: 'var(--cf-ink)' }}>Le entregas en mano</span>
              </div>
              <span className="text-lg font-bold font-mono-display" style={{ color: 'var(--cf-green-dark)' }}>
                {formatMoney(enMano)}
              </span>
            </div>
          </div>
