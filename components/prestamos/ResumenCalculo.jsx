...
export default function ResumenCalculo({ calculo, visible = true }) {
  ...
  const items = [
    ...
    { label: labelCuota,       value: fmt(cuotaDiaria),  accent: 'var(--cf-green-dark)',
      sub: tieneUltimaCuotaDiferente ? `Última cuota: ${fmt(ultimaCuota)}` : null },
    { label: 'Total interés',  value: fmt(totalInteres), accent: 'var(--cf-gold-dark)' },
    { label: 'Fecha de cierre', value: fmtFecha(fechaFin), accent: 'var(--cf-ink-2)' },
  ]

  return (
    <div className="border border-[var(--cf-border)] rounded-[12px] p-4 space-y-3" style={{ background: 'var(--cf-card)' }}>
      ...
      <div className="grid grid-cols-2 gap-3">
        {items.map(({ label, value, accent, sub }) => (
          <div
            key={label}
            className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[10px] px-3 py-2.5"
          >
            <p className="text-[10px] mb-0.5" style={{ color: 'var(--cf-ink-3)' }}>{label}</p>
            <p
              className="text-base font-bold leading-tight font-mono-display"
              style={{ color: value === '$0' || value === '—' ? 'var(--cf-ink-3)' : accent }}
            >
              {value ?? '—'}
            </p>
            {sub && <p className="text-[10px] mt-0.5" style={{ color: 'var(--cf-ink-3)' }}>{sub}</p>}
          </div>
        ))}
      </div>
