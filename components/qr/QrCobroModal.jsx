      ...
      <Modal open={open} onClose={handleClose} title="Cargando..." size="sm">
        <div className="flex flex-col items-center gap-4 py-8">
          <div
            ...
            style={{ borderColor: 'var(--cf-gold)', borderTopColor: 'transparent' }}
          />
      ...
      <Modal open={open} onClose={handleClose} title="Error" size="sm">
        <div className="flex flex-col items-center gap-4 py-6">
          <div className="w-14 h-14 rounded-full flex items-center justify-center" style={{ background: 'rgba(239,68,68,0.1)' }}>
            <svg className="w-7 h-7" style={{ color: 'var(--cf-red-dark)' }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            {cliente.cedula && (
              <div className="flex items-center gap-2 text-sm">
                <svg className="w-4 h-4 shrink-0" style={{ color: 'var(--cf-ink-4)' }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                ...
                <span style={{ color: 'var(--cf-ink-2)' }}>{cliente.cedula}</span>
              </div>
            )}
            {cliente.telefono && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <a href={`tel:${cliente.telefono}`} style={{ color: 'var(--cf-ink-2)' }}>{cliente.telefono}</a>
              </div>
            )}
            {cliente.direccion && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span style={{ color: 'var(--cf-ink-2)' }}>{cliente.direccion}</span>
              </div>
            )}
            {cliente.ruta?.nombre && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span style={{ color: 'var(--cf-ink-2)' }}>Ruta: {cliente.ruta.nombre}</span>
              </div>
            )}
          </div>
          ...
          {activos.length === 0 ? (
            ...
            <div className="flex flex-col gap-2.5">
              <p className="text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--cf-ink-4)' }}>
              ...
              {activos.map(p => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: mora > 0 ? 'rgba(239,68,68,0.06)' : 'var(--cf-surface)',
                      border: `1px solid ${mora > 0 ? 'rgba(239,68,68,0.2)' : 'var(--cf-border)'}`,
                    }}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span className="text-base font-semibold font-mono-display" style={{ color: 'var(--cf-ink)' }}>
                          {formatMoney(cuota)}
                        </span>
                        <span className="text-xs" style={{ color: 'var(--cf-ink-4)' }}>/ cuota</span>
                      </div>
                      {mora > 0 && (
                        <span
                          className="text-xs font-medium px-2 py-0.5 rounded-full"
                          style={{ background: 'rgba(239,68,68,0.1)', color: 'var(--cf-red-dark)' }}
                        >
                          {mora}d mora
                        </span>
                      )}
                    ...
                    <div className="flex items-center justify-between text-xs mb-2" style={{ color: 'var(--cf-ink-2)' }}>
                      <span>Saldo: {formatMoney(saldo)}</span>
                      <span>Prestado: {formatMoney(p.montoPrestado)}</span>
                    </div>

                    {/* Barra de progreso */}
                    <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--cf-border)' }}>
                      <div
                        className="h-full rounded-full transition-all"
                        style={{
                          width: `${Math.min(100, pct)}%`,
                          background: mora > 0 ? 'var(--cf-red-dark)' : 'var(--cf-gold)',
                        }}
                      />
                    </div>
                    <div className="flex items-center justify-between mt-1.5">
                      <span className="text-xs" style={{ color: 'var(--cf-ink-4)' }}>{pct}% pagado</span>
                      <span
