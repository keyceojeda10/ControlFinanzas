...
const COLOR_MAP = {
  success: 'var(--cf-green-dark)',
  warning: 'var(--cf-gold-dark)',
  danger:  'var(--cf-red-dark)',
  info:    'var(--cf-ink-2)',
}
...
export default function AccionCard({ tool, input, displayData, onConfirm, onCancel }) {
    ...
    return (
      <div className="rounded-[12px] overflow-hidden my-1"
        ...
        <div className="px-4 pt-3 pb-2">
          <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--cf-ink-3)' }}>
            Opciones de soporte
          </p>
        </div>
  ...
  const accentColor = COLOR_MAP[displayData?.color] || 'var(--cf-ink-2)'
  ...
  return (
    <div className="rounded-[12px] overflow-hidden my-1"
      style={{
        ...
        border: `1px solid color-mix(in srgb, ${accentColor} 25%, var(--cf-border))`,
      }}>
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-b" style={{ borderColor: 'var(--cf-border)' }}>
        ...
        <div className="px-4 py-3 space-y-1.5">
          {fields.map((f, i) => (
            <div key={i} className="flex justify-between items-start gap-3">
              <span className="text-xs shrink-0" style={{ color: 'var(--cf-ink-3)' }}>{f.label}</span>
              <span className="text-xs font-medium text-right" style={{ color: 'var(--cf-ink)' }}>{f.value}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {status === 'done' && !escalationData && (
        <div className="px-4 py-3 space-y-2.5">
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20" style={{ color: 'var(--cf-green-dark)' }}>
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
            <p className="text-sm" style={{ color: 'var(--cf-green-dark)' }}>{resultMsg}</p>
          </div>
          {waUrl && (
            <a
              href={waUrl}
              ...
              className="flex items-center justify-center gap-2 w-full h-9 rounded-[10px] text-sm font-semibold transition-all active:scale-95"
      ...
      {status === 'error' && (
        <div className="px-4 py-3 space-y-2">
          <div className="flex items-start gap-2">
            <svg className="w-4 h-4 shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20" style={{ color: 'var(--cf-red-dark)' }}>
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
            </svg>
            <p className="text-xs" style={{ color: 'var(--cf-red-dark)' }}>{resultMsg}</p>
          </div>
          <button onClick={() => setStatus('pending')} className="text-xs underline" style={{ color: 'var(--cf-ink-3)' }}>
            Intentar de nuevo
          </button>
        </div>
      )}
      ...
      {(status === 'pending' || status === 'loading') && (
        <div className="flex gap-2 px-3 pb-3">
          ...
          <button
            ...
            style={{ background: accentColor, color: 'var(--cf-ink)' }}
          >
            {status === 'loading' ? (
              <>
    ...
    <>
      {escalationData.whatsappUrl && (
        <a
          ...
          style={{ background: 'color-mix(in srgb, var(--cf-green-dark) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--cf-green-dark) 30%, transparent)', color: 'var(--cf-green-dark)' }}
        >
