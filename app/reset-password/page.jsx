...
function ResetForm() {
  ...
  if (!token) {
    return (
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-[24px] p-8 text-center">
        <p className="text-[#ef4444] text-sm mb-4">Enlace inválido. Solicita un nuevo enlace de recuperación.</p>
        <Link href="/forgot-password" className="text-sm text-[#f5c518] hover:underline font-medium">
  ...
  if (exito) {
    return (
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-[24px] p-8 text-center">
        ...
        <p className="text-sm text-[#888888] mb-6">Tu contraseña ha sido restablecida exitosamente.</p>
        <Link
          href="/login"
          className="inline-flex items-center justify-center h-11 px-8 rounded-[12px] bg-[#f5c518] hover:bg-[#f0b800] text-[#0a0a0a] font-bold text-sm transition-all"
        >
          Ir al login
        </Link>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-[24px] p-8 space-y-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="confirm" className="text-[11px] font-medium text-[#888888] uppercase tracking-[0.05em]">
    ...
    <div className="min-h-dvh flex flex-col items-center justify-center bg-[#0a0a0a] px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#888888] mt-1">Ingresa tu nueva contraseña</p>
        </div>

        <Suspense fallback={<div className="h-48 bg-[#1a1a1a] rounded-[24px] animate-pulse" />}>
          <ResetForm />
        </Suspense>
      </div>
    </div>
