// app/api/pagos/ruta-extra/route.js — Crear preferencia para ruta extra
...
import { getPrecioRutaExtra } from '@/lib/planes'
...
export async function POST(req) {
  ...
  if (!planInfo || planInfo.rutaExtra <= 0) {
    return NextResponse.json({ error: 'Tu plan no permite agregar rutas extra' }, { status: 403 })
  }
  ...
  const precio = getPrecioRutaExtra(country)
  ...
  const preference = await preferenceApi.create({
    body: {
      items: [
        {
          id:          `ruta-extra-${orgId}`,
          title:       'Control Finanzas - Ruta adicional (1 mes)',
          unit_price:  precio,
          quantity:    1,
