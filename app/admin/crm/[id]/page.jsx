...
function InfoRow({ label, children }) {
  return (
    <div className="flex items-start justify-between gap-2 py-2 border-b border-[var(--cf-border)] last:border-0">
      <span className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide shrink-0 mt-0.5">{label}</span>
...
function SkeletonDetail() {
  return (
    <div className="space-y-4 animate-pulse">
      <div className="h-6 w-48 bg-[var(--cf-surface)] rounded" />
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-1 space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-32 bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px]" />
          ))}
        </div>
        <div className="lg:col-span-2">
          <div className="h-96 bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px]" />
        </div>
      </div>
    </div>
  )
...
export default function CRMDetailPage({ params }) {
  ...
  if (error) return (
    <div className="max-w-4xl mx-auto">
      <Link href="/admin/crm" className="inline-flex items-center gap-1.5 text-sm text-[var(--cf-ink-3)] hover:text-[var(--cf-ink)] mb-4 transition-colors">
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      {/* Back */}
      <Link href="/admin/crm" className="inline-flex items-center gap-1.5 text-sm text-[var(--cf-ink-3)] hover:text-[var(--cf-ink)] transition-colors">
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
        </svg>
        Volver a CRM
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {/* Org info */}
          <div className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px] p-4">
            <div className="flex items-start justify-between gap-2 mb-3">
              <div>
                <h2 className="text-base font-bold text-[var(--cf-ink)]">{data.orgNombre}</h2>
                <p className="text-[10px] text-[var(--cf-ink-3)]">ID: {id}</p>
              </div>
            ...
            <div className="space-y-0">
              {data.orgTelefono && (
                <InfoRow label="Teléfono">
                  <span className="text-xs text-[var(--cf-ink-3)]">{data.orgTelefono}</span>
                </InfoRow>
              )}
              {data.ciudad && (
                <InfoRow label="Ciudad">
                  <span className="text-xs text-[var(--cf-ink-3)]">{data.ciudad}</span>
                </InfoRow>
              )}
              <InfoRow label="Registro">
                <div className="text-right">
                  <p className="text-xs text-[var(--cf-ink-3)]">{formatDate(data.createdAt)}</p>
                  <p className="text-[10px] text-[var(--cf-ink-3)]">{diasDesde(data.createdAt)}</p>
                </div>
              </InfoRow>
            </div>
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <select
                  ...
                  className="mt-1 w-full h-9 px-3 rounded-[10px] border border-[var(--cf-border)] bg-[var(--cf-card)] text-sm text-[var(--cf-ink)] focus:outline-none focus:border-[var(--cf-blue)]"
                >
              ...
              <div>
                <label className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide">Fuente de registro</label>
                <select
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={saveOrg}
                  disabled={saving}
                  className="flex-1 h-9 rounded-[10px] bg-[var(--cf-blue)] text-[var(--cf-ink)] text-xs font-medium hover:bg-[var(--cf-blue)] transition-colors disabled:opacity-50"
                >
                  {saving ? 'Guardando...' : 'Guardar cambios'}
                </button>
                {saveMsg && (
                  <span className={`text-xs ${saveMsg === 'Guardado' ? 'text-[var(--cf-green-dark)]' : 'text-[var(--cf-red-dark)]'}`}>
                    {saveMsg}
                  </span>
                )}
              </div>
          ...
          {/* Owner info */}
          <div className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px] p-4">
            <p className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide font-medium mb-3">Propietario</p>
            <div className="space-y-0">
              <InfoRow label="Nombre">
                <span className="text-xs text-[var(--cf-ink-3)]">{data.ownerNombre}</span>
              </InfoRow>
              <InfoRow label="Email">
                <span className="text-xs text-[var(--cf-ink-3)] break-all">{data.ownerEmail}</span>
              </InfoRow>
              {data.ownerPhone && (
                <InfoRow label="Celular">
                  <span className="text-xs text-[var(--cf-ink-3)]">{data.ownerPhone}</span>
                </InfoRow>
              )}
          ...
          {/* Subscription */}
          <div className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px] p-4">
            <p className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide font-medium mb-3">Suscripción</p>
            {data.suscripcion ? (
              <div className="space-y-0">
                <InfoRow label="Plan">
                ...
                {data.suscripcion.vencimiento && (
                  <InfoRow label="Vencimiento">
                    <div className="text-right">
                      <p className="text-xs text-[var(--cf-ink-3)]">{formatDate(data.suscripcion.vencimiento)}</p>
                      {data.suscripcion.diasRestantes !== undefined && (
                        <p className={`text-[10px] ${data.suscripcion.diasRestantes > 7 ? 'text-[var(--cf-green-dark)]' : data.suscripcion.diasRestantes > 0 ? 'text-[var(--cf-gold-dark)]' : 'text-[var(--cf-red-dark)]'}`}>
              ...
              <p className="text-xs text-[var(--cf-ink-3)]">Sin suscripción activa</p>
            )}
          </div>
          ...
          {data.actividad && (
            <div className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px] p-4">
              <p className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide font-medium mb-3">Actividad</p>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: 'Clientes',  value: data.actividad.clientes  ?? 0, color: 'var(--cf-ink)' },
                  { label: 'Préstamos', value: data.actividad.prestamos ?? 0, color: 'var(--cf-blue)' },
                  { label: 'Rutas',     value: data.actividad.rutas     ?? 0, color: 'var(--cf-blue)' },
          ...
          <div className="flex gap-2">
            {phone && (
              <a
                ...
                className="flex-1 flex items-center justify-center gap-2 h-10 rounded-[12px] bg-[rgba(34,197,94,0.1)] border border-[rgba(34,197,94,0.2)] text-[var(--cf-green-dark)] text-xs font-medium hover:bg-[rgba(34,197,94,0.2)] transition-colors"
              >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 h-10 rounded-[12px] bg-[rgba(59,130,246,0.1)] border border-[rgba(59,130,246,0.2)] text-[var(--cf-blue)] text-xs font-medium hover:bg-[rgba(59,130,246,0.2)] transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* ------------------------------------------------------------------ */}
        <div className="lg:col-span-2">
          <div className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px] p-4 h-full flex flex-col gap-4">
            <p className="text-xs font-semibold text-[var(--cf-ink-3)] uppercase tracking-wide">
              Notas · {(data.notas ?? []).length}
            </p>

            {/* Add note form */}
            <div className="space-y-3 pb-4 border-b border-[var(--cf-border)]">
              <textarea
                value={notaBody}
                onChange={(e) => setNotaBody(e.target.value)}
                rows={3}
                placeholder="Agrega una nota de seguimiento..."
                className="w-full px-3 py-2 rounded-[12px] border border-[var(--cf-border)] bg-[var(--cf-card)] text-sm text-[var(--cf-ink)] placeholder-[#555555] focus:outline-none focus:border-[var(--cf-blue)] resize-none"
              />
              <div className="flex gap-2 items-center">
                <select
                  value={notaEstado}
                  onChange={(e) => setNotaEstado(e.target.value)}
                  className="h-9 px-3 rounded-[10px] border border-[var(--cf-border)] bg-[var(--cf-card)] text-xs text-[var(--cf-ink)] focus:outline-none focus:border-[var(--cf-blue)]"
                >
                ...
                <button
                  onClick={addNota}
                  disabled={addingNota}
                  className="ml-auto h-9 px-5 rounded-[10px] bg-[var(--cf-gold)] text-[#1a1a2e] text-xs font-bold hover:bg-[#e5b516] transition-colors disabled:opacity-50"
                >
                  {addingNota ? 'Agregando...' : 'Agregar nota'}
                </button>
              </div>
              {notaError && <p className="text-xs text-[var(--cf-red-dark)]">{notaError}</p>}
            </div>

            {/* Timeline */}
            <div className="flex-1 overflow-y-auto space-y-3 max-h-[500px] lg:max-h-[600px]">
              {(data.notas ?? []).length === 0 ? (
                <p className="text-sm text-[var(--cf-ink-3)] text-center py-8">Sin notas aún. Agrega la primera nota de seguimiento.</p>
              ) : (
                (data.notas ?? []).map((n, i) => (
                  <div key={n.id ?? i} className="flex gap-3">
                    {/* Timeline dot */}
                    <div className="flex flex-col items-center">
                      <div className="w-2.5 h-2.5 rounded-full bg-[var(--cf-fill)] border-2 border-[var(--cf-blue)] mt-1 shrink-0" />
                      {i < (data.notas ?? []).length - 1 && (
                        <div className="w-px flex-1 bg-[var(--cf-fill)] mt-1" />
                      )}
                    </div>

                    {/* Note card */}
                    <div className="flex-1 pb-3">
                      <div className="bg-[var(--cf-card)] border border-[var(--cf-border)] rounded-[12px] px-3 py-2.5 space-y-1.5">
                        <div className="flex items-start justify-between gap-2">
                          <span className="text-xs font-medium text-[var(--cf-ink-3)]">
                            {n.adminNombre ?? 'Admin'}
                          </span>
                        ...
                        <p className="text-sm text-[var(--cf-ink)] whitespace-pre-wrap">{n.contenido}</p>
                        {n.estadoCambio && (
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-[var(--cf-ink-3)]">Estado cambiado a</span>
