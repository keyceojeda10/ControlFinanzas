...
export default function AdminTicketDetallePage() {
  ...
  if (!ticket) return <p className="text-sm text-[var(--cf-ink-3)] text-center py-12">Ticket no encontrado</p>

  const estado = ESTADO_BADGE[ticket.estado] || ESTADO_BADGE.abierto

  return (
    <div className="max-w-3xl mx-auto">
      <button onClick={() => router.push('/admin/soporte')} className="text-xs text-[var(--cf-ink-3)] hover:text-[var(--cf-ink)] transition-colors mb-3 flex items-center gap-1">
      ...
      <Card className="mb-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h1 className="text-[25px] font-semibold text-[var(--cf-ink)] mb-1.5">{ticket.asunto}</h1>
        ...
        <p className="text-xs text-[var(--cf-ink-3)] leading-relaxed mb-4">{ticket.descripcion}</p>

        {/* Info del usuario */}
        <div className="bg-[var(--cf-card)] border border-[var(--cf-border)] rounded-[10px] p-3 mb-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-[var(--cf-ink)]">{ticket.user?.nombre}</p>
              <p className="text-[10px] text-[var(--cf-ink-3)]">{ticket.user?.email} — {ticket.organization?.nombre} ({ticket.organization?.plan})</p>
            </div>
          </div>
        </div>
        ...
        {ticket.solicitaContacto && (
          <div className={`rounded-[10px] p-3 mb-4 border ${ticket.contactoAtendido ? 'bg-[rgba(34,197,94,0.05)] border-[rgba(34,197,94,0.15)]' : 'bg-[rgba(245,158,11,0.05)] border-[rgba(245,158,11,0.15)]'}`}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-[var(--cf-ink)]">Solicita contacto telefónico</p>
                <p className="text-sm font-bold text-[var(--cf-gold)] mt-0.5">{ticket.telefonoContacto || 'Sin teléfono'}</p>
              </div>
        ...
        {/* Cambiar estado */}
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-[var(--cf-ink-3)]">Estado:</span>
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-full text-[10px] font-medium transition-all ${
                ticket.estado === e
                  ...
                  : 'bg-[var(--cf-surface)] text-[var(--cf-ink-3)] hover:text-[var(--cf-ink)] border border-[var(--cf-border)] disabled:opacity-50'
              }`}
            >
      ...
      {modalEstado && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px] p-5 w-full max-w-md">
            <h3 className="text-sm font-bold text-[var(--cf-ink)] mb-1">
              {modalEstado === 'resuelto' ? 'Marcar como resuelto' : 'Cerrar ticket'}
            </h3>
            <p className="text-xs text-[var(--cf-ink-3)] mb-3">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-[10px] bg-[var(--cf-card)] border border-[var(--cf-border)] px-3 py-2 text-xs text-[var(--cf-ink)] placeholder-[#555555] focus:outline-none focus:border-[var(--cf-gold)] resize-none"
              autoFocus
            />
            <div className="flex items-center justify-end gap-2 mt-3">
              <button
                onClick={() => { setModalEstado(null); setNotaResolucion('') }}
                className="px-3 py-1.5 rounded-[10px] text-xs text-[var(--cf-ink-3)] hover:text-[var(--cf-ink)] border border-[var(--cf-border)] transition-colors"
              >
                Cancelar
              </button>
              <button
                ...
                className="px-3 py-1.5 rounded-[10px] text-xs font-semibold bg-[var(--cf-gold)] text-[#1a1a2e] hover:bg-[#d4a817] transition-colors disabled:opacity-50"
              >
      ...
      <Card padding={false}>
        ...
        <div ref={chatRef} className="px-4 py-4 space-y-3 max-h-[400px] overflow-y-auto">
          {mensajes.length === 0 ? (
            <p className="text-xs text-[var(--cf-ink-3)] text-center py-6">No hay mensajes. Responde al usuario aquí.</p>
          ) : (
            mensajes.map(m => {
                ...
                return (
                  <div key={m.id} className="flex justify-center">
                    <div className="max-w-[90%] rounded-[12px] px-4 py-3 bg-[rgba(139,92,246,0.08)] border border-[rgba(139,92,246,0.2)]">
                      <p className="text-xs text-[var(--cf-blue)] leading-relaxed whitespace-pre-wrap font-medium">{m.contenido}</p>
        ...
        <form onSubmit={enviarMensaje} className="px-4 py-3 border-t border-[var(--cf-border)] flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 h-9 rounded-[10px] bg-[var(--cf-card)] border border-[var(--cf-border)] px-3 text-xs text-[var(--cf-ink)] placeholder-[#555555] focus:outline-none focus:border-[var(--cf-gold)]"
          />
