...
export default function MetricasPage() {
  ...
  if (!stats) return <p className="text-[var(--color-danger)] text-sm">Error cargando métricas</p>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <Card>
          <p className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wide mb-4">Distribución de planes</p>
          {planDist.length > 0 ? (
        ...
        {/* Churn mensual */}
        <Card>
          <p className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wide mb-4">Churn mensual</p>
          <ResponsiveContainer width="100%" height={200}>
      ...
      {uso && (
        <>
          <div className="pt-4">
            <h2 className="text-lg font-bold text-[var(--color-text-primary)]">Uso del producto</h2>
            <p className="text-xs text-[var(--color-text-muted)] mt-0.5">Usuarios activos y features más usados</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {/* Top features */}
            <Card>
              <p className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wide mb-3">Features más usados</p>
              <div className="space-y-2">
                {uso.topFeatures.map((f) => (
                  <div key={f.evento} className="flex items-center justify-between">
                    <span className="text-xs text-[var(--color-text-secondary)]">{f.evento}</span>
                    <span className="text-xs font-bold text-[var(--color-text-primary)] font-mono-display">{f.count}</span>
                  </div>
                ))}
                {uso.topFeatures.length === 0 && <p className="text-xs text-[var(--color-text-muted)]">Sin datos aún</p>}
              </div>
            </Card>

            {/* Top pages */}
            <Card>
              <p className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wide mb-3">Páginas más visitadas</p>
              <div className="space-y-2">
                {uso.topPages.map((p) => (
                  <div key={p.pagina} className="flex items-center justify-between">
                    <span className="text-xs text-[var(--color-text-secondary)]">{p.pagina}</span>
                    <span className="text-xs font-bold text-[var(--color-text-primary)] font-mono-display">{p.count}</span>
                  </div>
                ))}
                {uso.topPages.length === 0 && <p className="text-xs text-[var(--color-text-muted)]">Sin datos aún</p>}
              </div>
            </Card>
          </div>
        </>
