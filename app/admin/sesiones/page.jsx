...
export default function SesionesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-[25px] font-semibold text-white">Sesiones Multi-dispositivo</h1>
        <p className="text-xs text-[var(--color-text-muted)]">Detecta usuarios con actividad desde multiples IPs o dispositivos</p>
      </div>

      {/* Métricas */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <Card>
          <p className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wide">Usuarios activos</p>
          <p className="text-2xl font-bold text-[var(--color-accent)]">{data.totalUsuariosActivos}</p>
          <p className="text-[10px] text-[#555]">ultimas {horas}h</p>
        </Card>
        <Card>
          <p className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wide">Sospechosos</p>
          <p className="text-2xl font-bold" style={{ color: data.totalSospechosos > 0 ? 'var(--color-danger)' : 'var(--color-success)' }}>
            {data.totalSospechosos}
          </p>
          <p className="text-[10px] text-[#555]">2+ IPs o 3+ dispositivos</p>
        </Card>
        <Card className="col-span-2 sm:col-span-1">
          <p className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-wide">Periodo</p>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-[8px] text-xs font-medium border transition-all ${
            verTodos
              ? 'border-[var(--color-accent)] text-[var(--color-accent)] bg-[rgba(245,197,24,0.08)]'
              : 'border-[#333] text-[var(--color-text-muted)] hover:border-[#555]'
          }`}
        >
      ...
      {data.usuarios.length === 0 ? (
        <Card>
          <div className="text-center py-8">
            <svg className="w-10 h-10 mx-auto mb-3 text-[var(--color-success)]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
            ...
            <Card key={u.user.id}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-[var(--color-text-muted)] truncate">{u.user.email}</p>
                  {u.user.organization && (
                ...
                <div className="text-right shrink-0">
                  <div className="flex items-center gap-3 text-[11px]">
                    <span className="flex items-center gap-1" title="IPs distintas">
                      ...
                      <span className={u.totalIps >= 2 ? 'text-[var(--color-danger)] font-semibold' : 'text-[var(--color-text-muted)]'}>
                    ...
                    <span className="flex items-center gap-1" title="Dispositivos distintos">
                      ...
                      <span className={u.totalDispositivos >= 3 ? 'text-[var(--color-danger)] font-semibold' : 'text-[var(--color-text-muted)]'}>
                        {u.totalDispositivos}
                      </span>
                    </span>
                  </div>
