...
export default function SesionesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-[25px] font-semibold text-[var(--cf-ink)]">Sesiones Multi-dispositivo</h1>
        <p className="text-xs text-[var(--cf-ink-3)]">Detecta usuarios con actividad desde multiples IPs o dispositivos</p>
      </div>

      {/* Métricas */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        <Card>
          <p className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide">Usuarios activos</p>
          <p className="text-2xl font-bold text-[var(--cf-gold)]">{data.totalUsuariosActivos}</p>
          <p className="text-[10px] text-[#555]">ultimas {horas}h</p>
        </Card>
        <Card>
          <p className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide">Sospechosos</p>
          <p className="text-2xl font-bold" style={{ color: data.totalSospechosos > 0 ? 'var(--cf-red-dark)' : 'var(--cf-green-dark)' }}>
            {data.totalSospechosos}
          </p>
          <p className="text-[10px] text-[#555]">2+ IPs o 3+ dispositivos</p>
        </Card>
        <Card className="col-span-2 sm:col-span-1">
          <p className="text-[10px] text-[var(--cf-ink-3)] uppercase tracking-wide">Periodo</p>
          <p className="text-sm text-[var(--cf-ink)] mt-1">{data.periodo}</p>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-[8px] text-xs font-medium transition-all ${
                horas === p.key
                  ...
                  : 'text-[var(--cf-ink-3)] hover:text-white'
              }`}
            >
              {p.label}
            </button>
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-[8px] text-xs font-medium border transition-all ${
            verTodos
              ? 'border-[var(--cf-gold)] text-[var(--cf-gold)] bg-[rgba(245,197,24,0.08)]'
              : 'border-[#333] text-[var(--cf-ink-3)] hover:border-[#555]'
          }`}
        >
      ...
      {data.usuarios.length === 0 ? (
        <Card>
          <div className="text-center py-8">
            <svg className="w-10 h-10 mx-auto mb-3 text-[var(--cf-green-dark)]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-semibold text-[var(--cf-ink)] truncate">{u.user.nombre}</p>
                  ...
                  <p className="text-[11px] text-[var(--cf-ink-3)] truncate">{u.user.email}</p>
                  {u.user.organization && (
                ...
                <div className="text-right shrink-0">
                  <div className="flex items-center gap-3 text-[11px]">
                    <span className="flex items-center gap-1" title="IPs distintas">
                      <svg className="w-3.5 h-3.5 text-[var(--cf-ink-3)]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" />
                      </svg>
                      <span className={u.totalIps >= 2 ? 'text-[var(--cf-red-dark)] font-semibold' : 'text-[var(--cf-ink-3)]'}>
                    ...
                    <span className="flex items-center gap-1" title="Dispositivos distintos">
                      ...
                      <span className={u.totalDispositivos >= 3 ? 'text-[var(--cf-red-dark)] font-semibold' : 'text-[var(--cf-ink-3)]'}>
                        {u.totalDispositivos}
                      </span>
                    </span>
                  </div>
