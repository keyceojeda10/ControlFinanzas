  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[25px] font-semibold text-[var(--cf-ink)]">Organizaciones</h1>
        ...
        {hayFiltros && (
          <button
            onClick={limpiarTodo}
            className="text-[11px] text-[var(--cf-ink-3)] hover:text-[var(--cf-ink)] transition-all"
          >
            Limpiar filtros
          </button>
        )}
      ...
      <div className="space-y-3">
        <form onSubmit={handleSearch} className="flex gap-2">
          ...
          />
          <button type="submit" className="h-9 px-4 rounded-[12px] bg-[var(--cf-blue)] text-[var(--cf-ink)] text-xs font-medium hover:bg-[var(--cf-blue)] transition-all shrink-0">
            Buscar
          </button>
        </form>
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            className="h-8 px-2 rounded-[10px] border border-[var(--cf-border)] bg-[var(--cf-card)] text-xs text-[var(--cf-ink)] focus:outline-none focus:border-[var(--cf-blue)]"
          >
          ...
          <span className="text-[10px] text-[var(--cf-ink-3)]">desde</span>
          <input
            type="date"
            value={fechaDesde}
            ...
            className="h-8 px-2 rounded-[10px] border border-[var(--cf-border)] bg-[var(--cf-card)] text-xs text-[var(--cf-ink)] focus:outline-none focus:border-[var(--cf-blue)]"
          />
          <span className="text-[10px] text-[var(--cf-ink-3)]">hasta</span>
          <input
            type="date"
            value={fechaHasta}
      ...
      {loading ? <SkeletonTable rows={5} /> : orgsFiltradas.length === 0 ? (
        <p className="text-sm text-[var(--cf-ink-3)] text-center py-8">No se encontraron organizaciones</p>
      ) : (
        <div className="bg-[var(--cf-surface)] border border-[var(--cf-border)] rounded-[16px] overflow-hidden">
          {/* Header */}
          <div className="hidden sm:grid grid-cols-7 gap-2 px-4 py-2.5 text-[10px] text-[var(--cf-ink-3)] font-medium uppercase border-b border-[var(--cf-border)]">
          ...
          {orgsFiltradas.map((o) => (
            <Link
              ...
              className="grid grid-cols-2 sm:grid-cols-7 gap-2 px-4 py-3 border-b border-[var(--cf-border)] last:border-0 hover:bg-[var(--cf-fill)] transition-all items-center"
            >
              <div className="col-span-2">
                <p className="text-sm font-medium text-[var(--cf-ink)]">{o.nombre}</p>
                {o.ownerEmail && (
                  <p className="text-[11px] text-[var(--cf-ink-3)] truncate">{o.ownerEmail}</p>
                )}
              ...
              <p className="text-sm text-[var(--cf-ink-3)] text-center">{o.usuarios}</p>
              <p className="text-sm text-[var(--cf-ink-3)] text-center">{o.clientes}</p>
              <div className="text-center">
                ...
                {o.suscripcion ? (
                  <div>
                    ...
                    {(o.suscripcion.montoCOP ?? 0) > 0 && (
                      <p className="text-[11px] text-[var(--cf-green-dark)] mt-0.5">pagado</p>
                    )}
                  </div>
                ) : (
                  <span className="text-[10px] text-[var(--cf-ink-3)]">Sin suscripción</span>
                )}
              </div>
            </Link>
          ))}
