...
export default function CajaPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setCierres(Array.isArray(cierresData.cierres) ? cierresData.cierres : [])
      setPrestamos(Array.isArray(prestamosData) ? prestamosData : [])
      if (cierresData.fechaDisplay) {
        setFechaDisplay(cierresData.fechaDisplay)
      }
    } catch {
    ...
    return (
      <div className="max-w-xl mx-auto space-y-4">
        <div className="mb-2">
          ...
          <p className="text-sm text-[#555555] mt-0.5">{fechaDisplay || '—'}</p>
        </div>
