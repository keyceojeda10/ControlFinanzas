...
export default function CapitalPage() {
  ...
  return (
    <div className="max-w-3xl lg:max-w-6xl mx-auto p-4">
      <div className="mb-5">
        <h1 className="text-[25px] font-semibold text-[var(--color-text-primary)]">Capital</h1>
        <p className="text-sm text-[var(--color-text-muted)] mt-0.5">Tu fondo de préstamos — dinero disponible para prestar</p>
      </div>
      {bannerVisible && (
        <div className="mb-4 rounded-[12px] px-3.5 py-2.5 flex items-start gap-2.5" style={{ background: 'color-mix(in srgb, var(--color-teal) 8%, var(--color-bg-card))', border: '1px solid color-mix(in srgb, var(--color-teal) 20%, var(--color-border))' }}>
          ...
          <button onClick={cerrarBanner} className="shrink-0 w-5 h-5 flex items-center justify-center rounded-full transition-colors hover:bg-[rgba(255,255,255,0.1)]" style={{ color: 'var(--color-text-muted)' }} title="Cerrar">
