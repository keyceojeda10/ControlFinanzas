...
export function buildFirstMessage(nombre) {
  ...
  // Enfoque psicológico: NO interrogar, hablar en tercera persona,
  // reducir defensas, generar identificación indirecta.
  // El objetivo es obtener UNA respuesta, no vender.
  return [
...
export function buildLeadKeyboard(leadId, waLink) {
  ...
  // Fila 1: WhatsApp (URL button) + Precio (callback)
  const row1 = []
  if (waLink) {
    row1.push({ text: '📱 WhatsApp', url: waLink })
  }
  row1.push({ text: '💰 Precio', callback_data: `precio:${leadId}` })
  keyboard.push(row1)

  // Fila 2: Contactado + Descartar
  keyboard.push([
...
export async function sendFollowupReminder(lead, tipo) {
  ...
  switch (tipo) {
    ...
    case '24h': // >24h sin respuesta
      ...
      msgTemplate = `Hola ${saludo}, ¿quieres que te guíe con una videollamada rápida de 5 minutos? Te muestro cómo funciona el sistema en vivo.\n\nSi prefieres, pruébalo tú mismo: https://app.control-finanzas.com/registro\n14 días gratis, sin tarjeta.`
      break
    case '48h': // >48h último intento
      emoji = '⏰'
      titulo = 'Último intento'
      detalle = `Lleva <b>${horasTranscurridas}h</b> sin respuesta`
      msgTemplate = `${saludo}, no quiero molestarte más. Si en algún momento quieres organizar tu cartera de préstamos, aquí tienes tu prueba gratis:\n\nhttps://app.control-finanzas.com/registro\n\n¡Éxitos! 🤝`
      break
