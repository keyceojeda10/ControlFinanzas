...
import { prettyCantClientes, prettyEsPrestamista, prettyMetodoActual, prettyPlanInteres, prettyConsent } from '@/lib/fb-leads'
...
// Bot de Escalamientos (leads calientes) — canal DEDICADO para que no se
// pierdan entre el ruido de las notificaciones de la app. Si no esta
// configurado, cae al bot de notificaciones (comportamiento actual).
const ESCAL_BOT_TOKEN = process.env.TELEGRAM_ESCAL_BOT_TOKEN || NOTIF_BOT_TOKEN
const ESCAL_CHAT_ID = process.env.TELEGRAM_ESCAL_CHAT_ID || NOTIF_CHAT_ID
...
import { getLocale, getTimezone } from './i18n.js'

function formatFecha(createdTime, country = 'co') {
  const opts = {
  ...
  return date.toLocaleString(getLocale(country), opts)
}
...
// bot = 'leads' (default) | 'notif' | 'escalamiento'
async function telegramAPI(method, body, bot = 'leads') {
  const token = bot === 'escalamiento' ? ESCAL_BOT_TOKEN
    : bot === 'notif' ? NOTIF_BOT_TOKEN
    : BOT_TOKEN
  if (!token) {
...
function getChatId(bot = 'leads') {
  return bot === 'escalamiento' ? ESCAL_CHAT_ID
    : bot === 'notif' ? NOTIF_CHAT_ID
    : CHAT_ID
}
...
export function buildPrecioMessage() {
  ...
  return [
    ...
    ``,
    `📌 <b>${i.nombre}</b> — $${i.precio.toLocaleString('es-CO')}/mes`,
    `   ${i.maxUsuarios} usuario, hasta ${i.maxClientes} clientes, ${i.maxRutas} ruta`,
    ``,
...
export async function sendLeadNotification({ nombre, telefono, cantClientes, esPrestamista, metodoActual, planInteres, consent, anuncioId, createdTime, leadgenId }, leadId) {
  ...
  const mensaje = buildFirstMessage(nombre)
  ...
  const lines = [
    ...
    `👥 ${escapeHtml(prettyCantClientes(cantClientes)) || 'No especificó'} clientes`,
  ]
  const prest = prettyEsPrestamista(esPrestamista)
  if (prest) lines.push(prest)
  const metodo = prettyMetodoActual(metodoActual)
  if (metodo) lines.push(`🛠 Método actual: ${escapeHtml(metodo)}`)
  const plan = prettyPlanInteres(planInteres)
  if (plan) lines.push(`🎯 Plan de interés: ${escapeHtml(plan)}`)
  const cons = prettyConsent(consent)
  if (cons) lines.push(cons)
  lines.push(
    `📅 ${fecha}`,
    ``,
    `💬 <b>Mensaje listo:</b>`,
...
export async function sendFollowupReminder(lead, tipo) {
  ...
  switch (tipo) {
    ...
    case '7d': // registrado hace 7d — check-in único
      ...
      msgLabel = 'Check-in'
      msgTemplate = `${saludo}, cómo te ha ido con el sistema? Te ha servido?\n\nSi quieres seguir usándolo después del día 14, el plan inicial son $${PLANES.starter.precio.toLocaleString('es-CO')}/mes y te incluye todo lo que ya estás usando.\n\nMe dices si tienes alguna pregunta.`
      break
    default:
      return null
  }
