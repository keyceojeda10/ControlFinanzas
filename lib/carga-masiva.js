...
const ALIAS_COLUMNAS = {
  ...
  diasPlazo: [
    'diasplazo', 'dias plazo', 'dias', 'plazo', 'dias de plazo', 'term', 'cuotas',
    'numero de cuotas', 'num cuotas', 'plazo dias', 'plazo en dias', 'duracion',
    'duración', 'periodo', 'tiempo', 'cantidad de cuotas', 'no cuotas',
  ],
  frecuencia: [
...
export function detectarColumnas(headersOriginales) {
  ...
  for (const header of headersOriginales) {
    ...
    if (!mejorCampo) {
      for (const [campo, aliases] of Object.entries(ALIAS_COLUMNAS)) {
        if (usados.has(campo)) continue
        if (aliases.some(a => norm.includes(normalizar(a)) || normalizar(a).includes(norm))) {
          mejorCampo = campo
          break
        }
      }
    ...
    if (mejorCampo) {
      mapeo[header] = mejorCampo
      usados.add(mejorCampo)
    } else {
      sinMapear.push(header)
    }
  }
...
export function validarFila(fila, indice, cedulasExistentes) {
  ...
  } else if (cedula.length < 4 || cedula.length > 20) {
    errores.push('Cédula debe tener entre 4 y 20 caracteres')
  } else {
