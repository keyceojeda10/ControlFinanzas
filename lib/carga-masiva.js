...
import { deducirCondiciones } from '@/lib/importar/deducir-condiciones'
...
/* ══ LA CÉDULA DE UNA FILA — I5, 24 sep 2026 ═════════════════════════════════
 *
 * La misma cuenta que `validarFila` hacía suelta: la trimeada sin puntos ni
 * espacios, o el `SIN-<NOMBRE>` generado si no trae cédula pero sí nombre.
 * `validarCartera` la necesita ANTES de validar, para preguntarle a la base
 * qué clientes ya existen — y tiene que preguntar con la MISMA llave que
 * `validarFila` va a mirar, o los `SIN-<NOMBRE>` de un PDF nunca se encuentran
 * (la planilla de Crossbox no trae cédula: sin esto, cada reintento del mismo
 * archivo se veía como clientes nuevos, y `excedePlan` podía bloquear sin
 * motivo). Antes `validarCartera` sacaba la cédula con `.replace(/\D/g, '')`
 * (solo dígitos) mientras esta función limpiaba puntos y espacios: dos llaves
 * distintas para el mismo cliente.
 */
export function cedulaDeFila(fila) {
  const nombre = String(fila.nombre ?? '').trim()
  const cedula = String(fila.cedula ?? '').trim().replace(/[.\s]/g, '')
  if (cedula) return cedula
  if (!nombre) return ''
  return 'SIN-' + nombre.toUpperCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^A-Z0-9]/g, '-').replace(/-+/g, '-').slice(0, 30)
}
 ...
 */
export function validarFila(fila, indice, cedulasExistentes, huellasExistentes = new Map(), opciones = {}) {
  const errores = []
  const advertencias = []
  ...
  if (fila.fechaCorte && sinCondiciones) {
    ...
    if (deduccion.ok) {
      fila = {
        ...
        /* Solo cuando el total deducido ES el saldo del archivo (deduccion.totalEsSaldo):
           nada de abono previo, ni siquiera los pesos del redondeo. Un «Nunca» que cayó en
           la otra rama (saldo por debajo del capital, fila 46 de la planilla real) SÍ tiene
           un saldo que dice cuánto se abonó, y no se puede borrar: se inventaría deuda que
           no existe. */
        ...(deduccion.totalEsSaldo ? { saldoActual: '' } : {}),
      }
  ...
  let existente = null
  // La cédula cruda (sin la generación de SIN-<NOMBRE>) decide qué aviso dar;
  // `cedula` —la que de verdad se guarda— sale de `cedulaDeFila`, la MISMA
  // cuenta que usa `validarCartera` para preguntarle a la base. Ver su comentario.
  const cedulaCruda = String(fila.cedula ?? '').trim().replace(/[.\s]/g, '')
  let cedula = cedulaDeFila(fila)
  if (!cedulaCruda) {
    if (!nombre) {
  ...
  let correccion = null
  const erroresAntesDelPrestamo = errores.length
