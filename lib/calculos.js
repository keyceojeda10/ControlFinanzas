 ...
 */
/**
 * redondeo: 'exacto' | 'redondeado' | 'cerrado'
 *   - exacto:      al múltiplo de $100 (moneda mínima práctica en Colombia)
 *   - redondeado:  al múltiplo de $500
 *   - cerrado:     al múltiplo de $1.000
 * En los 3 casos las cuotas son parejas de inicio a fin.
 */
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio, frecuencia = 'diario', cuotaManual, redondeo = 'exacto' }) {
  const monto  = Number(montoPrestado)
  const tasa   = Number(tasaInteres)
  const dias   = Number(diasPlazo)
  const freq   = frecuencia || 'diario'
  const rd     = redondeo === 'cerrado' ? 1000 : redondeo === 'redondeado' ? 500 : 100
  ...
  // Cuota redondeada al múltiplo de rd más cercano hacia arriba.
  // Todas las cuotas son iguales y el totalAPagar se ajusta a cuota × numPeriodos.
  let cuotaDiaria, totalAPagarFinal
  if (numPeriodos <= 1) {
    cuotaDiaria = Math.max(rd, Math.ceil(totalAPagar / rd) * rd)
    totalAPagarFinal = cuotaDiaria
  } else {
    const cuotaBase = totalAPagar / numPeriodos
    cuotaDiaria = Math.max(rd, Math.ceil(cuotaBase / rd) * rd)
    totalAPagarFinal = cuotaDiaria * numPeriodos
  }
  const ultimaCuota = cuotaDiaria
...
export function tienePeriodoEsperadoHoy(prestamo, hoySinCobro = false, diasExcluidos = [], festivos = []) {
  ...
  // Diaria: cualquier dia cobrable cuenta como periodo esperado,
  // salvo que hoy sea dia sin cobro (festivo o dia excluido).
  if (freq === 'diario') {
    return !hoySinCobro
  }

  // No diaria (semanal/quincenal/mensual): hoy debe coincidir con el ciclo
  // calculado por calcularProximoCobro, que respeta el dia ancla configurado.
  const diasPeriodo = obtenerDiasPorPeriodo(freq)
  const diff = Math.floor((hoy.getTime() - inicio.getTime()) / DAY_MS)
  if (diff < 0) return false

  // Si existe dia ancla (diaCobroSemana o diaCobroMes), respetar ancla.
  // Si no, ciclo regular desde fechaInicio.
  if (prestamo.diaCobroSemana != null && (freq === 'semanal' || freq === 'quincenal')) {
    // diaCobroSemana: 0=dom, 6=sab (formato Date.getDay()).
    const diaHoy = hoy.getUTCDay()
    if (diaHoy !== Number(prestamo.diaCobroSemana)) return false
    // Ademas, verificar que sea el ciclo correcto (cada 1 o 2 semanas).
    return diff % diasPeriodo === 0
  }
