...
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio, frecuencia = 'diario', cuotaManual, modoInteres, redondeo, interesAdelantado = false, diaCobroMes, diaCobroMes2, capitalExtra }) {
  ...
  if (modo === 'lineal') {
    ...
    for (let n = 1; n <= numPeriodos; n++) {
      ...
      const capital = esUltimo ? monto - capitalAsignado : capitalRedondeado
      const extra = extraMapLineal.get(n) || 0
      capitalAsignado += capital + extra
      const cuotaTotal = capital + extra + Math.round(interesPeriodo)
      saldoRestante = Math.max(0, saldoRestante - capitalPorPeriodo - extra)
  ...
  const capitalPorPeriodo = monto / numPeriodos
  const interesPorPeriodo = interesTotal / numPeriodos
  const tablaAmortizacion = []
  let saldoRestante = monto
  for (let n = 1; n <= numPeriodos; n++) {
    const extra = extraMap.get(n) || 0
    const esUltimo = n === numPeriodos
    const capital = esUltimo ? saldoRestante : Math.round(capitalPorPeriodo)
    const interes = esUltimo ? Math.round(saldoRestante * (interesTotal / monto)) : Math.round(interesPorPeriodo)
    saldoRestante = Math.max(0, saldoRestante - capital - extra)
    tablaAmortizacion.push({
      numeroPeriodo: n,
      capital: Math.round(capital) + extra,
      interes,
      cuotaTotal: cuotaRegular + extra,
      saldoRestante: Math.round(saldoRestante),
      fechaEsperada: calcFechaEsperada(n),
      esExtra: extra > 0,
      montoExtra: extra,
  ...
  return {
    ...empaquetar(totalAPagarFinal, cuotaRegular, modo),
    ultimaCuota: tablaAmortizacion[numPeriodos - 1]?.cuotaTotal || 0,
    tablaAmortizacion,
    capitalExtra: [...extraMap.entries()].map(([numeroPeriodo, m]) => ({ numeroPeriodo, monto: m })),
  }
