...
export function calcularProximoCobro(prestamo) {
  ...
  // La primera cuota se cobra un periodo DESPUES del desembolso
  // (el dia del inicio el cliente recibe el dinero, no paga).
  const primerCobro = new Date(inicioMed.getTime() + diasPeriodo * 86400000)

  // Aun no llega el primer cobro
  if (primerCobro > hoy) return primerCobro

  // Contar cuantos pagos reales se han registrado (excluyendo ajustes)
  const pagosReales = Array.isArray(prestamo.pagos)
    ? prestamo.pagos.filter((p) => !TIPOS_AJUSTE.includes(p.tipo)).length
    : 0

  // La cuota N (1-indexed) toca el dia: inicio + N * diasPeriodo
  // Si ya se registraron N pagos, la proxima cuota es la N+1
  const proximaCuotaNum = pagosReales + 1
  const fechaProxima = new Date(inicioMed.getTime() + proximaCuotaNum * diasPeriodo * 86400000)

  // Si la fecha calculada ya paso (cliente esta atrasado), mostrar la mas cercana:
  // si es diario y hay atraso, devolver hoy (toca cobrar ya).
  // Para frecuencias mayores, si el dia programado ya paso, igual mostramos ese dia
  // (el cliente ve "te toca sab 11 abr" aunque ya sea 12 — es atraso).
  if (fechaProxima < hoy && diasPeriodo === 1) return hoy
