...
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio: fechaInicioRaw, frecuencia = 'diario', cuotaManual, modoInteres, redondeo, interesAdelantado = false, diaCobroMes, diaCobroMes2, capitalExtra }) {
  ...
  const calcFechaEsperada = (n) =>
    fechaDePeriodo(n, { fechaInicio, freq, diasPeriodo, diaCobroMes, diaCobroMes2 })
...
/**
 * La fecha del cobro numero `n`, con el calendario del proyecto.
 *
 * ── POR QUE ES UNA FUNCION EXPORTADA Y NO UN CIERRE ────────────────────────
 * Vivia dentro de `calcularPrestamo` como `calcFechaEsperada`, y ahi solo la
 * podia usar quien estuviera creando un prestamo. `lib/dinero/tabla.js` —la
 * tabla derivada de G6— necesita las MISMAS fechas para un prestamo que ya
 * existe, y la unica alternativa era copiarla: la duodecima replica de una
 * regla de calendario en un proyecto donde las fechas ya tienen su propia nota
 * («un solo calendario»).
 *
 * El cierre ahora delega aqui, asi que no hay dos calendarios que puedan
 * separarse con el tiempo. Ni una linea de logica cambia.
 */
