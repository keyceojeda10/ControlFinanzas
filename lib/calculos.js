...
export function calcularDiasMora(prestamo, diasExcluidos = [], festivos = []) {
  ...
  if (tieneTablaAmortizacion(prestamo)) {
    ...
    if (!primeraVencidaFecha) return 0
    const manana = new Date(hoy.getTime() + DAY_MS)
    const diasAtraso = calcularDiasTranscurridosCobrables(primeraVencidaFecha, manana, diasExcluidos, festivos)
...
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio, frecuencia = 'diario', cuotaManual, modoInteres, redondeo }) {
  ...
  if (modo === 'lineal') {
    ...
    for (let n = 1; n <= numPeriodos; n++) {
      ...
      fechaEsperada.setDate(fechaEsperada.getDate() + n * diasPeriodo)
...
export function tienePeriodoEsperadoHoy(prestamo, hoySinCobro = false, diasExcluidos = [], festivos = []) {
  ...
  // el usuario eligio anclar por numero de dia (ej. "cobra los 5").
 ...
 */
export function recalcularTablaSoloInteresDesdeSaldo({ saldoInicial, tasaInteres, numPeriodosRestantes, primerNumeroPeriodo, fechaBase, diasPeriodo }) {
  const tasa = Number(tasaInteres)
  const tabla = []
  let saldoRestante = saldoInicial
  for (let i = 0; i < numPeriodosRestantes; i++) {
 ...
 * diasGracia: dias de gracia antes de que empiece a correr
 * Retorna { diasMoraEfectivos, montoBase, montoMoratorio, tope }
 * El moratorio se calcula como interes simple diario sobre el monto en mora.
...
export function calcularInteresMoratorio(prestamo, diasExcluidos = [], festivos = [], tasaMoratorio = 0, diasGracia = 5) {
  ...
  const diasMora = calcularDiasMora(prestamo, diasExcluidos, festivos)
  const diasMoraEfectivos = Math.max(0, diasMora - diasGracia)
  if (diasMoraEfectivos <= 0) return nulo

  const montoBase = calcularMontoEnMora(prestamo, diasExcluidos, festivos)
  if (montoBase <= 0) return nulo
...
export function calcularLiquidacionAnticipada(prestamo, fechaLiquidacion = new Date(), diasExcluidos = [], festivos = []) {
  ...
  function interesPara(meses) {
    ...
    } else if (modo === 'solo_interes') {
      interes = capital * (tasa / 100) * meses
    } else if (modo === 'manual' || modo === 'saldo' || modo === 'lineal') {
  ...
  const noRecalculable = (modo === 'manual' || modo === 'saldo' || modo === 'lineal')
