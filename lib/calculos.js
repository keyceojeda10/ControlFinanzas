 ...
 */
/* ⚠ EL SUELO DE GRACIA, EXPORTADO PARA QUE LA PANTALLA PUEDA DECIRLO.
 *
 * Préstamos Rincón: «aunque tiene intereses moratorio, NO SE PUEDE APLICAR».
 * Tenía la tasa puesta (0,85 %) y la gracia en CERO, a propósito. Sus 20
 * préstamos activos son mensuales, así que este suelo le convertía ese cero en
 * QUINCE y el moratorio no salía nunca: 0 aplicables de 2 en mora. Medido
 * contra producción.
 *
 * El suelo puede ser buena política —un mensual con un día de atraso no debería
 * generar mora—, pero la pantalla decía «días que deben pasar antes de que se
 * empiece a calcular» y eso era FALSO para todo lo que no fuera diario. Se
 * configuraba un número y aplicaba otro, sin avisar.
 *
 * Sale de aquí para que la pantalla de configuración enseñe el número REAL por
 * frecuencia. Si se copiara allí, un día cambiaría uno y no el otro. */
export const GRACIA_MINIMA_POR_FRECUENCIA = { diario: 0, semanal: 7, quincenal: 10, mensual: 15 }

/** Los días de gracia que de verdad se aplican a un préstamo de esa frecuencia. */
export function graciaEfectiva(diasGracia = 5, frecuencia = 'diario') {
  return Math.max(Number(diasGracia) || 0, GRACIA_MINIMA_POR_FRECUENCIA[frecuencia] ?? 0)
}
...
export function calcularInteresMoratorio(prestamo, diasExcluidos = [], festivos = [], tasaMoratorio = 0, diasGracia = 5) {
  ...
  const freq = prestamo.frecuencia || 'diario'
  const gracia = graciaEfectiva(diasGracia, freq)
