...
import { obtenerDiasPorPeriodo, sumarMeses, fechaDePeriodo, primerCobroMensual } from './dinero/calendario.js'
...
export { obtenerDiasPorPeriodo, sumarMeses, fechaDePeriodo, primerCobroMensual }
...
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio: fechaInicioRaw, frecuencia = 'diario', cuotaManual, modoInteres, redondeo, interesAdelantado = false, diaCobroMes, diaCobroMes2, capitalExtra }) {
  ...
  const primerCobro = hayCorteMensual ? primerCobroMensual(fechaInicio, diaCobroMes) : null
  ...
  const calcFechaEsperada = prorrateaPrimero
    ? (n) => (n <= 1 ? new Date(primerCobro) : sumarMeses(primerCobro, n - 1, diaCobroMes))
    : (n) => fechaDePeriodo(n, { fechaInicio, freq, diasPeriodo, diaCobroMes, diaCobroMes2 })
  ...
  const empaquetar = (totalAPagar, cuota, modoUsado) => ({
    ...
    // Para que la pantalla pueda decir POR QUE la primera cuota es distinta.
    // Una cuota mas baja sin explicacion se lee como un error del sistema.
    ...(prorrateaPrimero ? { prorrateoPrimerPeriodo: true, diasPrimerPeriodo, primerCobro } : {}),
  })
