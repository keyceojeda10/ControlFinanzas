...
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio, frecuencia = 'diario' }) {
  ...
  // Interés mensual proporcional al plazo
  const meses = dias / 30
  const interesBase = monto * (tasa / 100) * meses
  const totalBase  = monto + interesBase

  // Redondear cuota al múltiplo de 50 más cercano (mínimo 50) — moneda mínima en Colombia
  const cuotaBase   = totalBase / numPeriodos
  const cuotaDiaria = Math.max(50, Math.round(cuotaBase / 50) * 50)

  // Total a pagar = cuota × cuotas, así todos los valores cierran sin sobrantes
