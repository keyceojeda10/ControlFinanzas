...
import { totalDelReparto, capitalDelPrestamo, fraccionArrastrada } from '@/lib/dinero/capital-base'
...
export function interesesSinPagar(prestamo) {
  ...
  /* ⚠ UN PAGO «COMPLETO» TAMBIÉN PAGA INTERÉS, Y AQUÍ NO SE CONTABA.
   *
   * Es la misma regla que ya aplica `calcularCapitalRestante` unas líneas más
   * abajo —«cubren primero el interés devengado sin pagar; lo que sobra es
   * capital»— y tenerla en un sitio y no en el otro deja al préstamo diciendo
   * dos cosas: capital al día y a la vez meses de interés sin pagar.
   *
   * Se ve al MIGRAR, que es cuando aparece un pago «completo» en un abierto: el
   * abono previo se guarda con ese tipo. Medido en el espejo con un préstamo de
   * $1.000.000 al 20% de hace dos meses y $400.000 ya abonados —justo los dos
   * meses de interés—: nacía con 31 días de mora y dos períodos sin pagar. */
  const devengadoTotal = periodos.reduce((a, d) => a + (Number(d.interes) || 0), 0)
  const porEtiqueta = suma('intereses')
  let bolsa = porEtiqueta
    + Math.min(suma('completo'), Math.max(0, devengadoTotal - porEtiqueta))
 ...
 */
/**
 * El día en que empezó la mora que tiene HOY: la primera cuota vencida sin
 * cubrir (tabla), el próximo cobro que ya pasó (lineal) o el interés cerrado
 * más viejo sin pagar (abierto). `null` si no está en mora.
 *
 * Es la misma ancla con la que `calcularDiasMora` cuenta los días; se sacó
 * aparte (8 sep 2026) para que el interés moratorio descuente lo que ya se
 * aplicó EN ESTA racha y no en una mora de hace meses.
 */
export function fechaInicioMora(prestamo, diasExcluidos = [], festivos = [], ahora = Date.now()) {
  if (prestamo.estado != null && prestamo.estado !== 'activo') return null
  if (!prestamo.cuotaDiaria || prestamo.cuotaDiaria <= 0) return null
  if (calcularSaldoPendiente(prestamo) <= 0) return null
  ...
  if (esAbiertoConDevengo(prestamo)) {
    ...
    return inicioDiaColombia(`${sinPagar[0].periodo}T05:00:00.000Z`)
  }
...
export function calcularDiasMora(prestamo, diasExcluidos = [], festivos = [], ahora = Date.now()) {
   ...
   * (Esa regla vive en `fechaInicioMora`, que es donde se decide el ancla.) */
  const ancla = fechaInicioMora(prestamo, diasExcluidos, festivos, ahora)
  if (!ancla) return 0
  const hoy = inicioDiaColombia(ahora)
  ...
  if (esAbiertoConDevengo(prestamo)) {
    return Math.max(0, Math.round((hoy - ancla) / 86400000))
  }
  ...
  const diasAtraso = calcularDiasTranscurridosCobrables(ancla, manana, diasExcluidos, festivos)
 ...
 *
 * ⚠ DOS MODOS, Y EL DE SIEMPRE ES EL QUE VIENE SIN NADA (21 sep 2026).
 *
 *   · sin opciones — lo que el cliente DEBE de capital: `montoPrestado` entero.
 *     Lo usan quienes deciden plata: el mínimo para renovar, el cierre
 *     anticipado, el interés cobrable y el que se devenga en un abierto. No cambia.
 *   · `{ paraReparto: true }` — tu plata que sigue en la calle, para INFORMAR.
 *     Sin el interés que se arrastró de una renovación (nunca salió de la caja) y
 *     con el total pactado si una renovación reescribió el de la deuda. Es la
 *     gemela de `capitalEnCalle` en reparto.js; las dos dicen lo mismo.
 *
 * Se eligió así a propósito: si a una pantalla que informa se le olvida la
 * opción, enseña la cifra de antes; si la opción fuera la de por defecto y a
 * quien decide una deuda se le olvidara quitarla, cambiaría lo que se cobra.
 */
export function calcularCapitalRestante(prestamo, { paraReparto = false } = {}) {
  const montoBase = paraReparto ? capitalDelPrestamo(prestamo) : prestamo?.montoPrestado
  /* ⚠ Para informar, un capital de verdad en CERO es un dato, no un hueco: la
     cartulina de quien financió un saldo que ya era todo interés. Con `null`, los
     que suman caen al `?? p.montoPrestado` y contaban la deuda entera como plata
     tuya en la calle. */
  if (paraReparto && !(montoBase > 0) && Number(prestamo?.montoPrestado) > 0) return 0
  ...
  /* ⚠ Y EL PAGO DECLARADO COMO SOLO INTERÉS TAMPOCO SE REPARTE.
   *
   * `repartirPagado` ya lo sacaba —está declarado, no se adivina nada— y esta
   * función nunca se enteró: metía ese dinero en el reparto y le bajaba el
   * capital al préstamo. O sea que la FICHA daba por devuelto un capital que
   * nadie devolvió, mientras los informes contaban el correcto.
   *
   * Medido contra producción el 27 ago 2026: 17 préstamos vivos en 10 negocios,
   * $859.157 de capital que la ficha se comía. El mayor: prestó $1.000.000, le
   * pagaron $142.857 de puro interés, y su ficha decía que ya le habían
   * devuelto esa parte del capital. */
  const soloInteres = (prestamo.pagos ?? [])
  ...
  if (tieneTablaAmortizacion(prestamo)) {
    ...
    const deLaTabla = interesAcumuladoTabla(prestamo.cuotasAmortizacion, aRepartir)
    /* Para informar, lo que la tabla llama capital lleva dentro el interés que
       arrastró de una renovación: la misma cuenta que `repartirPagado`. */
    const arrastrado = paraReparto ? Math.round((aRepartir - deLaTabla) * fraccionArrastrada(prestamo)) : 0
    const capitalDevuelto = aRepartir - deLaTabla - arrastrado + abonos
  ...
  /* ⚠ Y LA FRACCIÓN TAMPOCO MIRA EL INTERÉS COBRADO APARTE.
   *
   * Es la trampa de los $7.311 que está explicada entera en `repartirPagado`:
   * cobrar interés suelto sube `totalAPagar`, y con el total subido los pagos
   * VIEJOS se vuelven a repartir con una fracción que no era la suya. Capital
   * que sigue en la calle aparece como devuelto sin que nadie devolviera nada.
   *
   * Allí se arregló en agosto y aquí no, y esta copia es la que pinta la ficha
   * del préstamo. Se descuenta de los dos lados, igual que allá. */
  const totalAPagar = (paraReparto ? (totalDelReparto(prestamo) || montoBase) : (prestamo?.totalAPagar ?? montoBase)) - soloInteres
...
/* ══ LO QUE SE LIQUIDA AL RENOVAR: UNA SOLA CUENTA PARA TODOS ════════════════
 *
 * ⚠ ESTO ESTABA EN DOS SITIOS Y NO DECÍAN LO MISMO. El servidor de renovar lo
 * arregló el 5 ago 2026 con el caso de PRESTA MIL; la PANTALLA se quedó con la
 * regla vieja —usar el capital siempre que hubiera uno— y desde entonces
 * enseñaba una entrega inflada que el servidor luego no registraba.
 *
 * Lo cazó un prestamista el 9 sep 2026, y su cuenta era la correcta:
 *
 *   «el crédito de 500, me le gané 200 en intereses y entonces tiene un saldo
 *    de 200. Entonces yo cojo los 500, le resto el saldo que tiene y lo que
 *    queda es para entregarle al cliente, sería 300. Y en el sistema me hace
 *    un ejemplo 350 o 370»
 *
 * Los 350 salen del reparto proporcional: de los 500 que ya pagó, el 71,4 % se
 * imputa a capital, así que «capital pendiente» son 142.857 y la pantalla
 * ofrecía entregar 357.143. Medido sobre la cartera entera: **5.224 de 6.032
 * préstamos activos (86,6 %) en 263 negocios** enseñaban una cifra distinta de
 * la que el servidor iba a registrar.
 *
 * LA REGLA, que ahora vive AQUÍ y solo aquí:
 *
 *   · SIN tabla de amortización → se liquida LA DEUDA. Es lo que el cliente
 *     pactó devolver y todavía no ha devuelto, y es lo que el cobrador lleva
 *     en la cartulina. Son el 94 % de la cartera.
 *   · CON tabla → manda el capital: ahí el interés futuro no está devengado y
 *     cobrarlo al renovar sería cobrar un interés que nunca corrió.
 */
export function minimoParaRenovar(prestamo) {
  const capital = calcularCapitalRestante(prestamo)
  return tieneTablaAmortizacion(prestamo) && capital != null
    ? capital
...
export function calcularCuotasEnMora(prestamo, diasExcluidos = [], festivos = []) {
  ...
  const porDias = Math.max(1, Math.ceil(diasMora / diasPeriodo))
  /* ⚠ NUNCA MÁS CUOTAS VENCIDAS DE LAS QUE QUEDAN POR PAGAR.
   *
   * `porDias` cuenta los PERÍODOS que han pasado, y el plazo no es un tope: un
   * diario que lleva 340 días sin pagar decía «340 cuotas vencidas» debiendo 19,
   * y un semanal de 8 cuotas, «14 cuotas vencidas» debiendo 4. Al lado salía la
   * plata bien —`calcularMontoEnMora` ya va topado al saldo—, así que el mismo
   * renglón daba una cifra cierta y otra imposible: «95 días en mora · 14 cuotas
   * vencidas · $136.000», con cuota de $34.000.
   *
   * Medido el 19 sep 2026 contra el espejo: 960 de 1.974 préstamos en mora (49%)
   * contaban de más. Sale en la ficha, en la ruta, en «Pagar mora · N cuotas» y
   * en Cobros de hoy.
   *
   * La plata NO cambia: donde el tope aprieta, `cuotas × cuota` ya superaba el
   * saldo y `calcularMontoEnMora` se quedaba con el saldo igual. El pago único
   * no tiene cuota que dividir y tiene UNA sola que vencer. */
  const pendientes = calcularCuotasPendientes(prestamo)
  return Math.min(porDias, pendientes > 0 ? pendientes : 1)
}
...
export function interesCobrableAhora(prestamo) {
  ...
  return filasDelInteresCobrable(prestamo).reduce((a, f) => a + f.debe, 0)
}

/**
 * QUÉ FILAS SE LLEVA UN PAGO DE INTERÉS, Y CUÁNTO LE TOCA A CADA UNA.
 *
 * ⚠ LA COMPARTEN LAS DOS VÍAS, y por eso existe. Esta regla vivía escrita dos
 * veces y cada copia decía una cosa:
 *   · `interesCobrableAhora` —la que VALIDA el pago y SUGIERE el monto— suma
 *     las cuotas vencidas MÁS el interés de la que viene (lo pidió Crediya el
 *     14 ago: le pagaban la quincena por adelantado);
 *   · el reparto del servidor —el que APUNTA el pago en la tabla— saltaba toda
 *     cuota que aún no vencía.
 * O sea: el servidor cobraba el interés de la cuota que viene y no lo apuntaba
 * en ninguna fila. Cuando esa cuota vencía, la app lo pedía OTRA VEZ.
 *
 * Reportado el 21 sep 2026 por Inversiones Don Pacho: Adriana pagó $600.000 de
 * interés —la app le sugirió $360.000 tras un primer pago de $240.000—, y la
 * tabla solo apuntó $440.000. Los $160.000 de la cuota del 20 oct no estaban en
 * ninguna parte. Medido en producción ese día: 15 préstamos en 7 negocios,
 * $1.381.933 de interés cobrado sin apuntar.
 *
 * Orden: las vencidas primero, de la más vieja a la más nueva; luego LA
 * siguiente que aún no vence (una sola). Una fila con el capital ya cubierto no
 * cuenta: no hay cuota que pagar ahí.
 *
 * @returns {Array<{ numeroPeriodo: number, debe: number, vencida: boolean }>}
 */
export function filasDelInteresCobrable(prestamo, hoy = inicioDiaColombia()) {
  if (!tieneTablaAmortizacion(prestamo)) return []
  ...
  for (const fila of filas) {
    ...
    if (inicioDiaColombia(fila.fechaEsperada) <= hoy) {
      if (debe > 0) salida.push({ numeroPeriodo: fila.numeroPeriodo, debe, vencida: true })
      continue
    }
    ...
    if (!yaTomeLaSiguiente && debe > 0) {
      salida.push({ numeroPeriodo: fila.numeroPeriodo, debe, vencida: false })
      yaTomeLaSiguiente = true
    }
  }
  return salida
}
...
/**
 * LO INVERSO: cuánto le quita a cada fila BORRAR un pago de interés.
 *
 * Borrar un pago de interés no tocaba `interesPagado`: la cuota se quedaba con
 * el interés dado por pagado sin plata detrás, y la app dejaba de pedirlo. El
 * reparto va de la cuota más vieja a la más nueva, así que se deshace al revés,
 * desde la más nueva que tenga interés apuntado.
 *
 * @returns {Array<{ numeroPeriodo: number, quitar: number }>}
 */
export function deshacerPagoDeInteres(prestamo, monto) {
  ...
  for (const fila of filas) {
    if (restante <= 0) break
    const tiene = Math.max(0, fila.interesPagado || 0)
    if (tiene <= 0) continue
    const quitar = Math.min(restante, tiene)
    salida.push({ numeroPeriodo: fila.numeroPeriodo, quitar })
    restante -= quitar
  }
  return salida
}
...
/**
 * Cuánto de un pago de interés va a cada fila. Lo que no quepa no se reparte:
 * el servidor ya rechaza un pago mayor que `interesCobrableAhora`, que es la
 * suma de estas mismas filas.
 */
export function repartoDelPagoDeInteres(prestamo, monto, hoy = inicioDiaColombia()) {
  let restante = Math.max(0, Number(monto) || 0)
  const salida = []
  for (const f of filasDelInteresCobrable(prestamo, hoy)) {
    if (restante <= 0) break
    const aplicar = Math.min(restante, f.debe)
    salida.push({ numeroPeriodo: f.numeroPeriodo, aplicar })
    restante -= aplicar
  }
  return salida
}
...
// tan importado.
export const PERIODOS_POR_MES = { diario: 30, semanal: 4, quincenal: 2, mensual: 1 }
 ...
 */
/* ⚠ `ahora` ES UN ARGUMENTO, como en `calcularDiasMora` y sus hermanas.
 *
 * No lo era, y su prueba se lo pasaba igual —cuarto argumento, ignorado en
 * silencio—: pasaba en verde porque la cuenta con la fecha REAL coincidía con
 * la esperada, y se puso roja sola el 15 de septiembre de 2026 al cambiar el
 * mes. Una función de dinero que no se puede fijar en el tiempo no se puede
 * probar: solo se sabe si acierta el día que se mira. */
export function calcularProximoCobro(prestamo, diasExcluidos = [], festivos = [], ahora = Date.now()) {
  ...
  } else if (freq === 'quincenal' && (anclaMes || Number.isInteger(prestamo?.diaCobroMes2))) {
    /* ⚠ EL AVISO DE ARRIBA SE CUMPLIO, Y EN QUINCENAL.
     *
     * `fechaBase` es `fechaInicio + N x 15 dias` y luego `aplicarDiaAncla` la
     * empuja al dia de cobro. Eso NO es el calendario del prestamo: se salta el
     * primer cobro entero cuando el dia de corte cae antes de que pasen los 15
     * dias, que es el caso normal.
     *
     * El del cliente, al peso: presta el 10 de agosto y cobra el 16 y el 31.
     *
     *   lo pactado          16 ago -> 31 ago -> 16 sep -> 30 sep
     *   lo que decia        31 ago -> 16 sep -> 30 sep -> 16 oct
     *
     * El 16 de agosto no existia. Por eso, al pagar la cuota vencida, el
     * comprobante saltaba al 16 de SEPTIEMBRE y el prestamista lo leyo como que
     * el sistema le anulaba la del 31: «Cuando esta pagando es la anterior y la
     * del 31 la anula». No la anulaba — nunca la habia tenido.
     *
     * `fechaDePeriodo` cuenta las apariciones REALES de los dias de cobro, y es
     * la que genera la tabla de los prestamos que la tienen y la que usa el
     * abierto. Los sin tabla iban por su cuenta. Ahora hay un solo calendario.
     *
     * MEDIDO EN PRODUCCION el 27 ago 2026: de los 93 quincenales vivos sin
     * tabla y sin fecha puesta a mano, 69 en 8 negocios tenian el cobro 15 o 16
     * dias tarde — una quincena justa.
     *
     * Mensual y semanal con dia de la semana NO se tocan: comprobado que sus
     * dos calendarios ya coinciden. Y diario se queda con `aplicarDiaAncla`,
     * que es quien sabe de dias sin cobro y festivos.
     *
     * ⚠ Y BASTA CON UNO DE LOS DOS DIAS. La primera version exigia `diaCobroMes`
     * y dejaba fuera al que rellena solo «Segundo cobro» —la pantalla lo
     * permite—: ese prestamo volvia al calendario viejo. Es lo que el
     * prestamista siguio viendo el 28 de agosto, un dia despues del arreglo. */
    proximoCobro = fechaDePeriodo(proximaCuotaNum, {
      fechaInicio: inicioMed,
      freq,
      diasPeriodo,
      diaCobroMes: prestamo.diaCobroMes,
      diaCobroMes2: prestamo.diaCobroMes2,
    })
  } else {
...
export function tienePeriodoEsperadoHoy(prestamo, hoySinCobro = false, diasExcluidos = [], festivos = []) {
  ...
  /* ══ EL DIA CERRADO NO BORRA LA CUOTA: LA CORRE AL SIGUIENTE ═════════════
   *
   * Esto solo lo miraba la frecuencia DIARIA. Un semanal cuyo ciclo caia en un
   * dia marcado sin cobro sumaba igual a «lo que toca cobrar hoy», con el
   * negocio cerrado, mientras la ficha de la ruta —que si respeta el dia sin
   * cobro— decia «0 cobros programados hoy». En la misma pantalla se leia
   * «$173.000 por cobrar hoy» al lado de «0 cobros programados hoy».
   *
   * Medido en el espejo un domingo (20 sep 2026): de 266 prestamos no diarios
   * que «tocaban hoy», 23 en 13 negocios caian en dia cerrado, $2.787.500.
   * Habia casos con `diasSinCobro [0,1,2,3,4,5]` —solo se cobra el sabado— que
   * aun asi decian que hoy tocaba.
   *
   * El dueño, 20 sep 2026: «se mueve al dia siguiente, es lo logico». Y es lo
   * que el sistema YA hace con los festivos en `calcularProximoCobro`, que los
   * salta hacia adelante en cualquier frecuencia.
   *
   * Asi que:
   *   · hoy cerrado            → hoy no vence nada (la cuota espera);
   *   · hoy abierto            → vence lo de hoy Y lo que cayo en los dias
   *                              cerrados que vienen justo antes.
   *
   * La cuota NO se pierde de la meta: aparece el primer dia en que de verdad
   * se puede cobrar. El limite de 7 dias hacia atras es el tamaño de la semana:
   * mas alla no hay racha de dias cerrados posible (siete dias cerrados serian
   * un negocio que no cobra nunca). */
  if (hoySinCobro) return false

  const cerrado = (fecha) => (
    (diasExcluidos?.length > 0 && diasExcluidos.includes(fecha.getUTCDay()))
    || esFestivo(fecha, festivos)
  )

  const candidatos = [hoy]
  for (let i = 1; i <= 7; i++) {
    const antes = new Date(hoy.getTime() - i * DAY_MS)
    if (!cerrado(antes)) break
    if (antes < inicio) break
    candidatos.push(antes)
  }

  return candidatos.some((dia) => tocaPorCalendario(prestamo, prestamoSinPagos, inicio, dia, freq, diasExcluidos, festivos))
}

/**
 * ¿`dia` es una fecha de cobro del calendario de este prestamo?
 *
 * Es el cuerpo que `tienePeriodoEsperadoHoy` tenia dentro, sacado aparte para
 * poder preguntarlo por una fecha que no sea hoy — es lo que hace falta para
 * arrastrar la cuota de un dia cerrado al siguiente dia cobrable.
 */
function tocaPorCalendario(prestamo, prestamoSinPagos, inicio, dia, freq, diasExcluidos, festivos) {
  // No diaria: `dia` debe ser EXACTAMENTE un dia de cobro del calendario.
  const diasPeriodo = obtenerDiasPorPeriodo(freq)

  // Mensual: toca cobrar si es el dia ancla del mes, recortado al ultimo dia
  ...
  // El guardia `dia > inicio` mantiene la REGLA CLAVE de arriba: el primer cobro es
  ...
  if (prestamo.diaCobroMes != null && prestamo.diaCobroSemana == null) {
    const diaDelMes = dia.getUTCDate()
    if (diaDelMes === Number(prestamo.diaCobroMes)) return true
    if (prestamo.diaCobroMes2 != null && diaDelMes === Number(prestamo.diaCobroMes2)) return true
    return false
  }
  ...
  if (prestamo.diaCobroSemana != null && (freq === 'semanal' || freq === 'quincenal')) {
    ...
    const primerCobro = calcularProximoCobro(prestamoSinPagos, diasExcluidos, festivos)
    if (!primerCobro) return false
    const diffPrimero = Math.floor((dia.getTime() - inicioDiaColombia(primerCobro).getTime()) / DAY_MS)
    return diffPrimero >= 0 && diffPrimero % diasPeriodo === 0
  }

  // Sin ancla: ciclo regular desde fechaInicio. El primer cobro es a los
  // `diasPeriodo` dias. Toca si (dia - inicio) es multiplo positivo del periodo.
  const diff = Math.floor((dia.getTime() - inicio.getTime()) / DAY_MS)
...
export function calcularInteresMoratorio(prestamo, diasExcluidos = [], festivos = [], tasaMoratorio = 0, diasGracia = 5) {
  ...
  /* ⚠ LO QUE SE OFRECE ES LO QUE FALTA, NO EL ACUMULADO. El cálculo cubre
     todos los días de la mora desde que empezó; si ayer ya se aplicó el día 1,
     hoy se ofrece solo el día 2. Sin este descuento la pantalla volvía a listar
     a los mismos 8 préstamos con la misma cifra nada más aplicarles: en el
     espejo, tres pulsaciones del mismo lote cobraron $2.968 de mora por $855
     (8 sep 2026). Solo se descuentan los moratorios de ESTA racha. */
  const yaAplicado = moratorioYaAplicado(prestamo.pagos, fechaInicioMora(prestamo, diasExcluidos, festivos))
  const montoMoratorio = Math.max(0, Math.min(montoCalculado, tope) - yaAplicado)

  return { diasMoraEfectivos, montoBase, montoMoratorio, tope, yaAplicado, topado: tope > 0 && montoCalculado >= tope, aplicable: montoMoratorio > 0 }
}

/** Suma de los recargos de interés moratorio aplicados al préstamo desde
 *  `desde` (el inicio de la mora actual). Los reconoce por la nota con la que
 *  los escriben las dos pantallas del moratorio. */
export function moratorioYaAplicado(pagos = [], desde = null) {
  const limite = desde ? new Date(desde).getTime() : -Infinity
  let suma = 0
  for (const pg of pagos || []) {
    if (pg?.tipo !== 'recargo') continue
    if (!/^inter[eé]s moratorio/i.test(String(pg.nota || '').trim())) continue
    if (new Date(pg.fechaPago).getTime() < limite) continue
    suma += Number(pg.montoPagado) || 0
  }
 ...
 */
/* ══ LOS AJUSTES YA APLICADOS NO SON INTERÉS FUTURO ═══════════════════════
 *
 * Un recargo sube `totalAPagar` y un descuento lo baja, y ninguno de los dos
 * cuenta como pago. Así que el saldo ya los lleva dentro, pero el cierre
 * anticipado se calcula como `capital + interés devengado`: el recargo se caía
 * fuera y aparecía como interés perdonable.
 *
 ...
 * único «interés futuro» era el recargo de mora que él acababa de aplicar. O
 * sea: el sistema le ofrecía regalar el moratorio en el mismo cuadro donde se
 * lo acababa de cobrar. En producción, 210 préstamos activos con recargos en
 * 20 negocios, 35,3 millones de pesos expuestos.
 *
 * Es el mismo error que ya se corrigió para el préstamo abierto —«un descuento
 * inventado sobre plata ya devengada»—, en el otro camino. El interés de las
 * cuotas que NO han corrido se sigue perdonando: eso no lo debe nadie todavía.
 * Lo que ya se causó, se cobra. */
export function ajustesYaAplicados(prestamo) {
  let n = 0
  for (const pg of prestamo?.pagos ?? []) {
    if (pg?.tipo === 'recargo') n += Number(pg.montoPagado) || 0
    else if (pg?.tipo === 'descuento') n -= Number(pg.montoPagado) || 0
  }
  return Math.round(n)
}
...
export function calcularLiquidacionAnticipada(prestamo, fechaLiquidacion = new Date(), diasExcluidos = [], festivos = []) {
  ...
  /* Recargos menos descuentos ya aplicados. `totalAPagar` los lleva dentro, así
     que el cierre de hoy también tiene que llevarlos. */
  const ajustes = ajustesYaAplicados(prestamo)
  ...
  if (tieneTablaAmortizacion(prestamo)) {
    ...
    function modalidadTabla(interesDevengado, etiqueta, meses) {
      // + los recargos y − los descuentos que ya se aplicaron: son deuda causada,
      // no interés futuro. Ver `ajustesYaAplicados`.
      const totalCierre = capital + interesDevengado + ajustes
  ...
  function modalidad(meses, etiqueta) {
    ...
    // Mismo criterio que con tabla: lo ya ajustado se cobra.
    const totalCierre = capital + interesDevengado + ajustes
    const restanteHoy = Math.max(0, Math.round(totalCierre - totalPagadoReal))
    const interesPerdonado = Math.max(0, Math.round(saldoActual - restanteHoy))
    return {
