...
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio: fechaInicioRaw, frecuencia = 'diario', cuotaManual, modoInteres, redondeo, interesAdelantado = false, diaCobroMes, diaCobroMes2, capitalExtra, primerCobro: primerCobroImpuesto }) {
  ...
  if (modo === 'saldo') {
    ...
    const ultimaCuota = tablaAmortizacion[numPeriodos - 1]?.cuotaTotal || 0
    ...
    const ultimaDesencajada = numPeriodos > 1
...
export function cobradoHoy(prestamo) {
  const hoy = inicioDiaColombia().toISOString().slice(0, 10)
  return (prestamo?.pagos ?? []).reduce((suma, p) => {
    if (TIPOS_AJUSTE.includes(p.tipo)) return suma
    if (inicioDiaColombia(p.fechaPago).toISOString().slice(0, 10) !== hoy) return suma
    return suma + (Number(p.montoPagado) || 0)
  }, 0)
}
