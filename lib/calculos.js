...
export function calcularCapitalRestante(prestamo) {
  ...
  // Los abonos explicitos a capital bajan capital directo, sin repartirse: el
  // prestamista dijo «esto es capital».
  const abonos = Math.min(
    (prestamo.pagos ?? []).filter(p => p.tipo === 'capital').reduce((a, p) => a + (p.montoPagado ?? 0), 0),
    monto,
  )
  const aRepartir = Math.max(0, pagado - abonos)

  // El saldo pendiente es el techo: un prestamo no puede tener mas capital vivo
  // en la calle del que falta por cobrarle. Sin este tope, los 850 prestamos
  // cerrados por debajo de lo prestado daban aqui una cifra y otra en
  // `capitalEnCalle` — $1.547.682 contra $1.230.000 sobre el mismo prestamo.
  // El tope se aplica siempre que haya total pactado. Un saldo de CERO es un
  // dato, no un hueco: significa pagado del todo, y ahi no queda capital vivo.
  if (totalAPagar <= 0) return Math.max(0, Math.round(monto - aRepartir - abonos))

  const saldo = Math.max(0, totalAPagar - pagado)
  const conTope = (restante) => Math.max(0, Math.round(Math.min(restante, saldo)))
  ...
  // inventa pesos.
  const fraccion = Math.min(1, Math.max(0, (totalAPagar - monto) / totalAPagar))
  const capitalDevuelto = aRepartir - Math.round(aRepartir * fraccion) + abonos
  return conTope(monto - capitalDevuelto)
}
...
export function recalcularTablaDesdeSaldo({ saldoInicial, tasaInteres, numPeriodosRestantes, primerNumeroPeriodo, fechaBase, diasPeriodo }) {
  ...
  for (let i = 0; i < numPeriodosRestantes; i++) {
    ...
    fechaEsperada.setUTCDate(fechaEsperada.getUTCDate() + (i + 1) * diasPeriodo)
    tabla.push({
...
export function recalcularTablaSaldoDesdeSaldo({ saldoInicial, tasaInteres, numPeriodosRestantes, primerNumeroPeriodo, fechaBase, diasPeriodo, frecuencia }) {
  ...
  for (let k = 0; k < n; k++) {
    ...
    saldoRestante = Math.max(0, saldoRestante - capitalReal)
    const fechaEsperada = new Date(fechaBase)
    fechaEsperada.setUTCDate(fechaEsperada.getUTCDate() + (k + 1) * diasPeriodo)
    tabla.push({
