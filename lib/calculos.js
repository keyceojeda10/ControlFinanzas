...
function calcularDiasTranscurridosCobrables(inicio, fecha, diasExcluidos = []) {
  const diasCalendario = Math.floor((fecha - inicio) / DAY_MS)
  const diasDescontados = diasExcluidos.length > 0
    ? contarDiasExcluidos(inicio, fecha, diasExcluidos)
    : 0
...
export function calcularDiasMora(prestamo, diasExcluidos = []) {
  ...
  const diasTranscurridos = calcularDiasTranscurridosCobrables(inicio, ahora, diasExcluidos)
...
export function calcularProximoCobro(prestamo, diasExcluidos = []) {
  ...
  // Buscar la fecha cuyo "día cobrable transcurrido" coincida con la cuota objetivo.
