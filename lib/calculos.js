...
/**
 * Calcula la fecha del próximo cobro de un préstamo segun su frecuencia.
 * Para diario devuelve hoy si no ha pagado, o mañana si ya pago.
 * Para semanal/quincenal/mensual devuelve la fecha del proximo ciclo,
 * tomando en cuenta si ya pago en el ciclo actual.
 * Devuelve un Date (medianoche Colombia) o null si no aplica.
 */
export function calcularProximoCobro(prestamo) {
  ...
  // Hoy en Colombia (medianoche)
  const ahoraCol = new Date(Date.now() - 5 * 60 * 60 * 1000)
  const hoy = new Date(ahoraCol.getFullYear(), ahoraCol.getMonth(), ahoraCol.getDate())

  const inicio = new Date(prestamo.fechaInicio)
  const inicioMed = new Date(inicio.getFullYear(), inicio.getMonth(), inicio.getDate())

  // Aún no empieza
  if (inicioMed > hoy) return inicioMed

  // ¿Ya pagó hoy o en el ciclo actual?
  const yaPagoEnCicloActual = (() => {
    if (!Array.isArray(prestamo.pagos) || prestamo.pagos.length === 0) return false
    // Inicio del ciclo actual: el último día de cobro <= hoy
    const diasDesdeInicio = Math.floor((hoy - inicioMed) / 86400000)
    const ciclosCompletos = Math.floor(diasDesdeInicio / diasPeriodo)
    const inicioCicloActual = new Date(inicioMed.getTime() + ciclosCompletos * diasPeriodo * 86400000)
    return prestamo.pagos.some((p) => {
      if (TIPOS_AJUSTE.includes(p.tipo)) return false
      const fp = new Date(p.fechaPago)
      const fpCol = new Date(fp.getTime() - 5 * 60 * 60 * 1000)
      const fpMed = new Date(fpCol.getFullYear(), fpCol.getMonth(), fpCol.getDate())
      return fpMed >= inicioCicloActual && fpMed <= hoy
    })
  })()

  const diffDias = Math.floor((hoy - inicioMed) / 86400000)

  if (freq === 'diario') {
    return yaPagoEnCicloActual
      ? new Date(hoy.getTime() + 86400000) // mañana
      : hoy
  }

  // Frecuencias no diarias: si hoy cae en el ciclo y no ha pagado, es hoy
  if (diffDias % diasPeriodo === 0 && !yaPagoEnCicloActual) return hoy

  // Si no, el próximo ciclo
  const siguientePeriodo = yaPagoEnCicloActual
    ? Math.floor(diffDias / diasPeriodo) + 1
    : Math.ceil(diffDias / diasPeriodo)
  return new Date(inicioMed.getTime() + siguientePeriodo * diasPeriodo * 86400000)
}
