...
export function tieneTablaAmortizacion(prestamo) {
  return ['lineal', 'solo_interes', 'lineal_dinamico'].includes(prestamo?.modoInteres) && Array.isArray(prestamo?.cuotasAmortizacion) && prestamo.cuotasAmortizacion.length > 0
}
...
export function calcularPrestamo({ montoPrestado, tasaInteres, diasPlazo, fechaInicio, frecuencia = 'diario', cuotaManual, modoInteres, redondeo, interesAdelantado = false, diaCobroMes, diaCobroMes2, capitalExtra }) {
  ...
  if (modo === 'saldo') {
    ...
    // Con extras: iterar para hallar la cuota regular que amortiza el saldo
    // a cero en numPeriodos, considerando abonos extra de capital.
    // Busqueda binaria de la cuota regular optima.
    ...
    const cuotaRegular = ceil100((lo + hi) / 2)
    ...
    for (let n = 1; n <= numPeriodos; n++) {
      ...
      const capitalReal = esUltimo ? saldoRestante : Math.min(capitalTotal, saldoRestante)
...
export function calcularLiquidacionAnticipada(prestamo, fechaLiquidacion = new Date(), diasExcluidos = [], festivos = []) {
  ...
  if (tieneTablaAmortizacion(prestamo)) {
    ...
    // En lineal_dinamico la tabla se recalcula tras cada pago, asi que el
    // capital restante real se obtiene distribuyendo pagos efectivos por las
    // cuotas (interes primero, luego capital). Para lineal/solo_interes la
    // tabla es fija y el metodo original (capital - sum tabla) es correcto.
    let capitalRestante
    if (modo === 'lineal_dinamico') {
      ...
      capitalRestante = capital - capitalCubierto
      // El interes del proximo periodo debe recalcularse sobre el capital real
      if (primeraPendiente) {
      ...
      interesVencido = totalPagadoReal - (capital - capitalRestante)
    } else {
      capitalRestante = capital - capitalPagadoEnTabla
    }
    ...
    const intMesCompleto = modo === 'lineal_dinamico'
      ? (primeraPendiente ? primeraPendiente.interes : 0)
      : interesVencido + (primeraPendiente ? primeraPendiente.interes : 0)
    const intProporcional = modo === 'lineal_dinamico'
      ? (primeraPendiente ? interesProporcional : 0)
      : interesVencido + interesProporcional
