...
/** El cuerpo exacto que espera Meta, o `null` si no queda ningún botón válido
 *  (entonces el mensaje se manda como texto normal).
 *
 *  Está separado del envío para poder comprobar la FORMA sin mandar nada: si el
 *  payload va mal, Meta responde 400 y no manda absolutamente nada, así que el
 *  fallo se descubriría con un cliente delante. */
export function payloadDeBotones(phone, texto, botones = []) {
...
export async function sendButtons(phone, texto, botones = []) {
  const payload = payloadDeBotones(phone, texto, botones)
  /* Sin botones válidos esto es un texto normal, y mandarlo como interactivo
     sería un 400. Se degrada en vez de fallar: el mensaje importa más que el
     formato. */
  if (!payload) return sendText(phone, texto)
