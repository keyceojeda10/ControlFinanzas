...
//
// ⚠⚠ GENERACIÓN ANTERIOR. `FUNCIONES_SISTEMA`, de aquí abajo, NO es lo que el
// bot puede decir hoy: eso vive en `lib/bot-v2/producto.js`. Esta lista está
// incompleta (le faltan el Excel, Lucas IA, el simulador y los modos de
// interés) y quien la lea creyendo que es la buena diagnosticará mal — pasó el
// 1 sep 2026. Se conserva solo porque tres pruebas leen los topes de plan de
// este fichero.
...
/* ⚠ AQUÍ ESTABA `VIDEOS`, con los siete enlaces de YouTube y la lista de
   reproducción. Fuera: los videos son de marzo y enseñan la interfaz de antes
   del rediseño de julio.

   Este archivo ya solo lo alcanza `test-multiagente.js` —el agente vivo es
   `lib/bot-v2/agente`, que además tiene la regla escrita de no mandar videos—
   pero las URLs se quitan igual. Una constante exportada que sigue ahí es
   exactamente por donde vuelven. */
