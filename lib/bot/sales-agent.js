...
export async function responder(lead, historial, entrante) {
  ...
  function postProcesar(mensaje) {
    ...
    // Inyectar link de reset si menciona contraseña pero no tiene la URL
    if (/contrase[nñ]a|reset|recuperar.*cuenta|no puede entrar/i.test(msg) && !msg.includes('reset-password')) {
      msg += `\n\n${RESET_URL}`
    }
