...
export async function responder(lead, historial, entrante) {
  ...
  function postProcesar(mensaje) {
    ...
    // Limpiar caracteres no-latinos (chino, arabe, etc.) que DeepSeek a veces mezcla
    msg = msg.replace(/[^\x00-\x7F -ɏḀ-ỿ -⁯←-➿\u{1F300}-\u{1FAFF}]/gu, '')
    // Corregir "15 días" a "14 días"
    msg = msg.replace(/15 d[ií]as (de prueba |gratis|sin compromiso)/gi, s => s.replace('15', '14'))
    // Quitar nombres propios inventados ("soy Laura de Control Finanzas")
    msg = msg.replace(/soy \w+ de Control Finanzas/gi, 'soy Daniela de Control Finanzas')
    ...
    const botMencionaLink = /registr|inscrib|probar|prueb.*gratis|dias gratis|14 dias/i.test(msg)
    const botDiceAquiEsta = /aqu[ií].*(?:link|enlace)|te (?:dejo|comparto|env[ií]o).*(?:link|enlace)|link para|enlace para/i.test(msg)
...
export async function generarSeguimiento(lead, historial, numeroIntento = 1) {
  ...
  const estrategiaPorIntento = {
    ...
    2: 'Menciona una FUNCION CONCRETA del sistema que sea relevante al dolor del lead. Ejemplos reales: "el sistema le calcula la mora automatica", "cada cobrador cierra caja y usted ve el cuadre", "funciona sin internet". NUNCA inventes testimonios, cifras de recuperacion ni historias de otros clientes. Solo habla de lo que el sistema HACE.',
    3: `Ofrece VIDEOLLAMADA: "Si quiere, le agendamos una llamada de 15 min para mostrarle todo en vivo. Le parece?" Si acepta, marca escalar=true.`,
    4: 'Ultimo empujon suave. Menciona UNA funcion real que no hayas dicho: "el sistema funciona hasta sin internet" o "puede ver las ganancias del dia, semana y mes". Breve. NUNCA inventes datos ni testimonios.',
    5: 'Cierre final: "Quedo atento si en algun momento lo necesita." Deja la puerta abierta sin presion. No mandes link ni vendas.',
  }
  const estrategia = estrategiaPorIntento[numeroIntento] || estrategiaPorIntento[5]

  const instruccion = `Eres Daniela, asesora de Control Finanzas (sistema de cartera y cobros para prestamistas). Estas haciendo SEGUIMIENTO a un prospecto (${lead.nombre || 'prestamista'}) que no ha avanzado. Intento ${numeroIntento} de 5.
...
- Cant clientes: ${(lead.cantClientes || '').replace(/_/g, ' ') || 'no especificado'}
...
- PROHIBIDO INVENTAR: NUNCA escribas "un prestamista me conto", "un cliente nos dijo", "otro usuario", "X% de reduccion", historias de terceros ni cifras de rendimiento. SOLO puedes hablar de funciones reales del sistema. Si tu mensaje contiene una historia inventada, sera eliminado.
- NO digas que eres bot ni IA. Tu nombre es Daniela, usalo si te presentas.
- NO inventes funciones, monedas, cupones ni integraciones. Si no sabes, escala.
- Son 14 dias gratis, NUNCA digas 15.
- NUNCA le digas al lead "escribanos al..." ni le des un numero de telefono. Si necesita ayuda humana, dile "ya un asesor lo va a contactar".
...
Responde usando la herramienta seguimiento_lead.`
  ...
  function sanitizarMensaje(msg) {
    ...
    m = m.replace(/15 d[ií]as (de prueba |gratis|sin compromiso)/gi, s => s.replace('15', '14'))
    m = m.replace(/(?:un|otro|cierto) (?:prestamista|cliente|usuario)[\s\S]{0,200}(?:me (?:cont[oó]|dijo)|nos (?:cont[oó]|dijo)|me escribi[oó])[\s\S]{0,200}?[.!?]/gi, '')
    m = m.replace(/(?:me (?:cont[oó]|dijo)|nos (?:cont[oó]|dijo)) (?:que |un )[\s\S]{0,200}?[.!?]/gi, '')
    m = m.replace(/(?:reducir|ahorrar) (?:hasta |un )?\d+%/gi, '')
    m = m.replace(/soy \w+ de Control Finanzas/gi, 'soy Daniela de Control Finanzas')
    m = m.replace(/\n{3,}/g, '\n\n').trim()
    return m || 'Buenas, ¿qué tal? Quedo atento si necesita algo.'
  }
