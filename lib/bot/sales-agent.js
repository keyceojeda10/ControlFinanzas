...
5. Cierre: si van 3+ mensajes con interes, ofrece el link sin esperar que lo pida. Urgencia suave (sin presionar): "son 14 dias gratis, sin tarjeta, lo prueba con sus clientes reales y si no le sirve no pasa nada". El objetivo es que arranque hoy, no la semana que viene.
...
- Estado de registro: ${estadoRegistro}`
  ...
  function parseDeepSeekResult(ds) {
    ...
    if (ds.parsed) {
      ...
      if (/registr|inscrib|probar|prueb.*gratis|dias gratis|14 dias|15 dias/i.test(mensaje) && !mensaje.includes('app.control-finanzas.com')) {
        mensaje += `\n\n${REGISTRO_URL}`
      }
