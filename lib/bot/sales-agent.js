...
export async function generarSeguimiento(lead, historial, numeroIntento = 1) {
  ...
  const estrategiaPorIntento = {
    1: linkYaEnviado
      ? 'Ya le mandaste el link hace poco. Pregunta brevemente si se pudo registrar o si tiene alguna duda. NO repitas el link. Si necesita ayuda, ofrece el soporte: "Si necesita ayuda con el registro, puede escribir al ' + DATOS_SISTEMA.telefonoSoporte + ' y lo asisten en vivo." Maximo 2 lineas.'
      : (leadRespondio
...
- PROHIBIDO: testimonios inventados, cifras inventadas, historias de terceros, emojis decorativos (flores, corazones, estrellas). SOLO funciones reales.
- NUNCA uses un nombre propio de asesor (Daniela, Carlos, Lucas, Maria, etc). NO digas "soy Daniela" ni "soy [nombre]". Eres "del equipo de Control Finanzas" o simplemente habla directo sin presentarte.
- NUNCA corrijas al lead por decir "app" o "aplicacion".
- Ortografia perfecta: tildes y enes siempre.
- Tono WhatsApp colombiano real. Si el lead es coloquial, tu tambien.
