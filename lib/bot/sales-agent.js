...
export async function responder(lead, historial, entrante) {
  ...
  // Red de seguridad: si el bot explicó algo del sistema pero no cerró con prueba gratis, inyectar cierre
  const EXPLICA_SISTEMA = /sistema|calcula|automatico|registra|control|intereses|mora|ganancias|cobrador|cartera|prestamo/i
  const YA_CIERRA = /gratis|prueb|quiere.*link|quiere.*probarlo|mande.*link|registro\?r=2/i
  const ES_ESCALAMIENTO = /ya le pas[eé]|soporte|301 199/i
  function garantizarCierre(msg, escalar) {
    if (escalar) return msg
    if (yaRegistrado) return msg
    if (!EXPLICA_SISTEMA.test(msg)) return msg
    if (YA_CIERRA.test(msg)) return msg
    if (ES_ESCALAMIENTO.test(msg)) return msg
    return msg + `\n\nPuede probarlo ${DATOS_SISTEMA.diasPrueba} dias gratis, sin tarjeta. Quiere que le mande el link?`
  }

  // Red de seguridad: si escala y no incluyó el número de soporte, inyectarlo
  ...
  // Red de seguridad: detectar si debió escalar pero no lo hizo
  ...
  try {
    ...
    if (ds.parsed) {
      ...
      return forzarEscalamiento({
        mensaje: garantizarCierre(garantizarSoporte(postProcesar(o.mensaje), escalar), escalar),
...
export async function generarSeguimiento(lead, historial, numeroIntento = 1) {
  ...
  const estrategiaPorIntento = {
    1: linkYaEnviado
      ? 'Ya le mandaste el link. Pregunta si lo pudo abrir o si tiene duda. NO repitas el link. Si dice "ok" o "luego lo miro", ofrece un video corto: "Le mando un video de 1 minuto para que vea cómo funciona, sin tener que registrarse: ' + VIDEOS.primerosPasos + '"'
      : (leadRespondio
        ? 'Retoma desde lo último que se dijo. Aporta UN beneficio concreto del sistema.'
        : 'No ha respondido. Envía un video del sistema como gancho: "Le dejo un video de 1 minuto para que vea cómo funciona Control Finanzas: ' + VIDEOS.primerosPasos + '". NO repitas el mismo mensaje de antes.'),
    2: videoEnviado
      ? 'Ya mandaste video. Menciona UNA función real que NO hayas dicho y que resuelva un dolor concreto. Ejemplo: "Cada cobrador cierra caja al final del día y usted ve al instante si cuadra".'
      : 'Envía un video corto del sistema como gancho. Di "Le dejo un video para que vea cómo es por dentro:" y pon ' + VIDEOS.primerosPasos + '. Después cierra: "¿Quiere probarlo ' + DATOS_SISTEMA.diasPrueba + ' días gratis?"',
    3: videollamadaOfrecida
      ? 'Ya ofreciste videollamada. Menciona UNA función real nueva. Si todavía no mandaste video, manda uno: ' + VIDEOS.crearPrestamo
      : 'Ofrece videollamada: "Si quiere, le agendamos una videollamada de 15 min para mostrarle todo en vivo. ¿Le parece?" Si no acepta, tienes el siguiente intento.',
    4: 'Último mensaje de valor. Algo diferente a TODO lo que hayas dicho. Opciones: un video que no hayas mandado (' + VIDEOS.registrarPago + '), o una función nueva en 2 líneas máx.',
    5: 'Cierre suave: "Quedo atenta si en algún momento lo necesita." Sin presión, sin link, sin video.',
  }
  ...
  const instruccion = `Eres Daniela, asesora COMERCIAL de Control Finanzas (cartera y cobros para prestamistas). Tu único trabajo es VENDER. Seguimiento a ${lead.nombre || 'prestamista'}. Intento ${numeroIntento}/5.

Hora Colombia: ${fechaCol} (${franja}). Usa el saludo correcto.
Datos: método=${lead.metodoActual || '?'}, clientes=${(lead.cantClientes || '').replace(/_/g, ' ') || '?'}, temp=${lead.temperatura || 0}
${avisos}
...
- Máx 2-3 líneas. Nada de párrafos.
- Cada followup DIFERENTE al anterior. Lee lo que ya dijiste.
- Si no le interesa: darPorPerdido=true.
- 14 días de prueba (no 15).
- PROHIBIDO: "un prestamista me contó", "un cliente nos dijo", cifras inventadas, historias de terceros. SOLO funciones reales.
- Tu nombre es Daniela. No eres bot ni IA.
- NUNCA corrijas al lead por decir "app" o "aplicación".
- Ortografía perfecta: tildes y eñes siempre ("mañana", "señor", "información", NO "manana", "senor", "informacion").
