...
export async function responder(lead, historial, entrante) {
  ...
  // Post-procesamiento robusto: ultima linea de defensa contra alucinaciones
  function postProcesar(mensaje) {
    ...
    // 3. Normalizar nombre: cualquier "soy X de Control" -> "soy Daniela de Control"
    ...
    // 6. Eliminar numeros de telefono sueltos (3XX XXX XXXX o +57...)
    msg = msg.replace(/\b(?:\+?57\s?)?3\d{2}[\s.-]?\d{3}[\s.-]?\d{4}\b/g, '')
    msg = msg.replace(/(?:escriba?n?o?s?\s+al|contacte?n?o?s?\s+(?:al|en))\s+\S+/gi, match => {
      if (/control-finanzas/i.test(match)) return match
      return 'ya un asesor lo va a contactar'
    })

    // 7. Eliminar cupones inventados
    msg = msg.replace(/c[oó]digo\s+["']?\w+["']?/gi, '')
    msg = msg.replace(/cup[oó]n\s+["']?\w+["']?/gi, '')

    // 8. URLs truncadas o rotas
    ...
    // 9. Limpiar espacios y lineas vacias multiples
    msg = msg.replace(/\n{3,}/g, '\n\n').replace(/  +/g, ' ').trim()

    // 10. Link de registro: inyectar si el bot menciona registro/link pero no incluyo URL
    ...
    // Inyectar link de reset para soporte/cliente
    if (tipoAgente !== 'ventas' && /contrase[nñ]a|reset|recuperar.*cuenta|no puede entrar/i.test(msg) && !msg.includes('reset-password')) {
      msg += `\n\n${RESET_URL}`
    }

    // Anti-spam de link: no repetir si el bot anterior ya lo tenia (salvo que el lead lo pida)
    ...
    // Si despues de toda la limpieza quedo vacio, fallback seguro
    if (!msg.trim()) msg = 'Quedo atenta, me avisa si tiene alguna duda.'
...
export async function generarSeguimiento(lead, historial, numeroIntento = 1) {
  ...
  // Analizar historial para saber que ya se dijo
  ...
  const instruccion = `Eres Daniela, asesora de Control Finanzas (cartera y cobros para prestamistas). Seguimiento a ${lead.nombre || 'prestamista'}. Intento ${numeroIntento}/5.
...
- NUNCA des numeros de telefono. Para ayuda: "ya un asesor lo va a contactar".
...
Responde con la herramienta seguimiento_lead.`
  ...
  function sanitizarMensaje(msg) {
    ...
    m = m.replace(/\b(?:\+?57\s?)?3\d{2}[\s.-]?\d{3}[\s.-]?\d{4}\b/g, '')
    m = m.replace(/(?:escriba?n?o?s?\s+al|contacte?n?o?s?\s+(?:al|en))\s+\S+/gi, match => {
      if (/control-finanzas/i.test(match)) return match
      return 'ya un asesor lo va a contactar'
    })
