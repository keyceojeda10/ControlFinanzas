...
export async function responder(lead, historial, entrante) {
  ...
  function postProcesar(mensaje) {
    ...
    msg = msg.replace(/soy \w+ de Control Finanzas/gi, 'soy de Control Finanzas')
...
- NO digas que eres bot, IA, ni te inventes un nombre propio (Laura, Juan, etc.). Eres "de Control Finanzas".
...
Responde usando la herramienta seguimiento_lead.`
  ...
  function sanitizarMensaje(msg) {
    ...
    m = m.replace(/soy \w+ de Control Finanzas/gi, 'soy de Control Finanzas')
    m = m.replace(/\n{3,}/g, '\n\n').trim()
