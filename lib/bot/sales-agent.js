...
import { clasificar, KEYWORDS_PAGO } from './router'
...
function limpiarMensaje(msg) {
  ...
  // Quitar emojis decorativos (flores, corazones, estrellas, brillos, fuego)
  ...
  // Eliminar menciones de funciones que NO existen
  m = m.replace(/(?:tiendas?|corresponsales?|efecty|baloto|puntos? de pago|bancolombia|nequi|daviplata|PSE|pasarela)/gi, '')
  m = m.replace(/(?:firma (?:electr[oó]nica|digital)|geolocalizaci[oó]n|GPS del cobrador)/gi, '')
  m = m.replace(/(?:notificaci[oó]n(?:es)? autom[aá]tica|app store|play store|google play)/gi, '')

  // Eliminar nombres propios de asesor
  m = m.replace(/\bsoy (?:Daniela|Carlos|Lucas|Mar[ií]a|Andrea|Sofia|Juan|Pedro|Santiago|Camilo|Valentina)\b/gi, '')
  m = m.replace(/\b(?:mi nombre es|me llamo) \w+/gi, '')

  // Eliminar frases sobre "depende del numero de clientes"
  m = m.replace(/(?:depende|var[ií]a|seg[uú]n) (?:del? |la )?(?:n[uú]mero|cantidad) de clientes/gi, '')

  // Quitar markdown (negritas, cursivas, listas con viñetas)
...
export async function responder(lead, historial, entrante) {
  ...
  function forzarEscalamiento(resultado) {
    ...
    if (yaRegistrado && KEYWORDS_PAGO.some(k => textoLead.includes(k)) && !resultado.escalar) {
  ...
  function construirResultado(parsed, usage) {
    ...
    return forzarEscalamiento({
      ...
      enviarGuia: yaRegistrado ? guiaValida(o.enviarGuia) : null,
      usage,
    })
  }
...
export async function generarSeguimiento(lead, historial, numeroIntento = 1) {
  ...
  const estrategiaPorIntento = {
    1: linkYaEnviado
      ? 'Ya le mandaste el link. Pregunta brevemente si se pudo registrar o si tiene alguna duda. NO repitas el link. Si necesita ayuda: "Puede escribir al ' + DATOS_SISTEMA.telefonoSoporte + ' y lo asisten en vivo." Maximo 2 lineas.'
      : (leadRespondio
        ? 'Retoma desde lo ultimo que se dijo. Aporta UN beneficio concreto de la lista de funciones reales.'
        : 'No ha respondido. Envia un video como gancho: "Le dejo un video de 1 minuto para que vea como funciona Control Finanzas: ' + VIDEOS.primerosPasos + '". NO repitas el mismo mensaje de antes.'),
    2: videoEnviado
      ? 'Ya mandaste video. Menciona UNA funcion de la LISTA DE FUNCIONES REALES que NO hayas dicho.'
      : 'Envia un video corto: "Le dejo un video para que vea como es por dentro:" y pon ' + VIDEOS.primerosPasos + '. Cierra: "Quiere probarlo ' + DATOS_SISTEMA.diasPrueba + ' dias gratis?"',
    3: videollamadaOfrecida
      ? 'Ya ofreciste videollamada. Menciona UNA funcion REAL nueva de la lista. Si no mandaste video: ' + VIDEOS.crearPrestamo
      : 'Ofrece videollamada: "Si quiere, le agendamos una videollamada de 15 min para mostrarle todo en vivo. Le parece?"',
    4: 'Ultimo mensaje de valor. Algo diferente a TODO lo anterior. Un video que no hayas mandado (' + VIDEOS.registrarPago + ') o UNA funcion REAL en 2 lineas.',
  ...
  const instruccion = `Eres el asistente virtual de Control Finanzas (sistema de cartera y cobros para prestamistas). Seguimiento a ${lead.nombre || 'prestamista'}. Intento ${numeroIntento}/5.
...
- Recibos por WhatsApp listos para enviar
...
- GPS del cobrador, geolocalizacion
- Notificaciones automaticas a deudores
- Firma electronica
- Integracion con bancos
- App nativa en tiendas de apps
- Cualquier funcion que NO este en la lista de arriba
...
- ${DATOS_SISTEMA.diasPrueba} dias de prueba (NO 15).
- NUNCA inventes funciones. Si no esta en la lista, NO EXISTE.
- NUNCA uses un nombre propio (Daniela, Carlos, Lucas, Maria, etc). Habla directo sin presentarte.
...
- Ortografia perfecta: tildes y enes.
- Tono WhatsApp colombiano real.
- PROHIBIDO: testimonios inventados, cifras inventadas, emojis decorativos.
