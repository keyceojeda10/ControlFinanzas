...
//
// ⚠ NO SE DICE «llevamos más de dos años»: la primera organización es del 1 de
// marzo de 2026, o sea seis meses. Es la respuesta a «¿es confiable?» —la
// pregunta que MÁS convierte a pago (21 %)— y ahí una cifra que el cliente
// puede desmentir destruye justo lo que el mensaje viene a construir. Si la
// empresa lleva dos años con otro producto, se cambia `textoConfianza` y ya.
...
export function saludoDeAnuncio() {
  return {
    texto: `Hola, bienvenido a Control Finanzas, el sistema de cartera y cobros hecho para prestamistas.

Aquí llevas tus clientes, tus préstamos, tus cobradores y tus rutas desde el celular. Se acabaron el cuaderno y sumar de noche.
...
Tenemos videos tutoriales paso a paso para cada parte del sistema, y si prefieres que te ayude una persona, nuestro soporte técnico está en el ${SOPORTE}, ${HORARIO}.
...
Y si tu cartera está en el cuaderno, le tomas foto a las hojas y de ahí salen los clientes. Hasta 30 fotos de una vez.`,
      botones: [
        { id: 'cf_probar', titulo: 'Empezar gratis' },
        { id: 'cf_precio', titulo: 'Cuánto cuesta' },
...
Lo mejor es verlos mientras vas probando: son ${DIAS_PRUEBA} días gratis y no pedimos tarjeta.`,
      botones: [
        { id: 'cf_probar', titulo: 'Empezar gratis' },
        { id: 'cf_precio', titulo: 'Cuánto cuesta' },
        { id: 'cf_humano', titulo: 'Hablar con alguien' },
...
Entras, eliges tu plan y pagas con tarjeta o con Nequi. Si dejas puesto el cobro automático se te renueva solo cada mes y no tienes que volver a entrar a pagar.
...
Los primeros pasos están en video aquí: ${LINK_VIDEOS}${PREGUNTA_LIBRE}${POR_TELEFONO}`,
    ...
    cf_at_clientes: {
      texto: `Tu cartera no hace falta escribirla a mano.

En el menú entras a «Pasar mi cuaderno», le tomas foto a las hojas y de ahí salen los nombres y los montos. Tú revisas, corriges lo que esté mal y guardas. Hasta 30 fotos de una vez.

Está explicado en video aquí: ${LINK_VIDEOS}

Dime en qué parte se te quedó trabada y lo miramos.${POR_TELEFONO}`,
      botones: [
        { id: 'cf_videos', titulo: 'Ver los videos' },
        { id: 'cf_humano', titulo: 'Hablar con alguien' },
      ],
...
export function textoConfianza(datos) {
  ...
  const cabeza = negocios >= 100
    ? `Claro que sí. Más de ${miles(negocios)} prestamistas ya abrieron su cuenta con nosotros`
    : 'Claro que sí. Somos un equipo colombiano y el sistema se usa todos los días'
  const cola = prestamos >= 100
    ? `, y ahora mismo hay ${miles(prestamos)} préstamos vivos corriendo dentro del sistema.`
    : '.'
...
Y no hace falta que me creas: métele dos o tres clientes y míralo tú mismo. Son ${DIAS_PRUEBA} días gratis y no pedimos tarjeta.`
}
