...
/* ⚠ CADA PLAN SE VENDE POR PARA QUIÉN ES, NO POR CUÁNTO CUESTA.
 *
 * Una tabla de topes y precios no vende: obliga al cliente a adivinar cuál es
 * el suyo. Lo que engancha es reconocerse —«cobro yo solo», «ya tengo
 * cobradores»— y ver qué se le abre al subir.
 *
 * Los números salen de `lib/planes.js`; lo único escrito a mano es a quién le
 * sirve cada uno. Y el cuerpo tiene que caber en 1024 caracteres, que es el
 * tope de WhatsApp cuando el mensaje lleva botones: por eso se venden los tres
 * primeros y los de arriba se nombran en una línea. */
function textoPrecios(country = 'co') {
  const P = (k) => pesos(getPrecioPlan(k, country))
  const tope = (k) => PLANES_CONFIG[k].maxClientes.toLocaleString('es-CO')
  const arriba = PLANES_CONFIG.professional.maxClientes.toLocaleString('es-CO')
  return `Tenemos un plan para cada tamaño de negocio:

INICIAL — ${P('starter')} al mes
Hasta ${tope('starter')} clientes y una ruta. Para el que cobra solo y quiere por fin tener la cartera organizada de una vez.

BÁSICO — ${P('basic')} al mes
Hasta ${tope('basic')} clientes, y ya con reportes: cuánto entró, cuánto está en calle y cuánto ganaste de verdad.

CRECIMIENTO — ${P('growth')} al mes
Si ya tienes cobradores. Tu cuenta de administrador más la del cobrador, ${PLANES_CONFIG.growth.maxRutas} rutas y hasta ${tope('growth')} clientes. Y te abre Lucas, que te contesta «¿quién me debe más?» o «¿cuánto gané este mes?» y te arma el reporte solo.

De ahí para arriba hay planes hasta ${arriba} clientes y ${PLANES_CONFIG.professional.maxUsuarios} cuentas.

Los primeros ${DIAS_PRUEBA} días son gratis y no pedimos tarjeta.`
}
...
export function saludoDeAnuncio() {
  return {
    texto: `Hola, bienvenido a Control Finanzas, el sistema número uno de cartera y cobros para prestamistas en Latinoamérica.

Aquí llevas tus clientes, tus préstamos, tus cobradores y tus rutas desde el celular. Sin cuadernos y sin sumar de noche.
...
export function respuestaAtasco() {
  return {
    texto: `Entiendo. ¿En qué parte te quedaste?${TAMBIEN_POR_TELEFONO}`,
...
function respuestas(country) {
  return {
    ...
    cf_como: {
      ...
      texto: `Es sencillo de usar y hace más de lo que parece.

Registras tus clientes y tus préstamos, y el sistema calcula las cuotas, los intereses y la mora solo. Y como cada quien presta distinto, eliges el modo de interés que se ajusta a como prestas tú.

Cada pago le manda su comprobante por WhatsApp con un toque. Armas rutas por zona, le das cuenta propia a cada cobrador y ves quién pagó y cuánto se recaudó a medida que van cobrando.

Los reportes los bajas en PDF o en Excel cuando los necesites. Y funciona sin internet: el cobrador cobra igual y todo se sube cuando vuelve la señal.

Si tu cartera está en el cuaderno, le tomas foto a las hojas y de ahí salen los clientes. Hasta 30 fotos de una vez.`,
      botones: [
        { id: 'cf_probar', titulo: 'Empezar gratis' },
        { id: 'cf_precio', titulo: 'Cuánto cuesta' },
      ],
    ...
    cf_probar: {
      ...
      texto: `Listo. Abres tu cuenta aquí, sin tarjeta:

${LINK_REGISTRO}

Te toma dos minutos. Apenas entres te digo cómo pasar tu cartera sin escribirla.`,
      botones: [],
    },
    cf_humano: {
      texto: `Claro. Cuéntame qué necesitas y te respondo por aquí mismo.${TAMBIEN_POR_TELEFONO}`,
      botones: [],
...
En el menú entras a «Pasar mi cuaderno», le tomas foto a las hojas y de ahí salen los nombres y los montos. Tú solo revisas y guardas.

Dime en qué parte se te quedó trabada y lo miramos.${TAMBIEN_POR_TELEFONO}`,
      botones: [],
      avisar: 'se atascó cargando clientes',
    },
    cf_at_otra: {
      texto: `Cuéntame qué pasó y lo vemos por aquí mismo.${TAMBIEN_POR_TELEFONO}`,
      botones: [],
...
Tu información es tuya y te la puedes llevar cuando quieras, en PDF o en Excel.

Y no hace falta que me creas: métele dos o tres clientes y míralo tú mismo. No pedimos tarjeta.`
}
