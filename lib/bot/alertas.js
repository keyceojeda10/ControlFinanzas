...
/* ⚠ AQUÍ VIVÍA `alertarReporteDiario`, Y SE FUE EL 8 SEP 2026.
 *
 * Estaba escrita entera, con su endpoint en `api/cron/whatsapp-bot-report`… y
 * no estaba en el crontab del servidor. O sea que llevaba meses sin mandarse
 * una sola vez. Su bloque de A/B era además el de julio, el de los hooks, no
 * el del clasificador de intención, que es el que hay que vigilar ahora.
 *
 * Decisión del dueño al enterarse: «si se necesita y es superimportante lo
 * hacemos, si no lo quitamos». Lo que de verdad hacía falta —el A/B del v3—
 * va ahora en el mensaje de la autocrítica, que sí corre todas las noches
 * (`lib/bot-v2/aprendizaje.js`, `resumirAB`). La tasa de entrega tiene su
 * propia alerta, `alertarFallosEntrega`, que salta sola cuando cae. */
