...
// Plantillas segmentadas por metodoActual del formulario de Facebook.
// Cada una ataca el dolor especifico del lead sin repetir preguntas
// que ya respondio en el formulario.
const TEMPLATES_POR_METODO = {
  cuaderno_papel: 'contacto_cuaderno',
  libreta:        'contacto_cuaderno',
  excel_sheets:   'contacto_excel',
  app_basica:     'contacto_app',
  no_llevo_control: 'contacto_sin_control',
}
const TEMPLATE_FALLBACK = process.env.WHATSAPP_TEMPLATE_INICIAL || 'contacto_inicial'
...
export async function procesarNuevoLead(data) {
  ...
  // Seleccionar plantilla segun datos del formulario.
  // Si tenemos metodoActual, usamos una plantilla segmentada que va directo
  // al dolor del lead sin repetir la pregunta generica.
  const nombreVar = primerNombre(lead.nombre) || 'que tal'
  const metodo = lead.metodoActual || metodoActual || ''
  const templateName = TEMPLATES_POR_METODO[metodo] || TEMPLATE_FALLBACK
  ...
  if (wa.configurado() && botActivo) {
    ...
    try {
      await wa.sendTemplate(telefono, templateName, { nombre: nombreVar }, TEMPLATE_LANG)
      ...
      if (templateName !== TEMPLATE_FALLBACK) {
        ...
        try {
          ...
          await wa.sendTemplate(telefono, TEMPLATE_FALLBACK, { nombre: nombreVar }, TEMPLATE_LANG)
          await prisma.botConversacion.create({
            data: { botLeadId: lead.id, rol: 'bot', texto: textoFallback },
          })
          await prisma.botLead.update({
            where: { id: lead.id },
            data: { proximoSeguimiento: new Date(Date.now() + 1 * 24 * 3600000) },
...
function construirTextoPlantilla(templateName, nombre) {
  ...
  return textos[templateName] || textos.contacto_inicial
}
