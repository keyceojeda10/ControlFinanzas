...
function siguienteVariante() {
  const variante = abContador % 2 === 0 ? 'A' : 'B'
  abContador++
  return variante
}
...
export async function procesarNuevoLead(data) {
  ...
  const variante = siguienteVariante()
  ...
  const leadVariante = lead.abVariante || variante
  const templateName = AB_TEMPLATES[leadVariante] || TEMPLATE_FALLBACK
