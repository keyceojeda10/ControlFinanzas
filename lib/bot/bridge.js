...
import { APERTURAS_UTILITY, textoAperturaUtility } from '@/lib/bot-v2/respuestas-fijas'
...
/* ⚠ EL PRIMER MENSAJE ES DE UTILIDAD; EL HOOK VA DESPUÉS (7 sep 2026).
   `solicitud_recibida` (categoría UTILITY en Meta) confirma la solicitud y
   pide respuesta: se entrega a todos, también a los números del experimento
   130472 que no reciben marketing, y si contestan se abre la ventana de 24 h
   donde el pitch va como texto libre (`pitchTrasSolicitud`). A quien no
   contesta le llega `contacto_v2` a las 4 h (ver `elegirPlantilla` en
   lib/bot-v2/sender.js). La variable permite volver al hook directo sin
   desplegar: sin ella, todo sigue como antes. */
const TEMPLATE_HOOK = process.env.WA_TEMPLATE_PRIMER_CONTACTO || 'contacto_v2'
const esAperturaDeUtilidad = (nombre) => Object.hasOwn(APERTURAS_UTILITY, nombre)
  ...
  if (wa.configurado() && botActivo) {
    ...
    try {
      ...
      await prisma.botConversacion.create({
        data: { botLeadId: lead.id, rol: 'bot', texto: textoPlantilla, wamid: wa.wamidDe(envio), proveedor: 'plantilla', promptId: templateName, etapa: 'APERTURA' },
      })
      // Tras la apertura de utilidad, el hook de venta el MISMO día si no
      // contesta (4 h); tras el hook directo, como siempre, al día siguiente.
      const esperaMs = esAperturaDeUtilidad(templateName) ? 4 * 3600000 : 24 * 3600000
      await prisma.botLead.update({
        where: { id: lead.id },
        data: { proximoSeguimiento: new Date(Date.now() + esperaMs) },
      })
      ...
      if (templateName !== TEMPLATE_FALLBACK) {
        ...
        try {
          ...
          await prisma.botConversacion.create({
            data: { botLeadId: lead.id, rol: 'bot', texto: textoFallback, wamid: wa.wamidDe(envioFb), proveedor: 'plantilla', promptId: TEMPLATE_FALLBACK, etapa: 'APERTURA' },
          })
          await prisma.botLead.update({
            where: { id: lead.id },
...
function construirTextoPlantilla(templateName, nombre) {
  const textos = {
    // Las dos aperturas de utilidad, cada una con su texto real.
    ...Object.fromEntries(Object.keys(APERTURAS_UTILITY).map((k) => [k, textoAperturaUtility(nombre, k)])),
