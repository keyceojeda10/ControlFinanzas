...
const APERTURA_UTILITY = 'solicitud_recibida'
...
export async function procesarNuevoLead(data) {
  ...
  if (wa.configurado() && botActivo) {
    ...
    try {
      ...
      const esperaMs = templateName === APERTURA_UTILITY ? 4 * 3600000 : 24 * 3600000
      await prisma.botLead.update({
        where: { id: lead.id },
...
function construirTextoPlantilla(templateName, nombre) {
  const textos = {
    solicitud_recibida: textoAperturaUtility(nombre),
