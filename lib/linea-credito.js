...
// Meses transcurridos entre dos fechas (proporcional, base 30 dias).
//
// ── SE CUENTAN DIAS ENTEROS, NO MILISEGUNDOS ──
//
// La proporcion por milisegundos hacia que el interes empezara a correr al
// instante: un desembolso de $8.000.000 al 10% mostraba «Intereses pendientes
// $1,04» TREINTA Y CUATRO SEGUNDOS despues de registrarlo, y la cifra subia sola
// mientras se miraba la pantalla.
//
// Eso contradice al propio producto. La pantalla dice «Interes fijo mensual» y
// la ayuda de la opcion dice, literal, que «el interes se calcula sobre lo que
// el cliente debe AL DIA DEL CORTE». Un numero que cambia cada segundo no es lo
// que el prestamista va a cobrar, y si se lo enseña al cliente no cuadra.
//
// Contando dias enteros se conserva lo que la proporcion vino a arreglar —el dia
// del corte son 0 dias y el interes es 0, sin duplicarse sobre el ya cortado— y
// desaparece el ruido de menos de un dia. Ademas es como cuenta un gota a gota:
// por dias, no por relojes.
...
function calcularInteresFijoMensual(linea, desde, hasta, tasa) {
  ...
  // Proporcional al tiempo transcurrido: sin esto, el interes "del periodo actual"
  // valia un mes completo apenas se generaba un corte (desde=ahora, 0 dias) y se
  // duplicaba sobre el interes ya cortado. Tambien evita cobrar un mes el dia 1.
  return Math.round(saldoAlFinal * tasa * mesesEntre(desde, hasta))
}
...
function calcularInteresAlCorte(linea, desde, hasta, tasa) {
  ...
  return Math.round((saldoAnterior + desembolsosPeriodo) * tasa * mesesEntre(desde, hasta))
}
