...
/**
 * ¿Está en la prueba gratis o en un demo? Lo dice `planDemoHasta`: el registro
 * lo pone SIN `planOriginal` (solo el demo del panel pone los dos), así que
 * mirar `planOriginal` dejaba fuera la prueba de todos los registros.
 */
export function enPrueba(org, ahora = new Date()) {
  return Boolean(org?.planDemoHasta && new Date(org.planDemoHasta) > ahora)
}

/**
 * ¿Agregar se paga aparte, ya? Con un plan vigente, sí:
 *   · si lo pagó, siempre, también durante un demo del panel (si no, un cliente
 *     de Crecimiento con demo de Empresarial se ponía 50 gratis);
 *   · si es de $0, salvo en la prueba: una cortesía no regala adicionales,
 *     pero en la prueba se eligen para pagarlos con el plan.
 * (revisiones del 24 sep 2026)
 */
export function planVigente(ultima, org, ahora = new Date()) {
  if (!ultima || ultima.estado !== 'activa' || !ultima.fechaVencimiento) return false
  if (new Date(ultima.fechaVencimiento) <= ahora) return false
  if (ultima.montoCOP > 0) return true
  return !enPrueba(org, ahora)
}
...
export async function cambiarAdicionales({ organizationId, tipo, delta, ahora = new Date() }) {
  ...
  if (delta > 0 && e.cobrandoPlan) {
    /* El cobro en vuelo va con los de ahora y, al aprobarse, no recorta: uno
       subido gratis ahora se quedaría sin pagar. */
    return { error: 'Estamos cobrando tu plan en este momento. Intenta de nuevo en unos minutos.', status: 409 }
  }
...
export async function aplicarCompraAdicionales({ organizationId, plan, adicionales, montoCOP, gatewayId, referencia, country = 'co' }) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      /* Con el tope en la misma escritura: dos compras a la vez no pasan de
         50. Si no cabe, se deshace también el apunte del libro. */
      const suma = await tx.organization.updateMany({
        where: {
          id: organizationId,
          cobradoresAdicionales: { lte: MAX_ADICIONALES - cobradores },
          rutasAdicionales: { lte: MAX_ADICIONALES - rutas },
        },
        data: {
      ...
      if (suma.count === 0) throw Object.assign(new Error('tope'), { tope: true })
    ...
    if (err?.tope) return { ok: false, motivo: `Pasaría del tope de ${MAX_ADICIONALES}` }
    throw err
  }
}
