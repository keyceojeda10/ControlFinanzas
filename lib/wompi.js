...
export async function cobrarConFuente({ fuenteId, montoCOP, email, referencia }) {
  ...
  if (!res.ok) {
    const motivo = json?.error?.messages ? JSON.stringify(json.error.messages) : (json?.error?.reason ?? `HTTP ${res.status}`)
    /* `http` separa «la fuente no existe» (422: es un rechazo) de «llave mala»
       o «Wompi caído» (401 / 5xx: no es culpa del cliente). */
    return { ok: false, motivo, estado: null, id: null, http: res.status }
  }
  const d = json?.data ?? {}
  /* ⚠ NO SE ACTIVA NADA AQUÍ. La transacción nace PENDING y quien activa el
     plan es el webhook (o `reconciliar`, en lib/cobro-intento.js, si el webhook
     no llegó), los dos por `activarPlanPagado`, que no activa dos veces. */
  return { ok: true, id: d.id, estado: d.status, mensaje: d.status_message ?? null, motivo: null, http: res.status }
}

/**
 * La transacción creada con esta referencia, o null si Wompi no tiene ninguna.
 * Sirve cuando el proceso se cayó después de mandar el cobro y antes de apuntar
 * el id: sin esto no habría forma de saber si salió.
 */
export async function transaccionPorReferencia(referencia) {
  const priv = process.env.WOMPI_PRIVATE_KEY
  if (!priv) throw new Error('WOMPI_PRIVATE_KEY no configurada')
  const res = await fetch(`${WOMPI_BASE}/transactions?reference=${encodeURIComponent(referencia)}`, {
    headers: { Authorization: `Bearer ${priv}` },
  })
  if (!res.ok) throw new Error(`Wompi no contestó la consulta por referencia: HTTP ${res.status}`)
  const json = await res.json().catch(() => null)
  const lista = Array.isArray(json?.data) ? json.data : []
  return lista.find((t) => t?.reference === referencia) ?? null
}
...
/* ⚠ UN PAGO DEL PLAN DICE CON CUÁNTOS ADICIONALES SE COBRÓ: `mensualc2r1` es
   un mes con 2 cobradores y 1 ruta. Lo usan `activarPlanPagado` (los deja en
   lo pagado, nunca más) y «por revisar» (separa el plan de los adicionales).
   Las referencias viejas, sin eso, se siguen leyendo igual. */
export function referenciaDeCobro(orgId, plan, periodo = 'mensual', adicionales = null, { automatico = false } = {}) {
  /* La `a` final: cobro AUTOMÁTICO. Ése no recorta los adicionales al
     aprobarse (uno comprado aparte mientras iba en vuelo ya está pagado). */
  const conAdicionales = adicionales
    ? `${periodo}c${Math.max(0, adicionales.cobradores | 0)}r${Math.max(0, adicionales.rutas | 0)}${automatico ? 'a' : ''}`
    : periodo
  return `cf-${orgId}-${plan}-${conAdicionales}-${Date.now()}`
}

/* ══ LA COMPRA DE ADICIONALES VA EN LA MISMA REFERENCIA ═════════════════════
 *
 * Agregar cobradores o rutas con el plan ya pagado es un pago aparte (los días
 * que faltan para renovar). Su referencia lleva CUÁNTOS se compraron donde el
 * plan lleva el periodo: `cf-<org>-growth-adic1x0-<ts>` es 1 cobrador y 0 rutas.
 *
 * ⚠ Quien lea la referencia y vea `periodo: 'adicionales'` NO puede pasársela a
 * `activarPlanPagado`: le alargaría el plan 30 días por el precio de un
 * cobrador. El webhook la desvía antes (lib/adicionales.js). */
export function referenciaDeAdicionales(orgId, plan, { cobradores = 0, rutas = 0 } = {}) {
  return `cf-${orgId}-${plan}-adic${cobradores}x${rutas}-${Date.now()}`
}
...
export function leerReferencia(ref) {
  ...
  const adic = /^adic(\d{1,3})x(\d{1,3})$/.exec(periodo)
  if (adic) {
    return { orgId, plan, periodo: 'adicionales', adicionales: { cobradores: Number(adic[1]), rutas: Number(adic[2]) }, ts }
  }
  if (periodo.startsWith('adic')) return null
  const pagados = /^(mensual|trimestral|semestral|anual)c(\d{1,3})r(\d{1,3})(a?)$/.exec(periodo)
  if (pagados) {
    return {
      orgId, plan, periodo: pagados[1], ts,
      conAdicionales: { cobradores: Number(pagados[2]), rutas: Number(pagados[3]), automatico: pagados[4] === 'a' },
    }
  }
