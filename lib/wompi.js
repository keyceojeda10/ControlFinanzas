...
export function referenciaDeCobro(orgId, plan, periodo = 'mensual', adicionales = null) {
  const conAdicionales = adicionales
    ? `${periodo}c${Math.max(0, adicionales.cobradores | 0)}r${Math.max(0, adicionales.rutas | 0)}`
    : periodo
...
export function leerReferencia(ref) {
  ...
  const pagados = /^(mensual|trimestral|semestral|anual)c(\d{1,3})r(\d{1,3})$/.exec(periodo)
  if (pagados) {
    return { orgId, plan, periodo: pagados[1], conAdicionales: { cobradores: Number(pagados[2]), rutas: Number(pagados[3]) }, ts }
  }
