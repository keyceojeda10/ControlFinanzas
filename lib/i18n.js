...
// ── El número TAL Y COMO LO PIDE WHATSAPP ──────────────────────────────────
//
// ⚠ NO ES LO MISMO QUE `formatearTelefonoIntl`, y por eso está aparte.
//
// WhatsApp no identifica a la gente por su número de teléfono: la identifica
// por un número suyo que casi siempre coincide. En Argentina NO coincide.
//
// El móvil argentino se escribe con diez dígitos —código de área sin el 0 y
// abonado: `362 5325911`—, pero desde fuera del país lleva un **9** delante
// del área, y WhatsApp usa siempre la forma de fuera: `+54 9 362 5325911`.
// Sin ese 9 el enlace se abre y WhatsApp contesta «no es un número válido».
//
// Un cliente de Argentina lo reportó el 12 ago 2026, y con el fallo doble:
// nadie le pasaba el país, así que a sus clientes les pegábamos **+57**, el
// indicativo de Colombia. La captura decía literalmente
// «+573625325911 no es un número de teléfono válido».
//
// Si mañana entra otro país con una regla propia —México llevó un `1` durante
// años y ya no— se añade aquí y en ningún otro sitio.
const PREFIJO_WHATSAPP = {
  ar: '549',
}

/**
 * El número con el que se arma `wa.me/…`, o `null` si no cuadra con el país.
 *
 * El `null` es parte del contrato: es lo que permite a la pantalla apagar el
 * botón en vez de abrir un enlace que va a dar error. Un `wa.me` roto no falla
 * en la app —falla en WhatsApp, delante del cliente del prestamista—.
 */
export function telefonoParaWhatsApp(telefono, country = paisPorDefecto()) {
  if (!telefono) return null
  const cfg = getCountryConfig(country)
  const pais = cfg.phonePrefix.replace('+', '')
  const wa = PREFIJO_WHATSAPP[country] ?? pais

  let d = String(telefono).replace(/\D/g, '')
  if (!d) return null

  // Ya viene en la forma de WhatsApp: se devuelve tal cual.
  if (d.startsWith(wa) && d.length === wa.length + cfg.phoneDigits) return d

  // Viene internacional «normal» (+54 362…): se le mete el 9 que falta.
  if (d.startsWith(pais) && d.length === pais.length + cfg.phoneDigits) {
    return wa + d.slice(pais.length)
  }

  /* El 0 de marcar dentro del país no viaja: fuera se marca +593 99…, no
     +593 099… ⚠ Y ECUADOR Y VENEZUELA LO CUENTAN DENTRO DE SUS DÍGITOS —sus
     ejemplos son `0991234567` y `04121234567`—, así que exigir `phoneDigits` a
     secas los dejaba en `null`: el botón de WhatsApp apagado para dos países
     enteros, sin dar ningún error. Por eso vale también la regla del propio
     país, que es la que sabe si el 0 contaba o no.
     (`formatearTelefonoIntl` sigue con ese agujero; no se toca aquí porque es
     la de MARCAR y tiene sus propias pruebas.) */
  if (d.startsWith('0')) d = d.slice(1)
  if (d.length === cfg.phoneDigits || cfg.phoneRegex.test(d)) return wa + d
