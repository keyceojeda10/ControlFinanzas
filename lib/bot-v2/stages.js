...
export function detectarStage(historial, textoEntrante, yaRegistrado, lead) {
  ...
  if (!preguntaDeVerdad && CONFIRMACION_CORTA.test(crudo) && (botOfrecioPrueba || counts.total >= 5)) {
    return 'CIERRE'
  }
...
export function etapaDesdeIntencion(intencion, { historial = [], yaRegistrado = false, lead = {} } = {}) {
  ...
  switch (intencion.intencion) {
    ...
    case 'confirmacion':
      ...
      return { etapa: (botOfrecioPrueba || intencion.intencionCompra >= 70) ? 'CIERRE' : 'VALOR' }
