...
export function sanitizar(mensaje, textoLead = '') {
  ...
  // ⚠ ESTA GUARDA ES TEMPORAL Y HAY QUE RELAJARLA, PERO TODAVIA NO.
  //
  // El cobro recurrente con Wompi existe desde el 1 sep 2026: el cliente guarda
  // su tarjeta o su Nequi en /configuracion/plan y a partir de ahi se le cobra
  // sin volver a entrar. PERO la renovacion mes a mes la hace el cron
  // `/api/cron/cobro-recurrente`, que sigue APAGADO (falta
  // COBRO_RECURRENTE_ACTIVO=1 y programarlo). Mientras siga apagado, prometer
  // «se le cobra solo todos los meses» es prometer algo a medias, asi que la
  // oracion se sigue borrando entera.
  //
  // CUANDO EL CRON ESTE ENCENDIDO: quitar `cobro|pago` de la segunda linea y
  // `tarjeta` de la tercera. Lo que NO se toca nunca es el debito bancario: no
  // se le toca ninguna cuenta del banco, se le cobra al medio que el mismo
  // guardo. El bot llego a decirle a una lead "se debita de tu cuenta bancaria
  // todos los meses", que es la pregunta mas sensible del embudo y es falsa.
  //
