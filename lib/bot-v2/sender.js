...
import { esAperturaUtility } from './respuestas-fijas.js'
...
function elegirPlantilla(historial) {
  ...
  const mensajesBot = historial.filter(m => m.rol === 'bot')
  const textosBot = mensajesBot.map(m => (m.texto || '')).join(' ')
  const linkEnviado = textosBot.includes('app.control-finanzas.com/registro')

  if (linkEnviado) return TEMPLATE_POSTLINK
  // Solo le llegó la apertura de utilidad y no contestó: ahora sí, el hook.
  if (!leadRespondio && mensajesBot.length === 1 && esAperturaUtility(mensajesBot[0].texto)) return TEMPLATE_HOOK
