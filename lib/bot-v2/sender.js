...
import { REINTENTO_EN_VENTANA_MS, reintentarEnVentana, tipoDeSeguimiento, ordenarPorPrioridad, intencionDeClasificacion } from './cadencia.js'
import { esAperturaUtility, esRespuestaGenerica } from './respuestas-fijas.js'
...
const DESCRIPCION_PLANTILLA = {
  ...
  contacto_v2: '(hook de venta: le conte que registra el prestamo y el sistema le calcula cuotas, intereses, ganancias del dia y mora; que cada cobrador entra con su usuario y usted ve en tiempo real cuanto cobro; y que lo puede probar gratis 14 dias)',
...
// El hook de venta, para quien recibió la apertura de utilidad y no contestó
// (7 sep 2026). Es el mismo que manda lib/bot/bridge.js cuando abre directo.
const TEMPLATE_HOOK = 'contacto_v2'
const TEMPLATE_LANG = process.env.WHATSAPP_TEMPLATE_LANG || 'es'

// La decisión es pura y vive en cadencia.js; aquí solo se traduce a los
// nombres aprobados en Meta.
const PLANTILLA_POR_TIPO = {
  postlink: TEMPLATE_POSTLINK,
  hook: TEMPLATE_HOOK,
  descubrimiento: TEMPLATE_DESCUBRIMIENTO,
  lead: TEMPLATE_SEGUIMIENTO,
}

function elegirPlantilla(historial) {
  const tipo = tipoDeSeguimiento(historial, { esApertura: esAperturaUtility, esGenerico: esRespuestaGenerica })
  return PLANTILLA_POR_TIPO[tipo] || TEMPLATE_SEGUIMIENTO
}
...
export async function enviarSeguimientos(limite = 3) {
  ...
  /* Se piden más de los que caben y se queda con los que más valen (sprint 8).
     Antes iba solo por atraso y el lote se gastaba en quien nunca contestó
     mientras el que preguntó el precio ayer esperaba a la pasada siguiente. */
  const posibles = await prisma.botLead.findMany({
    where: {
      botActivo: true,
    ...
    take: Math.min(limite * 6, 90),
  })
  // La última intención de cada uno sale de la traza, en una sola consulta.
  const intencionPorLead = new Map()
  if (posibles.length > 1) {
    const trazas = await prisma.botConversacion.findMany({
      where: { botLeadId: { in: posibles.map((l) => l.id) }, rol: 'bot', clasificacion: { not: null } },
      select: { botLeadId: true, clasificacion: true, createdAt: true },
      orderBy: { createdAt: 'desc' },
      take: 400,
    })
    for (const t of trazas) {
      if (!intencionPorLead.has(t.botLeadId)) intencionPorLead.set(t.botLeadId, intencionDeClasificacion(t.clasificacion))
    }
  }
  const candidatos = posibles.length > limite ? ordenarPorPrioridad(posibles, intencionPorLead, limite) : posibles
      ...
      if (seg.mensaje) {
        ...
        if (abierta && leadRespondio) {
          ...
          await prisma.botConversacion.create({
            data: { botLeadId: lead.id, rol: 'bot', texto: seg.mensaje, wamid: wa.wamidDe(envio), ...(seg.traza || {}) },
          })
        } else {
          ...
          await prisma.botConversacion.create({
            data: {
              ...
              proveedor: 'plantilla', promptId: template, etapa: 'SEGUIMIENTO',
            },
          })
        }
      }
