...
describe('de la intención a la etapa', () => {
  ...
  it('una confirmación solo cierra si el bot ofreció la prueba o la intención de compra es alta', () => {
    const pregunta = [{ rol: 'bot', texto: '¿Sabe cuánto le deben hoy?' }]
    expect(etapaDesdeIntencion(I('confirmacion'), { historial: pregunta }).etapa).toBe('VALOR')
    expect(etapaDesdeIntencion(I('confirmacion'), { historial: conLink }).etapa).toBe('CIERRE')
    expect(etapaDesdeIntencion(I('confirmacion', { intencionCompra: 85 }), { historial: pregunta }).etapa).toBe('CIERRE')
  })
