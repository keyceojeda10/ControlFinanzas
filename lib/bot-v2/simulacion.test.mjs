...
suite('Simulación de flujos reales del bot', () => {
  ...
  it('10) [FIX 26-ago] Un «Sí» a la pregunta de deuda NO salta al link: explora', async () => {
    ...
    // → el bot ofrecía el link sin explorar. Ahora debe seguir explorando
    // (preguntar volumen, cobradores, etc.) y NO mandar el link todavía.
    estado.registrado = false
    ...
    const m = (d.mensaje || '').toLowerCase()
    // No debe mandar el link de registro ni ofrecer la prueba todavía.
    expect(m).not.toContain('app.control-finanzas.com/registro')
    // Debe hacer otra pregunta de descubrimiento o dimensionar.
