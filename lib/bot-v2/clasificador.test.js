...
describe('clasificador — preguntas operativas de REGISTRADOS van a soporte', () => {
  const reg = { yaRegistrado: true }

  it('escala el "cómo/dónde hago X" del sistema', () => {
    expect(tipo('como cierro la caja del dia', reg)).toBe('escalar')
    expect(tipo('donde registro un abono a capital', reg)).toBe('escalar')
    expect(tipo('como le asigno clientes a un cobrador?', reg)).toBe('escalar')
    expect(tipo('puedo cambiar la fecha de un prestamo?', reg)).toBe('escalar')
  })

  it('escala las preguntas de PLATA (donde equivocarse cuesta caro)', () => {
    expect(tipo('el interes se calcula sobre saldo o sobre el total?', reg)).toBe('escalar')
    expect(tipo('que pasa si un cliente paga de mas?', reg)).toBe('escalar')
    expect(tipo('como quito el saldo en negativo', reg)).toBe('escalar')
  })

  it('NO escala la charla normal de un registrado', () => {
    expect(tipo('listo, gracias', reg)).toBe('ventas')
    expect(tipo('ok perfecto', reg)).toBe('ventas')
    expect(tipo('buenos dias', reg)).toBe('ventas')
  })

  it('NO toca las conversaciones de PREVENTA (ahi sí se responde y se vende)', () => {
    expect(tipo('como funciona el sistema?')).toBe('ventas')
    expect(tipo('como se registran los pagos?')).toBe('ventas')
    expect(tipo('que pasa si un cliente no paga?')).toBe('ventas')
    expect(tipo('donde veo las ganancias?')).toBe('ventas')
  })
})

describe('[BOT v3 · sprint 2] lo que la traza de 733 turnos demostró que se escapaba', () => {
  it('«no pude iniciar» y sus hermanos son soporte duro, registrado o no', () => {
    for (const t of ['estoy intentando entrar para reiniciarme y no pude, no pude iniciar', 'no puedo ingresar', 'no me abre la sesión', 'se me cerró la cuenta']) {
      expect(clasificar(t).tipo, t).toBe('escalar')
    }
  })
  it('«vídeo» con tilde también pide tutoriales', () => {
    expect(clasificar('Me puedes enviar algun vídeo de cómo se hace').tipo).toBe('tutoriales')
    expect(clasificar('tienen videos?').tipo).toBe('tutoriales')
  })
})
