...
describe('stages — [FIX] un «Sí» a una pregunta de descubrimiento NO es compra', () => {
  ...
  /* ⚠ 8 sep 2026: ofrecer la prueba ya no basta. El pitch de apertura la
     menciona, así que un «sí» al segundo mensaje se llevaba el enlace sin que
     el lead hubiera contado nada de su negocio, que es el «cierre prematuro»
     que señalaron las tres auditorías y que el dueño resumió como «no vende».
     Ahora hace falta que el lead haya puesto algo suyo, o que el bot le haya
     preguntado derecho si le manda el enlace. */
  it('si el bot ofreció la prueba y el lead YA contó algo, el «dale» cierra', () => {
    const h = [
      { rol: 'bot', texto: 'Sabe cuanto le deben?' },
      { rol: 'lead', texto: 'no, sumo a mano cada noche' },
  ...
  it('pero si NO ha contado nada, el «dale» va a descubrir, no al enlace', () => {
    const h = [
      { rol: 'bot', texto: 'Le contamos como funciona? Son 14 dias gratis, sin tarjeta.' },
    ]
    expect(detectarStage(h, 'dale', false, {})).not.toBe('CIERRE')
  })

  it('y si el bot preguntó derecho si le manda el enlace, el «sí» es un sí', () => {
    // Caso real Lead-34: «¿Quiere que le mande el link?» → «Si».
    const h = [
      { rol: 'bot', texto: '14 dias gratis, sin tarjeta ni compromiso. Quiere que le mande el link?' },
    ]
    expect(detectarStage(h, 'Si', false, {})).toBe('CIERRE')
  })
