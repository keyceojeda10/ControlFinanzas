...
const VERBOS = [
  ...
  [/\b(?:traslad[a-z]*|mover|muev[a-z]*)\b/g, 'trasladar'], // «lo que pasa es que» NO es trasladar: es muletilla
...
const norm = (t) => canon(String(t || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim())
...
const PIDE_COMO = /\bcomo\b|\bc[oó]mo\b|\bdonde\b|\bd[oó]nde\b|\bse puede\b|\bpuedo\b|\bse hace\b|\bhago\b|\bhacer\b|\bme ayuda\b|\bme explica\b|\bme ense[ñn]a\b|\bpasos\b|\bmanera de\b|\bforma de\b|\bqu[eé] hago\b|\bque hago\b|\bno (?:se|s[eé]) (?:como|c[oó]mo)\b|\bensename\b|\bense[ñn]eme\b/i
...
const FRASES = Object.entries(GUIAS).map(([slug, g]) => ({
  ...
  frases: [...String(g.claves).split(',').map(norm), norm(g.titulo)]
    .filter(Boolean)
    .map((f) => ({ f, palabras: f.split(' ').filter((w) => w.length > 3 && !PALABRAS_VACIAS.has(w)) })),
}))
...
/* Palabra COMPLETA, no subcadena: con `includes` a secas, «registro» casaba
   ...
   se volvía elástico. */
const tienePalabra = (t, w) => new RegExp(`(?:^|\\s)${w}(?:s|es)?(?:$|\\s)`).test(t)
 ...
 */
export function elegirGuia(texto, { minimo = 4, paraRegistrado = true } = {}) {
  const t = norm(texto)
