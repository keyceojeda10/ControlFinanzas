 ...
 *  ⚠ SHA1 Y NO UN HASH CASERO: la medición se hace en SQL
 *  (`.auditoria/_ab-intencion.sh`) con `SHA1(l.id)`, y los dos lados tienen
 *  que repartir IGUAL o el A/B no mide nada. Último dígito hex par → semántica. */
export function varianteDe(leadId, modo = process.env.BOT_INTENCION || 'off') {
  ...
  return parseInt(hex.at(-1), 16) % 2 === 0 ? 'semantica' : 'regex'
}
