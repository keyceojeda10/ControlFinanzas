...
export const FUNCIONES = [
  ...
  // Verificado el 1 sep 2026 contra el codigo. Faltaban las tres y son
  // argumentos de venta: la importacion por Excel es la via natural para quien
  // ya tiene su cartera en un archivo, los modos de interes son LO que hace que
  // el sistema se adapte a como presta cada quien, y el simulador cierra ventas
  // en la calle.
  'Importar clientes desde un Excel o CSV (app/api/carga-masiva): sube el archivo que ya tenga y el sistema detecta las columnas solo, sin plantilla',
  'Modos de interes distintos para que el calculo se ajuste a como presta cada quien (lib/calculos.js, modoInteres)',
  'Simulador de cuota para ensenarle al cliente lo que pagaria, sin registrar nada (/prestamos/simulador)',
]
...
export const NO_EXISTE = [
  ...
  // ⚠ ESTA LISTA CAMBIO EL 1 SEP 2026, Y LA DISTINCION IMPORTA.
  //
  // El cobro recurrente con Wompi YA EXISTE: el cliente guarda su tarjeta o su
  // Nequi desde /configuracion/plan y a partir de ahi se le cobra solo. Asi que
  // «cobro automatico» dejo de ser mentira y salio de la lista — antes borraba
  // la frase entera del mensaje y dejaba un hueco.
  //
  // Lo que SIGUE siendo falso, y por eso se queda, es el DEBITO BANCARIO: no se
  // le toca ninguna cuenta del banco, se le cobra al medio de pago que el mismo
  ...
  //
  // ⚠ PENDIENTE: la renovacion mes a mes la hace el cron
  // `/api/cron/cobro-recurrente`, que a esta fecha esta APAGADO (falta
  // COBRO_RECURRENTE_ACTIVO=1 y programarlo). Mientras siga apagado, lo que
  // funciona es guardar el medio y el primer cobro; la renovacion automatica
  // no. No prometer «se renueva solo todos los meses» hasta encenderlo.
  ...
  // ⚠ AQUI ESTABA 'inteligencia artificial para el prestamista', Y SE
  // CONTRADECIA CON LA LISTA DE ARRIBA: Lucas IA esta en FUNCIONES desde que se
  // corrigio, pero `sanitizar()` BORRA del mensaje todo lo que casa con esta
  // lista. Comprobado pasando la frase por el sanitizador:
  //
  //   antes:   «Lucas es la inteligencia artificial para el prestamista que
  //             responde sobre su negocio.»
  //   despues: «Lucas es la que responde sobre su negocio.»
  //
  // O sea que el mensaje salia mutilado al cliente justo al nombrar el mayor
  // argumento de venta de los planes altos. Quitado el 1 sep 2026.
]
