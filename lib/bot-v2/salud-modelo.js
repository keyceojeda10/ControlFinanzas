...
export async function saludDelModelo(prisma, { minutos = 60, ahora = Date.now() } = {}) {
  ...
  /* ⚠ SE ACOTA POR LOS DOS LADOS. Con solo `gte` la ventana es «desde hace una
     hora hasta el fin de los tiempos», que en producción da igual —`ahora` es
     ahora— pero hace la función incomprobable contra el histórico: al apuntarla
     a un día de agosto contaba también todo lo que vino después y salía «sano»
     el día que estuvo caído. Lo cazó la comprobación contra el espejo. */
  ...
  const [porProveedor, mensajesDeLeads] = await Promise.all([
    ...
    prisma.botConversacion.count({ where: { rol: 'lead', createdAt: { gte: desde, lte: hasta } } }),
  ])
  ...
  if (principal === 0 && respaldo === 0) {
    return mensajesDeLeads >= MINIMO_PARA_CREERSELO
      ? { estado: 'mudo', principal, respaldo, mensajesDeLeads, minutos }
      : { estado: 'quieto', principal, respaldo, mensajesDeLeads, minutos }
  }

  if (principal === 0 && respaldo >= MINIMO_PARA_CREERSELO) {
    return { estado: 'respaldo', principal, respaldo, mensajesDeLeads, minutos }
  }

  return { estado: 'sano', principal, respaldo, mensajesDeLeads, minutos }
}

/** El aviso, en las palabras de quien lo va a leer a las tres de la mañana. */
export function avisoDelModelo(salud) {
  /* La ventana se dice, no se da por sabida: es un parámetro, y escribir «la
     última hora» a pelo deja el aviso mintiendo el día que se cambie. */
  const ventana = (salud.minutos ?? 60) === 60 ? 'la última hora'
    : (salud.minutos ?? 60) % 60 === 0 ? `las últimas ${(salud.minutos) / 60} horas`
    : `los últimos ${salud.minutos} minutos`

  if (salud.estado === 'mudo') {
    return '🔴 <b>El bot no está contestando</b>\n\n' +
      `Llegaron ${salud.mensajesDeLeads} mensajes de leads en ${ventana} y ` +
  ...
  return '🟡 <b>El bot está con el modelo de respaldo</b>\n\n' +
    `En ${ventana}: 0 llamadas al principal y ${salud.respaldo} al de reserva. ` +
