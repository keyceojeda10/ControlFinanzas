...
/* Los 20 vivos de la playlist «Tutoriales Control Finanzas V2».
...
export const VIDEOS = [
  ...
  { id: '1LHUf6P5GgQ', n: 2, titulo: 'Cómo instalar la app en el celular', dur: '1:12',
    /* ⚠ Solo INSTALAR. «¿Por dónde entro?» NO es este vídeo: es el enlace, y lo
     contesta `ESCALAMIENTO.acceso_registrado` sin gastar un turno del modelo. */
    claves: 'instalar app, descargar app, instalar la aplicacion, descargar la aplicacion, poner el icono, acceso directo, instalar en el celular, instalar en el computador, android, iphone, dejar la app en el telefono' },
  ...
  { id: 'ohsPvDXqca8', n: 19, titulo: 'Los buscadores: cómo encontrar cualquier cosa', dur: '2:04',
    claves: 'buscador, buscar, encontrar una pantalla, atajos' },
  /* ⚠ El 20 existe y está publicado, pero NO se manda a quien pregunta por
     dónde entra: eso es el enlace. Se queda para lo único que ni el enlace ni
     el vídeo de instalar cubren: recuperar la contraseña. */
  { id: 'yRFQesUKq08', n: 20, titulo: 'Cómo volver a entrar si cerraste la aplicación', dur: '1:10',
    claves: 'olvide la contraseña, no me acuerdo de la clave, recuperar la contraseña, cambiar la contraseña, perdi la clave, no recuerdo mi clave' },
  { id: 'feeT2GEhvdw', n: 21, titulo: 'Cómo asignarle una ruta a un cobrador', dur: '1:01',
  ...
  /* ⚠ `soloRegistrado`: a quien todavía no tiene cuenta NO se le manda cómo se
     paga. Es el único vídeo del que enseñar de más hace daño: en plena venta,
     contestar «así se paga» a quien no se ha registrado adelanta la caja antes
     de la prueba gratis. Al cliente registrado, en cambio, es lo que pide —34
     preguntas de 27 personas el 2 sep, el grupo más grande. */
  { id: 'pufODWUplzo', n: 22, titulo: 'Cómo pagar el plan: todas las formas', dur: '1:24', soloRegistrado: true,
    claves: 'como pago el plan, donde pago el plan, pagar la mensualidad, pagar la suscripcion, metodo de pago, medio de pago, cobro automatico, que se cobre solo, pagar yo cada mes, mensual trimestral o anual, cambiar de plan, subir de plan, mejorar el plan, ver mi plan, donde esta mi plan, con que tarjeta se paga, pagar con nequi' },
].map((v) => ({ ...v, url: url(v.id, v.desde), clave: `${v.id}#${v.desde || 0}` }))

/** El de pagar, que acompaña al enlace cuando un cliente dice que quiere pagar. */
export const VIDEO_PAGO = VIDEOS.find((v) => v.n === 22)
...
export function elegirVideo(texto, { registrado = false } = {}) {
  const t = norm(texto)
  if (!t || t.split(' ').length < 2) return null
  const clave = mejorCoincidencia(t, ENTRADAS, { minimo: 4 })
  const v = clave ? VIDEOS.find((x) => x.clave === clave) : null
  /* El que aún no tiene cuenta no recibe los de después de pagar. */
  return v && v.soloRegistrado && !registrado ? null : v
}
