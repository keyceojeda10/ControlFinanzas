...
import { VIDEOS, VIDEO_PAGO } from './videos.js'
...
export const ESCALAMIENTO = {
  ...
  intencion_pago_registrado: `Se paga desde su misma cuenta, en la sección de planes:\n\n${EMPRESA.linkPago}\n\nAhí elige si prefiere que se cobre solo cada mes o pagar usted cuando toque, y queda activo de una.\n\nSi lo quiere ver antes, aquí está paso a paso (${VIDEO_PAGO.dur}):\n\n${VIDEO_PAGO.url}\n\nY si prefiere que le ayuden, escriba al ${EMPRESA.telefonoSoporte}, de ${EMPRESA.horarioSoporte}.`,
...
/* ══ EL PITCH, EN A/B (9 sep 2026) ═════════════════════════════════════════
 *
 * El `largo` es el que aprobó el dueño y lleva desde el 7 sep. El `corto` sale
 * de lo único que está medido sobre qué convierte aquí: el test A/B de julio
 * con 194 leads, donde el **dolor directo** ganó 9,4 veces a la pregunta
 * amable, con MENOS respuestas (38 % contra 57 %) y más ventas. Filtrar es
 * bueno: contesta quien siente el problema.
 *
 * Y hay una razón estructural, medida sobre el propio texto: el largo **quema
 * seis funciones de golpe**, entre ellas cobradores y tiempo real. O sea que le
 * gasta su mejor argumento al que tiene cobradores ANTES de saber si los tiene.
 * Se vio en una conversación real del 9 sep: el bot le repitió tres veces «ve
 * al segundo lo que cobró cada uno» porque ya lo había dicho todo de entrada.
 *
 * ⚠ NO se sustituye a ciegas, se reparte: ese texto abre TODAS las
 * conversaciones y cambiarlo por corazonada es lo que ya salió mal una vez. El
 * reparto es el mismo mecanismo del A/B de intención —SHA1 del id del lead, la
 * misma mitad siempre— y la traza guarda cuál salió (`promptId: pitch:corto`),
 * así que a los pocos días se cuenta en SQL quién registra más.
 */
export function varianteDePitch(leadId, modo = process.env.BOT_PITCH || 'ab') {
  if (modo === 'corto') return 'corto'
  if (modo === 'largo') return 'largo'
  const hex = createHash('sha1').update(String(leadId || '')).digest('hex')
  return parseInt(hex.at(-1), 16) % 2 === 0 ? 'corto' : 'largo'
}

export function pitchTrasSolicitud({ nombre, diasPrueba = 14, variante = 'largo' } = {}) {
  const n = primerNombreLimpio(nombre)
  const saludo = n ? `Claro, ${n}.` : 'Claro.'

  /* Tres líneas: qué es, el dolor que convirtió 9,4×, y la pregunta que decide
     el argumento siguiente. Lo que se quita no se pierde: la prueba se ofrece
     cuando ya sienta el problema, y las funciones salen una a una según lo que
     conteste. */
  if (variante === 'corto') {
    return `${saludo} Usted registra el préstamo y el sistema le lleva las cuotas, los intereses y la mora. Sin sumar a mano.

La mayoría me dice «más o menos tengo tanto en la calle», pero nunca el número exacto. Aquí lo ve en dos segundos.
