...
function leadPideLink(texto) {
  ...
  if (/(?:mand|env[ií]|pas|comp[aá]rt)[aeiou]*(?:me|nos)\s*(?:el |un |de nuevo |otra vez )?(?:link|enlace|url|click)/i.test(t)) return true
  ...
  if (/(?:mand[ae]me|env[ií]ame|p[aá]same)\s+(?:el\s+)?(?:link|enlace|click)/i.test(t)) return true
...
function procesarLink(mensaje, yaRegistrado, historial, textoEntrante) {
  ...
  // Si el lead PIDE explicitamente el link, SIEMPRE incluirlo aunque ya se envio
...
export async function responder(lead, historial, entrante) {
  ...
  if (lead.metodoActual) {
    ...
    } else if (metodo.includes('excel') || metodo.includes('hoja')) {
