...
function detectarStage(historial, textoEntrante, yaRegistrado) {
  ...
  // Lead ya registrado: solo responder o escalar
  if (yaRegistrado) return 'POST_LINK'

  // Pregunta de precios
  if (/(?:cu[aá]nto|precio|costo|valor|plan(?:es)?|mensual|cuanto (?:vale|cuesta|sale))/i.test(texto)) {
    return 'PRECIOS'
  }
...
export async function generarSeguimiento(lead, historial, numeroIntento = 1) {
  ...
  const prompt = prompts.promptSeguimiento({
    ...
    linkEnviado, videoEnviado, videollamadaOfrecida, franja,
