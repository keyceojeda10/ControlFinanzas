...
import { respuestaEscalamiento, RECHAZO, TUTORIALES, esPrimerTurnoTrasApertura, esRespuestaGenerica, pitchTrasSolicitud } from './respuestas-fijas.js'
...
import { nivelDeSoporte, elegirGuia, yaSeMandoGuia, textoDeGuia } from './soporte.js'
...
export async function responder(lead, historial, entrante) {
  ...
  if (tipo === 'escalar') {
    ...
    if (yaRegistrado && (razon === 'soporte' || razon === 'soporte_registrado')) {
      ...
      // Una guía por conversación: cuatro capturas seguidas dos veces es spam.
      if (nivel === 'navegacion' && !yaSeMandoGuia(historial)) {
        const slug = elegirGuia(entrante.texto)
        if (slug) {
          return { mensaje: textoDeGuia(slug, EMPRESA.telefonoSoporte), enviarGuia: slug,
            temperatura: lead.temperatura || 50, escalar: false,
            usage: { modelo: 'fijo', tokensIn: 0, tokensOut: 0, costoUsd: 0 }, traza: FIJO('guia', { promptId: `guia:${slug}` }) }
        }
      }
      /* Con las capturas apagadas, el que contesta «cómo se hace tal cosa» es
         el VÍDEO. Los tutoriales se publicaron el 24 de agosto y sí enseñan la
         interfaz de hoy, comprobado fotograma a fotograma contra el espejo. */
  ...
  /* ══ SOPORTE EN TRES NIVELES AL QUE YA ES CLIENTE (BOT v3 · sprint 8) ══════
   *
   * El grupo más grande de lo que escribe la gente ya registrada (27 % de 204
   * preguntas medidas el 2 sep) es «cómo hago tal cosa». Para eso hay 35 guías
   * visuales con 119 capturas hechas por el equipo, publicadas y servidas, y
   * el webhook cableado para mandarlas: lo único que faltaba era esta decisión,
   * que devolvía `enviarGuia: null` a fuego desde que se escribió.
   *
   * Va DESPUÉS del escalamiento semántico y ANTES del prompt, y solo para
   * registrados. Las preguntas de plata y los fallos técnicos NO se contestan
   * aquí: van a una persona, que es la regla de oro de este sistema. */
  /* El cliente que pregunta «cómo se hace tal cosa» y NO cayó en el camino de
     soporte —«¿dónde descargo la app?» se clasifica como venta— también merece
     su respuesta. Guía si las capturas estuvieran al día; vídeo mientras no. */
  if (yaRegistrado && !yaSeMandoGuia(historial)) {
    const slug = nivelDeSoporte(entrante.texto, { yaRegistrado }) === 'navegacion' ? elegirGuia(entrante.texto) : null
    if (slug) {
      return { mensaje: textoDeGuia(slug, EMPRESA.telefonoSoporte), enviarGuia: slug,
        temperatura: lead.temperatura || 50, escalar: false,
        usage: { modelo: 'fijo', tokensIn: 0, tokensOut: 0, costoUsd: 0 }, traza: FIJO(`${clasificacionTraza}>guia`, { promptId: `guia:${slug}` }) }
    }
    const video = elegirVideo(entrante.texto)
    if (video) {
      return { mensaje: textoDeVideo(video, { registrado: true }), temperatura: lead.temperatura || 50, escalar: false,
  ...
  switch (stage) {
    ...
    case 'VALOR': {
      const linkEnviado = historial.some(m => m.rol === 'bot' && /app\.control-finanzas\.com\/registro/.test(m.texto || ''))
      prompt = prompts.promptValor({ nombre: lead.nombre, historial: hist, dolorDetectado, linkEnviado, contexto })
      break
    }
    case 'CIERRE':
