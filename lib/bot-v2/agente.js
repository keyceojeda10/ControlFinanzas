...
function detectarStage(historial, textoEntrante, yaRegistrado) {
  ...
  if (/(?:cu[aá]nto|precio|costo|valor|plan(?:es)?|mensual|cuanto (?:vale|cuesta|sale)|forma de pago|como (?:se )?paga|cómo (?:se )?paga)/i.test(texto)) {
    return 'PRECIOS'
  }

  // Objecion
  if (/(?:caro|costoso|muy caro|no tengo plata|pensarlo|voy a pensar|ya tengo|ya uso|me da miedo|desconfi|no s[eé] usar|dif[ií]cil)/i.test(texto)) {
    return 'OBJECION'
  }
  ...
  // Pocas interacciones: descubrimiento
  if (counts.total <= 3) return 'DESCUBRIMIENTO'

  // Confirmacion corta despues de valor = avanzar a cierre (no repetir)
  if (counts.total > 3 && CONFIRMACION_CORTA.test(textoEntrante?.trim())) {
    return 'CIERRE'
  }

  // Mas interacciones: valor o cierre
  if (counts.total <= 6) return 'VALOR'
