...
import { respuestaEscalamiento, RECHAZO, TUTORIALES, ACCESO, esPrimerTurnoTrasApertura, esRespuestaGenerica, pitchTrasSolicitud, varianteDePitch } from './respuestas-fijas.js'
...
export async function responder(lead, historial, entrante) {
  ...
  if (tipo === 'tutoriales') {
    ...
    const suyo = elegirVideo(entrante.texto, { registrado: yaRegistrado })
    if (suyo) {
      return {
  ...
  if (tipo === 'escalar') {
    ...
    if (yaRegistrado && (razon === 'soporte' || razon === 'soporte_registrado')) {
      ...
      const video = elegirVideo(entrante.texto, { registrado: true })
      if (video) {
  ...
  if (!['PRECIOS', 'OBJECION', 'POST_LINK'].includes(stage)
    ...
    /* En A/B desde el 9 sep: el largo que aprobó el dueño contra uno de tres
       líneas con el dolor que convirtió 9,4× en julio. La traza guarda cuál
       salió, así que a los pocos días se cuenta quién registra más. */
    const variante = varianteDePitch(lead.id)
    return {
      mensaje: pitchTrasSolicitud({ nombre: lead.nombre, diasPrueba: EMPRESA.diasPrueba, variante }),
      temperatura: Math.max(45, lead.temperatura || 0),
      escalar: false,
      usage: { modelo: 'fijo', tokensIn: 0, tokensOut: 0, costoUsd: 0 },
      traza: FIJO(clasificacionTraza, { etapa: stage, promptId: `pitch:${variante}` }),
    }
  }
  ...
  const ctxValidar = { textoLead: entrante.texto, contexto, stage, historial, linkPermitido, pais: lead.pais || lead.country || null, yaRegistrado }
...
export async function generarSeguimiento(lead, historial, numeroIntento = 1) {
  ...
  /* Un ángulo que no se le haya dado todavía. Regla 6 de la guía de ventas de
     marzo: «el follow-up no es rogar, es dar valor; cada mensaje de seguimiento
     debe tener un ángulo diferente». Y hace falta: de los 803 leads que
     contestaron en 90 días, 616 no se registraron. Si el seguimiento repite lo
     que ya dijo, no recupera a ninguno. */
  const ctxSeg = contextoDe(msgs, { lead, yaRegistrado: false })
  const libres = argumentosPara({ segmento: segmentoDe('', msgs), usados: ctxSeg.argumentosUsados || [] })
  const argumentoNuevo = libres.length
    ? `${libres[0].dolor}\n  · ${libres[0].coste}\n  · ${libres[0].prueba}`
    : null
