...
export async function responder(lead, historial, entrante) {
  ...
  if (!['PRECIOS', 'OBJECION', 'POST_LINK'].includes(stage)
    ...
    return {
      ...
      enviarGuia: null,
      usage: { modelo: 'fijo', tokensIn: 0, tokensOut: 0, costoUsd: 0 },
      traza: FIJO('ventas', { etapa: stage, promptId: 'pitchTrasSolicitud' }),
    }
  }
  ...
  if (!resultado) {
    return {
      ...
      traza: { etapa: stage, clasificacion: 'ventas', proveedor: 'ninguno', promptId: promptIdDe(stage, prompt, hist, lead.nombre, entrante.texto) },
    }
  }
  ...
  let latenciaMs = resultado._latenciaMs || 0
