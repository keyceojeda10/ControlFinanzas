...
export function promptValor({ nombre, historial, dolorDetectado, linkEnviado = false, contexto }) {
...
${linkEnviado ? 'El cierre ya se hizo: ahora toca CONVERSAR. Conecta UNA funcion con lo que acaba de decir o contesta su pregunta, y termina con una pregunta corta o un siguiente paso concreto.' : 'NO ofrezcas prueba gratis ni dias de prueba en esta etapa. NO envies link de registro. Solo conecta UNA funcion con su dolor. El cierre viene despues.'}
...
export function promptObjecion({ nombre, historial, objecion, contexto }) {
  return `Eres el asistente virtual de Control Finanzas. Conversacion con ${nombre || 'un prestamista'}.
...
export function promptSeguimiento({ nombre, historial, intento, metodo, clientes, linkEnviado, videollamadaOfrecida, leadRespondio, franja, ultimaIntencion = null }) {
  let estrategia
...
DE QUÉ VENÍA HABLANDO: ${RETOMAR[ultimaIntencion]}` : ''}
