...
/* ⚠ «se LE debita» se escapaba: el patrón pedía «se» pegado al verbo y la forma
   más natural en español mete el pronombre en medio («a su deudor se le debita
   la cuota»). Cazado el 9 sep 2026 escribiendo la prueba de la excepción.
   Va aparte porque el débito de una cuenta es falso SIEMPRE: al cliente se le
   cobra al medio de pago que él guardó, no se le toca ninguna cuenta. */
const RX_DEBITO_CUENTA = /\bse\s+(?:le\s+|les\s+)?d[eé]bit[ao]\b/i
const RX_COBRO_AUTO = /\b(?:cobro|pago|d[eé]bito)\b[^.\n]{0,30}\bautom[aá]tic[oa]\b|\bautom[aá]tic\w*[^.\n]{0,40}\b(?:cuenta\s+bancaria|tarjeta|banco)\b/i
/* ⚠ DOS «COBROS AUTOMÁTICOS» DISTINTOS, Y SOLO UNO ES MENTIRA ────────────────
 *
 *  · Que el sistema le cobre solo AL DEUDOR del prestamista —debitarle la
 *    cuenta, descontarle la cuota— NO existe. Prometerlo es la alucinación que
 *    más veces se ha visto, y sigue siendo violación dura.
 *  · Que el prestamista deje pagada SU suscripción sola cada mes SÍ existe
 *    desde el 1 sep 2026: Wompi tokenizado, `COBRO_RECURRENTE_ACTIVO=1` en el
 *    `.env` de producción y el cron de las 13:00 corriendo (comprobado el 9 sep
 *    2026, `candidatos:0` porque aún nadie guarda su medio de pago). Desde el 9
 *    sep tiene su vídeo, el 22. Marcarlo como invento hacía regenerar una
 *    respuesta CORRECTA y dejaba al bot contradiciendo al tutorial.
 *
 * La excepción es estrecha a propósito: solo con el cliente YA REGISTRADO y
 * solo si la frase nombra lo NUESTRO. «Su plan» a secas no vale: en boca de un
 * prestamista, el plan suele ser el de pagos de su deudor. */
const RX_NUESTRA_SUSCRIPCION = /\bsuscripci[oó]n\b|\bmensualidad\b|\bplan\b[^.\n]{0,30}\b(?:control finanzas|sistema|app|aplicaci[oó]n|plataforma)\b|\b(?:pagar|paga|pago de)\b[^.\n]{0,20}\b(?:el plan|su plan|la app|el sistema)\b/i
 ...
 */
export function validar(mensaje, { textoLead = '', contexto = null, stage = null, historial = [], linkPermitido = true, pais = null, yaRegistrado = false } = {}) {
  ...
  if (RX_DEBITO_CUENTA.test(m) || (RX_COBRO_AUTO.test(m) && !(yaRegistrado && RX_NUESTRA_SUSCRIPCION.test(m)))) motivos.push('cobro_automatico')
  ...
  /* ⚠ POR PARECIDO, NO POR INCLUSIÓN. Comparando la frase entera se escapaba la
     misma pregunta con las palabras en otro orden, que es como la repite un
     modelo. Visto en producción el 9 sep, dos turnos seguidos:
       «¿Ahorita cuántos clientes más o menos tiene en cartera?»
       «¿cuántos clientes tiene más o menos en cartera?»
     El lead ya había contestado «más de 100». */
  if (contexto?.ultimaPreguntaBot) {
    const previa = new Set(normalizar(contexto.ultimaPreguntaBot).split(' ').filter((w) => w.length > 3))
    for (const q of m.match(/[^.!?\n]*\?/g) || []) {
      const ahora = normalizar(q).split(' ').filter((w) => w.length > 3)
      if (previa.size < 3 || ahora.length < 3) continue
      const comunes = ahora.filter((w) => previa.has(w)).length
      if (comunes / Math.max(ahora.length, previa.size) >= 0.7) { motivos.push('pregunta_repetida'); break }
    }
  }
  ...
  /* ⚠ EN LAS ETAPAS DE VENTA, UNA RESPUESTA SIN PREGUNTA NI ENLACE ES UNA
     RESPUESTA MUERTA. Antes esto solo saltaba con un «quedo atento» pasivo, y
     por eso pasaban 244 de 679 respuestas reales (35 %) que informan y se
     quedan ahí: «Listo, cuando lo haga me avisa». El dueño lo llamó por su
     nombre: «contesta preguntas, no incita, no deja deseando». */
  if (['VALOR', 'DESCUBRIMIENTO', 'OBJECION'].includes(stage) && !m.includes('?') && !RX_LINK.test(m)) motivos.push('sin_siguiente_paso')
...
export function correccionPara(motivos, { contexto, textoLead } = {}) {
  ...
  const L = {
    ...
    cobro_automatico: 'Prometiste que el sistema le cobra solo al DEUDOR. Eso no existe: el sistema no debita cuentas de nadie. (Lo que sí existe, y solo para el plan del propio prestamista, es dejar pagada su suscripción cada mes.)',
    ...
    sin_siguiente_paso: 'Tu respuesta se queda ahí: no pregunta nada ni propone nada. En venta eso es una respuesta muerta. Termina con UNA pregunta corta sobre su negocio, o con el siguiente paso concreto.',
...
export function leadPideLink(texto) {
  ...
  if (/(?:c[oó]mo|donde|dónde|por donde|por dónde)\s+(?:entro|ingreso|accedo|abro|entra uno|se entra|se ingresa)(?:\s+(?:a|al|en|el|la))*\s*(?:sistema|app|aplicaci[oó]n|plataforma|programa)?/i.test(t)) return true
  /* Medido el 2 sep: «volver a entrar» son 38 de 204 preguntas de registrados, y
     casi ninguna dice «link». Dicen que perdieron la aplicación o preguntan cómo
     se llama. La respuesta a todas es la misma: el enlace. */
  if (/se me (?:perdi[oó]|borr[oó]|desapareci[oó]|fue)\s+(?:la\s+)?(?:app|aplicaci[oó]n|pagina|p[aá]gina|sistema)/i.test(t)) return true
  if (/(?:c[oó]mo|cual|cu[aá]l)\s+(?:se llama|es)\s+(?:la\s+)?(?:app|aplicaci[oó]n|pagina|p[aá]gina)/i.test(t)) return true
  if (/no (?:encuentro|hallo|veo)\s+(?:la\s+)?(?:app|aplicaci[oó]n|pagina|p[aá]gina)/i.test(t)) return true
  if (/(?:volver a|puedo|quiero)\s+(?:entrar|ingresar)\b/i.test(t)) return true
