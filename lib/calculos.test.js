...
describe('calcularPrestamo', () => {
  ...
  describe('modo manual', () => {
    ...
    it('auto-extiende periodos si cuota × plazo < capital + interes', () => {
      // Prestamo: $1M a 10% mensual, 2 meses, cuota manual $100K
      // Interes fijo = 1M * 10% * 2 = $200K → minTotal = $1.2M
      // Periodos originales (mensual) = 2 → 100K * 2 = 200K < 1.2M
      // Periodos necesarios = ceil(1.2M / 100K) = 12
      const r = calcularPrestamo({
        montoPrestado: 1000000, tasaInteres: 10, diasPlazo: 60,
        fechaInicio: fecha('2026-01-01'), frecuencia: 'mensual',
        modoInteres: 'manual', cuotaManual: 100000,
      })
      expect(r.cuotaDiaria).toBe(100000)
      expect(r.numPeriodos).toBe(12)
      expect(r.totalAPagar).toBe(1200000)
      expect(r.totalInteres).toBe(200000)
    })

    it('no extiende si cuota cubre el total normalmente', () => {
      // $500K a 10%, 1 mes diario, cuota 25K
      // Interes = 500K * 10% * 1 = 50K → minTotal = 550K
      // 25K * 30 = 750K > 550K → no se extiende
      const r = calcularPrestamo({
        montoPrestado: 500000, tasaInteres: 10, diasPlazo: 30,
        fechaInicio: fecha('2026-01-01'), frecuencia: 'diario',
        modoInteres: 'manual', cuotaManual: 25000,
      })
      expect(r.numPeriodos).toBe(30)
      expect(r.totalAPagar).toBe(750000)
    })
  })
})
...
describe('saldoPendiente y capitalRestante', () => {
  ...
  /* ── CAMBIO DE CONVENCION (1 ago 2026) ────────────────────────────────
     Estas pruebas fijaban la CASCADA: el pago cubria todo el interes primero
     y hasta que no entraban los $200.000 el capital no bajaba un peso.

     Ahora es PROPORCIONAL: cada peso lleva su parte. Medido sobre la cartera
     real, la diferencia entre las dos convenciones son $264.614.219 en
     «capital en la calle» — la cifra con la que el prestamista decide si
     puede prestar mas, y que la cascada inflaba un 8%.

     Ver `lib/dinero/reparto.js` para el porque de la eleccion. */
  describe('capitalRestante sin tabla (modo fijo) — proporcional', () => {
    it('pagado del todo, no queda capital', () => {
      const p = crearPrestamo({
    ...
    it('un pago parcial SI baja el capital, en su proporcion', () => {
      const p = crearPrestamo({
        montoPrestado: 1000000, totalAPagar: 1200000,
        pagos: [crearPago(100000)], totalPagado: 100000,
      })
      // De cada peso, 1.000.000/1.200.000 = 5/6 es capital.
      // 100.000 × 5/6 = 83.333 devueltos → quedan 916.667.
      // Con la cascada quedaban 1.000.000: el pago no bajaba nada.
      expect(calcularCapitalRestante(p)).toBe(916667)
    })

    it('un abono a capital SI baja capital entero, sin repartirse', () => {
      const p = crearPrestamo({
        montoPrestado: 1000000, totalAPagar: 1200000,
        pagos: [{ ...crearPago(100000), tipo: 'capital' }], totalPagado: 100000,
      })
      // El prestamista dijo «esto es capital»: se respeta.
      expect(calcularCapitalRestante(p)).toBe(900000)
    })
  })
})
...
describe('cuotasPendientes', () => {
  ...
  it('con tabla: cuenta cuotas no pagadas completamente', () => {
    ...
    /* Pagar la primera cuota es plata que ENTRA: mueve `totalPagado`, no solo la
       casilla de la fila. La prueba ponía únicamente `cuotasAmortizacion[0].pagado`
       y con eso fijaba el mecanismo viejo —contar fila por fila— en vez de la
       conducta. Ese mecanismo es justo el que dejaba «al día» a quien había
       abonado por otra vía, porque `fila.pagado` solo lo llenan los pagos
       completo/parcial. Ver `coberturaDeLaTabla`. */
    const primera = p.cuotasAmortizacion[0]
    primera.pagado = primera.cuotaTotal
    p.totalPagado = primera.cuotaTotal
