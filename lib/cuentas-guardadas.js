...
export async function abrirCuentaGuardada({ id, llave, pin }, ahora = Date.now()) {
  ...
  // Si un borrado del 5.º intento fallido anterior no se completó, la fila
  // sigue viva con el contador ya agotado: sin este corte, cada pedido nuevo
  // le regala otra comparación de bcrypt a quien ya no debería tener ninguna.
  if (fila.pinHash && fila.intentosPin >= MAX_INTENTOS_PIN) {
    await prisma.cuentaGuardada.delete({ where: { id: fila.id } }).catch(() => {})
