...
describe('el agente devuelve la traza con las columnas exactas', () => {
  ...
  it('las respuestas fijas llevan proveedor fijo y su clasificación', () => {
    ...
    expect(A).toMatch(/traza: FIJO\(`escalar:\$\{razonFinal\}`\)/)
    // El promptId dice qué variante del pitch salió: así se cuenta el A/B en SQL.
    expect(A).toMatch(/traza: FIJO\(clasificacionTraza, \{ etapa: stage, promptId: `pitch:\$\{variante\}` \}\)/)
  })
