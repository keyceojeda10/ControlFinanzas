...
// ── Tercera tanda del 20 sep 2026, tras probarlo el dueño en su iPhone ──────
describe('la tarjeta del cobro, en el teléfono', () => {
  const ficha = lee('app/(dashboard)/prestamos/[id]/page.jsx')
  it('los botones pueden encogerse y el icono no', () => {
    // «El botón de imprimir se sale de la caja y el icono de WhatsApp está diminuto».
    const i = ficha.indexOf('function AccionesDelCobro(')
    const comp = ficha.slice(i, i + 2600)
    expect(comp).toContain('[&>button]:min-w-0')
    expect(comp).toContain('[&_svg]:shrink-0')
    expect(comp).toMatch(/className="flex gap-2 w-full min-w-0/)
  })
})

describe('«12 días» en un préstamo semanal', () => {
  const nuevo = lee('app/(dashboard)/prestamos/nuevo/page.jsx')
  it('la unidad se monta como nodo NUEVO al cambiar la frecuencia (WebKit no repinta el viejo)', () => {
    expect(nuevo).toMatch(/<span key=\{`unidad-\$\{frecuencia\}`\}[^>]*>\s*\{UNIDAD_PLAZO\[frecuencia\]\}/)
  })
})

describe('el resumen del préstamo, sin verde ni degradado', () => {
  const nuevo = lee('app/(dashboard)/prestamos/nuevo/page.jsx')
  it('ninguna de las dos vistas lleva borde ni degradado verdes', () => {
    expect(nuevo).not.toMatch(/linear-gradient\(135deg, color-mix\(in srgb, var\(--cf-green-dark\)/)
    expect(nuevo).not.toMatch(/border: '1px solid color-mix\(in srgb, var\(--cf-green-dark\) 25%/)
    const tarjeta = nuevo.slice(nuevo.indexOf('const SectionCard ='), nuevo.indexOf('function EditableRow('))
    expect(tarjeta).not.toContain('linear-gradient')
  })
  it('editar se nota: el renglón entero es un botón de 48px con su pastilla', () => {
    const fila = nuevo.slice(nuevo.indexOf('function EditableRow('), nuevo.indexOf('function CifraResumen('))
    expect(fila).toContain('min-h-[48px]')
    expect(fila).toContain('aria-label={`Cambiar ${label}`}')
    expect(fila).toContain('w-7 h-7 rounded-full')
  })
  it('las cifras calculadas van grandes en las DOS vistas, y el aviso sale una sola vez', () => {
    expect(nuevo.split('<CifraResumen rotulo="Total a pagar"').length - 1).toBe(2)
    expect(nuevo.split('<AvisoPorCobro aviso={avisoDelPorcentaje(modoInteres, calculo,').length - 1).toBeLessThanOrEqual(2)
    const confirmar = nuevo.slice(nuevo.indexOf('{/* ═══ PASO 2: CONFIRMAR + FIRMA ═══ */}'))
    expect(confirmar.split('<AvisoPorCobro').length - 1).toBe(1)
  })
})
