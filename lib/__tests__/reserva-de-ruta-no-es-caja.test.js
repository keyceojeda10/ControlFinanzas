...
describe('⚠ y la OTRA vía: lo que dicen las operaciones', () => {
  /* Arreglar `afectaCaja` dejó la banda cuadrando, pero apareció un aviso
   * nuevo: «$200.000 de préstamos que no cuadran». Él lo vio y preguntó «¿qué
   * faltaría ahí, hermano?».
   *
   * Era el MISMO fallo por el otro lado. Las consultas que calculan «cuánto se
   * entregó de verdad» toman, por préstamo, el movimiento MÁS RECIENTE — y la
   * reserva de ruta es posterior al desembolso, así que le pisaba su importe:
   * decía $600.000 donde se entregaron $500.000, dos veces.
   *
   * Reproducido contra su base: el libro decía $2.950.000 y las operaciones
   * $3.150.000. Exactamente los $200.000 del aviso.
   *
   * Es la cuarta vez en este repo que arreglo una vía y dejo la otra. */
  const leer = (r) => require('fs').readFileSync(require('path').resolve(process.cwd(), r), 'utf8')

  for (const ruta of ['lib/dinero/desembolsado.js', 'app/api/caja/route.js']) {
    it(`${ruta.split('/').pop()}: ninguna consulta de desembolso cuenta la reserva`, () => {
      const src = leer(ruta)
      // Cada `tipo: 'desembolso'` tiene que traer su `ajusteArranqueRuta: false`
      // dentro del mismo `where`.
      const consultas = src.split("tipo: 'desembolso',").slice(1)
      expect(consultas.length, 'no encuentro las consultas de desembolso').toBeGreaterThan(0)
      for (const [i, trozo] of consultas.entries()) {
        const where = trozo.slice(0, trozo.indexOf('},') + 1)
        expect(where, `la consulta ${i + 1} de ${ruta} vuelve a contar la reserva de ruta`)
          .toMatch(/ajusteArranqueRuta:\s*false/)
      }
    })
  }
})
