...
describe('⚠ el reverso que pone el sistema no se puede borrar', () => {
  const capitalApi = leer('app/api/capital/movimientos/[id]/route.js')

  it('se rechaza, aunque sea de tipo «ajuste»', () => {
    /* Pasaba la lista de manuales porque ES un ajuste, pero no es de nadie: lo
       pone el sistema al eliminar un gasto. Borrarlo deja huérfano al asiento
       del gasto, que sigue restando para siempre sin nada detrás.

       Le pasó al cliente que se fue: a las 11:50 borró justo ese. */
    expect(capitalApi).toMatch(/movimiento\.tipo === 'ajuste' && movimiento\.referenciaTipo === 'gasto'/)
  })

  it('y se explica por qué, no solo que no', () => {
    /* «No se puede» a secas es lo que hace que la gente pruebe otra cosa peor. */
    expect(capitalApi).toMatch(/seguiría restando de tu caja/)
  })

  it('los ajustes de verdad manuales se siguen pudiendo borrar', () => {
    /* La pantalla de «Mi plata» sigue siendo suya: lo que se cierra es el
       reverso del sistema, no lo que el prestamista asentó. */
    expect(capitalApi).toMatch(/const TIPOS_MANUALES = \['capital_inicial', 'inyeccion', 'retiro', 'ajuste'\]/)
  })
})
