...
describe('WhatsApp abre las PLANTILLAS', () => {
  ...
  it('la firma del negocio sale de `useAuth`, no de un campo inventado', () => {
    ...
    /* Se comprueban los CAMPOS, no el orden ni la lista entera: al entrar
       `esOwner` para el «Deshacer el cobro», una prueba que copiaba la
       desestructuración al pie de la letra se puso roja sin que nada se
       hubiera roto. */
    expect(codigo).toMatch(/= useAuth\(\)/)
    for (const campo of ['user', 'orgNombre', 'ocultarSaldoWA', 'organizationId']) {
      expect(codigo, `falta ${campo} en useAuth`).toMatch(new RegExp(`\\{[^}]*\\b${campo}\\b[^}]*\\} = useAuth\\(\\)`))
    }
    expect(codigo, 'volvió el campo inventado').not.toMatch(/organizacionNombre/)
