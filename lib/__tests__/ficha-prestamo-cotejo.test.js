...
describe('las tres reglas de la lamina', () => {
  ...
  it('2 · el interes UNA sola vez, en «como se pacto»', () => {
    ...
    // Las mismas palabras del prestamista, ahora como rótulo de cada cifra
    // (19 sep 2026: «números grandes, con sus sumas»). La ganancia va en el chip
    // de «me paga», que es la que la trae.
    expect(ficha).toMatch(/<LineaCifra rotulo="Le presté" cifra=\{prestado\}/)
    expect(ficha).toMatch(/<LineaCifra rotulo="Me paga" cifra=\{totalAPagar\}\s*\n\s*chip=\{ganancia && !\/sin plazo\//)
