...
const wizard   = sinComentarios(leer('components', 'onboarding', 'OnboardingWizard.jsx'))
...
describe('T05-04 · las tres vías cuadran con el asistente', () => {
  ...
  it('el asistente también ofrece las tres', () => {
    /* ⚠ ESTA PRUEBA LEÍA UN ARCHIVO QUE YA NO SE RENDERIZA.
       Miraba `WizardCartulina.jsx`, que era la pantalla de la foto del
       asistente. Cuando el asistente pasó al lector de tanda, ese archivo
       quedó huérfano —y la prueba seguía en verde afirmando que «el asistente
       ofrece las tres» sobre código que nadie ejecuta—. Ahora lee el
       asistente de verdad. */
    expect(wizard, 'falta la foto').toMatch(/metodo === 'foto'/)
    expect(wizard, 'falta el Excel').toMatch(/metodo === 'excel'/)
    expect(wizard, 'falta el manual').toMatch(/metodo: 'cero'/)
  })

  it('las tres se ofrecen en la misma pantalla, con las mismas palabras', () => {
    expect(pantalla).toContain("destino: '/carga-masiva'")
    // El asistente monta esa misma pantalla en vez de tener su propia lista.
    expect(wizard).toMatch(/<TraerCartera/)
  })
})
