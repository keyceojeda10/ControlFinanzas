...
describe('«sin asignar» en negativo no se pinta como un faltante', () => {
  // Comprobado en producción: pasa en 1 de los 6 negocios que usan capital por
  // ruta («Invenciones J & J»: global $311.600, en rutas $29.478.397). La causa
  // es sana — los `ajusteArranqueRuta` restan del global pero NO de la
  // sub-bolsa, así que una ruta que ES el negocio entero acaba por encima.
  // La aritmética está bien; el rótulo era el que mentía.
  const t = sinComentarios(leer('components/caja/CajaPorRuta.jsx'))

  it('cambia el rótulo en vez de enseñar un menos en rojo', () => {
    expect(t).toMatch(/Las rutas suman más que el global/)
    expect(t).not.toMatch(/var\(--cf-red\)['"]?\s*:\s*['"]#F3F3F6/)
  })

  it('dice que no falta plata', () => {
    expect(t).toMatch(/No falta plata/)
  })
})
