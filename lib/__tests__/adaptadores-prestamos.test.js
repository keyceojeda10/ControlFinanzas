...
describe('T02-06 · el contexto, los estados y las tres cifras', () => {
  ...
  it('la tira lleva cuota y vencimiento, y el atraso EN PLATA', () => {
    ...
    expect(c.map((x) => x.etiqueta)).toEqual(['Cuota', 'Atraso', 'Vence'])
    expect(c[1].valor).toMatch(/48\.000/)
    expect(c[1].tono).toBe('contra')
  })

  it('al dia, el atraso sale en «Nada» en vez de desaparecer', () => {
    // Una columna que aparece y desaparece obliga a releer la tira en CADA
    // tarjeta. ⚠ Decía «LE FALTA $0» pegado a una deuda de $540.000, y «le
    // falta» es como la ficha llama a la DEUDA: se leía «no debe nada» (19 sep
    // 2026). Con cero se dice «ATRASO · Nada», en gris, como en la ficha.
    const atraso = cifrasDe({ ...base, diasMora: 0, montoEnMora: 0 }, 'CO').find((x) => x.clave === 'atraso')
    expect(atraso.etiqueta).toBe('Atraso')
    expect(atraso.valor).toBe('Nada')
    expect(atraso.tono).toBe('apagado')
  })
  ...
  it('sin el dato medido, la columna NO se inventa', () => {
    ...
    expect(c.some((x) => /ganancia/i.test(x.etiqueta))).toBe(false)
    // El atraso, igual: sin `montoEnMora` no hay columna, no hay un $0 falso.
    expect(c.some((x) => x.etiqueta === 'Atraso')).toBe(false)
  })
  ...
  it('el pagado dice la FECHA, no la cuota', () => {
    ...
    /* ⚠ «12 jul», SIN el «de». La prueba lo pedia CON preposicion porque asi lo
       devolvia el `es-CO` del ICU nuevo, y eso era el fallo, no el contrato: la
       docstring de `fechaCorta` dice «12 de jul» pero la de `proximoCobroTexto`
       —que escribe la misma columna— dice «12 ago». Las dos fechas de la tira
       salian con formatos distintos, y la larga se recortaba en la columna de
       74px. Esta prueba estaba fijando la deriva. */
    expect(c).toMatch(/terminado 12 jul/)
