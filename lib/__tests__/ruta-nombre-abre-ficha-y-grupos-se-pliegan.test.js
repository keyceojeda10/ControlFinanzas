...
describe('el nombre y la foto llevan a la ficha del cliente', () => {
  ...
  it('lo llevan el nombre y la foto, no un botón nuevo', () => {
    /* Se comprueba que el mismo juego de manejadores (`identidad`) se aplica
       DOS veces: al avatar y al nombre. Que sean esos dos y no otro sitio es lo
       que se pidió; un botón más en una tarjeta que ya tiene cinco controles es
       lo que se pidió NO hacer. */
    const puestos = [...tarjeta.matchAll(/\{\.\.\.identidad\}/g)]
    expect(puestos.length, 'el nombre o la foto dejaron de abrir la ficha').toBeGreaterThanOrEqual(2)
  })
