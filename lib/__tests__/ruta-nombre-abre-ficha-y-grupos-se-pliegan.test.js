...
describe('el nombre y la foto llevan a la ficha del cliente', () => {
  ...
  it('⚠ está en las DOS pantallas que usan esta tarjeta', () => {
    ...
    //
    // ⚠ Y GUARDANDO EL SITIO ANTES DE SALTAR. Aquí decía `router.push` pelado, y
    // ese era justo el camino que perdía la posición de la lista: al volver de
    // la ficha aparecía arriba del todo. Ver `sitio-de-la-lista`.
    const i = rutaPag.indexOf('onAbrirCliente={')
    const bloque = rutaPag.slice(i, i + 400)
    // El destino no cambia; desde el 24 ago va por `irGuardando`, que guarda
    // el sitio antes de saltar. Ver `abono-sigue-pendiente.test.js`.
    expect(bloque).toMatch(/irGuardando\([\s\S]{0,90}\/clientes\/\$\{fila\.id\}`\)/)
    // El orden —guardar y luego navegar— vive ahora dentro de `irGuardando`,
    // que es la puerta unica por la que salen las tres vias.
    const puerta = rutaPag.slice(rutaPag.indexOf('const irGuardando ='), rutaPag.indexOf('const irGuardando =') + 200)
    expect(puerta.indexOf('guardarContextoRuta')).toBeGreaterThan(-1)
    expect(puerta.indexOf('guardarContextoRuta')).toBeLessThan(puerta.indexOf('router.push'))
  })
})
...
describe('las secciones de la vista agrupada se pliegan', () => {
  it('el rótulo es el plegador', () => {
    ...
    /* ⚠ Vive FUERA del render de la página desde el 20 sep 2026: un componente
       declarado dentro de otro se vuelve a crear en cada render y React
       destruye su nodo, así que el botón perdía el foco del teclado a mitad de
       uso. Lo caza `react/no-unstable-nested-components`. */
    const i = rutaPag.indexOf('function RotuloGrupo(')
    expect(i, 'el rótulo volvió a declararse dentro del render').toBeGreaterThan(0)
    expect(rutaPag.indexOf('export default function RutaDetallePage'), 'quedó dentro de la página otra vez')
      .toBeGreaterThan(i)
    const bloque = rutaPag.slice(i, i + 1600)
    expect(bloque, 'el rótulo volvió a ser un <div> sin acción').toMatch(/<button/)
    expect(bloque).toMatch(/onClick=\{onPlegar\}/)
    expect(rutaPag).toMatch(/onPlegar=\{\(\) => plegarGrupo\(g\.clave\)\}/)
