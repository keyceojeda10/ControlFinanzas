...
describe('dónde sale', () => {
  ...
  describe('todas las superficies que mueven plata', () => {
    it('el cobro de la calle: «Cuota» no cobra de un toque', () => {
      /* Era `onClick={() => onCobrarCuota?.(p)}`: un roce en el bolsillo
         registraba un pago, y es la pantalla más usada en la calle. */
      expect(ATAJOS).toMatch(/onClick=\{\(\) => \(tactil \? setDeslizarCuota\(p\.id\) : onCobrarCuota\?\.\(p\)\)\}/)
      expect(ATAJOS).toMatch(/deslizarCuota === p\.id \? \(/)
      expect(ATAJOS).toMatch(/<DeslizarParaConfirmar/)
    })

    it('«Otro monto» también: es la misma plata', () => {
      const i = ATAJOS.indexOf('montoAbierto === p.id')
      expect(i).toBeGreaterThan(-1)
      expect(ATAJOS.slice(i, i + 2200)).toMatch(/tactil \? \(\s*<>\s*<DeslizarParaConfirmar/)
    })
