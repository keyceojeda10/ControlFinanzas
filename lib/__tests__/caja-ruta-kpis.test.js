...
describe('nada se usa antes de declararse (TDZ)', () => {
  // Puse `gestionPorRuta` DESPUÉS del `porRuta` que lo consume y el endpoint
  // entero reventó con «Cannot access 'ao' before initialization»: la pantalla
  // decía «No se pudo cargar la caja del cobrador».
  //
  // Las 2.027 pruebas seguían verdes. Un `const` leído antes de su línea no
  // falla al compilar ni al importar — solo cuando corre. Es el mismo fallo que
  // ya tumbó el migrador y la carga masiva.
  it('gestionPorRuta se declara antes de usarse', () => {
    expect(api.indexOf('const gestionPorRuta'))
      .toBeLessThan(api.indexOf('gestion: r.rutaId'))
  })

  it('deltaPorRuta se declara antes del porRutaMap que lo lee', () => {
    expect(api.indexOf('const deltaPorRuta'))
      .toBeLessThan(api.indexOf('saldoApertura: Math.round'))
  })

  it('los recargos se reparten después de recibirlos', () => {
    expect(api.indexOf('const [cobros, gastos'))
      .toBeLessThan(api.indexOf('for (const r of recargos)'))
  })
})
