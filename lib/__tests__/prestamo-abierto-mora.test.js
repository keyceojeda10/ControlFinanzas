...
/* ── EL CAPITAL EN LA CALLE, QUE ES LO QUE VE LA CARTERA ─────────────────── */
describe('pagar interés no baja el capital en la calle', () => {
  it('en un abierto, el reparto no toca el capital', async () => {
    const { capitalEnCalle } = await import('@/lib/dinero/reparto')
    /* La cartera suma `capitalEnCalle` préstamo a préstamo. Si el pago de
       interés se repartiera —que es lo que hace el reparto proporcional—, el
       informe diría que el cliente devolvió capital que no devolvió. */
    const p = abierto({
      totalAPagar: 759_000,
      totalPagado: 69_000,
      devengos: [{ periodo: '2026-06-15', interes: 69_000, capitalBase: 690_000 }],
      pagos: [{ tipo: 'intereses', montoPagado: 69_000, fechaPago: dia('2026-06-16') }],
    })
    expect(capitalEnCalle(p)).toBe(690_000)
  })

  it('y el abono a capital sí lo baja, entero', () => {
    return import('@/lib/dinero/reparto').then(({ capitalEnCalle }) => {
      const p = abierto({
        totalAPagar: 759_000,
        totalPagado: 269_000,
        devengos: [{ periodo: '2026-06-15', interes: 69_000, capitalBase: 690_000 }],
        pagos: [
          { tipo: 'intereses', montoPagado: 69_000, fechaPago: dia('2026-06-16') },
          { tipo: 'capital', montoPagado: 200_000, fechaPago: dia('2026-06-20') },
        ],
      })
      expect(capitalEnCalle(p)).toBe(490_000)
    })
  })
})
