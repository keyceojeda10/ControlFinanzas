...
describe('CUÁL de los préstamos es el clavo', () => {
  ...
  it('⚠ y en la HOJA DE COBRO, que es donde peor se notaba', () => {
    ...
    // Con varios préstamos va en la fila de cada uno; con uno solo, en el bloque.
    expect(hojaCobro).toMatch(/\{x\.esClavo && <EtiquetaClavo \/>\}/)
    expect(hojaCobro).toMatch(/p\?\.esClavo && <EtiquetaClavo \/>/)
