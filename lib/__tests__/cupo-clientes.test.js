...
import { PLANES_CONFIG, LIMITES_PLAN, cuposDe, selectCupos } from '@/lib/planes'
...
const IMPORTAR = 'lib/importar-cartera.js'
const LIMITES = 'lib/limites-plan.js'
const USO = 'app/api/plan/uso/route.js'

// Desde el 24 sep 2026 la suma vive en `cuposDe` (lib/planes.js), con los
// adicionales pagados: los sitios que la usan piden `selectCupos` y leen el
// cupo de ahí. La suma se prueba ejecutándola, no leyendo su texto.
describe('la suma del cupo (cuposDe)', () => {
  it('suma el cupo de clientes al del plan', () => {
    expect(cuposDe({ plan: 'starter', clientesExtra: 50 }).clientes).toBe(PLANES_CONFIG.starter.maxClientes + 50)
    expect(cuposDe({ plan: 'starter' }).clientes).toBe(PLANES_CONFIG.starter.maxClientes)
  })

  it('el select que la acompaña pide el plan y el cupo', () => {
    expect(selectCupos).toMatchObject({ plan: true, clientesExtra: true })
  })
})

describe('los dos sitios que BLOQUEAN', () => {
  it('crear un cliente suma el cupo extra', () => {
  ...
  it('crear un cliente lee el plan de la BASE, no del token', () => {
  ...
  it('la importación usa el cupo de la BASE, con el plan de la base', () => {
    const src = leer(IMPORTAR)
    expect(src, 'no lee el cupo de la organización')
      .toMatch(/organization\.findUnique\(\{[\s\S]{0,120}select: selectCupos/)
    expect(src).toMatch(/const cupo = cuposDe\(orgCupo, orgCupo\?\.plan \|\| plan\)/)
    expect(src).toMatch(/const limiteClientes = cupo\.clientes/)
  })
...
describe('los tres que INFORMAN', () => {
  it('lib/limites-plan suma el cupo', () => {
    ...
    expect(src).toContain('select: selectCupos')
    expect(src).toMatch(/const cupo = cuposDe\(org\)/)
    expect(src).toMatch(/const limiteClientes = cupo\.clientes/)
  ...
  it('api/plan/uso devuelve el límite CON el cupo', () => {
    ...
    const src = leer(USO)
    expect(src).toContain('select: selectCupos')
    expect(src).toMatch(/const cupo = cuposDe\(org, planReal\)/)
    expect(src).toMatch(/const limiteClientes = cupo\.clientes/)
