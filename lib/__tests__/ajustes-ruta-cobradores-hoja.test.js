...
describe('«Crear cobrador» no se va bajo el scroll', () => {
  ...
  it('en el teléfono se aparta de la pastilla y en PC no, que allí no hay', () => {
    ...
    expect(pc.slice(0, 160), 'en PC no hay pastilla: no se le reserva su alto')
      .not.toMatch(/--cf-nav-inset/)
    expect(pc.slice(0, 160), 'pero sí queda un respiro abajo').toMatch(/(bottom:\s*16px|padding-bottom:\s*16px)/)
  })

  it('en PC el botón no es una franja de lado a lado', () => {
    // Medía 1.068px de ancho y tapaba la lista como una barra dorada.
    const pc = GLOBALS.slice(GLOBALS.indexOf('@media (min-width: 1024px)', GLOBALS.indexOf('.cf-crear-sticky')))
    expect(pc.slice(0, 220)).toMatch(/\.cf-crear-sticky\s*>\s*button\s*\{[^}]*max-width/)
  })
})
