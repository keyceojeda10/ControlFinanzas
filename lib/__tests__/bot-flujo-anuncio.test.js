...
import {
  ...
  BOTONES_ENTRADA, BOTONES_ATASCO, pareceAtascado, respuestaAtasco,
...
describe('⚠ el tráfico se separa, el bot de siempre no se toca', () => {
  it('solo atiende a quien viene de un anuncio', () => {
    expect(esDeAnuncio({ anuncioId: '120244629437740778' })).toBe(true)
    expect(esDeAnuncio({ anuncioId: null })).toBe(false)
  ...
  it('un lead que ya existía y ahora llega por un anuncio se marca', () => {
    ...
    expect(webhook).toMatch(/if \(ref && !existente\.anuncioId\)/)
  })
...
describe('⚠ lo que más paga tiene respuesta, y lo que menos no se lleva la conversación', () => {
  ...
  it('la respuesta de confianza no inventa cifras que no tiene', () => {
    ...
    expect(sinDatos).toContain('exportarlos')
  })
...
describe('⚠ el atasco no se deriva, se atiende', () => {
  ...
  it('⚠ NUNCA manda al 301 199 3001', () => {
    /* 274 derivaciones a ese número en julio y agosto; 4 tickets de soporte en
       toda la historia del producto. Convierte cerca del 1 %. */
    expect(flujo).not.toMatch(/301\s*199\s*3001/)
    expect(flujo).not.toMatch(/3011993001/)
    for (const b of BOTONES_ATASCO) {
      expect(respuestaDeBoton(b.id).texto).not.toMatch(/301\s*199\s*3001/)
    }
  })
})
