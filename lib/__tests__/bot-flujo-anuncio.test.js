...
describe('⚠ lo que más paga tiene respuesta, y lo que menos no se lleva la conversación', () => {
  ...
  it('la respuesta de confianza no inventa cifras que no tiene', () => {
    ...
    /* ⚠ «abrieron su cuenta», no «nos usan»: 587 son las organizaciones
       REGISTRADAS y 353 las que han cargado cartera. La frase dice lo que el
       número significa. */
    expect(conDatos).toMatch(/abrieron su cuenta/i)
    ...
    /* Y lo que se afirma tiene que estar comprobado: respaldo diario (cron
       `cf-respaldo`, cifrado, con copia fuera) y copia descargable
       (`app/api/cuenta/backup`). Nada de «dos años», que son seis meses. */
    expect(sinDatos).toMatch(/copia de seguridad todos los días/i)
    expect(sinDatos).not.toMatch(/\b(dos|2)\s+años\b/i)
  })
  ...
  it('y con pocos negocios no se presume de un número que no impresiona', () => {
    ...
    expect(t).toMatch(/equipo colombiano/i)
  })
})
