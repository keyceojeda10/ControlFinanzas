...
describe('⚠ nada contesta con el bot apagado', () => {
  it('los atajos de botones van DESPUÉS de las guardas', () => {
    /* Estaban antes: un botón pulsado seguía contestando aunque el bot
       estuviera apagado para ese lead —porque lo estaba atendiendo una
       persona— y le pisaba la respuesta al humano. */
    const apagadoLead  = webhook.indexOf('if (botApagado)')
    const apagadoGlobal = webhook.indexOf('!config.botActivo')
    for (const atajo of ['probar bot cf', 'esDeAnuncio(lead) && botonId', 'esBotonDeCartera(botonId)']) {
      const i = webhook.indexOf(atajo)
      expect(i, `no encontré «${atajo}»`).toBeGreaterThan(-1)
      expect(i, `«${atajo}» contesta con el bot apagado`).toBeGreaterThan(apagadoLead)
      expect(i, `«${atajo}» contesta con el bot global apagado`).toBeGreaterThan(apagadoGlobal)
    }
  })

  it('y hay una llave para probar el flujo nuevo desde un teléfono', () => {
    /* El flujo solo atiende a quien viene de un anuncio, así que hasta que la
       campaña arranque nadie lo recibe. La llave evita tener que tocar
       producción a mano para probarlo. */
    expect(webhook).toMatch(/clave === 'probar bot cf'/)
    expect(webhook).toMatch(/clave === 'salir bot cf'/)
    expect(webhook).toMatch(/desdeAnuncioWa: entrar/)
  })
})
...
describe('⚠ lo que dijeron las 1.047 conversaciones reales', () => {
  ...
  it('⚠ y al que YA está dentro no se le ofrece la prueba que ya tiene', () => {
  ...
  it('⚠ y NINGUNA versión pega el link de pago en la lista de precios', () => {
    /* «El link para pagar cuando apenas estás dando la información de los
       precios es completamente innecesario, y los va a llevar a un sitio donde
       ni siquiera están registrados» — el dueño. Es ruido en un mensaje que ya
       es largo, y el botón «Quiero pagar ya» está justo debajo. */
    for (const reg of [false, true]) {
      expect(respuestaDeBoton('cf_precio', { registrado: reg }).texto)
        .not.toContain('/configuracion/plan')
    }
    /* Pero el botón sí lleva ahí, que es donde toca. */
    expect(respuestaDeBoton('cf_precio').botones.map(b => b.id)).toContain('cf_pagar')
    expect(respuestaDeBoton('cf_pagar').texto).toContain('/configuracion/plan')
  })

  it('y el sistema no se vende como si fuera solo del celular', () => {
    /* «Hay que aclarar que se puede gestionar desde la computadora, desde una
       tablet, desde el celular y todo eso» — el dueño. */
    expect(saludoDeAnuncio().texto).toMatch(/celular.*computador.*tablet/i)
  })
