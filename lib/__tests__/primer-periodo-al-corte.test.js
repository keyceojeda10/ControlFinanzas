...
describe('los modos sin tabla: la fecha sí, el interés no', () => {
  /* «Cobro los 30» significa el 30 que viene, y eso no depende de cómo se
     calcule el interés: la FECHA se corrige en todos los modos. El PRORRATEO no,
     porque en `fijo`, `unico` y `manual` el interés no se devenga por periodo
     —es un total pactado repartido en cuotas iguales— y no hay dónde escribir un
     primer periodo distinto sin cambiar el precio que el prestamista cotizó. */
  const args = (modo) => ({
    montoPrestado: 2000000, tasaInteres: 5, diasPlazo: 360,
  ...
  it.each(['fijo', 'unico', 'manual'])('%s empieza a cobrar en el primer corte', (modo) => {
    const con = calcularPrestamo({ ...args(modo), diaCobroMes: 30 })
    // Antes vencía el 30/8/2027: la primera cuota se iba al 30 de SEPTIEMBRE,
    // 48 días después de entregar, y todo el calendario nacía corrido un mes.
    expect(enBogota(con.primerCobro)).toBe('30/8/2026')
    expect(enBogota(con.fechaFin)).toBe('30/7/2027')
  })

  it.each(['fijo', 'unico', 'manual'])('%s no prorratea: el total no se toca', (modo) => {
    const sin = calcularPrestamo(args(modo))
    const con = calcularPrestamo({ ...args(modo), diaCobroMes: 30 })
    expect(con.prorrateoPrimerPeriodo).toBeUndefined()
    expect(con.totalAPagar).toBe(sin.totalAPagar)
    expect(con.cuotaDiaria).toBe(sin.cuotaDiaria)
  })

  it('⚠ sin día de corte siguen sin `primerCobro`, y nada se mueve', () => {
    /* Es lo que deja intactos a los préstamos que ya están en la calle: la
       columna nace en null y `fechaDePeriodo` ni entra en la rama nueva. */
    for (const modo of ['fijo', 'unico', 'manual', 'saldo', 'solo_interes']) {
      const r = calcularPrestamo(args(modo))
      expect(r.primerCobro, `${modo} se inventó un primer cobro sin día de corte`).toBeUndefined()
      expect(enBogota(r.fechaFin)).toBe('13/8/2027')
    }
  })
})

describe('la cuota que se ANUNCIA es la normal, no la del tramo corto', () => {
  /* `cuotaDiaria` no es solo un rótulo: la mora y las cuotas pendientes dividen
     por él. Con la primera fila prorrateada, un globo de $100.000 al mes se
     guardaba como «$56.667 mensuales» y la tarjeta del cliente mentía. */
  it('globo: anuncia $100.000, aunque la primera sea de $56.667', () => {
    const r = delVideo({ modoInteres: 'solo_interes', diaCobroMes: 30 })
    expect(r.cuotaDiaria).toBe(100000)
    expect(r.tablaAmortizacion[0].cuotaTotal).toBe(56667)
  })

  it('sobre saldo: sigue anunciando la cuota francesa', () => {
    expect(delVideo({ diaCobroMes: 30 }).cuotaDiaria).toBe(225700)
  })

  it('lineal: anuncia la primera cuota COMPLETA', () => {
    const r = calcularPrestamo({
      montoPrestado: 1200000, tasaInteres: 5, diasPlazo: 180,
      fechaInicio: '2026-08-13', frecuencia: 'mensual', modoInteres: 'lineal', diaCobroMes: 30,
    })
    expect(r.cuotaDiaria).toBe(r.tablaAmortizacion[1].cuotaTotal)
  })
})
