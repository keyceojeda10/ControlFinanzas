...
describe('el detalle sale del MISMO bucle que las cifras', () => {
  ...
  // La gráfica de los 7 días se mudó a un componente que comparten el resumen y
  // el Inicio: el dueño pidió que allá se viera como aquí.
  const semana = fs.readFileSync('components/cf/SemanaDeCobros.jsx', 'utf8')
  ...
  it('un día sin plata lo dice UNA vez, y las partes nunca van pegadas', () => {
    // «0 cobrosNo entró plata ese día.»: tres trozos sueltos sin separador.
    // Es la rama EXCLUYENTE del día vacío: si monto es 0, no se pinta nada más.
    // ⚠ La gráfica se mudó a `SemanaDeCobros`, que ahora comparten el resumen y
    //   el Inicio; la regla es la misma y vive allí.
    expect(semana).toContain("{dia.monto === 0 ? 'No entró plata ese día.' : (")
    // Y el otro lado siempre mete « · » antes de lo que sigue.
    expect(semana).toContain("{dia.cobros != null ? ' · ' : ''}")
  })

  it('la pantalla: las barras y los azulejos se TOCAN', () => {
    expect(semana).toMatch(/onClick=\{\(\) => setElegido\(/)   // las barras, en SemanaDeCobros
