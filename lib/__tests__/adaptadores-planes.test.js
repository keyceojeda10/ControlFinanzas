...
import { tramosDePlan, limiteInicial, loQueDesbloquea, escaleraPara, recomendarPlan, planMinimo, pesoDelPlan, adaptarPlanExcedido, razonDelPlan } from '@/lib/adaptadores/planes'
...
describe('tramosDePlan', () => {
  it('los límites salen de PLANES_CONFIG, no de la pantalla', () => {
    ...
    // Inicial baja a 100 a propósito: con 150 la gente se quedaba ahí mucho
    // tiempo y el salto a Básico —que solo añade techo— no llegaba nunca.
    expect(t[0].limite).toBe(100)
  ...
  // El numero de clientes es el PEOR criterio para elegir plan: alguien con 60
  // clientes puede pagar Crecimiento solo por tener un cobrador. Manda el
  // nombre, y debajo lo que desbloquea.
  it('manda el nombre del plan, no el techo de clientes', () => {
    const t = tramosDePlan('co', String)
    expect(t[0].texto).toBe('Inicial')
    expect(t[1].texto).toBe('Básico')
    expect(t[0].techo).toBe('hasta 100 clientes')
  })

  it('lo que desbloquea sale de PLANES_CONFIG y pone el cobrador primero', () => {
    const t = tramosDePlan('co', String)
    // ⚠ YA NO SON IDÉNTICOS. Básico llegó a los REPORTES (nivel 1) por
    // decisión de negocio: era el motivo que faltaba para que quien está en
    // Inicial ($39.000) suba a Básico ($59.000). Antes solo cambiaba el techo
    // de clientes, que a mucha gente no le aprieta todavía.
    //
    // Esta prueba lo deja fijado: la frase la arma `tramosDePlan` LEYENDO
    // `PLANES_CONFIG`, así que la pantalla de planes lo anuncia sola. Si
    // alguien vuelve `reportesNivel` a 0, aquí salta.
    expect(t[0].desbloquea).toBe('tú solo · 1 ruta')
    expect(t[1].desbloquea).toBe('tú solo · 1 ruta · reportes')
    // El salto de verdad es Crecimiento: ahí aparece el cobrador.
    // Y es UNO, no dos: maxUsuarios cuenta todas las cuentas de la
    // organización, y el dueño es una de ellas. Decir «2 personas cobrando»
    // vendía el doble de lo que se entrega.
    expect(t[2].desbloquea).toContain('tú + 1 cobrador')
    expect(t[2].desbloquea).not.toContain('2 personas')
    expect(t[2].desbloquea).toContain('3 rutas')
  })
...
describe('adaptarPlanExcedido', () => {
  ...
  it('el rótulo y el título salen del plan real, no del handoff', () => {
    ...
    expect(v.rotulo).toBe('Plan Inicial · 100 clientes')
    expect(v.titulo).toBe('Tienes 160 clientes y tu plan cubre 100')
...
// Quien ya contestó «tengo cobradores» no puede ver planes sin cobradores:
// sería ofrecerle un plan donde lo que acaba de decir que hace no se puede.
describe('la escalera depende del perfil', () => {
  it('con cobradores empieza en el primero que los permite', () => {
    const e = escaleraPara('equipo')
    expect(e[0]).toBe('growth')
    expect(e).not.toContain('starter')
    expect(e).not.toContain('basic')
  })

  it('cobrando solo se ve la escalera de entrada', () => {
    expect(escaleraPara('solo')[0]).toBe('starter')
    expect(escaleraPara(undefined)[0]).toBe('starter')
  })

  it('todos los que se ofrecen a un equipo permiten de verdad más de un usuario', () => {
    for (const id of escaleraPara('equipo')) {
      expect(PLANES_CONFIG[id].maxUsuarios).toBeGreaterThan(1)
    }
  })

  it('los tramos que ve un equipo empiezan en Crecimiento', () => {
    const t = tramosDePlan('co', String, 3, 'equipo')
    expect(t[0].texto).toBe('Crecimiento')
    expect(t.map((x) => x.texto)).not.toContain('Inicial')
  })
})
