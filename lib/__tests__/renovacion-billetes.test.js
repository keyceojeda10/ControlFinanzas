...
describe('el código', () => {
  it('redondea la diferencia y guarda la exacta aparte', () => {
    expect(src, 'ya no se calcula la diferencia exacta')
      .toContain('const diferenciaExacta = montoFinal - minimoRenovacion')
    // `montoFinal` es lo que mandó la pantalla, salvo al financiar (ahí es la deuda: entrega 0).
    expect(src).toContain('const montoFinal = financiar ? minimoRenovacion : Number(montoPrestado)')
  ...
  it('el aviso del historial también', () => {
    expect(src).toMatch(/entregó \$\{pesos\(diferencia\)\}/)
  })
})
