...
import { interesPorPorcentaje, interesPorPorcentajeMensual, mesesDelPlazo, tasaDeFinanciar, vistaFinanciar, MODO_FINANCIAR } from '@/lib/financiar'
import { minimoParaRenovar, calcularPrestamo } from '@/lib/calculos'
...
/* ⚠ EL % ES AL MES, COMO AL CREAR UN PRÉSTAMO (Inversiones Jk, 25 sep 2026):
   «el cliente me está debiendo 660, yo le presto el 15 %, se lo voy a financiar
   a dos meses […] deberían de ser 858, y solamente suma un solo mes, 760». */
describe('el porcentaje de financiar es AL MES', () => {
  it('debe $660.400, 15 % a 8 semanas: lo mismo que un préstamo nuevo ($859.200)', () => {
    const interes = interesPorPorcentajeMensual(660400, 15, { periodos: 8, frecuencia: 'semanal' })
    expect(interes).toBe(198120)
    const v = vistaFinanciar({ deuda: 660400, interes, periodos: 8, frecuencia: 'semanal', fechaInicio: '2026-09-25' })
    const nuevo = calcularPrestamo({ montoPrestado: 660400, tasaInteres: 15, frecuencia: 'semanal', diasPlazo: 56, modoInteres: 'fijo' })
    expect(v.total).toBe(859200)
    expect(v.total).toBe(nuevo.totalAPagar)
    expect(v.cuota).toBe(107400)
  })

  it('a un mes (30 días o 4 semanas) no cambia nada: PRESTA MIL queda igual', () => {
    expect(interesPorPorcentajeMensual(1030000, 20, { periodos: 30, frecuencia: 'diario' })).toBe(interesPorPorcentaje(1030000, 20))
    expect(interesPorPorcentajeMensual(1030000, 20, { periodos: 4, frecuencia: 'semanal' })).toBe(interesPorPorcentaje(1030000, 20))
  })

  it('6 semanas son mes y medio', () => {
    expect(mesesDelPlazo({ periodos: 6, frecuencia: 'semanal' })).toBe(1.5)
    expect(interesPorPorcentajeMensual(1000000, 20, { periodos: 6, frecuencia: 'semanal' })).toBe(300000)
  })

  it('la hoja convierte el % con los meses del plazo, y lo dice', () => {
    const hoja = leer('components/prestamos/FinanciarSaldo.jsx')
    expect(hoja).toMatch(/interesPorPorcentajeMensual\(deuda, porcentajeDe\(pct\), \{ periodos: cuotas, frecuencia \}\)/)
    expect(leer('components/pantallas/Gestion.jsx')).toMatch(/rotulo="Qué porcentaje al mes"/)
  })
})
