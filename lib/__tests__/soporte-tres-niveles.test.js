...
import { elegirGuia, nivelDeSoporte, yaSeMandoGuia, textoDeGuia, GUIAS_ENCENDIDAS } from '@/lib/bot-v2/soporte'

/* Las guías están APAGADAS (`BOT_GUIAS` no está en «on»): las capturas son del
   15 de agosto y la interfaz cambió. Estas pruebas miden el SELECTOR, que sigue
   ahí para el día que se regeneren, así que le pasan `forzar`. */
const elegirGuia_ = (t, o = {}) => elegirGuia(t, { ...o, forzar: true })
...
describe('nivelDeSoporte: qué contesta el bot y qué no', () => {
  ...
  it('con las guías apagadas, «cómo hago tal cosa» no dispara envío', () => {
    // Apagadas el 8 sep: las capturas enseñan botones que ya no existen.
    expect(GUIAS_ENCENDIDAS).toBe(false)
    expect(nivelDeSoporte('como hago para editar las fecha inicial del prestamo', reg)).toBe(null)
  })
  it('el selector sigue acertando para el día que se regeneren', () => {
    expect(elegirGuia_('como hago para editar las fecha inicial del prestamo')).toBe('editar-eliminar-prestamo')
    expect(elegirGuia_('Esto puedo enviar recibos por whatsapp? Puedo editar los textos?')).toBe('enviar-recibo-whatsapp')
  })
...
describe('elegirGuia: mejor ninguna que la equivocada', () => {
  it('acierta en las preguntas reales que se midieron', () => {
    expect(elegirGuia_('como hago para editar las fecha inicial del prestamo y la fecha final')).toBe('editar-eliminar-prestamo')
    expect(elegirGuia_('Yo puedo agregar el capital inicial? Es decir si tengo un capital de 20 millones')).toBe('inyectar-capital')
    expect(elegirGuia_('Esto puedo enviar recibos por whatsapp? Puedo editar los textos?')).toBe('enviar-recibo-whatsapp')
  })
  it('entiende el verbo conjugado, que es como escribe la gente', () => {
    ...
    expect(elegirGuia_('Como se crea una ruta a ver')).toBe('crear-ruta')
    expect(elegirGuia_('como creo una ruta a ver')).toBe('crear-ruta')
    expect(elegirGuia_('como agrego un cliente')).toBe('crear-cliente')
    expect(elegirGuia_('como elimino un prestamo')).toBe('editar-eliminar-prestamo')
  })
  it('un relato largo no es una pregunta puntual', () => {
    // Casaban palabras por acumulación: este pedía «trasladar cliente».
    expect(elegirGuia_('Buenas tardes, lo que pasa es que yo necesito una induccion, porque aqui no me dan una induccion de nada y yo tengo mis clientes y quiero saber bien todo el tema porque no me quiero equivocar con la plata de la gente que es lo mas delicado que hay en este negocio y usted sabe como es eso de complicado cuando uno empieza')).toBe(null)
  })
  it('«lo que pasa es que» es una muletilla, no el verbo trasladar', () => {
    expect(elegirGuia_('lo que pasa es que necesito una induccion porque aqui no me dan')).toBe(null)
  })
  ...
  it('nombrar algo no es preguntar cómo se hace', () => {
    expect(elegirGuia_('ya cree la ruta y quedo bien')).toBe(null)
    expect(elegirGuia_('tengo tres cobradores')).toBe(null)
  })
  it('a un registrado nunca se le manda «cómo crear una cuenta»', () => {
    ...
    expect(elegirGuia_('Quiero registrar a un cliente que ya lleva tres cuotas pagadas, como lo registro?')).not.toBe('crear-cuenta')
    expect(elegirGuia_('como me registro', { paraRegistrado: true })).not.toBe('crear-cuenta')
  })
  it('si dos guías empatan, no se manda ninguna', () => {
    expect(elegirGuia_('como hago')).toBe(null)
  })
})
