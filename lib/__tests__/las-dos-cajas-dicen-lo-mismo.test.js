...
describe('⚠ un DESCUENTO no puede encender la alarma', () => {
  ...
  it('`saltoAsientos` solo cuenta lo que afecta a la caja', () => {
    ...
    /* ⚠ EL ANCLA CAMBIÓ DE FORMA, NO DE INTENCIÓN. El filtro dejó de ser una
       línea suelta el 27 ago: ahora el asiento excluido, antes de seguir, se
       apunta con su nombre y su signo (`clasificarSinEfecto`), porque si no la
       banda descuadraba por su importe sin decir de dónde salía. Lo que esta
       prueba protege sigue igual: que `saltoAsientos` se cuente DESPUÉS del
       filtro, o los descuentos vuelven a encender la alarma. */
    const iFiltro = src.indexOf('if (!afectaCaja(m)) {')
