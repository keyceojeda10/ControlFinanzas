...
describe('⚠ el índice lleva al informe, no a su pantalla', () => {
  ...
  /* ⚠ HAY DOS MANERAS DE PONER UN ANCLA Y LAS DOS VALEN.
     ...
     nadie pinta. */
  const PINTA_DINAMICO = FUENTES.includes('id={`informe-${informe.id}`}')

  for (const i of INFORMES.filter((x) => x.ancla)) {
    it(`«${i.titulo}» tiene su ancla puesta`, () => {
      const nombre = i.ancla.slice(1)
      const literal = FUENTES.includes(`id="${nombre}"`)
      const dinamica = PINTA_DINAMICO && new RegExp(`id: '${i.id}',`).test(FUENTES)
      expect(literal || dinamica, `nada pinta ${i.ancla} para «${i.titulo}»`).toBe(true)
    })
  }
