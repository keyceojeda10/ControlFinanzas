...
import { INFORMES, PERIODOS, informesDelPlan, informeBloqueado, buscarInforme, destinoDe } from '@/lib/reportes/catalogo'
...
describe('el catálogo está bien escrito', () => {
  ...
  it('⚠ la pantalla de un informe EXISTE en el disco', () => {
    /* Escribí la primera versión del catálogo con `/reportes/entro`,
       `/reportes/calle` y cuatro más que me inventé pensando en cómo quedaría
       la pantalla nueva. Ninguna existía: seis renglones llevando a un 404.
       Ahora todas son la MISMA pantalla dinámica, así que basta con que esté —
       pero hay que comprobarlo contra el disco, no contra mi memoria. */
    const ruta = resolve(process.cwd(), 'app/(dashboard)/reportes/[informe]/page.jsx')
    expect(existsSync(ruta), 'no existe /reportes/[informe]').toBe(true)
  })

  it('⚠ y `destinoDe` lleva ahí, no a un ancla', () => {
    /* La versión anterior de esta prueba se saltaba los doce con un
       `if (!i.pantalla) continue`. Al quitar `pantalla` del catálogo siguió
       pasando en verde sin mirar nada — que es peor que fallar. */
    for (const i of INFORMES) {
      expect(destinoDe(i), `«${i.titulo}» no lleva a su pantalla`).toBe(`/reportes/${i.id}`)
    }
  })

  it('cada informe dice con qué parámetros habla su API', () => {
    /* Sin `params`, la pantalla tendría un `if` por informe y el duodécimo se
       quedaría sin filtro sin que nadie se entere. */
    for (const i of INFORMES) {
      expect(Array.isArray(i.params), `«${i.titulo}» no declara params`).toBe(true)
    }
  })
...
describe('⚠ el índice lleva al informe, no a su pantalla', () => {
  ...
  /* ⚠ AQUÍ SE LEÍAN TRES ARCHIVOS Y NADIE USABA EL RESULTADO: las pruebas de
     ancla se fueron con las anclas y `FUENTES` se quedó. Reventó el día que
     `components/pantallas/Bajar.jsx` se borró al unificar los volcados — un
     `ENOENT` que tumbaba el fichero entero de pruebas por leer algo que no se
     miraba. */

  /* ⚠ HAY DOS MANERAS DE PONER UN ANCLA Y LAS DOS VALEN.
     Unas están escritas a mano —`id="informe-seguros"`— y las de los informes
     con período las pinta UN componente para todos: `id={`informe-${x.id}`}`.
     Mi primera versión solo buscaba la literal y suspendía a los dos nuevos,
     que sí estaban puestos. El ancla dinámica se da por buena únicamente si
     ADEMÁS la pantalla le pasa ese id, o el renglón apuntaría a un ancla que
     nadie pinta. */
  /* ⚠ LAS PRUEBAS DE ANCLA SE FUERON CON LAS ANCLAS. Cada informe tiene su
     pantalla, así que ya no hay un `id="informe-x"` que pintar dentro de una
     pantalla larga. Dejarlas habría sido dejar pruebas que no miran nada. */
...
describe('⚠ un filtro que no filtra es peor que no tenerlo', () => {
  /* «Cómo rindió el negocio» declaraba cuatro períodos y su API
     —`dashboard/analiticas`— no lee un solo parámetro: siempre contesta el mes
     en curso. Los cuatro chips se pintaban, se pulsaban y devolvían lo mismo.
     Se cazó pidiendo los cuatro y comparando las respuestas, no leyendo el
     código: en el catálogo los períodos «estaban bien puestos». */
  it('el que ofrece varios períodos sabe recibir el filtro', () => {
    const mentirosos = INFORMES.filter((i) => (i.periodos?.length ?? 0) > 1 && (i.params?.length ?? 0) === 0)
    expect(
      mentirosos.map((i) => i.titulo),
      'ofrecen elegir período y su API no recibe ningún parámetro con el que filtrar',
    ).toEqual([])
  })

  it('y el que no recibe filtros ofrece un solo período', () => {
    for (const i of INFORMES.filter((x) => (x.params?.length ?? 0) === 0)) {
      expect(i.periodos?.length ?? 0, `«${i.titulo}» no puede filtrar y ofrece ${i.periodos?.length} períodos`).toBe(1)
    }
  })
})
