...
describe('la pantalla', () => {
  ...
  it('pinta la banda como BLOQUE OSCURO, no en ambar (Adenda 5 · E11)', () => {
    /* Era una banda dorada maciza. «Mismo error que la tarjeta del panel: el
       fondo dorado apaga los montos» —quedan del color de su propia caja— «y
       deja las barras invisibles».

       Compacta a propósito: 16px de radio y 16 de relleno, no los 20 del
       titular del panel, porque aquí es un resumen sobre una lista. */
    expect(jsx, 'volvio el fondo dorado')
      .not.toMatch(/background: 'var\(--cf-gold\)', color: 'var\(--cf-gold-ink\)'/)
    expect(jsx).toMatch(/background: BLOQUE\.fondo/)
    expect(jsx).toMatch(/borderRadius: 16, padding: '16px 18px'/)
  })

  it('y sus dos cifras son recaudado y te faltan', () => {
    // La gramatica del sistema, la misma que el titular del panel. Antes eran
    // «esperado» y «recaudado»: dos datos sin relacion visible entre ellos.
    expect(jsx).toContain('Recaudado hoy')
    expect(jsx).toContain('Te faltan')
    expect(jsx, 'la resta tiene que venir hecha del adaptador').toMatch(/banda\.faltan/)
  })

  it('con su barra y su porcentaje', () => {
    // Sin ella los dos montos son datos sueltos y nadie sabe si el dia va bien.
    expect(jsx).toMatch(/banda\.porcentaje/)
    expect(jsx).toMatch(/banda\.detalleCorto/)
  })
