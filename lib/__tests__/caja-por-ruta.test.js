...
// ⚠ `clienteId` va en el molde: los pagos de verdad SIEMPRE lo traen, y de él
// depende contar personas en vez de pagos («2 de 5 cobros», no «3 de 2»). Un
// molde sin él probaba algo que no existe.
let nCliente = 0
const pago = (extra = {}) => ({
  montoPagado: 10000, tipo: 'completo', metodoPago: 'efectivo',
  rutaId: 'r1', rutaNombre: 'Ruta #1', cobradorNombre: 'Pepito',
  clienteId: `c${++nCliente}`, ...extra,
})
...
describe('el nombre de la ruta no depende de /api/rutas', () => {
  it('con el nombre en el propio capital, la fila se pinta sin la otra lista', () => {
    // El fallo real: `rutasDisponibles` solo se cargaba al abrir el REPORTE, así
    // que en esta pestaña llegaba vacía, la fila se quedaba sin nombre y no se
    // pintaba. La pantalla decía «no hay cobros ni préstamos» con capital dentro.
    const cap = {
      porRuta: [{ rutaId: 'r1', nombre: 'Ruta Norte', saldoCapital: 3096800, capitalHabilitado: true }],
      global: 14357755, enRutas: 3096800, sinAsignar: 11260955, negativas: 0,
    }
    const filas = agruparCajaPorRuta([], [], undefined, [], cap, null)  // <- sin `rutas`
    expect(filas).toHaveLength(1)
    expect(filas[0].nombre).toBe('Ruta Norte')
  })
})

describe('«N de M cobros» compara personas con personas', () => {
  it('dos pagos del MISMO cliente cuentan como un cobro', () => {
    // `cobrosHoy` de /api/rutas son los CLIENTES a los que toca cobrar hoy.
    // Contando pagos salía «3 de 2 cobros», que se lee como un fallo.
    const filas = agruparCajaPorRuta([
      pago({ clienteId: 'c1', montoPagado: 30000 }),
      pago({ clienteId: 'c1', montoPagado: 20000, metodoPago: 'transferencia' }),
      pago({ clienteId: 'c2', montoPagado: 45000 }),
    ], [{ id: 'r1', cobrosHoy: 2 }])
    expect(filas[0].subtitulo).toContain('2 de 2 cobros')
    expect(filas[0].subtitulo).not.toContain('3 de 2')
    // Y la plata NO cambia: siguen siendo tres pagos.
    expect(filas[0].bruto).toBe(95000)
  })

  it('sin programados, dice cuántos pagos hubo', () => {
    const filas = agruparCajaPorRuta([
      pago({ clienteId: 'c1' }), pago({ clienteId: 'c1' }),
    ], [])
    expect(filas[0].subtitulo).toContain('2 cobros')
  })
})
