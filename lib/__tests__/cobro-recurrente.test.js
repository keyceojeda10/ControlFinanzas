...
import { MAX_FALLOS, whereCobroVivo } from '@/lib/cobro-automatico'
...
describe('⚠ el cron no puede cobrar de más', () => {
  ...
  it('cobra lo que vence antes de la próxima pasada, nunca un día antes de eso', () => {
    /* Era «solo lo que YA venció», y un plan que vencía a las 13:10 no se cobraba
       ...
       El anticipo es de una pasada —24 h— y está en lib/cobro-automatico.js. */
  ...
  it('⚠ la ventana se vuelve a mirar en la suscripción más reciente', () => {
    ...
    const bucle = cron.indexOf('for (const org of orgs)')
  ...
  it('se para a los tres rechazos', () => {
    /* El límite vive en lib/cobro-automatico.js: el mismo que decide si se
       avisa y si se deja entrar. */
    expect(MAX_FALLOS).toBe(3)
    expect(whereCobroVivo.cobroFallos).toEqual({ lt: 3 })
    expect(cron).toMatch(/\.\.\.whereCobroVivo,/)
