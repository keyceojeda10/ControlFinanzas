...
describe('⚠ la pantalla del plan: suscribirse es lo predeterminado', () => {
  ...
  it('⚠ la suscripción SIEMPRE cobra el precio mensual', () => {
    ...
    expect(calculo).toMatch(/precioPeriodo\(orgPrecio, planKey, periodoEfectivo, inicioPeriodo\)/)
  })
...
describe('⚠ la hoja de suscripción', () => {
  ...
  it('el precio que enseña es el que se va a cobrar', () => {
    ...
    expect(pantalla).toMatch(/precioMensual=\{estado\?\.proximoCobro\?\.plan === info\.key\s*\? estado\.proximoCobro\.monto\s*: precioPeriodo\(orgPrecio, info\.key, 'mensual', inicioPeriodo\)\.total\}/)
