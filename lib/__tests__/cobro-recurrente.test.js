...
describe('⚠ un solo camino para activar el plan', () => {
  ...
  it('al mandar solo se reinicia si no hay rechazo de este periodo', () => {
    ...
    expect(intento).toMatch(/return lanzarCobro\(\{ org, plan, montoCOP: precio\.monto, reiniciarFallos: !vigente, origen \}\)/)
  })
...
describe('⚠ la pantalla del plan: suscribirse es lo predeterminado', () => {
  ...
  it('⚠ la suscripción SIEMPRE cobra el precio mensual', () => {
    ...
    expect(calculo).toMatch(/precioCheckout\(orgPrecio, planKey, periodoEfectivo, inicioPeriodo\)/)
  })
...
describe('⚠ la hoja de suscripción', () => {
  ...
  it('el precio que enseña es el que se va a cobrar', () => {
    /* El primer cobro lo calcula el servidor con lib/precio-plan.js (precio
       preferencial y último pago incluidos). La hoja enseña ese mismo
       `proximoCobro`; si el plan elegido es otro, el mes de ese plan con la
       misma función. Desde el 12 sep 2026 el `descuento` % no lo mira nadie. */
    expect(pantalla).toMatch(/precioMensual=\{estado\?\.proximoCobro\?\.plan === info\.key\s*\? estado\.proximoCobro\.monto\s*: precioCheckout\(orgPrecio, info\.key, 'mensual', inicioPeriodo\)\.total\}/)
    expect(intento).toMatch(/const precio = montoDelCobro\(\{ org, plan, pagada, ultima, ahora \}\)/)
    for (const [nombre, src] of [['pantalla', pantalla], ['intento', intento], ['crear', crear], ['cron', cron]]) {
      expect(src, nombre).not.toMatch(/descuento/)
    }
  })
...
describe('⚠ el cobro al guardar el medio', () => {
  ...
  it('cobra el MES, nunca un trimestre ni un año', () => {
    ...
    expect(intento).toMatch(/const referencia = referenciaDeCobro\(org\.id, plan, 'mensual', adicionalesDe\(org, plan\), \{ automatico: true \}\)/)
  ...
  it('el checkout manual también escribe la referencia con el par unificado', () => {
    expect(crear).toMatch(/referenciaDeCobro\(orgId, plan, periodo, adicionalesDe\(org, plan\)\)/)
