...
describe('⚠ la pantalla del plan: suscribirse es lo predeterminado', () => {
  ...
  it('el interruptor NO vive dentro del desplegable de planes', () => {
    /* El botón que más se pulsa es «Renovar mi plan», que está fuera. Si el
       interruptor solo apareciera dentro de «Cambiar de plan», la mayoría
       renovaría sin enterarse de que existe la suscripción. */
    const interruptor = pantalla.indexOf("setModoPago(m.key)")
    const desplegable = pantalla.indexOf('{showPlanes && (')
    expect(interruptor).toBeGreaterThan(-1)
    expect(desplegable).toBeGreaterThan(-1)
    expect(interruptor).toBeLessThan(desplegable)
  })
