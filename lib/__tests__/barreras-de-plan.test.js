...
const BARRERAS = {
  ...
  /* ⚠ BAJÓ DE 2 A 1 (9 sep 2026). Estaba en 2 mientras el catálogo de informes
     anunciaba `calle` como nivel 1: la pantalla lo ofrecía, `descargar` dejaba
     pasar y este API contestaba 403 al final. Lo reportó un cliente recién
     subido a Básico y afectaba a los 102 negocios de nivel 1. El cruce de
     abajo impide que vuelva a desalinearse. */
  'reportes/cartera': 1,
  ...
  /* Los dos que pidió Préstamos Rincón, que es cliente de Básico: en nivel 2 no
     los alcanzaría el que los pidió. Y son la cuenta que se lleva al contador,
     no un análisis de gestión. */
  'reportes/contador': 1,
  'reportes/cuentas': 1,

  /* Los cuatro volcados que eran `/reportes/bajar`. Nivel 1, EL MISMO que tenía
     `reportes/exportar`: al unificarlos en el índice no se le puede subir el
     precio a quien ya los bajaba. */
  'reportes/datos': 1,

  /* ⚠ NO TIENE NIVEL PROPIO: exige el DEL INFORME que le pidan.
     Es la puerta única de descarga de los doce, así que fijarle un número aquí
     sería contradecir al catálogo — o dejaría bajar «Tus cobradores» (nivel 2)
     a quien solo alcanza el nivel 1. Llama a `exigeNivelReportes(session,
     informe.nivel)`, y la prueba de abajo comprueba que lo hace. */
  'reportes/descargar': 'del informe',
...
describe('barreras de plan en los reportes', () => {
  ...
  for (const [clave, nivel] of Object.entries(BARRERAS)) {
    it(`${clave} → ${nivel === null ? 'abierto a propósito' : `nivel ${nivel}`}`, () => {
      ...
      /* La puerta única de descarga no tiene nivel propio: exige el del informe
         que le pidan. Con un número fijo, o bloquearía «Para el contador»
         (nivel 1) a quien sí lo alcanza, o dejaría bajar «Tus cobradores»
         (nivel 2) a quien no. */
      if (nivel === 'del informe') {
        expect(fuente).toMatch(/exigeNivelReportes\(\s*session\s*,\s*informe\.nivel\s*\)/)
...
/* ══ LA CAUSA DE RAÍZ: EL CATÁLOGO Y EL API TIENEN QUE DECIR LO MISMO ═══════
 *
 * El fallo del 9 sep no fue que faltara una barrera —había dos, y la prueba de
 * arriba las veía las dos—: fue que **no coincidían**. El catálogo prometía
 * nivel 1 y el API que calcula los datos pedía 2, así que el informe se
 * anunciaba, se abría y reventaba al final. Comprobar cada barrera por su lado
 * no lo caza; hay que cruzarlas.
 */
describe('cada informe del catálogo lo alcanza el plan que promete', () => {
  const nivelQueExige = (ruta) => {
    const f = `app/api/${ruta.replace(/^\/api\//, '').split('?')[0]}/route.js`
    if (!existsSync(f)) return null
    const m = readFileSync(f, 'utf8').match(/exigeNivelReportes\(session,\s*([^)]+)\)/)
    if (!m) return null
    const n = Number(m[1].trim())
    return Number.isFinite(n) ? n : null      // `informe.nivel` se resuelve solo
  }

  it('⚠ ningún API pide MÁS nivel del que su informe anuncia', async () => {
    const { INFORMES } = await import('@/lib/reportes/catalogo')
    const rotos = []
    for (const inf of INFORMES) {
      for (const ruta of [inf.ver, inf.bajar].filter(Boolean)) {
        const exige = nivelQueExige(ruta)
        if (exige != null && exige > inf.nivel) {
          rotos.push(`${inf.id}: el catálogo promete ${inf.nivel} y ${ruta} exige ${exige}`)
        }
      }
    }
    expect(rotos, `informes que se ofrecen y luego dan 403:\n${rotos.join('\n')}`).toEqual([])
  })

  it('y «Lo que está en la calle» lo alcanza Básico, que es de donde salió el caso', async () => {
    const { INFORMES } = await import('@/lib/reportes/catalogo')
    const { nivelReportes } = await import('@/lib/planes')
    const calle = INFORMES.find((i) => i.id === 'calle')
    expect(nivelReportes('basic')).toBeGreaterThanOrEqual(calle.nivel)
    expect(nivelQueExige(calle.ver)).toBeLessThanOrEqual(nivelReportes('basic'))
    expect(nivelQueExige(calle.bajar)).toBeLessThanOrEqual(nivelReportes('basic'))
    // Y el Inicial sigue fuera a propósito.
    expect(nivelReportes('starter')).toBeLessThan(calle.nivel)
  })
})
