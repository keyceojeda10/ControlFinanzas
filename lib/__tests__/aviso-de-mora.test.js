...
describe('se avisa una vez, al cruzar', () => {
  ...
  it('⚠ la fila de resumen se lleva las llaves de los que no cupieron', () => {
    /* Sin esto el guion NO es idempotente: los que se quedan fuera del tope no
       quedan anotados y la corrida siguiente los vuelve a coger. Probado en el
       espejo: la segunda corrida creó 48 filas más sin que se atrasara nadie. */
    const resumen = codigo.slice(codigo.indexOf('resumen: true') - 500, codigo.indexOf('resumen: true') + 200)
    expect(resumen).toMatch(/href:/)
    expect(resumen).toMatch(/llaves: restantes\.map/)
    expect(codigo).toMatch(/Array\.isArray\(d\.llaves\)/)
  })

  it('no avisa de una deuda que no sabe nombrar', () => {
    /* 71 avisos del espejo decían «se atrasó… Debe $0»: todos de cuota única,
       donde el pago es al final y `calcularMontoEnMora` devuelve 0 a propósito
       mientras `calcularDiasMora` sí cuenta días. Un aviso que dice «debe $0»
       enseña a ignorar los avisos. */
    expect(codigo).toMatch(/monto <= 0\) continue/)
  })
})
...
describe('el tope no se calla', () => {
  it('lo que no cabe se dice, y lleva a la lista entera', () => {
    ...
    expect(codigo).toMatch(/otros \$\{restantes\.length\} clientes/)
