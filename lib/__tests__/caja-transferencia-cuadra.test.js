...
describe('«Le queda en la ruta» y lo que entrega', () => {
  // El pie de esa cifra decía «entraTotal − salioTotal», pero esos subtotales
  // son ahora la cuenta de los BILLETES: su resta da lo que hay que entregar,
  // NO lo que queda en la ruta. Se explicaba con una cuenta que da otro número
  // distinto del que está justo encima.
  //
  // Lo que sí es cierto —y es lo que ahora dice— es que las dos cifras se
  // diferencian exactamente en lo que entró a la cuenta.
  const quedaEnLaRuta = (d) =>
    d.apertura + d.cobradoEfectivo + d.cobradoDigital - d.prestadoEfectivo - d.gastos

  it('la ruta menos el banco es lo que entrega', () => {
    const c = cuenta()
    expect(quedaEnLaRuta(RUTA5) - RUTA5.cobradoDigital).toBe(c.suma)
    // Con las cifras de su captura: 160.548 − 179.000 = −18.452
    expect(quedaEnLaRuta(RUTA5)).toBe(160548)
    expect(c.suma).toBe(-18452)
  })

  it('sin transferencias las dos cifras son la misma', () => {
    const d = { ...RUTA5, cobradoDigital: 0 }
    expect(quedaEnLaRuta(d)).toBe(cuenta(d).suma)
  })
})
