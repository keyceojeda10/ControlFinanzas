...
// OJO: `ruta` (singular) es el adaptador del DETALLE; `rutas` (plural) el de la
// lista. Son dos archivos, y sus nombres se distinguen en una letra.
import { formatearKm } from '@/lib/adaptadores/ruta'
...
describe('T27-02 · «3,4 km» en la cabecera', () => {
  it('por debajo del kilometro se dice en METROS', () => {
    // «0,8 km» obliga a traducir mentalmente para saber si son dos cuadras o
    // veinte, y esta cifra existe para decidir de un vistazo si la ruta cabe
    // en una mañana.
    expect(formatearKm(850)).toBe('850 m')
    expect(formatearKm(120)).toBe('120 m')
  })

  it('a partir del kilometro, con coma y UN decimal', () => {
    // Coma porque es como se escribe en Colombia. Un solo decimal porque la
    // precision de mas es falsa: la distancia sale en linea recta entre
    // puntos, no por las calles que se caminan de verdad.
    expect(formatearKm(3400)).toBe('3,4 km')
    expect(formatearKm(12000)).toBe('12,0 km')
  })

  it('sin distancia NO devuelve «0 km»', () => {
    // La API manda `null` cuando hay menos de dos clientes con coordenadas. Un
    // «0 km» ahi se leeria como que la ruta entera esta en el mismo portal.
    expect(formatearKm(null)).toBeNull()
    expect(formatearKm(0)).toBeNull()
    expect(formatearKm(undefined)).toBeNull()
    expect(formatearKm('hola')).toBeNull()
  })
})
