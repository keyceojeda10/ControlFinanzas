...
describe('T15-02 · tres salidas por préstamo', () => {
  it('cada préstamo lleva Cuota, Otro monto y No pagó', () => {
    // El texto es condicional: «Ya cobrado» cuando ese prestamo ya se cobro hoy.
    expect(atajos).toMatch(/p\.pagadoHoy \? 'Ya cobrado' : 'Cuota'/)
    expect(atajos).toMatch(/>Otro monto</)
    expect(atajos).toMatch(/>No pagó</)
  })

  it('el prestamo YA COBRADO hoy no se vuelve a cobrar de un toque', () => {
    ...
    expect(atajos).toMatch(/disabled=\{ocupado \|\| !\(cuota > 0\) \|\| p\.pagadoHoy\}/)
    // «Otro monto» SIGUE vivo: un segundo abono el mismo dia es legitimo; lo
    // que no puede pasar es de un toque y sin decidirlo.
    expect(atajos).toMatch(/<Boton disabled=\{ocupado\} onClick=\{\(\) => \{ setMonto\(''\); setMontoAbierto\(p\.id\) \}\}>Otro monto<\/Boton>/)
  })
