...
describe('las TRES puertas se abren a la vez', () => {
  ...
  it('el auto-completar por misiones cumplidas tampoco', () => {
    /* Tercera puerta: una cuenta parada que ya tiene sus misiones hechas la
       cumple, así que sin excluir lo reabierto se cierra otra vez en la misma
       petición.

       ⚠ Se comprueba la INTENCIÓN, no el nombre de la variable. Esta prueba
       exigía literalmente `coreCompleto`, y al cambiar la regla —cerrar cuando
       está hecho todo lo comprobable, no solo el core— falló por el renombrado
       aunque la guardia seguía intacta. Una prueba que fija un identificador
       no protege la conducta: estorba al arreglarla. */
    const guardia = api.match(/if \(!reapertura && (\w+) &&/)
    expect(guardia, 'la tercera puerta empieza por !reapertura').not.toBeNull()
    // Y esa condición se calcula sobre las misiones, no sobre un subconjunto.
    expect(api).toMatch(new RegExp(`const ${guardia[1]} = .*misiones|const ${guardia[1]} = .*every`))
  })
