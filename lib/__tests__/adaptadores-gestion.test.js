...
import {
  adaptarRecargo, atajosDeRecargo,
...
describe('T19-05 · corregir', () => {
  it('la consecuencia lleva EL NÚMERO REAL de pagos, campo por campo', () => {
    ...
    expect(r.peligrosos.find((c) => c.clave === 'monto').consecuencia).toBe('Recalcula 22 pagos')
  ...
  it('los recargos y descuentos NO cuentan como pagos', () => {
    // No son cobros: mueven la deuda en los papeles. Contarlos infla el número que
    // se usa para asustar.
    ...
    expect(adaptarCorregir(conAjustes).peligrosos[0].consecuencia).toBe('Recalcula 1 pago')
  })

  it('sin pagos, corregir es SEGURO y lo dice', () => {
    const r = adaptarCorregir({ ...P, pagos: [] })
    expect(r.aviso).toMatch(/corregir es seguro/)
