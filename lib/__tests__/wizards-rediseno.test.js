...
describe('T01-06 · la franja de acción es UNA sola pieza', () => {
  ...
  it('el contenido reserva hueco para la franja', () => {
    ...
    //
    // Se busca por la FORMA, no por el literal entero: la clase se ataba a
    // `max-w-2xl mx-auto pb-40 lg:pb-36` clavado, y al ensanchar la caja para
    // las dos columnas de T16-00 (`xl:max-w-[1076px]`) esta prueba se cayó sin
    // que el hueco hubiera cambiado. Lo que importa es que el `pb` exista.
    const caja = prestamo.match(/className="max-w-2xl[^"]*"/)?.[0]
    expect(caja, 'ya no hay caja `max-w-2xl` en la pantalla').toBeTruthy()
    expect(caja, 'la caja perdió el hueco de abajo y la franja tapa el último campo')
      .toMatch(/\bpb-\d+\b[\s\S]*\blg:pb-\d+\b/)
  })
})
...
/* Desde el 19 sep «Cliente creado» es su propio componente (`ClienteCreado`,
   la pantalla aprobada): el botón «Cargar otro cliente» vive allí y el REINICIO
   del formulario sigue aquí, como su `onOtro`. Se ancla a ese manejador, que
   termina en `window.scrollTo`. */
function reinicioDeCadena() {
  const desde = cliente.indexOf('onOtro={() => {')
  if (desde === -1) return null
  const fin = cliente.indexOf('window.scrollTo', desde)
  return fin === -1 ? null : cliente.slice(desde, fin)
}
...
describe('T07-03 · cargar clientes en cadena', () => {
  it('ofrece seguir con otro sin volver a la lista', () => {
    ...
    const creado = sinComentarios(leer('components', 'cf', 'ClienteCreado.jsx'))
    expect(creado).toMatch(/onClick=\{onOtro\}[\s\S]{0,400}>Cargar otro cliente</)
    expect(cliente).toMatch(/<ClienteCreado[\s\S]*onOtro=\{\(\) => \{/)
  })
