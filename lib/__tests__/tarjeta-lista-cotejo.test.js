...
describe('la tira de cifras — lo que faltaba', () => {
  ...
  it('dentro de la tarjeta el valor parte de 14, no de 15', () => {
    ...
    expect(primitivos).toMatch(/fontSize: \(enTarjeta \? 14 : sobreOscuro \? 16 : 15\) - pasoLargo\(c\.valor\)/)
  })

  it('y no puede desbordar su columna pase lo que pase', () => {
    // El ajuste de cuerpo aprieta; el recorte es el que garantiza. La etiqueta
    // ya lo llevaba desde siempre y el valor no llevaba nada.
    const bloque = primitivos.slice(primitivos.indexOf('export function TiraCifras'))
    const i = bloque.indexOf('className="cf-fig"')
    expect(bloque.slice(i, bloque.indexOf('}}>', i))).toMatch(/overflow: 'hidden'/)
  })
