...
describe('la tira de cifras — lo que faltaba', () => {
  ...
  it('los valores llevan cifras tabulares (regla global 1)', () => {
    ...
    expect(primitivos).toMatch(/<span className="cf-fig" style=\{\{/)
  })

  it('dentro de la tarjeta el valor parte de 14, no de 15', () => {
    // «Parte de» y ya no «es»: el cuerpo base sigue siendo el mismo, pero se le
    // resta un tramo cuando la cifra es larga. Sin eso, un atraso de siete
    // cifras se salia de su columna y se metia encima de la de al lado —«
    // $1.272.0004 0%», el atraso pegado al cumplimiento—, y en una pantalla de
    // cobro eso es pedirle al cliente la cifra equivocada.
    // Desde el 19 sep 2026 el cuerpo base se lo pasa a `CifraQueCabe`, que lo
    // encoge con el ancho real de la columna (antes, por tramos y con recorte).
    expect(primitivos).toMatch(/<CifraQueCabe valor=\{c\.valor\} base=\{enTarjeta \? 14 : sobreOscuro \? 16 : 15\}/)
  })

  it('y no puede desbordar su columna NI perder un dígito', () => {
    // El recorte «garantizaba» el ancho comiéndose el último cero: «$1.272.00».
    const bloque = primitivos.slice(primitivos.indexOf('export function TiraCifras'))
    expect(bloque).toMatch(/className="cf-cifra-col"/)
    expect(bloque).not.toMatch(/overflow: 'hidden', textOverflow: 'clip'/)
  })
...
describe('lo que aportan los adaptadores', () => {
  it('NINGUNO trae ya el rótulo «Deuda total»: T03 lo quita', () => {
    ...
    // En su sitio, bajo el monto, va cuantos prestamos tiene — con «debe»
    // delante, porque en la lista de PRÉSTAMOS esa misma esquina es lo pagado.
    expect(c.detalle).toBe('debe en 3 préstamos')
...
describe('el nombre del cliente tiene su línea entera', () => {
  it('la pastilla «Nuevo» NO comparte línea con el nombre', () => {
    ...
    expect(lineaDelNombre, 'la pastilla volvió a la línea del nombre')
      .not.toMatch(/Creado en las últimas 24 horas|<EtiquetaNuevo/)
  })
  ...
  it('y sí está en la fila de estado, con «Al día»', () => {
    ...
    // Desde el 19 sep 2026 es `EtiquetaNuevo`, la misma `Pastilla` que «Al día».
    expect(fila, 'la pastilla «Nuevo» ya no se pinta en la fila de estado')
      .toMatch(/<EtiquetaNuevo nuevo=\{nuevo\} \/>/)
  })
