...
describe('cuándo NO ofrece (lo que más importa)', () => {
  it('en mora, avisa en vez de ofrecer', () => {
    ...
    expect(s.titular).toContain('12 días de atraso')
  })
...
describe('la etiqueta de estado', () => {
  ...
  it('un atraso corto se dice, pero sin alarma', () => {
    /* ⚠ ESTE HUECO LO ENCONTRÉ EN EL ESPEJO, no en las pruebas. La oferta pedía
       estar al día y el aviso arrancaba a los 7 días: entre 1 y 6 el bloque
       DESAPARECÍA, y ese es el estado más común de todos.
       El caso real: ARIEL GIL, 3 préstamos pagados, tope puesto, 4 días de
       atraso — y la ficha no decía nada. */
    const s = generarTipCliente(cli({ montoMaximoPrestamo: 1000000 }), [prestamo({ diasMora: 4 })], { completados: 3, incumplidos: 0 })
    expect(s, 'con atraso corto vuelve a quedarse mudo').not.toBe(null)
    expect(s.tono, 'un atraso de 4 días no es una alarma').toBe('info')
    expect(s.monto, 'ofrece prestarle a quien va atrasado').toBeUndefined()
    expect(s.titular).toContain('4 días')
    expect(s.porque, 'no dice que suele pagar bien').toContain('3 préstamos completos')
  })

  it('«se atrasó un día» va en singular', () => {
    const s = generarTipCliente(cli({}), [prestamo({ diasMora: 1 })], { completados: 0 })
    expect(s.titular).toBe('Se atrasó un día.')
  })
})
