...
describe('la tira que se desplaza', () => {
  ...
  it('los chips se desplazan, pero el filtro NO', () => {
    // ⚠ ESTA PRUEBA DECIA LO CONTRARIO —«el filtro va DENTRO»— y estaba mal.
    // El dueño lo cazo a la primera con una captura: «paso lo que no queria,
    // que el filtro se desplazara junto con las opciones del menu horizontal;
    // el filtro no se puede mover porque si no por defecto sale escondido».
    //
    // Con cuatro o cinco estados la tira arranca desplazada y el ultimo
    // elemento nace fuera de la pantalla. Es el MISMO aviso que ya estaba
    // escrito para el conmutador de vista, y no lo aplique al filtro.
    const iChips = barra.indexOf('const chips = (')
    const iCorte = barra.indexOf('if (!onMasFiltros) return chips')
    expect(iChips, 'no se encontro el bloque de chips').toBeGreaterThan(0)
    expect(iCorte, 'no se encontro el corte').toBeGreaterThan(iChips)
    const scroll = barra.slice(iChips, iCorte)
    expect(scroll, 'los chips si se desplazan').toMatch(/overflowX: 'auto'/)
    expect(scroll, 'el filtro no puede estar dentro del scroll').not.toMatch(/onMasFiltros/)
    // El margen negativo se queda: es lo que hace que las pastillas lleguen al
    // borde de la pantalla.
    expect(scroll).toContain("margin: '0 calc(-1 * var(--cf-pad-screen))'")
  })

  it('el filtro anclado lleva su propio fondo', () => {
    // Los chips pasan por DEBAJO al deslizar —la tira se sale de su caja 20px
    // por lado— y sin fondo se verian encima del filtro.
    const anclado = barra.slice(barra.indexOf('if (!onMasFiltros) return chips'))
    expect(anclado).toContain("background: 'var(--cf-surface)'")
    expect(anclado).toContain("flex: 'none'")
  })
})
