...
describe('el SQL dice lo MISMO que el JavaScript', () => {
  ...
  const evaluarSql = (expr, { montoPagado, totalAPagar, montoPrestado, tipo = 'completo', cobradoDeInteres = 0, totalAPagarPrevio = null, interesArrastrado = null }) => {
    const js = expr
      ...
      .replace(
        /COALESCE\(\(SELECT SUM\(pi\.montoPagado\) FROM Pago pi WHERE pi\.prestamoId = pr\.id AND pi\.tipo = 'intereses'\), 0\) \* \(NOT EXISTS \(SELECT 1 FROM CuotaAmortizacion ca WHERE ca\.prestamoId = pr\.id\)\)/g,
        String(cobradoDeInteres),
      )
      /* ⚠ TANTOS `WHEN` COMO HAGA FALTA. El traductor entendía uno; al no
         repartirse el pago etiquetado como interés pasaron a dos, y con el
         abono a capital fuera del reparto, a TRES. Cada vez que se añadía uno
         el `CASE` entero se colaba crudo al `new Function` y reventaba con
         «Unexpected identifier 'WHEN'». Escrito en bucle no hay que volver. */
      .replace(/CASE (WHEN [\s\S]+?) ELSE ([\s\S]+?) END/g, (_, ramas, otro) => {
        const pares = ramas.split(/\bWHEN\b/).map((r) => r.trim()).filter(Boolean).map((r) => {
          const i = r.indexOf(' THEN ')
          return [r.slice(0, i), r.slice(i + ' THEN '.length)]
        })
        return pares.reduceRight((acc, [cond, val]) => `((${cond}) ? (${val}) : ${acc})`, `(${otro})`)
      })
    ...
    /* `COALESCE` y `NULLIF` se evalúan como en MariaDB: el primero que no sea
       NULL, y NULL si los dos son iguales. Los usan las dos piezas de la
       renovación (`totalDelRepartoSql`, `capitalDelPrestamoSql`). */
    const COALESCE = (...v) => v.find((x) => x != null) ?? null
    const NULLIF = (a, b) => (a === b ? null : a)
    return new Function('P_montoPagado', 'PR_totalAPagar', 'PR_montoPrestado', 'P_tipo',
      'PR_totalAPagarPrevio', 'PR_interesArrastrado', 'COALESCE', 'NULLIF',
      `return ${js}`)(montoPagado, totalAPagar, montoPrestado, tipo,
      totalAPagarPrevio, interesArrastrado, COALESCE, NULLIF)
  }
  ...
  /* ⚠ Y LAS DOS PIEZAS DE LA RENOVACIÓN (21 sep 2026). El viejo se cierra con el
     total igual a lo pagado y guarda el pactado en `totalAPagarPrevio`; el nuevo
     lleva dentro el interés sin cobrar en `interesArrastrado`. Si el SQL no las
     leyera, analíticas y el reparto a socios volverían a borrar esa ganancia
     mientras el Inicio la cuenta. */
  const RENOVADOS = [
    { nombre: 'viejo cerrado al renovar', montoPagado: 519000, totalAPagar: 519000, montoPrestado: 1000000, totalAPagarPrevio: 1200000 },
    { nombre: 'nuevo con interés arrastrado', montoPagado: 200000, totalAPagar: 1177200, montoPrestado: 981000, interesArrastrado: 113500 },
    { nombre: 'nuevo que era todo interés', montoPagado: 50000, totalAPagar: 240000, montoPrestado: 200000, interesArrastrado: 200000 },
  ]

  it('las renovaciones se reparten igual en SQL que en JavaScript', () => {
    const sql = repartoSql()
    for (const c of RENOVADOS) {
      const enJs = repartirPagado({
        montoPrestado: c.montoPrestado, totalAPagar: c.totalAPagar,
        totalAPagarPrevio: c.totalAPagarPrevio, interesArrastrado: c.interesArrastrado,
        totalPagado: c.montoPagado, pagos: [],
      })
      expect(Math.round(evaluarSql(sql.interes, c)), `interés · ${c.nombre}`).toBe(enJs.interes)
      expect(Math.round(evaluarSql(sql.capital, c)), `capital · ${c.nombre}`).toBe(enJs.capital)
  ...
  it('agregando POR PRÉSTAMO no pide `tipo`, que ahí no existe', () => {
    // ⚠ ESTO ROMPIÓ «¿CÓMO VA EL NEGOCIO?» EN PRODUCCIÓN. Analíticas llama con
    // `pago: 'pr'` para sumar el `totalPagado` del préstamo entero; al añadir el
    // `CASE WHEN pago.tipo = 'intereses'` la expresión pasó a exigir `pr.tipo`,
    // que en `Prestamo` no existe, y la pantalla devolvía «No pudimos cargar tus
    // números». La prueba de paridad no lo vio: solo evalúa `pago: 'p'`.
    const vida = repartoSql({ pago: 'pr', prestamo: 'pr', monto: 'totalPagado', porFila: false })
    expect(vida.interes).not.toMatch(/pr\.tipo/)
    expect(vida.capital).not.toMatch(/pr\.tipo/)

    // Y la forma por fila SÍ lo pregunta: es la que distingue el pago etiquetado.
    const porFila = repartoSql()
    expect(porFila.interes).toMatch(/p\.tipo = 'intereses'/)
  })
