...
// ── 20 sep 2026 · «ese "cómo paga" no se entiende una mierda» — el dueño ──────
import { resumenDe, diasCobrablesEnMes as diasMes, lecturaDe as leer2, comportamiento12Meses as doce } from '../comportamiento'

describe('«Cómo paga» dice las cosas con palabras y con plata', () => {
  const mes = (nombre, cumplio, pagado, esperado) => ({ nombre, corta: nombre.slice(0, 3), cumplio, pagado, esperado, estado: 'x' })

  it('los días sin cobro llegan como NÚMEROS y ya no se tiran', () => {
    // `obtenerDiasSinCobro` devuelve [0]. Solo se entendían nombres ('domingo'):
    // a un diario se le pedían los siete días y quien pagaba todo salía al 86 %.
    const septiembre = { desde: new Date(Date.UTC(2026, 8, 1)), hasta: new Date(Date.UTC(2026, 8, 30, 23, 59, 59)) }
    const p = { fechaInicio: new Date(Date.UTC(2026, 0, 1)) }
    expect(diasMes(septiembre, p, [])).toBe(30)
    expect(diasMes(septiembre, p, [0])).toBe(26)          // cuatro domingos menos
    expect(diasMes(septiembre, p, ['0'])).toBe(26)
    expect(diasMes(septiembre, p, ['domingo'])).toBe(26)  // la forma vieja sigue valiendo
  })

  it('cada mes trae su nombre: una letra no distingue agosto de abril', () => {
    const meses = doce({ prestamos: [], pagos: [], hasta: new Date(Date.UTC(2026, 8, 20)) })
    expect(meses.at(-1)).toMatchObject({ nombre: 'septiembre', corta: 'sept', anio: 2026 })
    expect(meses.at(-2)).toMatchObject({ nombre: 'agosto', corta: 'ago' })
  })

  it('«desde agosto», no «desde abril»', () => {
    const meses = [
      mes('mayo', 100, 1, 1), mes('junio', 100, 1, 1), mes('julio', 100, 1, 1),
      { ...mes('agosto', 0, 0, 1), etiqueta: 'A' }, { ...mes('septiembre', 0, 0, 1), etiqueta: 'S' },
    ]
    // Los tres últimos con dato son julio, agosto y septiembre; el primero torcido, agosto.
    expect(leer2(meses)).toContain('Desde agosto')
  })

  it('el veredicto sale de la plata de los últimos tres meses con dato', () => {
    const r = resumenDe([mes('julio', 100, 150000, 150000), mes('agosto', 0, 0, 150000), mes('septiembre', 0, 0, 75000)])
    expect(r).toMatchObject({ veredicto: 'mal', titulo: 'Viene fallando', pagado: 150000, esperado: 375000, pct: 40, desde: 'julio', hasta: 'septiembre', completos: 1 })
    expect(resumenDe([mes('agosto', 100, 9, 9), mes('septiembre', 95, 95, 100)]).veredicto).toBe('bien')
    expect(resumenDe([mes('septiembre', 70, 70, 100)])).toMatchObject({ veredicto: 'regular', cuantos: 1 })
  })

  it('sin un solo mes con préstamo no inventa un veredicto', () => {
    expect(resumenDe([{ cumplio: null }, { cumplio: null }])).toMatchObject({ veredicto: 'sin', conDato: 0 })
  })
})

describe('«Cómo paga»: una sola regla da el veredicto', () => {
  const mes = (nombre, cumplio, pagado, esperado) => ({ nombre, corta: nombre.slice(0, 3), cumplio, pagado, esperado })
  it('la tendencia solo dice si ANTES pagaba distinto; no repite ni contradice el título', () => {
    const bajo = resumenDe([mes('marzo', 100, 1, 1), mes('abril', 100, 1, 1), mes('mayo', 100, 100, 100), mes('junio', 40, 40, 100), mes('julio', 0, 0, 100)])
    expect(bajo.tendencia).toBe('Antes pagaba mejor: bajó desde junio.')
    // La racha mala empieza ANTES de los últimos tres meses: se dice desde ahí.
    const racha = resumenDe([mes('abril', 100, 1, 1), mes('mayo', 100, 1, 1), mes('junio', 39, 39, 100), mes('julio', 4, 4, 100), mes('agosto', 0, 0, 100), mes('septiembre', 0, 0, 100)])
    expect(racha.tendencia).toBe('Antes pagaba mejor: bajó desde junio.')
    const subio = resumenDe([mes('marzo', 0, 0, 1), mes('abril', 10, 1, 10), mes('mayo', 100, 100, 100), mes('junio', 100, 100, 100), mes('julio', 100, 100, 100)])
    expect(subio).toMatchObject({ veredicto: 'bien', tendencia: 'Antes pagaba peor: últimamente ha mejorado.' })
    // Sin meses anteriores, o sin cambio, no hay nada que añadir.
    expect(resumenDe([mes('junio', 80, 80, 100), mes('julio', 80, 80, 100)]).tendencia).toBeNull()
  })
  it('la tarjeta pinta la tendencia del resumen, no la frase de la otra regla', () => {
    const src = require('fs').readFileSync('components/pantallas/FichaCliente.jsx', 'utf8')
    expect(src).toContain('{resumen.tendencia ? ` ${resumen.tendencia}` : \'\'}')
    // Y un mes sin préstamo no se pinta como «0 %».
    expect(src).toMatch(/if \(sinPrestamo\) \{\s*return \(/)
    expect(src).toContain('Sin préstamo ese mes')
  })
})
