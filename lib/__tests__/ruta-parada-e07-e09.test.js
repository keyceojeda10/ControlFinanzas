...
describe('la tarjeta de la ruta es LA MISMA de cobros-hoy', () => {
  ...
  it('el display del carril va en la clase, nunca en el style', () => {
    ...
    // clase, asi que el `lg:hidden` no haria nada.
    //
    // El `lg:hidden` ahora es condicional (`carrilSiempre`): en /cobros-hoy el
    // carril se sigue quitando en PC —ahi no hay recorrido que numerar— y en la
    // ruta se queda, porque el numero es la posicion en el recorrido y el dueno
    // ya lo rebatio una vez: «no sale que numero de lista es en ruta, y eso es
    // importantisimo». Lo que esta prueba cuida sigue siendo lo mismo: que el
    // display no se escriba en el `style`.
    const bloque = tarjeta.match(/carrilSiempre \? '' : 'lg:hidden '[^>]*/)[0]
