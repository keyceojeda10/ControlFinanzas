...
describe('la frase de quien pregunta encuentra la acción', () => {
  ...
  it('⚠ la LISTA de clientes no lleva caja, y es a propósito', () => {
    /* La tuvo un día y el dueño la mandó quitar: «esa pantalla quedó
       terriblemente ubicada». Con los grupos de cobro fuera, lo único que le
       quedaba por ofrecer era «nuevo cliente», «filtros» y «cambiar vista», y
       las tres tienen botón a la vista. Un buscador que repite lo que ya se ve
       es ruido, y ahí costaba 83px por delante del título.

       Donde sí está es en la FICHA, que esconde de verdad. */
    const lista = leer('app/(dashboard)/clientes/page.jsx')
    expect(lista).not.toContain('QueNecesitas')
    expect(lista).not.toContain('grupoCobro')
  })
...
describe('caja y socios hablan como habla la gente', () => {
  ...
  it('⚠ «meter plata» en Caja LLEVA a Mi plata, no finge hacerlo aquí', () => {
    ...
    expect(src).toMatch(/Está en Capital/i)
