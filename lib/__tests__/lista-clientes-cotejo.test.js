...
describe('los chips, con las medidas de las dos laminas de lista', () => {
  ...
  it('el cuarto chip abre la HOJA DE FILTROS, no el modal de grupos', () => {
    ...
    // ⚠ ESTA PRUEBA FIJABA EL BOTON SUELTO ARRIBA, y el dueño pidio lo
    // contrario con la maqueta al lado: «quisiera dejar el filtro abajo, junto
    // al menu de Todos / Al dia / En mora, para que la barra de busqueda se
    // haga mas grande». El chip del final de la tira YA EXISTIA en
    // `BarraFiltros` y es lo que dibuja la maqueta; lo que faltaba era
    // pasarselo. Ver `controles-lista.test.js`.
    //
    // Lo que esta prueba defendia y SIGUE siendo cierto: abre `HojaFiltros`, no
    // el modal de grupos, y las dos pantallas hermanas hacen lo mismo.
    expect(pagina).toMatch(/onMasFiltros=\{\(\) => setHojaFiltros\(true\)\}/)
    expect(pagina).toMatch(/<HojaFiltros/)

    const prest = fs.readFileSync(path.join(process.cwd(), 'app/(dashboard)/prestamos/page.jsx'), 'utf8')
    for (const t of ['onMasFiltros={() => setHojaFiltros(true)}', 'flex-1 min-w-0']) {
  ...
  it('los filtros que se ofrecen son los que el servidor sabe filtrar', () => {
    // `/api/clientes` acepta `rutaId`. Los de mora y «le toca hoy» no, y
    // filtrarlos en el navegador solo miraria los 50 de la pagina: un filtro
    // que miente es peor que no tenerlo.
    //
    // Aqui tambien se exigia `grupo`. Los grupos de cobro se retiraron: 8 en
    // toda la historia, 4 de 439 negocios, y el ultimo el 15 de junio con el
    // boton todavia puesto.
