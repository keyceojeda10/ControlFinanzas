...
describe('la cadena de fotos no se queda coja', () => {
  /* ⚠ LA RESERVA DE RUTA GUARDA SU PAR DE FOTOS Y NO MUEVE EL SALDO. Si el
     recálculo no se las refresca, se quedan congeladas en el saldo que había el
     día que se escribieron: la cadena deja de ser contigua, `resumirLibro` lee
     la apertura del primer asiento y el cierre del último, y los dos mienten.
     Medido: 33 reservas rancias dejaban 9 org-días con el aviso rojo en 4
     negocios, $14.543.500 — y en uno la última foto decía $430.900 donde el
     capital real era $2.118.900. */
  const correr = async (movs, saldoInicial) => {
    const copia = new Map(movs.map((m) => [m.id, { ...m }]))
    let saldoEscrito = null
    const tx = {
      $queryRaw: async () => [{ id: 'c', saldo: saldoInicial }],
      capital: { create: async () => ({ id: 'c', saldo: 0 }),
        findUnique: async () => ({ id: 'c', saldo: saldoInicial }),
        update: async ({ data }) => { saldoEscrito = Math.round(data.saldo) } },
      movimientoCapital: { findMany: async () => movs.map((m) => ({ ...m })),
        update: async ({ where, data }) => Object.assign(copia.get(where.id), data) },
      ruta: { findMany: async () => [], update: async () => {} },
    }
    await recalcularSaldosCapital(tx, { organizationId: 'o' })
    return { filas: [...copia.values()], saldoEscrito }
  }

  it('le refresca la foto a la reserva de ruta, sin moverle el saldo', async () => {
    const movs = [
      { id: 'a', tipo: 'capital_inicial', monto: 1_000_000, saldoAnterior: 0, saldoNuevo: 1_000_000,
        rutaId: null, ajusteArranqueRuta: false, noMueveCapital: false, createdAt: '2026-08-01T10:00:00Z' },
      // La reserva, con la foto congelada en un saldo que ya no existe.
      { id: 'b', tipo: 'desembolso', monto: 300_000, saldoAnterior: 500_000, saldoNuevo: 500_000,
        rutaId: 'r1', ajusteArranqueRuta: true, noMueveCapital: false, createdAt: '2026-08-02T10:00:00Z' },
      { id: 'c', tipo: 'gasto', monto: 50_000, saldoAnterior: 1_000_000, saldoNuevo: 950_000,
        rutaId: null, ajusteArranqueRuta: false, noMueveCapital: false, createdAt: '2026-08-03T10:00:00Z' },
    ]
    const { filas, saldoEscrito } = await correr(movs, 950_000)
    const reserva = filas.find((f) => f.id === 'b')
    // Se le pone la foto del saldo que iba corriendo, las dos iguales.
    expect(reserva.saldoAnterior).toBe(1_000_000)
    expect(reserva.saldoNuevo).toBe(1_000_000)
    // Y el capital NO se mueve: la reserva sigue sin tocar el saldo global.
    expect(saldoEscrito).toBe(950_000)
  })

  it('con la foto rancia, la apertura del día miente y la banda descuadra', async () => {
    /* La forma real: la reserva es el PRIMER asiento del día y `resumirLibro`
       lee la apertura de su foto congelada. El día entero descuadra por una
       fila que no movió nada. */
    const historia = [
      { id: 'a', tipo: 'capital_inicial', monto: 1_000_000, saldoAnterior: 0, saldoNuevo: 1_000_000,
        rutaId: null, ajusteArranqueRuta: false, noMueveCapital: false, createdAt: '2026-08-01T10:00:00Z' },
      { id: 'b', tipo: 'desembolso', monto: 300_000, saldoAnterior: 500_000, saldoNuevo: 500_000,
        rutaId: 'r1', ajusteArranqueRuta: true, noMueveCapital: false, createdAt: '2026-08-05T10:00:00Z' },
      { id: 'c', tipo: 'gasto', monto: 50_000, saldoAnterior: 1_000_000, saldoNuevo: 950_000,
        rutaId: null, ajusteArranqueRuta: false, noMueveCapital: false, createdAt: '2026-08-05T11:00:00Z' },
    ]
    const delDia = (fs) => fs.filter((f) => f.createdAt.startsWith('2026-08-05'))
    // El saldo que enseña la pantalla es el cierre del libro.
    const banda = (fs) => { const lib = resumirLibro(delDia(fs), null)
      return lineasDeLaBanda({ libro: lib, saldo: lib.cierre }) }

    const antes = banda(historia)
    expect(antes.cuadra).toBe(false)
    expect(antes.saldo - antes.suma).toBe(500_000)   // la foto rancia, al peso

    const { filas } = await correr(historia, 950_000)
    expect(banda(filas).cuadra).toBe(true)
  })

  it('un asiento que no mueve el saldo tampoco lo mueve al recalcular', async () => {
    /* Era una mina: `esIngresoMovimiento` leía los dos saldos iguales como
       INGRESO y le SUMABA el monto al capital. Se disparaba al borrar o editar
       un movimiento, o al meter uno con fecha atrasada. */
    const movs = [
      { id: 'a', tipo: 'capital_inicial', monto: 1_000_000, saldoAnterior: 0, saldoNuevo: 1_000_000,
        rutaId: null, ajusteArranqueRuta: false, noMueveCapital: false, createdAt: '2026-08-01T10:00:00Z' },
      { id: 'b', tipo: 'ajuste', monto: 70_000, saldoAnterior: 1_000_000, saldoNuevo: 1_000_000,
        descripcion: 'Descuento aplicado - préstamo', rutaId: null,
        ajusteArranqueRuta: false, noMueveCapital: true, createdAt: '2026-08-02T10:00:00Z' },
    ]
    const { saldoEscrito } = await correr(movs, 1_000_000)
    expect(saldoEscrito).toBe(1_000_000)   // NO 1.070.000
  })
})

describe('el aviso también existe en el computador', () => {
  it('se pinta el texto, no el objeto', () => {
    /* `descuadre` es `{texto, diferencias}`. Pintarlo con `{descuadre}` hace que
       React reviente al renderizar un objeto como hijo: el dueño que abría la
       caja en el computador el día que no cuadraba no veía un aviso, veía la
       pantalla caerse. */
    const src = leer('components/pantallas/CajaEscritorio.jsx')
    expect(src).toContain('{descuadre?.texto && (')
    expect(src).toContain('{descuadre.texto}')
