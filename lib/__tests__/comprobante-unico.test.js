...
describe('los tres caminos sacan el mismo comprobante', () => {
  it('la ficha del préstamo monta `pantallas/Recibo`', () => {
    ...
    expect(registrar).toMatch(/import \{[^}]*\bRecibo\b[^}]*\} from '@\/components\/pantallas\/Recibo'/)
    expect(registrar).toMatch(/<Recibo\b/)
  })

  it('el cobro desde la ruta también, como ya hacía', () => {
    expect(ruta).toMatch(/import \{[^}]*\bRecibo\b[^}]*\}\s+from '@\/components\/pantallas\/Recibo'/)
  })
...
describe('no se perdió nada del comprobante viejo', () => {
  ...
  it('2 · el pago guardado sin señal se distingue del que ya subió', () => {
    ...
    // 5 sep 2026: el aviso dejó de ser una línea pequeña bajo «Pago registrado»;
    // ahora el TÍTULO dice «Guardado en el teléfono» y debajo lo que significa.
    expect(recibo).toMatch(/\{offline \? 'Guardado en el teléfono' : titulo\}/)
    expect(recibo).toMatch(/Todavía no está en el sistema/)
...
describe('«Guardar imagen» e «Imprimir» hacen algo', () => {
  ...
  it('las dos acciones viven en un solo sitio', () => {
    ...
    /* `guardarReciboImagen` se fue el 14 sep 2026: mandaba el PNG a la hoja del
       teléfono en el mismo toque y el cobrador no veía el papel hasta que ya
       estaba en el chat del cliente. Ahora los tres caminos abren la VISTA
       PREVIA, y el trabajo de entregar el archivo vive en `lienzo-compartir`. */
    const previa = readFileSync(resolve(RAIZ, 'components/recibos/HojaReciboPrevio.jsx'), 'utf8')
    const lienzo = readFileSync(resolve(RAIZ, 'lib/lienzo-compartir.js'), 'utf8')
    expect(previa).toMatch(/export default function HojaReciboPrevio/)
    expect(lienzo).toMatch(/export function compartirArchivo/)
    expect(acciones, 'volvió la acción que mandaba el recibo sin enseñarlo')
      .not.toMatch(/export function guardarReciboImagen/)
  ...
  it('⚠ y la RUTA los conecta: sus dos botones no hacían nada', () => {
    ...
    /* ⚠ SE COMPRUEBA QUE ESTÉN CONECTADOS, no cómo está escrito el `onClick`.
       La versión anterior exigía `onGuardarImagen={() => {`, con la llave, y se
       puso roja el día que los dos botones pasaron a llamar a una función común
       —que es una mejora—. Un contrato de forma, no de comportamiento. */
    ...
    expect(ruta, 'la ruta volvió a quedarse sin la acción de imprimir')
      .toMatch(/onImprimir=\{/)
    expect(ruta).toMatch(/<HojaReciboPrevio/)
    expect(ruta).toMatch(/imprimirRecibo\(/)
  })

  it('⚠ y se arma con la respuesta del pago, no con el estado de la pantalla', () => {
    /* EL FALLO DE PRESTA MIL, 21 ago: «el cliente me hizo un abono de 30 mil y
       ahí vuelve y dice saldo total 124, cuando debería quedar en 94».

       Los botones sacaban el préstamo de `ruta.clientes[…].prestamosActivos[0]`,
       o sea del estado de la pantalla, que al pulsar todavía era el de ANTES del
       cobro. El comprobante salía con «Total pagado $0», el saldo intacto y
       «Progreso 0%». Reproducido al peso: el API ya devolvía 19.400 y 446.200, y
       al generador le llegaban 0 y 465.600.

       La respuesta del POST es la única foto que no puede ir atrasada. */
    expect(ruta, 'ya no se guarda el préstamo que devolvió el cobro')
      .toMatch(/prestamo: data \?\? null/)
    expect(ruta, 'el comprobante volvió a leer el estado de la pantalla')
      .toMatch(/reciboCobro\?\.prestamo \?\?/)
    expect(ruta, 'sin el id del pago el comprobante sale sin «Referencia»')
      .toMatch(/id: reciboCobro\?\.pagoId/)
  })

  it('⚠ el nombre del negocio se parte en renglones, no se recorta', () => {
    /* PRESTA MIL usa de nombre una frase entera —«PRESTA MIL 3223846884 número
       SUPERVISOR para información sobre su crédito o reclamos»—. El dibujo
       encogía la letra en un bucle con suelo de 13px y lo que sobraba se salía
       del papel: en su comprobante se leía hasta «reclam». Es el dato por el que
       el cliente sabe a quién le pagó. */
    const com = readFileSync(resolve(RAIZ, 'components/ui/BotonCompartirRecibo.jsx'), 'utf8')
    expect(com, 'se perdió la función que parte el nombre en renglones')
      .toMatch(/function renglones\(/)
    expect(com, 'volvió el encogido con suelo, que recorta lo que no cabe')
      .not.toMatch(/while \(tam > 13/)
  })
  ...
  it('y en la ficha llaman a la acción, no a un paso intermedio', () => {
    ...
    expect(registrar).toMatch(/onGuardarImagen=\{\(\) => setPrevioAbierto\(true\)\}/)
    expect(registrar).toMatch(/<HojaReciboPrevio/)
