...
describe('las preferencias', () => {
  it('sin nada guardado, todo encendido y el resumen a las 9 de la noche', () => {
    ...
    expect(RESUMEN_HORA_POR_DEFECTO).toBe(21)   // lo pidió el dueño: «en la noche, 9, 10»
  })
...
describe('lo que se arregló de paso', () => {
  ...
  it('los avisos tienen su sección, para todos los roles, y no se llama como el módulo', () => {
    expect(leer('lib/adaptadores/configuracion.js')).toMatch(/\{ id: 'avisos',\s+nombre: 'Notificaciones'/)
  })
})

// ── Cazado revisando la app CON SESIÓN DE COBRADOR (19 sep 2026) ─────────────
describe('el cobrador no ve la plata del dueño', () => {
  it('⚠ `/api/mas` no le entrega ni una cifra del negocio', () => {
    /* No tenía barrera de rol: le devolvía el saldo del capital («$7.063.330
       listos para prestar»), gastos, perdidos y socios. Un endpoint sin barrera
       no falla: contesta. */
    const api = sinComentarios(leer('app/api/mas/route.js'))
    const iBarrera = api.indexOf("if (session.user.rol !== 'owner')")
    const iCapital = api.indexOf('prisma.capital.findFirst')
    expect(iBarrera).toBeGreaterThan(-1)
    expect(iBarrera, 'la barrera tiene que ir ANTES de leer el capital').toBeLessThan(iCapital)
    expect(api.slice(iBarrera, iBarrera + 120)).toMatch(/return Response\.json\(\{ esCobrador: true \}\)/)
  })

  it('y su «Más» es su menú, no el del dueño', () => {
    const pantalla = sinComentarios(leer('components/pantallas/PantallaMas.jsx'))
    const i = pantalla.indexOf('if (esCobrador) {')
    expect(i).toBeGreaterThan(-1)
    const suyo = pantalla.slice(i, pantalla.indexOf('const herramientas = ['))
    for (const prohibido of ["'/capital'", "'/socios'", "'/cobradores'", "'/actividad'", "'/dashboard/analiticas'", "'/carga-masiva'"]) {
      expect(suyo, `el cobrador no debe ver ${prohibido}`).not.toContain(prohibido)
    }
    // El rol sale de la sesión: si el API falla, no cae al menú del dueño.
    expect(leer('app/(dashboard)/mas/page.jsx')).toMatch(/esCobrador=\{esCobrador\}/)
  })
})
