...
describe('el reparto por medio de pago viene del API', () => {
  ...
  it('lo prestado en billetes excluye las transferencias', () => {
    /* Ya no se pregunta «¿es transferencia?» sino «¿entró al fajo?», porque
       desde el selector de cuenta una transferencia a la cuenta DEL COBRADOR sí
       es plata suya. Ver `lib/__tests__/de-quien-es-la-cuenta.test.js`. */
    expect(api).toMatch(/entraAlFajo\(d\.metodoPago, d\.metodoPagoId, cuentasCobrador\)/)
    expect(api, 'volvió a decidirlo a mano').not.toMatch(/filter\(\(d\) => d\.metodoPago !== 'transferencia'\)/)
  })
})
...
describe('⚠ y el otro número que suma con la calculadora: lo prestado', () => {
  /* «La idea es que me muestre ahí cuánto cobró el cobrador y cuánto prestó,
     así no me toca sacar cálculo con calculadora. Si prestó un millón, que me
     muestre prestó un millón.» — PRESTA MIL, 20 ago 2026.
  
     Medido ese día en su base, la ruta de JULIAN #7:
     ...
     billete nunca salió. Mismo enredo que el 119 del cobro, del otro lado de
     la cuenta. */
  const api = leer('app/api/caja/cobrador/[id]/route.js')
  const tarjeta = leer('components/caja/CajaCobradorDetalle.jsx')

  it('cada ruta lleva el valor colocado además del efectivo', () => {
    expect(api).toMatch(/b\.valorPrestadoDia \+= d\.montoPrestado \|\| d\.monto \|\| 0/)
    expect(api).toMatch(/valorPrestadoDia: Math\.round/)
  })

  it('⚠ «Prestado» sigue siendo el EFECTIVO: alimenta la resta del flujo', () => {
    /* Si se cambia por el valor de las cartulinas, la caja deja de cuadrar: el
       saldo absorbido de una renovación nunca salió del fajo. */
    expect(api).toMatch(/b\.prestadoDia \+= d\.monto \|\| 0/)
    expect(tarjeta).toMatch(/formatMoney\(ruta\.prestadoDia\)/)
  })

  it('las dos cifras salen juntas, y solo cuando difieren', () => {
    expect(tarjeta).toMatch(/\(ruta\.valorPrestadoDia \?\? 0\) > \(ruta\.prestadoDia \?\? 0\)/)
    expect(tarjeta).toMatch(/Cartulinas/)
    expect(tarjeta).toMatch(/Ya le debían/)
  })

  it('el total de lo prestado cierra sus dos renglones', () => {
    expect(tarjeta).toMatch(/Prestó en total/)
    expect(tarjeta).toMatch(/\(cr\.prestadoEfectivo \?\? cr\.prestado \?\? 0\) \+ \(cr\.prestadoDigital \?\? 0\)/)
  })
})
