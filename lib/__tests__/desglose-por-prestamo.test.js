...
describe('la pieza es UNA sola para las dos pantallas', () => {
  ...
  it('y al cobrar se recarga la lista', () => {
    ...
    // veces.
    expect(listaClientes).toMatch(/onSuccess=\{\(\) => fetchClientes\(/)
    expect(listaPrestamos).toMatch(/onSuccess=\{\(\) => fetchPrestamos\(/)
  })

  it('⚠ pero al cobrar NO se cierra la hoja: está enseñando el recibo', () => {
    /* Esta misma prueba pedía `setCobroRapido(null)` antes de recargar, y eso
       desmontaba la hoja en el instante en que enseñaba el comprobante: desde
       Clientes se cobraba y no salía recibo ni forma de mandarlo (el dueño,
       19 sep). La hoja se cierra con `onClose`, al salir del recibo. */
    for (const src of [listaClientes, listaPrestamos]) {
      const i = src.indexOf('<RegistrarPago')
      const hoja = src.slice(i, src.indexOf('/>', i))
      expect(hoja).toMatch(/onClose=\{\(\) => setCobroRapido\(null\)\}/)
      expect(hoja.slice(hoja.indexOf('onSuccess'))).not.toMatch(/setCobroRapido\(null\)/)
    }
  })
