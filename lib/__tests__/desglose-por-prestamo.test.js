...
describe('la tira del préstamo dice CUÁNDO SE COBRA, no cuándo vence', () => {
  it('«Cobra el» está, y era la que faltaba', () => {
    expect(rotulos(cifrasDe(base, 'CO'))).toContain('Cobra el')
  })
  ...
  it('«hoy» se pinta en oro y «vencido» en rojo', () => {
    const dame = (p) => cifrasDe(p, 'CO').find((c) => c.etiqueta === 'Cobra el')
    expect(dame({ ...base, proximoCobro: new Date().toISOString() }).tono).toBe('oro')
    expect(dame({ ...base, proximoCobro: enDias(-4) }).tono).toBe('contra')
...
describe('la fecha corta no lleva «de»', () => {
  ...
  it('y en la de cliente, que escribe la misma columna', () => {
    expect(proximoCobroTexto({ proximoCobro: '2026-08-29T05:00:00.000Z' })).toBe('29 ago')
  })
})
