...
describe('⚠ la imagen de la simulación', () => {
  ...
  it('dice que es una simulación, y solo cuando lo es', () => {
    /* Este papel se le enseña a alguien que todavía no ha firmado nada. Sin
       esta línea se lee como un compromiso.
       ⚠ Y el MISMO dibujante sirve para la tabla de un préstamo de verdad, que
       NO puede llamarse simulación: por eso el pie es un parámetro con este
       valor por defecto, y la otra vía lo cambia. */
    expect(dibujo).toMatch(/pie = 'Simulación · no es un compromiso ni una deuda registrada'/)
    expect(dibujo).toMatch(/ctx\.fillText\(pie, L, yPie\)/)
    const i = dibujo.indexOf('export function compartirTablaImagen')
    expect(i, 'falta la vía del préstamo real').toBeGreaterThan(-1)
    expect(dibujo.slice(i, i + 700)).toMatch(/rotulo: 'TABLA DEL PRÉSTAMO'/)
    expect(dibujo.slice(i, i + 700), 'a una deuda real no se le dice simulación').not.toMatch(/Simulación/)
  })
...
describe('⚠ un solo camino de canvas a WhatsApp', () => {
  ...
  it('`compartirLienzo` conserva las tres piezas', () => {
    ...
    expect(comun).toMatch(/link\.download = nombre/)
    expect(comun).toMatch(/export function descargarDatos/)
  })

  it('el recibo y la simulación salen por ella, no cada uno por su cuenta', () => {
    /* El recibo salía por `guardarReciboImagen`, que se fue el 14 sep 2026: la
       imagen pasa ahora por la VISTA PREVIA, y es ella la que entrega el
       archivo. La regla no cambia —un solo camino de canvas a WhatsApp—, solo
       cambia quién lo recorre. */
    const previa = leer('components/recibos/HojaReciboPrevio.jsx')
    expect(previa).toMatch(/from '@\/lib\/lienzo-compartir'/)
    expect(previa).toMatch(/compartirArchivo\(archivoRef\.current, \{/)
    expect(previa, 'volvió el navigator.share escrito a mano').not.toMatch(/navigator\.share\(/)
    expect(previa, 'volvió la descarga escrita a mano').not.toMatch(/link\.download|a\.download/)
