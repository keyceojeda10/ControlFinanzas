...
describe('el boton de cobrar', () => {
  it('se lleva un renglon entero', () => {
    ...
    /* ⚠ El boton ya no dice siempre «Cobrar»: a quien esta al dia se le adelanta,
       al que termino se le presta y al que lleva un año sin nada se le saca de
       la ruta. El texto sale de `contexto.accion`, con «Cobrar» de defecto. */
    const b = tarjeta.match(/style=\{\{[\s\S]{0,900}?\}\}\s*>\{contexto\?\.accion\?\.texto \?\? 'Cobrar'\}<\/button>/)
    expect(b, 'no encuentro el boton principal').toBeTruthy()
    expect(b[0]).toMatch(/width: '100%'/)
    expect(b[0]).not.toMatch(/flex: 1/)
  })

  it('y es mas alto que las secundarias', () => {
    const b = tarjeta.match(/style=\{\{[\s\S]{0,900}?\}\}\s*>\{contexto\?\.accion\?\.texto \?\? 'Cobrar'\}<\/button>/)[0]
