...
describe('la banda blanca: lo de hoy', () => {
  ...
  it('el resumen desglosa efectivo y digital', () => {
    ...
    expect(loDeHoy(ruta, fmt).resumen).toBe('efectivo $34.500 · digital $0')
  })
  ...
  it('el contador cuenta PUERTAS, no pagos', () => {
    // Quien anotó «no estaba» ya no es una puerta por tocar. Contando pagos se
    // quedaba en pendiente para siempre, y el botón «Empezar recorrido» —que sí
    // la descuenta— decía otra cosa a dos centímetros.
    const r = loDeHoy({ ...ruta, clientesVisitadosHoy: 3 }, fmt)
    expect(r.visita.texto).toBe('3 de 5')
    expect(r.visita.completo).toBe(false)
    expect(r.numeros.porVisitar).toBe(2)
  })

  it('sin el dato de visitas se cae a los que pagaron', () => {
    // Una pantalla que no lo manda todavía enseña algo razonable, no «0 de 5».
    expect(loDeHoy(ruta, fmt).visita.texto).toBe('1 de 5')
  })

  it('las visitas no pasan de lo que tocaba', () => {
    expect(loDeHoy({ ...ruta, clientesVisitadosHoy: 9 }, fmt).visita).toMatchObject({
      texto: '5 de 5', completo: true,
    })
  })

  it('sin cobros programados no dice «0 de 0»', () => {
    // El cero es un dato, pero «0 de 0» parece un error del sistema.
    const r = loDeHoy({ esperadoHoy: 0, recaudadoHoy: 0, clientesConCobroHoy: 0 }, fmt)
    expect(r.visita.texto).toBe('ninguna programada')
    expect(r.visita.completo).toBe(false)
  })
...
describe('el modo ruta: tres grupos', () => {
  ...
  it('la posición dice dónde vas, sin un «de» que compita con el contador', () => {
    // «parada 3 de 4» y «Visitas de hoy · 2 de 4» son dos cifras distintas del
    // mismo día a 20px una de otra: de ahí salen las preguntas.
    expect(p.posicion).toBe('parada 3')
  })

  it('la visita cerrada sin pago está hecha, pero no suma al fajo', () => {
    // ES EL FALLO CLÁSICO DE ESTA CASA: la misma cuenta en dos sitios y una de
    // las dos mintiendo. «No estaba» salía de la lista de pendientes en la
    // pantalla de la ruta, pero en el recorrido seguía en «Falta cobrar» hasta
    // la noche. Y al meterla entre las hechas, su CUOTA no puede sumarse: ese
    // total es el fajo que se entrega, y nadie puso un peso.
    const conCierre = partirRecorrido([
      { id: 1, orden: 1, nombre: 'A', cobradoHoy: true, montoCobrado: 20_000 },
      { id: 2, orden: 2, nombre: 'B', cobradoHoy: true, visitaCerrada: true, montoACobrar: 15_000 },
      { id: 3, orden: 3, nombre: 'C', montoACobrar: 12_000 },
    ], fmt)
    expect(conCierre.actual.nombre).toBe('C')
    expect(conCierre.cobradosTitulo).toBe('Ya hechos · 2')
    expect(conCierre.cobradosTotal).toBe('$20.000')
  })

  it('la parada cerrada sin pago no dice «cobrado»', () => {
    const [f] = adaptarRecorrido([
      { nombre: 'B', cobradoHoy: true, visitaCerrada: true, motivoCierre: 'no_estaba', montoACobrar: 15_000 },
    ], fmt)
    expect(f.detalle).toBe('no pagó · “no estaba”')
  })

  it('el abono parcial con la visita cerrada sí es dinero', () => {
    // «Abonó lo que pudo» cierra la visita Y entrega plata: dejarla fuera del
    // total descuadraría el fajo por lo bajo.
    const r = partirRecorrido([
      { id: 1, orden: 1, nombre: 'A', cobradoHoy: true, visitaCerrada: true, montoCobrado: 8_000, montoACobrar: 15_000 },
    ], fmt)
    expect(r.cobradosTotal).toBe('$8.000')
  })
