...
describe('«Claro» con pregunta detrás no es un sí', () => {
  /* ⚠ ESTA PRUEBA PEDÍA 'CIERRE' Y LA REGLA CAMBIÓ DEBAJO.
   *
   * La escribí cuando una confirmación corta cerraba siempre. Diez días después
   * `41a9793e` lo quitó a propósito, con la autocrítica medida delante:
   *
   *   Bot: «¿Sabe exactamente cuánto le deben hoy?»  →  Lead: «Sí»
   *   Bot: «Perfecto, aquí se registra en 2 minutos: <link>»
   *
   * El lead contestó una pregunta de diagnóstico y se le metió el link en la
   * cara. `slice(0, 3)` termina justo en una de esas preguntas —«¿usted ve al
   * segundo lo que cada uno cobró?»—, así que este caso ES el que se corrigió.
   *
   * Lo que aquella prueba protegía de verdad sigue vivo, y es lo de abajo: la
   * confirmación cierra cuando el bot YA ofreció la prueba. */
  it('«Claro» a una pregunta de diagnóstico NO es comprar', () => {
    const sinLink = trasElLink.slice(0, 3)
    expect(detectarStage(sinLink, 'Claro', false, leadFB)).toBe('VALOR')
  })

  it('pero «Claro» sí cierra cuando el bot acaba de ofrecer la prueba', () => {
    /* Tres mensajes, los mismos de arriba, cambiando SOLO el último del bot:
       en vez de otra pregunta de diagnóstico, el ofrecimiento. La conversación
       es igual de corta, así que lo que cierra es el ofrecimiento y nada más.
       Sin el link todavía, o `POST_LINK` se lo comería antes. */
    const ofrecio = [
      ...trasElLink.slice(0, 2),
      { rol: 'bot', texto: '¿Le muestro? Son 14 dias gratis y no pedimos tarjeta.' },
    ]
    expect(detectarStage(ofrecio, 'Claro', false, leadFB)).toBe('CIERRE')
  })
