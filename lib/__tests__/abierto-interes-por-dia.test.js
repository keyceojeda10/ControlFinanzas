...
describe('migrar un préstamo abierto que ya venía pagando', () => {
  /* $1.000.000 al 20% desde el 26 de junio, con $400.000 «ya abonados». Al
     crearlo, el sistema devenga los dos meses cerrados ($200.000 cada uno) y
     guarda el abono previo como `completo` — nunca pregunta si fue a intereses
     o a capital. Esos $400.000 son exactamente los dos meses de interés. */
  const migrado = {
    id: 'migrado', modoInteres: 'solo_interes', sinPlazo: true,
    montoPrestado: 1_000_000, tasaInteres: 20, frecuencia: 'mensual',
    fechaInicio: '2026-06-26', totalAPagar: 1_400_000, cuotasAmortizacion: [],
    pagos: [{ tipo: 'completo', montoPagado: 400_000, fechaPago: '2026-06-26' }],
    devengos: [
  ...
  it('nace al día, no con dos meses de mora', () => {
    // El abono cubre los dos períodos: no queda ninguno sin pagar.
    expect(interesesSinPagar(migrado)).toEqual([])
    expect(calcularDiasMora(migrado, [], [], HOY.getTime())).toBe(0)
  })

  it('y su capital sigue entero, que es la otra cara de lo mismo', () => {
    // Si el abono pagó el interés, no pagó capital. Las dos lecturas tienen
    // que decir lo mismo, o el préstamo dice a la vez que está al día y que
    // debe dos meses.
    expect(calcularCapitalRestante(migrado)).toBe(1_000_000)
    expect(calcularSaldoPendiente(migrado) - calcularCapitalRestante(migrado)).toBe(0)
  })

  it('si el abono hubiera sido a capital, el mes siguiente vale menos', () => {
    // La misma plata, dicha de otra forma: $400.000 menos de capital vivo son
    // $80.000 menos de interés cada mes, sobre un préstamo de un millón.
    const aCapital = { ...migrado, pagos: [{ tipo: 'capital', montoPagado: 400_000, fechaPago: '2026-06-26' }] }
    expect(calcularCapitalRestante(aCapital)).toBe(600_000)
    expect(periodoEnCursoAbierto(aCapital, HOY.getTime()).interesPeriodo).toBe(120_000)
    expect(periodoEnCursoAbierto(migrado, HOY.getTime()).interesPeriodo).toBe(200_000)
    // Y entonces SÍ debería los dos meses: el abono se fue al capital.
    expect(interesesSinPagar(aCapital).length).toBe(2)
  })
})
