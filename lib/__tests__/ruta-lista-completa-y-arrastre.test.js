...
describe('la tarjeta sabe decir que hoy no toca', () => {
  ...
  it('el botón grande no dice «Cobrar» en los cuatro casos', () => {
    ...
    expect(pagina).toMatch(/zona === 'sindeuda'\) router\.push\(`\/prestamos\/nuevo\?clienteId=/)
...
describe('dejarla apretada un momento y moverla', () => {
  ...
  it('⚠ y el bloqueo NO puede quedarse pegado', () => {
    /* Era un interruptor que se apagaba al tragarse el siguiente clic, pero
       tras un arrastre largo el navegador NO emite ninguno: la bandera se
       quedaba encendida y se comia el TOQUE SIGUIENTE, que si era de verdad.
       Medido en el espejo: se movia una tarjeta y la proxima que se tocaba ya
       no abria el cobro. Con la hora no hay estado que se quede pegado. */
