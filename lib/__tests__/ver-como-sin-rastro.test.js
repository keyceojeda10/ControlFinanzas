...
describe('la marca de solo lectura del teléfono', () => {
  ...
  it('en vista, las cinco funciones que ENCOLAN una escritura rechazan sin tocar IndexedDB', async () => {
    vi.stubGlobal('localStorage', almacen())
    const indexedDBEspia = { open: vi.fn() }
    vi.stubGlobal('indexedDB', indexedDBEspia)
    marcarSoloLectura(true)

    await expect(guardarPagoPendiente({})).rejects.toThrow(MENSAJE_EN_VISTA)
    await expect(guardarOrdenPendiente('ruta1', [])).rejects.toThrow(MENSAJE_EN_VISTA)
    await expect(guardarClientePendiente({})).rejects.toThrow(MENSAJE_EN_VISTA)
    await expect(guardarPrestamoPendiente({})).rejects.toThrow(MENSAJE_EN_VISTA)
    await expect(encolarMutacion({ tipo: 'cliente.update', entityId: 'c1', payload: {} })).rejects.toThrow(MENSAJE_EN_VISTA)
