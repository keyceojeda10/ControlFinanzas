...
describe('⚠ los dos endpoints usan la MISMA función', () => {
  ...
  it('⚠ el select de cobros PIDE `metodoPagoId`', () => {
    ...
    /* ⚠ SE ANCLA QUE SE PIDA, NO EN QUÉ LÍNEA. Esto exigía que `metodoPagoId`
       fuera justo la línea de encima de `prestamo: {`, y se puso rojo el día
       que se añadió `prestamoId` entre las dos —que es una mejora—. Un contrato
       de forma, no de comportamiento: lo que importa es que el campo viaje. */
    const i = cobrador.indexOf('const wherePagoCaja')
    const selectDeCobros = cobrador.slice(i, cobrador.indexOf('orderBy: { fechaPago:', i))
    expect(selectDeCobros).toMatch(/\bmetodoPagoId: true\b/)
  })
