...
describe('T02-04 / T08-01 · la forma, contra los estilos de las láminas', () => {
  ...
  it('lo raro va PLEGADO, y ahora abre su PROPIA hoja', () => {
    // ⚠ ESTA PRUEBA DECÍA LO CONTRARIO Y YA NO VALE.
    //
    // Fijaba que el enlace «cae al formulario de siempre»
    // (`setVerFormularioCompleto(true)`), que era cierto mientras el deslizador
    // de días solo viviera en el modal viejo. Ya no: «Abonar por días» tiene su
    // hoja (`components/pantallas/AbonoPorDias.jsx`) y el modal de antes del
    // rediseño quedó inalcanzable.
    //
    // El motor sigue sin reescribirse: la cola offline, el recibo, la foto de
    // evidencia, las coordenadas y el aviso de duplicado siguen donde estaban.
    expect(modal).toMatch(/onLoRaro=\{\(\) => setVerAbonoDias\(true\)\}/)
  ...
  it('CAPITAL E INTERÉS se resuelven EN la hoja, no saltando al modal viejo', () => {
    ...
    /* Cambiar de aplicación NO puede volver a mandar al formulario.
       ⚠ Fijaba la expresión EXACTA `onAplicacion={(a) => setTipo(a.id)}` y se
       puso en rojo cuando el manejador pasó a vaciar también el monto al entrar
       en «Solo interés» —heredar la cuota ahí subía la deuda $175.000 de un
       toque—. La conducta que importa no cambió: sigue resolviéndose dentro de
       la hoja. Reescrita sobre eso. */
    const manejador = modal.slice(modal.indexOf('onAplicacion='), modal.indexOf('explicacion={explicacionAplicacion}'))
    expect(manejador).toMatch(/setTipo\(a\.id\)/)
    expect(manejador, 'cambiar de aplicación volvió a saltar al formulario viejo')
      .not.toMatch(/setVerFormularioCompleto/)
