...
vi.mock('@/lib/prisma', () => ({
  prisma: {
    organization: {
      /* Solo lo que pide el `select`, como Prisma: un campo que falta en el
         `select` es `undefined` y el precio cae a lista EN SILENCIO. */
...
beforeEach(() => {
  vi.clearAllMocks()
  org = {
    id: 'org1', nombre: 'Negocio', plan: 'basic', planOriginal: null, country: 'co',
...
describe('cuándo se cobra', () => {
  ...
  it('a menos de 48 h de vencer, cobra el precio de UN mes', async () => {
...
describe('⚠ cuánto se cobra (lib/precio-plan.js)', () => {
  const montoEnviado = () => wompi.cobrarConFuente.mock.calls[0][0].montoCOP
  let warn
  ...
  it('preferencial que sigue vigente cuando empieza el mes: cobra el preferencial', async () => {
    Object.assign(org, { precioPreferencial: 40000, precioPreferencialPlan: 'basic', precioPreferencialHasta: new Date(ahora() + 10 * 24 * H) })
    subs = [subDePago(30, { montoCOP: 40000 })]
    expect((await cobrarAhoraSiToca({ orgId: 'org1' })).resultado).toBe('enviado')
    expect(montoEnviado()).toBe(40000)
    expect(warn).not.toHaveBeenCalled()
  })

  it('⚠ preferencial que termina antes de que empiece el mes: cobra lista, sin que nadie toque nada', async () => {
    Object.assign(org, { precioPreferencial: 40000, precioPreferencialPlan: 'basic', precioPreferencialHasta: new Date(ahora() + 10 * H) })
    subs = [subDePago(30, { montoCOP: 40000 })]
    await cobrarAhoraSiToca({ orgId: 'org1' })
    expect(montoEnviado()).toBe(59000)
  })

  it('definitivo: cobra el preferencial', async () => {
  ...
  it('⚠ pago fuera de lista que nadie revisó: no sube solo, y lo deja en el log', async () => {
    subs = [subDePago(30, { montoCOP: 45000 })]
    await cobrarAhoraSiToca({ orgId: 'org1' })
    expect(montoEnviado()).toBe(45000)
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('PRECIO POR REVISAR'))
  })

  it('el mismo pago, revisado en el panel y dejado a lista: cobra la lista', async () => {
    org.precioPagoRevisado = 45000
    subs = [subDePago(30, { montoCOP: 45000 })]
    await cobrarAhoraSiToca({ orgId: 'org1' })
    expect(montoEnviado()).toBe(59000)
    expect(warn).not.toHaveBeenCalled()
  ...
  it('⚠ Crecimiento con un cobrador y una ruta adicionales: cobra el plan MÁS ellos', async () => {
    Object.assign(org, { plan: 'growth', cobradoresAdicionales: 1, rutasAdicionales: 1 })
    subs = [subDePago(30, { plan: 'growth', montoCOP: 79000 })]
    await cobrarAhoraSiToca({ orgId: 'org1' })
    expect(montoEnviado()).toBe(79000 + 19000 + 29000)
    expect(warn).not.toHaveBeenCalled()
    /* La referencia dice con cuántos se cobró: al aprobarse quedan en eso. */
    const { referencia } = wompi.cobrarConFuente.mock.calls[0][0]
    expect(wompi.leerReferencia(referencia)).toMatchObject({ periodo: 'mensual', conAdicionales: { cobradores: 1, rutas: 1 } })
  })

  it('el último pago ya traía el cobrador ($98.000): no es un precio «por revisar»', async () => {
    Object.assign(org, { plan: 'growth', cobradoresAdicionales: 1 })
    subs = [subDePago(30, { plan: 'growth', montoCOP: 98000 })]
    await cobrarAhoraSiToca({ orgId: 'org1' })
    expect(montoEnviado()).toBe(98000)
    expect(warn).not.toHaveBeenCalled()
  })

  it('en Básico no hay adicionales: un número viejo en la columna no se cobra', async () => {
    Object.assign(org, { cobradoresAdicionales: 2 })
    await cobrarAhoraSiToca({ orgId: 'org1' })
    expect(montoEnviado()).toBe(59000)
  })

  it('⚠ pagó un trimestre: el cobro es de un mes, no del trimestre', async () => {
    subs = [subDePago(30, { montoCOP: 159300 })]
    await cobrarAhoraSiToca({ orgId: 'org1' })
    expect(montoEnviado()).toBe(59000)
  })
})
...
describe('⚠ el acceso vuelve con el pago APLICADO, no antes', () => {
  ...
  it('⚠ una referencia de adicionales en vuelo NO se pasa a activarPlanPagado', async () => {
    const ref = wompi.referenciaDeAdicionales('org1', 'growth', { cobradores: 1, rutas: 0 })
    Object.assign(org, { cobroRefPendiente: ref, cobroTxPendiente: 'tx9', cobroUltimoIntento: new Date() })
    wompi.consultarTransaccion.mockResolvedValue({ id: 'tx9', status: 'APPROVED', reference: ref, amount_in_cents: 1010000 })
