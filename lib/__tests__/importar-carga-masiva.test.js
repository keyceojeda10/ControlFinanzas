...
import { aCargaMasiva, diasDePlazo, cuotasDesdeDias, marcadorCedula, esMarcador, tasaMensualEquivalente } from '@/lib/importar/aCargaMasiva'
...
/* ══ 9 SEP 2026 · EL ARCHIVO QUE TRAE LOS DÍAS Y NO LAS CUOTAS ═══════════════
 *
 * El export de otro sistema traía «Dias credito» (30, 15, 21…) y ninguna
 * columna de cuotas. Sin cuotas no había plazo NI se podía convertir la tasa
 * plana, así que las 124 filas se descartaban y la pantalla decía «no hay
 * ninguna fila que se pueda importar» sin explicar por qué.
 */
describe('el plazo lo puede traer el archivo hecho', () => {
  const base = {
    nombre: 'Ana Ruiz', cedula: '11111111', telefono: '966589669',
    capital: 600000, interes: 20, nCuotas: null, diasPlazo: 30,
    frecuencia: 'quincenal', fechaInicio: '2026-09-04',
  }

  it('con los días del archivo se importa, aunque no venga el número de cuotas', () => {
    const { filas, descartadas } = aCargaMasiva([base], { semilla: 'x', hoy: '2026-09-09' })
    expect(descartadas).toHaveLength(0)
    expect(filas[0].diasPlazo).toBe(30)
  })

  it('y la tasa plana se convierte igual: las cuotas salen de los días', () => {
    // 30 días cobrando cada quince son 2 cuotas → 1 mes → 20 % plano = 20 % mensual.
    expect(cuotasDesdeDias('quincenal', 30)).toBe(2)
    const { filas } = aCargaMasiva([base], { semilla: 'x', hoy: '2026-09-09' })
    expect(filas[0].tasaInteres).toBe(20)
    expect(filas[0].tasaPlanaOrigen).toBe(20)
  })

  it('si el archivo trae las cuotas, mandan ellas y no la división', () => {
    const { filas } = aCargaMasiva([{ ...base, nCuotas: 6, diasPlazo: null, frecuencia: 'semanal' }],
      { semilla: 'x', hoy: '2026-09-09' })
    expect(filas[0].diasPlazo).toBe(42)
  })

  it('⚠ sin frecuencia sigue descartando: eso NO se adivina', () => {
    /* La frecuencia cambia la cuota y el interés, así que la pantalla la
       pregunta en vez de suponerla. Aquí solo se comprueba que sin ella no
       entra nada a la base. */
    const { filas, descartadas } = aCargaMasiva([{ ...base, frecuencia: null }],
      { semilla: 'x', hoy: '2026-09-09' })
    expect(filas).toHaveLength(0)
    expect(descartadas[0].motivo).toBe('no dice cada cuánto se cobra')
  })
})
