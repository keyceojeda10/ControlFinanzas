...
function componentesDelRedisenio() {
  const raices = ['components/cf', 'components/armazon', 'components/pantallas']
  const salida = []
  const bajar = (dir) => {
...
describe('los componentes del rediseño compilan', () => {
  ...
  for (const rel of ARCHIVOS) {
    it(rel.replace('components/', ''), async () => {
      ...
      // Y que de verdad haya transformado: si el JSX sigue crudo, el archivo
      // pasaría la sintaxis pero el build lo rechazaría igual.
      expect(res.code, `${rel}: el JSX salió sin transformar`).not.toMatch(/^\s*<[a-z]/m)
    })
  }
})
