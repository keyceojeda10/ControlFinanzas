...
function componentesDelRedisenio() {
  ...
  const raices = [
    ...
    // `components/prestamos` entra porque ahí vive el modal que ESCRIBE PAGOS. Le
    // monté la hoja nueva sin que el guardián lo mirara: un nombre mal escrito en
    // ese archivo es un cobro que no se registra.
...
describe('los componentes del rediseño compilan', () => {
  ...
  for (const rel of ARCHIVOS) {
    it(rel.replace('components/', ''), async () => {
      ...
      // Un .jsx VACÍO no es un fallo de sintaxis —oxc lo transforma sin una sola
      // queja y devuelve cadena vacía—, pero sí es un archivo muerto: quien lo
      // importe se encuentra `undefined` donde esperaba un componente. Aquí vivió
      // `PrestamoForm.jsx`, 0 bytes desde el commit inicial y sin un solo import,
      // hasta que se borró. Hubo un filtro por tamaño en el descubrimiento para
      // saltárselo; se quitó porque una exclusión silenciosa es justo el agujero
      // que este archivo existe para tapar: lo que no se mira, se pudre en verde.
      // Se comprueba ANTES de transformar para que el fallo diga «está vacío» y no
      // «no produjo código», que suena a avería del transformador.
      expect(fuente.trim(), `${rel} está vacío: escríbelo o bórralo`).not.toBe('')
...
describe('ningún componente usa un nombre que no existe', () => {
  ...
  for (const rel of ARCHIVOS) {
    it(rel.replace('components/', ''), () => {
      // SIN COMENTARIOS. Un `<CapitalTab />` escrito dentro de un comentario
      // —explicando por que se quito— no es codigo que se ejecute, y contarlo
      // como uso da un falso positivo. Es el tercer escaner del proyecto que
      // tropieza con lo mismo: la regla es que el texto de un comentario nunca
      // entra al analisis.
      const bruto = fs.readFileSync(path.join(process.cwd(), rel), 'utf8')
      const fuente = bruto
        .replace(/\{\/\*[\s\S]*?\*\/\}/g, '')
        .replace(/\/\*[\s\S]*?\*\//g, '')
        .replace(/^\s*\/\/.*$/gm, '')
      // Los imports SI se leen del archivo entero: un import comentado no
      // deberia contar, pero tampoco hace falta afinar tanto — lo que importa
      // es no inventar usos.
      const declarados = declaraciones(bruto)
