...
describe('login: que mensaje ve el usuario', () => {
  ...
  it('TODO mensaje que lanza lib/auth.js llega intacto al usuario', () => {
    // revisarCuenta/armarSesion (desactivada, suspendida, limite de plan,
    // VERIFY_EMAIL) viven en lib/auth-sesion.js desde la tarea 2; el login con
    // contraseña las sigue lanzando a través de authorize().
    const auth = fs.readFileSync(path.join(RAIZ, 'lib/auth.js'), 'utf8')
      + fs.readFileSync(path.join(RAIZ, 'lib/auth-sesion.js'), 'utf8')
