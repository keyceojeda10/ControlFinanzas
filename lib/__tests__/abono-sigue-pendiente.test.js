...
describe('abono parcial: la fila sigue viva', () => {
  ...
  it('conserva su tira de cifras', () => {
    ...
    // Y el cerrado sigue sin ella: ya esta tachado y con su hora.
    expect(filaDeCobro({ ...base, pagoHoy: true, cobroPendienteHoy: false }).cifras).toBeUndefined()
  })
...
describe('las acciones estan en todas las fichas', () => {
  it('no dependen de ser la parada actual', () => {
    expect(tarjeta).toMatch(/\{!cobrada && \(onLlamar \|\| onWhatsApp \|\| onMapa \|\| onMas\) && \(/)
