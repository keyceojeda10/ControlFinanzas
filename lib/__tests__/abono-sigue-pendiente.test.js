...
describe('la cifra no se sale de su columna', () => {
  it('el valor se ENCOGE hasta caber: ni pisa a la vecina ni se recorta', () => {
    /* ⚠ El recorte (`overflow: 'hidden'`) que garantizaba el ancho se comía el
       último cero: «$1.272.000» salía «$1.272.00» (19 sep 2026). Un número
       cortado es OTRO número. Ahora la columna es un contenedor y la cifra
       calcula su cuerpo con el ancho real. */
    const bloque = tira.slice(tira.indexOf('export function TiraCifras'))
    expect(bloque).toMatch(/className="cf-cifra-col"/)
    expect(bloque).toMatch(/<CifraQueCabe valor=\{c\.valor\}/)
    expect(bloque).not.toMatch(/textOverflow: 'clip'/)
    const css = require('fs').readFileSync(require('path').join(process.cwd(), 'app/tokens-2026.css'), 'utf8')
    expect(css).toMatch(/\.cf-cifra-col \{ container-type: inline-size; \}/)
    expect(css).toMatch(/font-size: min\(var\(--cf-cifra-tam, 14px\), calc\(100cqw \/ \(var\(--cf-cifra-n, 8\) \* 0\.62\)\)\)/)
  })
...
describe('volver a donde se iba', () => {
  ...
  it('el ancla de cada ficha existe', () => {
    ...
    //
    // ⚠ Va condicionada desde que el escritorio pinta la MISMA tarjeta: las dos
    // vistas están montadas a la vez —`hidden lg:block` esconde por CSS, no
    // desmonta— y dos nodos con el mismo `id` hacen que `getElementById`
    // devuelva el escondido. El ancla es de la lista de móvil, que es la que
    // guarda el sitio; lo que esta prueba cuida es que SIGA PUESTA ahí.
    expect(ruta).toMatch(/ancla=\{sinArrastre \? undefined : ANCLA_CLIENTE\(fila\.id\)\}/)
  ...
  it('⚠ el NOMBRE del cliente tambien guarda el sitio', () => {
    /* Este era el camino roto que reporto INVERSIONESJYM. `onClick` y `onMas`
       ...
       Sigue guardando, pero ya no lo hace por su cuenta: TODAS las salidas
       pasan por `irGuardando`. Ver el describe de abajo. */
    const i = ruta.indexOf('onAbrirCliente={')
    expect(i).toBeGreaterThan(-1)
    expect(ruta.slice(i, i + 200)).toMatch(/irGuardando\(/)
  })
})

/* ══════════════════════════════════════════════════════════════════════════
 *
 *  ⚠ TODAS LAS SALIDAS DE LA RUTA, NO SOLO LA QUE SE REPORTO
 *
 *  24 ago 2026. «Cuando esta en la vista agrupada y hace un prestamo, al
 *  volver ya no puede volver a donde estaba, se le pierde el foco. En la lista
 *  completa si le queda por donde iba.»
 *
 *  No era cosa de agrupar. Medido en el navegador contra el espejo, el
 *  `sessionStorage` venia VACIO despues de tocar «Prestarle» y LLENO despues
 *  de tocar el nombre del cliente, EN LAS DOS VISTAS. Lo que pasa es que la
 *  vista agrupada tiene un grupo «Listos para prestarles» que le pone el boton
 *  delante, asi que presta desde ahi; en la plana llega por la ficha.
 *
 *  Es el mismo fallo del comprobante de pago: se arreglo una via y se dejo la
 *  otra. Por eso ahora hay UNA sola puerta.
 * ══════════════════════════════════════════════════════════════════════════ */
describe('⚠ toda salida de la ruta guarda el sitio', () => {
  const ruta = leer('app/(dashboard)/rutas/[id]/page.jsx')

  it('`irGuardando` guarda ANTES de navegar', () => {
    const i = ruta.indexOf('const irGuardando =')
    expect(i, 'no existe la puerta unica').toBeGreaterThan(-1)
    const cuerpo = ruta.slice(i, i + 200)
    expect(cuerpo.indexOf('guardarContextoRuta')).toBeGreaterThan(-1)
    expect(cuerpo.indexOf('guardarContextoRuta')).toBeLessThan(cuerpo.indexOf('router.push'))
  })

  it('⚠ y se declara DESPUES de lo que usa', () => {
    /* `const` citado antes de su declaracion: tres veces en `caja/page.jsx`,
       pantalla en blanco que ninguna prueba de cifras caza. */
    expect(ruta.indexOf('const guardarContextoRuta =')).toBeLessThan(ruta.indexOf('const irGuardando ='))
  })

  it('«Prestarle» guarda el sitio', () => {
    const i = ruta.indexOf("zona === 'sindeuda'")
    expect(i).toBeGreaterThan(-1)
    const bloque = ruta.slice(i, i + 220)
    expect(bloque, 'salia pelado a /prestamos/nuevo').toMatch(/irGuardando\([\s\S]{0,90}\/prestamos\/nuevo/)
  })

  it('no queda ninguna salida pelada al cliente', () => {
    /* Anclado en el `onClick`, no en una frase: es la forma que tenian las
       tres vias rotas. */
    expect(ruta).not.toMatch(/onClick=\{\(\) => router\.push\(`\/clientes\//)
    expect(ruta).not.toMatch(/onClick=\{\(\) => router\.push\(`\/prestamos\/nuevo/)
  })
})
