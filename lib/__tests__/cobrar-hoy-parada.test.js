...
describe('las acciones tienen con que funcionar', () => {
  ...
  it('WhatsApp cae en la ficha cuando no hay telefono', () => {
    // Es donde se le puede poner uno. Abrir `wa.me/` vacio no lleva a ningun
    // sitio.
    expect(pagina).toMatch(/if \(tel\) window\.open\(`https:\/\/wa\.me\/\$\{tel\}`/)
    expect(pagina).toMatch(/else window\.location\.href = `\/clientes\/\$\{fila\.id\}`/)
  })
