...
describe('⚠ el paso sigue midiéndose después de cambiar de lector', () => {
  ...
  it('⚠ los dos lectores se distinguen en el mismo evento', () => {
    ...
    /* ⚠ NO SE CLAVA EL NÚMERO. Estaba en 3 y se cayó al añadir al lector de uno
       una cuarta salida —«esta foto es una lista»—, que también lleva su `via`.
       Lo que hay que garantizar es que TODAS las marcas la lleven, no cuántas
       hay: sin `via` los dos lectores caen en el mismo montón y no se puede
       saber cuál de los dos usó la gente. */
    const marcasLote = lote.match(/evento: 'cartulina_leida'/g) ?? []
    const marcasUna = una.match(/evento: 'cartulina_leida'/g) ?? []
    expect(lote.match(/via: 'lote'/g) ?? []).toHaveLength(marcasLote.length)
    expect(una.match(/via: 'una'/g) ?? []).toHaveLength(marcasUna.length)
    expect(marcasUna.length).toBeGreaterThanOrEqual(3)
  })
