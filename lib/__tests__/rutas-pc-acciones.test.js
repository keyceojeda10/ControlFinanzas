...
describe('la ruta en escritorio', () => {
  ...
  it('la hoja de WhatsApp tampoco', () => {
    // El tercero que cae en la misma trampa. El botón de WhatsApp de la tabla
    // llamaba a `setModalWA`, el estado cambiaba, y la hoja se montaba dentro
    // del `display:none`: en PC no pasaba NADA al pulsar. No se ve leyendo el
    // archivo —el `<HojaWhatsApp>` está y recibe sus datos—, solo pulsando.
    expect(iCierre, `falta el rótulo «${MARCA}»`).toBeGreaterThan(iMovil)
    const dentro = lineas
      .map((l, i) => ({ l, i }))
      .filter(({ l, i }) => i > iMovil && i < iCierre && l.includes('<HojaWhatsApp'))
    expect(dentro.length, `«<HojaWhatsApp>» dentro de lg:hidden en ${dentro.map((d) => d.i + 1)}`).toBe(0)
  })

  it('la tabla de PC deja escribirle al cliente', () => {
    // El botón existía solo en la tarjeta de móvil: desde el computador había
    // que sacar el teléfono para avisarle a alguien.
    expect(src, 'la tabla de escritorio no recibe `onWhatsApp`').toContain('onWhatsApp={(f) => {')
  })
  ...
  it('«Reordenar recorrido» tiene quién lo pinte en PC', () => {
    ...
    // Lo que importa es que exista una rama de ESCRITORIO que reaccione a
    // `ordenar`, no cómo esté escrito su `className`. Traía
    // `className="hidden lg:block"` clavado y dejó de encontrarlo en cuanto se
    // le añadió el ancho máximo: fallaba con la rama correcta delante. Es la
    // cuarta prueba de esta sesión que se cae por llevar dentro un valor que
    // luego cambia.
    expect(src, 'nadie reacciona a `ordenar` en la rama de escritorio')
      .toMatch(/\{modoVista === 'ordenar' && \(\s*\n\s*<div className="[^"]*\blg:block\b[^"]*">/)
  })
})
