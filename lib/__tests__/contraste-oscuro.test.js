...
describe('la tarjeta insignia del panel se lee en los dos temas', () => {
  ...
  it('los colores del bloque son fijos, no tokens que cambian con el tema', () => {
    /* La paleta se mudo a `components/cf/bloqueOscuro.js` cuando la banda del
       dia de Rutas paso a usar el mismo bloque: dos copias de los mismos hex se
       separan en cuanto se retoca una. */
    const mod = sinComentarios(leer('components/cf/bloqueOscuro.js'))
    const i = mod.indexOf('export const BLOQUE = {')
    expect(i, 'desapareció la paleta del bloque oscuro').toBeGreaterThan(-1)
    const paleta = mod.slice(i, mod.indexOf('}', i))
