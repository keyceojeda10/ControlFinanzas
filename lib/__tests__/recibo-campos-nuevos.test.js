...
describe('no rompe lo que ya habia', () => {
  it('los campos por defecto son los seis, y «Antes debía» va justo antes del saldo', () => {
    // El orden es la razón de ser del campo: la resta se lee de arriba abajo
    // (antes debía → pagó → queda). Si `saldoAntes` cae debajo de
    // `saldoPendiente`, el papel deja de contar una resta.
    expect(getDefaultCampos().map(c => c.campo)).toEqual([
      'totalPagado', 'saldoAntes', 'saldoPendiente', 'totalAPagar', 'cuota', 'progreso',
    ])
  })
