...
function condicionReal() {
  ...
  /* ⚠ `hayVentana` es «hay ventana de cobro, venga de un chip o de las dos
     fechas a mano». Se le pasa con los dos nombres para que la prueba siga
     valiendo si la expresión se reescribe con uno u otro. */
  return new Function('soloMora', 'diasMoraMin', 'listosRenovar', 'porVencer', 'hayVentana',
    'niUnPeso',
...
describe('los filtros que se calculan en JS ven la cartera entera', () => {
  ...
  const correr = (fn, x) => !!fn(x.soloMora, x.diasMoraMin, x.listosRenovar, x.porVencer,
    x.porVencer != null || !!x.hayRangoFechas, x.niUnPeso)
  ...
  const CASOS = [
    ...
    ['rango a mano',       { hayRangoFechas: true }],   // las dos fechas del calendario
    /* Se me olvidó al añadirlo, y esta prueba lo cazó: en el espejo la lista
       traía 32 donde la base decía 188, porque el endpoint paginaba PRIMERO. */
    ['ni un peso',         { niUnPeso: true }],
  ]
