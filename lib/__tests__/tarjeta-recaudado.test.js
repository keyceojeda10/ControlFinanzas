...
describe('el contenedor', () => {
  ...
  it('lleva borde, porque en oscuro el fondo de la app es EL MISMO color', () => {
    ...
    const i = hero.indexOf("data-bloque=\"recaudado\"")
    expect(i, 'se perdió la marca de la tarjeta').toBeGreaterThan(-1)
    expect(hero.slice(i, i + 1400)).toMatch(/border: '1px solid rgba\(255,255,255,\.14\)'/)
  })
