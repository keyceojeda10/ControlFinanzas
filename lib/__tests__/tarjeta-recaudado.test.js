...
describe('en escritorio son DOS bloques', () => {
  ...
  it('la gráfica se declara una vez y se pinta en los dos sitios', () => {
    ...
    expect((hero.match(/const grafica = hayGrafica \? \(/g) ?? []).length).toBe(1)
    expect((hero.match(/\{grafica\}/g) ?? []).length).toBe(2)
  })

  it('⚠ y es un ELEMENTO, nunca un componente declarado dentro de Hero', () => {
    /* Estaba como `function Grafica()` en el cuerpo de `Hero`. En cada render
       del padre la función es otra, React ve un tipo nuevo y DESMONTA el
       subárbol para montarlo de nuevo: el día elegido se borraba y los rótulos
       parpadeaban. El dueño lo reportó el 20 sep 2026 —«uno selecciona un día y
       se quita, y al principio hace un glitch»— y se midió en el espejo: las 14
       barras cambiaban de nodo en cada repintado, 0 de 14 sobrevivían.
       Lo vigila también `react/no-unstable-nested-components` en el lint. */
    expect(hero).not.toMatch(/function Grafica\(\)/)
    expect(hero).not.toMatch(/const Grafica = \(/)
  })
})
