...
describe('préstamo o mercancía', () => {
  it('sigue el mismo lenguaje que la frecuencia', () => {
    /* Se quedaron del lenguaje viejo mientras todo lo de alrededor cambiaba:
       píldoras de 22px y 44 de alto al lado de la frecuencia, que es
       rectangular de 12px y 48. Parecían de otra pantalla. */
    const i = src.indexOf("[['prestamo', 'Préstamo']")
    expect(i, 'el selector de tipo volvió a escribirse a mano').toBeGreaterThan(-1)
    const bloque = src.slice(i, i + 900)
    expect(bloque).toMatch(/h-12 rounded-\[12px\]/)
    expect(bloque, 'el activo tiene que marcarse en tinta, como la frecuencia')
      .toMatch(/background: 'var\(--cf-ink\)'/)
  })

  it('y NO usa `rounded-xl`, que en este tema son 22px', () => {
    /* `--radius-xl: 22px` en `globals.css`: `rounded-xl` NO son los 12 de
       Tailwind. Ninguno de los radios del tema (11/14/18/22) está en el canon,
       que manda 8/10/12/16/20. */
    const i = src.indexOf("[['prestamo', 'Préstamo']")
    expect(src.slice(i, i + 900)).not.toMatch(/rounded-(xl|lg|md|sm)\b/)
  })
})

describe('el token del fondo de la app no se usa como fondo de control', () => {
  it('en toda la pantalla de nuevo préstamo', () => {
    /* `--cf-surface` es el FONDO DE LA APP (#F4F4F1); `--cf-fill` es «control
       inactivo» (#F3F3EF). Los botones llevaban el primero, y mientras el
       formulario iba sobre el fondo gris colaba. En cuanto pasó a descansar
       sobre una hoja blanca, «Mercancía» se quedó sin nada que lo separara del
       papel: «en PC ni siquiera los veo». Eran 17 sitios. */
    const usos = (src.match(/background: 'var\(--cf-surface\)'/g) ?? []).length
    expect(usos, 'volvió a colarse `--cf-surface` como fondo de un control').toBe(0)
  })
})

describe('el mismo control por sus dos caminos', () => {
  it('«qué día cobras» está igual desde semanal y desde quincenal', () => {
    /* Sale por dos sitios y la primera vez arreglé solo el de arriba. Es el
       fallo que ya me costó dos días con el comprobante. */
    const veces = (src.match(/Que dia cobras\?/g) ?? []).length
    expect(veces, 'cambió el número de caminos: revisa que todos usen el mismo estilo').toBe(2)
    const conHelper = (src.match(/estiloOpcionMenor\(diaCobroSemana/g) ?? []).length
    expect(conHelper, 'una de las dos vías se quedó con el estilo viejo').toBe(4)
  })
})
