...
describe('cada prestamista decide si el reparto se enseña', () => {
   ...
   *  marea.» — un prestamista, 15 sep 2026, el día después de estrenarlo.
   *
   * No es quitarlo: a otros les resuelve la llamada de «abonué y el saldo casi
   * no se movió». Es que cada uno lo decida, como el resto del recibo. */
  it('es un campo más del checklist, y nace APAGADO', () => {
    const c = CAMPOS_PREDEFINIDOS.find((x) => x.campo === 'repartoDelPago')
    expect(c, 'desapareció el campo del checklist').toBeTruthy()
    expect(c.porDefecto, 'el reparto volvió a salir sin que nadie lo pidiera').toBe(false)
    // Y por tanto no está en los campos de fábrica.
    expect(getDefaultCampos().some((x) => x.campo === 'repartoDelPago')).toBe(false)
  })

  it('«Total a pagar» sigue siendo quitable, que también lo pidió', () => {
    expect(CAMPOS_PREDEFINIDOS.some((x) => x.campo === 'totalAPagar')).toBe(true)
  })

  it('no se pinta como fila: es el interruptor del bloque', () => {
    /* `resolverCampo` devuelve null para él, y las tres superficies filtran el
       null. Igual que `progreso`, que enciende la barra en vez de una fila. */
    expect(IMPRIMIR).toMatch(/if \(campo === 'repartoDelPago'\) return null/)
  })

  it('las dos superficies preguntan al checklist antes de pintarlo', () => {
    expect(COMPARTIR).toMatch(/campos\.some\(\(c\) => c\.campo === 'repartoDelPago'\)/)
    expect(IMPRIMIR).toMatch(/camposAUsar\.some\(\(c\) => c\.campo === 'repartoDelPago'\)/)
    // Y el bloque cuelga de esa respuesta, no de que haya cifra.
    expect(COMPARTIR).toMatch(/const reparto = quiereReparto \? repartoDeEstePago\(prestamo, pago\) : null/)
    expect(IMPRIMIR).toMatch(/const reparto = quiereReparto \? repartoDeEstePago\(prestamo, pago\) : null/)
  })
})
