...
describe('el recibo de pago abre la hoja', () => {
  it('los tres caminos del recibo la usan', () => {
    ...
    // Desde el 20 sep 2026 los tres estados son UNA tarjeta (`AccionesDelCobro`):
    // eran tres copias del mismo bloque y en el computador tres franjas estiradas.
    // Un solo botón ya no puede quedarse atrás en un estado y no en otro; lo que
    // hay que vigilar es que la tarjeta salga en los tres, o sea, que dependa
    // SOLO de que haya un último pago.
    const nuevos = src.split('<BotonAbrirHojaWA').length - 1
    expect(nuevos, 'el botón del recibo tiene que estar, y una sola vez').toBe(1)
    expect(src).toMatch(/\{ultimoPago && \(\s*<AccionesDelCobro/)
    expect(src).toMatch(/titulo=\{completado \? 'Préstamo saldado' : 'Cobro registrado'\}/)
  ...
  it('la hoja recibe el pago y lo suelta al cerrar', () => {
    ...
    // ⚠ NO se preselecciona la plantilla, y es la clave del asunto:
    // `preselectedTemplateId` enciende el modo avanzado, o sea el MODAL VIEJO.
    // Lo hice así en el primer intento y el recibo seguía abriendo la pantalla
    // que queremos retirar — se vio registrando un pago en el espejo, no en el
    // código. Con el pago basta: la hoja abre en la familia «Pago», que empieza
    // justo por `pago_confirmacion`.
    const desdeRecibo = src.split('<BotonAbrirHojaWA').slice(1).join('')
    expect(desdeRecibo, 'el recibo preselecciona plantilla y eso abre el modal viejo')
      .not.toMatch(/setWaSugerida\('pago_confirmacion'\)/)
  })
})
