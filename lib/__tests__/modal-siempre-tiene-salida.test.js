...
describe('siempre hay una salida visible', () => {
  it('con título O sin él', () => {
    // Dos ramas: la cabecera con título y la cabecera corta de solo la X.
  ...
  it('⚠ y ningún botón de cerrar es circular', () => {
    /* La regla ya estaba escrita en el canon —«un botón nunca es circular; el
       999px está reservado a cinco cosas y ninguna es un botón de acción»— y
       ESTA MISMA PRUEBA exigía lo contrario: pedía `rounded-full` y el fondo
       del círculo. Fijaba la implementación, y la implementación era el fallo
       que el dueño reportó con captura. */
    expect(sinNotas(modal), 'la X volvió a ser un círculo').not.toMatch(/rounded-full/)
  })

  it('⚠ la X no flota sobre el contenido', () => {
    /* El fallo: el botón era `absolute` contra el diálogo y el hueco que le
       hacía sitio era un `float-right` DENTRO del área que scrollea. Al
       deslizar, hueco y botón se separaban y el contenido pasaba por debajo de
       la X — se vio encima de la tarjeta del capital adeudado en «Renovar».

       En flujo eso no puede pasar, y por eso no hace falta reservar nada. */
    const codigo = sinNotas(modal)
    expect(codigo, 'la X volvió a ser absoluta').not.toMatch(/absolute top-3 right-3/)
    expect(codigo, 'volvió el hueco flotante').not.toMatch(/float-right/)
    expect(codigo, 'ya no se le cobra el hueco a todo el contenido').not.toMatch(/pr-16/)
  })
...
describe('los dos modales que traían su título dentro', () => {
  it('⚠ «Renovar» ya usa la cabecera del modal', () => {
    /* Mientras el título vivió dentro del cuerpo, el modal se quedaba sin
       cabecera y le tocaba la X flotante. Es la causa, no el síntoma. */
    const renovar = leer('components/prestamos/RenovarPrestamo.jsx')
    /* ⚠ ANCLADO EN LA CABECERA, NO EN EL TEXTO. El título dejó de ser una
       cadena fija el 31 ago: la misma hoja sirve para renovar y para cambiar el
       modo de cobro, y el título dice cuál de las dos es. Lo que esta prueba
       cuida es que el título siga siendo un PROP del modal —ahí es donde vive
       la X— y no vuelva al cuerpo. */
    const i = renovar.indexOf('<Modal')
    expect(i).toBeGreaterThan(-1)
    const props = renovar.slice(i, i + 700)
    expect(props).toMatch(/title=/)
    expect(props).toMatch(/subtitle=/)
    expect(props).toMatch(/Renovar el préstamo/)
  })

  it('la hoja de firma no lleva doble margen', () => {
    // `Firma` trae su propio padding; con el del modal encima el lienzo se
    // estrechaba y el título salía corrido.
    expect(leer('components/prestamos/FirmaDigital.jsx')).toMatch(/padding=\{false\}/)
  })
})
