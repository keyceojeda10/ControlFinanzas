...
describe('⚠ Nequi va primero porque es lo que usan', () => {
  ...
  it('y se dice cuándo se cobra al guardarlo', () => {
    /* Desde el 12 sep 2026 guardar cobra si el plan no está pago: decir «no se
       cobra nada» sería mentirle a quien está en prueba o vencido. */
    expect(hoja).toMatch(/Si tu plan ya está pago, hoy no se cobra nada; si no, se cobra al autorizar\./)
    expect(hoja).not.toMatch(/No se cobra nada ahora/i)
  })
})
