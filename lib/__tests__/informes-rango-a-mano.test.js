...
describe('⚠ el control sale donde sirve, y solo ahí', () => {
  ...
  it('y se puede quitar', () => {
    ...
    // La frase se reescribió («Manda el tramo que escribiste»); lo que no puede
    // faltar es que DIGA que el tramo escrito manda sobre las pastillas.
    expect(pantalla).toMatch(/Manda el tramo/)
  })

  /* ── LAS CAJAS NUNCA SALEN EN BLANCO ─────────────────────────────────────
     Reportado el 18 de agosto: «los selectores Desde y Hasta salen vacíos».
     Dos controles de fecha en blanco encima de unas cifras no dicen de qué día
     a qué día es lo que se está mirando. */
  it('las dos cajas enseñan el tramo de la pastilla mientras no se escriba otro', () => {
    expect(pantalla, 'no hay tramo derivado que enseñar').toMatch(
      /const rangoVista = rangoPuesto \? rango : rangoDe\(periodo\)/,
    )
    // Y las cajas tienen que leer de AHÍ, no del estado, o siguen en blanco.
    const leenDelEstado = /value=\{rango\[clave\]\}/.test(pantalla)
    expect(leenDelEstado, 'la caja lee del estado y volverá a salir vacía').toBe(false)
    expect(pantalla).toMatch(/value=\{rangoVista\[clave\]\}/)
  })

  it('tocar UNA sola caja ya deja el tramo completo', () => {
    /* Antes había que rellenar las dos para que filtrara: quien escribía solo
       «Desde» veía que no pasaba nada y creía que el control estaba roto. */
    expect(pantalla).toMatch(/setRango\(\{ \.\.\.rangoVista, \[clave\]: e\.target\.value \}\)/)
  })
})
