...
describe('el modelo puede devolver muchos clientes', () => {
  ...
  it('⚠ un renglón vacío del cuaderno no es un cliente', () => {
    ...
    /* ⚠ ANCLADO EN LA CONDICIÓN, NO EN LA LÍNEA ENTERA. Fijaba
       `.filter((c) => c.nombre || c.montoPrestado)` palabra por palabra, y se
       cayó al añadir las dudas por campo —que obligan a llevar el crudo al lado
       del normalizado—. Lo que hay que garantizar es que el renglón vacío se
       descarte, no cómo se llama la variable. */
    expect(lote).toMatch(/\.filter\([\s\S]{0,40}nombre \|\|[\s\S]{0,20}montoPrestado\)/)
  })
  ...
  it('la hoja de cuaderno se lee más grande que la cartulina', () => {
    /* A 1600px los números de la última columna se pierden.
 
       ⚠ 17 ago: esto fijaba el literal `maxTokens: 4096` y se rompió al subirlo
       a 12288. El número exacto no es el contrato —cada cliente engordó cuatro
       campos ese día y 4096 ya no daba para una hoja llena—; el contrato es que
       la lista tenga MUCHO más sitio que una cartulina suelta. Comprobado con
       una hoja de 60 renglones: con 4096 se cortaba, con 12288 salen los 60. */
    expect(lote).toMatch(/lado: 2000/)
...
describe('⚠ lo que la pantalla MUESTRA es lo que GUARDA', () => {
  const pantalla = leer('components/migrador/LoteFotos.jsx')
  // Sin las notas: explican el fallo nombrándolo. Ver `sinNotas` arriba.
  const codigo = sinNotas(pantalla)

  it('hay UN solo resolvedor de los valores efectivos', () => {
    /* Cazado corriendo el lote entero contra el espejo: el OCR devuelve
       `tasa: ''` cuando no la encuentra, y `''` NO es null — así que
       `f.tasa ?? base.tasa` daba `''` y `Number('')` es 0. El texto de la fila
       usaba `||` y decía «diario 20 %»… y se creaban los siete préstamos AL 0 %.

       Sobre una cartera de veinte millones son cuatro millones de interés que
       desaparecen sin un error a la vista. */
    expect(pantalla).toMatch(/function efectivo\(f, base\)/)
    expect(codigo, 'volvió el `??` que no cae con la cadena vacía')
      .not.toMatch(/f\.tasa \?\? base\.tasa/)
  })

  it('y lo usan el semáforo, el texto de la fila y el guardado', () => {
    // Tres cálculos sueltos del mismo dato es como se separaron la primera vez.
    const usos = (codigo.match(/efectivo\(f, base\)/g) ?? []).length
    expect(usos).toBeGreaterThanOrEqual(4)
    /* ⚠ LO QUE IMPORTA ES QUE EL GUARDADO SALGA DE `efectivo`, no el orden de
       los campos que desestructura: al añadir el modo de interés por fila hubo
       que sacar uno más de ahí, y la prueba literal se cayó por eso. */
    expect(pantalla).toMatch(/const \{[^}]*\} = efectivo\(f, base\)/)
  })

  it('la tasa vacía cae al valor de arriba, no a cero', () => {
    /* No se puede importar `efectivo` —vive dentro del componente— así que se
       comprueba la regla sobre el texto: `num()` descarta el 0 y el `??`
       encadena al defecto ya convertido a número. */
    expect(pantalla).toMatch(/const num = \(v\) => \{ const n = Number\(v\); return Number\.isFinite\(n\) && n > 0 \? n : null \}/)
    expect(pantalla).toMatch(/tasa: num\(f\.tasa\) \?\? num\(base\.tasa\)/)
  })
})
