...
describe('solo lectura', () => {
  ...
  it('las excepciones son EXACTAS, no un prefijo: un nombre parecido sigue bloqueado', () => {
    expect(vista('POST', '/api/auth/signout-foo')).toBe(true)
    expect(vista('POST', '/api/auth/sessions')).toBe(true)
    expect(vista('POST', '/api/errores-clientes')).toBe(true)
    expect(vista('POST', '/api/ver-como/volver-todo')).toBe(true)
    // salvo la barra final, y salvo el callback (ese sí es prefijo: /api/auth/callback/<proveedor>)
    expect(vista('POST', '/api/auth/signout/')).toBe(false)
    expect(vista('POST', '/api/auth/callback/cuenta-guardada')).toBe(false)
  })
