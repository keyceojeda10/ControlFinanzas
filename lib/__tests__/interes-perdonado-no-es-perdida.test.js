...
describe('⚠ el asiento se apunta pero no baja la caja', () => {
  ...
  it('⚠ los que leen la dirección por el saldo lo saltan', () => {
    ...
    const i = caja.indexOf('const deltaPorRuta = new Map()')
    expect(caja.slice(i, i + 900)).toMatch(/Math\.round\(m\.saldoAnterior\) === Math\.round\(m\.saldoNuevo\)\) continue/)
    // Y el de arranque de ruta NO se salta: ése sí mueve la bolsa a propósito.
    expect(caja.slice(i, i + 900)).toMatch(/!m\.ajusteArranqueRuta/)
