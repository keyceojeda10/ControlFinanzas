...
describe('⚠ el momento post-registro', () => {
  ...
  it('⚠ el teléfono no sustituye a la atención', () => {
    ...
    expect(ayuda.texto).toMatch(/Cuénteme en qué parte se quedó/)
    expect(ayuda.texto.indexOf('301 199 3001')).toBeGreaterThan(ayuda.texto.indexOf('Cuénteme'))
  ...
  it('el saludo dice el nombre cuando lo hay, y no se rompe cuando no', () => {
    ...
    expect(mensajeBienvenida('')).toMatch(/^Listo, su cuenta/)
    ...
    expect(mensajeBienvenida('Ana')).toMatch(/no se siente a escribir/i)
  })
})
