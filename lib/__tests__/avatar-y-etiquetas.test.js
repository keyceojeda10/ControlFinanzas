...
describe('lo que la licencia obliga a decir', () => {
  it('⚠ «Aventura» es CC BY 4.0, así que su autoría sale EN PANTALLA', async () => {
    /* Casi todo el juego es CC0 y no pide nada. `adventurer` sí: se puede usar
       y vender a cambio de nombrar a quien lo dibujó, donde se vea.

       Si mañana alguien borra esa línea del selector, esta prueba falla — y lo
       correcto entonces es quitar la sección, no la línea. */
    const { readFileSync } = await import('node:fs')
    const { resolve } = await import('node:path')
    const pantalla = readFileSync(resolve(process.cwd(), 'app/(dashboard)/configuracion/page.jsx'), 'utf8')
    const { AVATARS } = await import('@/lib/avatars')
    /* Las cuatro secciones CC BY y su autor. Si alguna sigue en el juego, su
       nombre TIENE que estar en el selector. */
    const CC_BY = {
      gestos: 'Lisa Wischofsky', sonrisas: 'Ashley Seo',
      garabatos: 'vijay verma', retratos: 'Micah Lanier',
    }
    const faltan = Object.entries(CC_BY)
      .filter(([cat, autor]) => AVATARS.some((a) => a.categoria === cat) && !pantalla.includes(autor))
      .map(([cat, autor]) => `${cat} → ${autor}`)
    expect(faltan, 'secciones CC BY sin su autor a la vista').toEqual([])
    if (Object.keys(CC_BY).some((c) => AVATARS.some((a) => a.categoria === c))) {
      expect(pantalla).toMatch(/CC BY 4\.0/)
    }
  })

  it('⚠ y no hay ningún personaje con dueño', async () => {
    /* Se pidieron Goku, Superman, Batman, Spider-Man e Iron Man con el
       argumento de «aún no somos tan grandes». No se metieron: son de Toei, DC,
       Marvel y Disney, y los avisos de retirada caen igual sobre negocios
       pequeños. Lo que sí hay es lo que más se le parece siendo de uso libre:
       pixel de videojuego, mando y dado, robots, y caras con gafas de pasta y
       camiseta estampada. */
    const { AVATARS } = await import('@/lib/avatars')
    const prohibidos = /goku|superman|batman|spider|iron ?man|hulk|thor|pikachu|mario|sonic|thanos|venom|deadpool|wolverine/i
    expect(AVATARS.filter((a) => prohibidos.test(a.id) || prohibidos.test(a.nombre))).toEqual([])
  })
})
