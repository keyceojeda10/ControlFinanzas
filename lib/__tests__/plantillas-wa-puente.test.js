...
describe('personalizar está a la vista', () => {
  ...
  it('el panel se abre sobre la plantilla que se está mirando', () => {
    ...
    expect(hj).toMatch(/onPersonalizar=\{secciones \?/)
...
describe('personalizar sin salir de la hoja', () => {
  ...
  it('el boton ya NO manda al modal viejo', () => {
    const i = tpl.indexOf('Personalizar este mensaje')
    const btn = tpl.slice(tpl.lastIndexOf('<button', i), i)
    expect(btn).toContain('onPersonalizar')
    expect(btn).not.toContain('onEditarPlantillas')
  })
