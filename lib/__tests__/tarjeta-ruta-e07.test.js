...
describe('la acción principal tiene su botón', () => {
  it('«Cobrar» existe y es DORADO, nunca verde', () => {
    /* Cobrar era lo único que no tenía botón: se hacía tocando la tarjeta
       ...
       alguien pagó. */
    /* ⚠ El texto ya no es fijo: sale de `contexto.accion`, con «Cobrar» de
       defecto. Lo que NO cambia es el color de la acción principal. */
    const i = src.indexOf("{contexto?.accion?.texto ?? 'Cobrar'}</button>")
    expect(i, 'no hay botón principal en la tarjeta').toBeGreaterThan(-1)
    const boton = src.slice(Math.max(0, i - 700), i)
    expect(boton).toMatch(/background: contexto\?\.accion\?\.tono === 'apagado' \? 'var\(--cf-card\)' : 'var\(--cf-gold\)'/)
  ...
  it('y no quita el gesto de siempre', () => {
    ...
    const i = src.indexOf("{contexto?.accion?.texto ?? 'Cobrar'}</button>")
    // Sin `contexto` —una visita de hoy— el botón sigue llamando a `onClick`,
    // que es el mismo gesto de tocar la tarjeta entera.
    expect(src.slice(Math.max(0, i - 700), i)).toMatch(/onClick=\{contexto\?\.accion \? onAccion : onClick\}/)
  })
})
