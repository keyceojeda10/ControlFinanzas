...
describe('el comprobante', () => {
  it('con los números del día, la barra se vuelve la billetera', () => {
    expect(RECIBO).toMatch(/\{conNumeros && <LlevasHoy progreso=\{progresoDia\} origenRef=\{refMonto\} \/>\}/)
    // Sin números, la barra de siempre: ningún camino se queda sin «cómo va el día».
    expect(RECIBO).toMatch(/\{progresoDia && !conNumeros && \(/)
  })
