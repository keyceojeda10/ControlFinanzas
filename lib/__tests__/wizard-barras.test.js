...
describe('las sombras de los wizards salen del canon', () => {
  ...
  it('crear cliente usa el token de hoja', () => {
    ...
    /* La ventana va holgada: con 320 caracteres, meter un comentario dentro del
       `style` empujaba `boxShadow` fuera y la prueba fallaba sin que la sombra
       se hubiera tocado. */
    const barra = cliente.match(/className="fixed left-0 right-0[^"]*bottom-0[^"]*"[\s\S]{0,900}/)[0]
