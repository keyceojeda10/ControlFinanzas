...
describe('la configuración de cada país es usable', () => {
  ...
  it('el símbolo de varias letras lleva su espacio', () => {
    /* `formatMoney` pega el símbolo a la cifra sin separación, que es lo
       correcto para `$` y `₲` —un solo glifo— pero deja «Bs500.000» pegado y se
       lee mal. El espacio va en el símbolo del país, no en la función: tocarla
       separaría el peso de su cifra en los otros diecisiete. */
    expect(formatMoney(500000, 'bo')).toBe('Bs 500.000')
    expect(formatMoney(500000, 'co')).toBe('$500.000')
    expect(formatMoney(500000, 'py')).toBe('₲500.000')
  })
