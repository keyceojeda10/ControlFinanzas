...
describe('en el resto de la interfaz, el techo solo puede bajar', () => {
  /* Eran 77 al medirlas por primera vez. Cada vez que se rediseña una pantalla
     este número baja y la prueba lo fija; la propia prueba avisa cuando el techo
     se ha quedado por encima de la realidad.

     Es la estética anterior repartida por las páginas que todavía no
  ...
  const TECHO_SOMBRAS = 36
  // 33 → 28 al unificar el comprobante de pago: la tarjeta de «Cuota N de M /
  // Saldo / Progreso» del comprobante viejo llevaba degradado Y sombra, y salió
  // entera. El trinquete hizo su trabajo — avisó de que se había quedado por
  // encima de la realidad.
  // 28 → 21 el 19 sep 2026, en la pasada de «lo que molesta a la vista»: salieron
  // el hero del cobrador, las dos tarjetas de Capital, el podio del ranking y las
  // de Soporte.
  const TECHO_DEGRADADOS = 16   // 20 sep 2026: fuera los del resumen del préstamo y la tarjeta del cobro
