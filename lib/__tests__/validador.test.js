...
describe('correccionPara: cada motivo le dice al modelo qué arreglar', () => {
  it('cubre todos los motivos que emite validar', () => {
    ...
    // Un motivo por línea; argumento_repetido añade debajo la lista de los que
    // quedan libres, sangrada, así que se cuentan solo las líneas de motivo.
    const lineas = texto.split('\n').filter((l) => !l.startsWith('  · '))
    expect(lineas).toHaveLength(todos.length)
    for (const l of lineas) { expect(l.startsWith('- ')).toBe(true); expect(l.length).toBeGreaterThan(30) }
    expect(texto).toContain('rutas')
    expect(texto).toContain('¿Cuántos cobradores tiene?')
  })
  it('al argumento repetido le dice CUÁLES le quedan, no solo que no repita', () => {
    // Medido en producción el 8 sep: un lead con 60 cartulinas recibió cuatro
    // mensajes seguidos con el mismo argumento, y la regeneración devolvió lo
    // mismo. «Usa otro» no sirve si el modelo no sabe cuáles hay.
    const t = correccionPara(['argumento_repetido'], { contexto: { argumentosUsados: ['saldo_calle', 'tiempo_real'] } })
    expect(t).toContain('saldo_calle')
    expect(t.split('\n  · ').length).toBeGreaterThan(5)
    expect(t).not.toContain('tiempo real cuanto cobro cada cobrador') // el que ya usó, fuera
  })
})
...
describe('lo que se vio en producción el 9 sep', () => {
  it('la misma pregunta con las palabras cambiadas de orden es una repetición', () => {
    /* Dos turnos seguidos, y el lead ya había contestado «más de 100»:
       «¿Ahorita cuántos clientes más o menos tiene en cartera?» y
       «¿cuántos clientes tiene más o menos en cartera?». */
    const contexto = { ultimaPreguntaBot: '¿Ahorita cuántos clientes más o menos tiene en cartera?', argumentosUsados: [] }
    expect(validar('Exacto. ¿cuántos clientes tiene más o menos en cartera?', { contexto, stage: 'VALOR' }).motivos)
      .toContain('pregunta_repetida')
  })
  it('pero una pregunta distinta sobre lo mismo no lo es', () => {
    const contexto = { ultimaPreguntaBot: '¿Ahorita cuántos clientes más o menos tiene en cartera?', argumentosUsados: [] }
    for (const m of ['Listo. ¿Y cuántos cobradores tiene en la calle?', '¿Hoy cómo hace el cuadre de la noche?']) {
      expect(validar(m, { contexto, stage: 'VALOR' }).motivos, m).not.toContain('pregunta_repetida')
    }
  })
})

describe('el pitch corto y el cierre nuevo', () => {
  it('el corto cabe en tres líneas y lleva el dolor que convirtió 9,4×', async () => {
    const { pitchTrasSolicitud } = await import('@/lib/bot-v2/respuestas-fijas')
    const t = pitchTrasSolicitud({ nombre: 'Carlos', variante: 'corto' })
    expect(t.split(/\s+/).length).toBeLessThan(60)          // el largo tiene 112
    expect(t).toMatch(/nunca el número exacto/)             // el dolor medido
    expect(t).toMatch(/¿Usted cobra solo o tiene cobradores\?$/)  // segmenta
    expect(t).not.toMatch(/gratis|prueba/i)                 // la prueba se guarda
  })
  it('y el largo sigue intacto, que es el que aprobó el dueño', async () => {
    const { pitchTrasSolicitud } = await import('@/lib/bot-v2/respuestas-fijas')
    const t = pitchTrasSolicitud({ nombre: 'Carlos', variante: 'largo' })
    expect(t.split(/\s+/).length).toBeGreaterThan(100)
    expect(t).toMatch(/celular, computador o tablet/)
  })
  it('el cierre pide recoger lo suyo y decirle qué hacer al entrar', async () => {
    const { promptCierre } = await import('@/lib/bot-v2/prompts')
    const p = promptCierre({ nombre: 'Ana', historial: '', contexto: null })
    expect(p).toMatch(/meta dos o tres de sus clientes/)
    expect(p).toMatch(/NUNCA cierres con «cualquier duda me escribe»/)
    expect(p).toContain('https://app.control-finanzas.com/registro')
  })
})
