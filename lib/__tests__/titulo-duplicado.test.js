...
describe('el título lo pone el armazón, no la página', () => {
  ...
  it('ninguna página con cabecera dibuja además su propio <h1>', () => {
    ...
    const CONOCIDAS = [
      // Las que TODAVIA repiten el titulo. Se va vaciando; NO puede crecer.
      '/carga-masiva', '/clientes/nuevo', '/clientes/x/editar',
      '/clientes/x/historial', '/configuracion/plan', '/migrador', '/reportes',
      // Estas dos son distintas: su <h1> es un DATO —el nombre del cobrador, el
      // asunto del ticket—, no el nombre de la pantalla. Se quedan a proposito,
      // pendientes de declararlo en la cabecera como se hizo en la caja del
      // cobrador, para que exista una sola vez.
      '/cobradores/x', '/soporte/x',
    ]
