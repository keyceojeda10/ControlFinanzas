...
describe('la tarjeta de cobro', () => {
  ...
  /* ⚠ HAY MÁS DE UN AVATAR. Estas dos pruebas miraban `indexOf` —el PRIMERO— y
     fijaban `2px` y `40`. Al añadirse la tarjeta compacta de quien ya pagó, su
     avatar (más pequeño, como toda ella) pasó a ser el primero y las dos
     fallaron sin que nada estuviera mal. Lo que importa es la regla, no la
     medida: TODOS llevan el anillo del estado y ninguno se puede aplastar. */
  const avatares = () => {
    const out = []
    let i = fila.indexOf('{iniciales}')
    while (i > -1) {
      out.push(fila.slice(Math.max(0, i - 700), i))
      i = fila.indexOf('{iniciales}', i + 1)
    }
    return out
  }

  it('el estado va en el anillo del avatar, en todos', () => {
    const todos = avatares()
    expect(todos.length).toBeGreaterThan(0)
    for (const a of todos) {
      expect(a, 'un avatar sin el anillo del estado').toMatch(/border: `[0-9.]+px solid \$\{color\}`/)
    }
  })

  it('ningún avatar se puede aplastar', () => {
    ...
    for (const a of avatares()) {
      for (const pieza of ["flex: 'none'", 'minWidth:', 'minHeight:', "aspectRatio: '1'"]) {
        expect(a, `a un avatar le falta ${pieza}`).toContain(pieza)
      }
    }
  })
