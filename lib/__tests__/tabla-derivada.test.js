...
describe('G6.1 · la tabla derivada cuadra con lo que la app ya calculó', () => {
  ...
  it('LA ÚLTIMA FILA CAE EN EL VENCIMIENTO, en los 144', () => {
    // ⚠ ESTA PRUEBA NO EXISTÍA Y POR ESO SE ME COLÓ. La anterior solo miraba
    // `fijo`/`mensual`, y en `unico` el paso salía de la frecuencia —1 día en
    // diario— así que la única cuota vencía AL DÍA SIGUIENTE del desembolso.
    // La cazó la medición contra el espejo: la mora de los 956 préstamos de
    // cuota única saltaba de $192M a $488M por mi derivación, no por el fallo
    ...
    for (const caso of CASOS) {
      ...
      const filas = derivarTabla(p)
      if (!filas.length) continue
      const ultima = new Date(filas[filas.length - 1].fechaEsperada).getTime()
      const fin = new Date(p._calc.fechaFin).getTime()
      if (ultima !== fin) fallos.push({ ...caso, ultima: new Date(ultima).toISOString(), fin: new Date(fin).toISOString() })
    }
    expect(fallos).toEqual([])
  })

  it('en cuota única el cobro va al FINAL del plazo, no al día siguiente', () => {
    const p = prestamoReal({ monto: 1000000, tasa: 20, dias: 30, frecuencia: 'diario', modoInteres: 'unico' })
    const filas = derivarTabla(p)
    expect(filas).toHaveLength(1)
    const dias = (new Date(filas[0].fechaEsperada) - new Date(p.fechaInicio)) / 86400000
    expect(dias).toBe(30)
  })
})
