...
describe('crear préstamo ya no tiene la cabecera en blanco', () => {
  ...
  it('el título dice EN QUÉ PASO se está', () => {
    ...
    //
    // ⚠ EN PC LOS PASOS SON OTROS (T16-00: cliente y condiciones van juntos),
    // así que el título sale de `PASOS_PC` o de `PASOS` según quepa. Fijar aquí
    // `PASOS[paso]` a secas era fijar la versión de teléfono como contrato.
    expect(src).toMatch(/titulo: \(unaPantalla \? .+ : PASOS\[paso\]\)\?\.label/)
  })
  ...
  it('y alimenta la espina de progreso', () => {
    ...
    expect(src).toMatch(/paso: unaPantalla \? .+ : paso \+ 1/)
    expect(src).toMatch(/total: unaPantalla \? PASOS_PC\.length : PASOS\.length/)
  })
