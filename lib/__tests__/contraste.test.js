...
describe('contraste · texto sobre el menú dorado', () => {
  // Los alfas se leen del propio componente: si alguien los baja, el test cae.
  function alfaDe(constante) {
    const linea = menu
      .split('\n')
      .find((l) => l.trim().startsWith(`const ${constante}`) && l.includes('rgba(58,41,0,'))
    const alfa = Number(linea?.split('rgba(58,41,0,')[1]?.split(')')[0])
    if (!Number.isFinite(alfa)) throw new Error(`no se pudo leer el alfa de ${constante}`)
    return alfa
  }

  it('la fecha (12px) pasa AA', () => {
    expect(contraste(sobre(TINTA_MENU, ORO, alfaDe('TINTA_2')), ORO)).toBeGreaterThanOrEqual(4.5)
  })

  it('los rótulos de grupo (10px) pasan AA', () => {
    expect(contraste(sobre(TINTA_MENU, ORO, alfaDe('ROTULO')), ORO)).toBeGreaterThanOrEqual(4.5)
  })

  it('los alfas del handoff (.55 y .62) NO pasaban — por eso se subieron', () => {
    expect(contraste(sobre(TINTA_MENU, ORO, 0.55), ORO)).toBeLessThan(4.5)
    expect(contraste(sobre(TINTA_MENU, ORO, 0.62), ORO)).toBeLessThan(4.5)
  })
})
