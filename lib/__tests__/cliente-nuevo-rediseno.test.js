...
describe('un solo campo obligatorio', () => {
  ...
  it('pero el marcador SIN- se sigue poniendo con el campo vacío', () => {
    ...
    //
    // ⚠ ESTO FIJABA LA EXPRESIÓN LITERAL `sinCedula || !form.cedula.trim()`, y
    // esa expresión tenía un fallo: acuñaba un marcador NUEVO en cada guardado,
    // así que al EDITAR a un cliente sin documento el API lo tomaba por un
    // cambio de cédula y devolvía «Cédula no válido». 1.574 clientes vivos no
    // se podían editar. Ver `editar-cliente-sin-cedula.test.js`.
    //
    // Se comprueba la conducta —hay marcador cuando no hay cédula escrita— y no
    // la línea exacta, que es lo que impedía corregirla.
    const bloque = src.slice(src.indexOf('const cedulaFinal'), src.indexOf('const payload'))
    expect(bloque).toMatch(/!sinCedula && form\.cedula\.trim\(\)/)
...
describe('los campos dejan de ir apilados a ancho completo', () => {
  ...
  it('el nombre es el que manda', () => {
    /* Ventana holgada: `sinComentarios` vacía los comentarios pero conserva su
       longitud, y el de este campo explica por qué NO lleva altura propia. */
    const i = src.indexOf('label="Nombre completo"')
    expect(src.slice(i, i + 1600)).toMatch(/cf-campo-grande/)
  })
})
