...
describe('⚠ el mes se corta en el mismo sitio en las dos pantallas', () => {
  /* Esto costó tres vueltas de encontrar y solo se ve comparando las dos
     pantallas del mismo negocio: la consulta base agrupaba restando el corte y
     la corrección por tabla usaba `toISOString()`, que es UTC. Un pago de las
     8:48 p.m. del 31 de julio caía en julio por un lado y en agosto por el
     otro. Eran $40.983 de diferencia en un solo negocio. */
  ...
  for (const f of ARCHIVOS) {
    const nombre = f.includes('pdf') ? 'el PDF' : 'la pantalla'

    it(`${nombre} arranca el mes a las 05:00 UTC`, () => {
      const src = crudo(f)
      expect(src, 'volvió `new Date(año, mes, 1)`, que usa la hora del servidor')
        .not.toMatch(/new Date\(hoy\.getFullYear\(\), hoy\.getMonth\(\)[^)]*, 1\)/)
      expect(src).toMatch(/HORA_CORTE = 5/)
    })

    it(`${nombre} deriva la clave del mes por una sola función`, () => {
      const src = crudo(f)
      expect(src).toMatch(/claveMesBogota/)
      expect(src, 'quedó una clave de mes sacada en UTC de una fecha de pago')
        .not.toMatch(/pago\.fechaPago\.toISOString\(\)\.slice\(0, 7\)/)
    })

    it(`${nombre} agrupa el SQL con el mismo corte`, () => {
      const src = crudo(f)
      const agrupaciones = src.match(/DATE_FORMAT\([^)]*\)/g) ?? []
      const sinCorte = agrupaciones.filter((a) => !/DATE_SUB\(/.test(a))
      expect(sinCorte, `quedan ${sinCorte.length} agrupaciones en UTC: ${sinCorte.join(' · ')}`)
        .toHaveLength(0)
    })
  }
})
