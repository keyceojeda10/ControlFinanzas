...
describe('un corte de la cuenta no le gasta intentos al lead', () => {
  const ruta = leer('app/api/webhook/whatsapp-cloud/route.js')

  it('⚠ los códigos de CORTE se atienden antes que el throttle', () => {
    /* No estaban en ninguna lista, así que cada rebote durante un corte quemaba
       un intento como si el mensaje hubiera llegado. Medido el 30 ago 2026 en
       pleno corte por facturación: 16 horas mudo, 25 de 25 fallando, y cada
       media hora un lead perdía otro de sus tres intentos contra un muro. */
    const fn = ruta.slice(ruta.indexOf('async function devolverIntentoSiThrottle'))
    const guarda = fn.indexOf('esCorteDeCuenta(codigo)')
    const throttle = fn.indexOf('CODIGOS_THROTTLE.has(codigo)')
    expect(guarda).toBeGreaterThan(-1)
    expect(guarda).toBeLessThan(throttle)   // antes, o el `return` del throttle se lo come
  })

  it('el corte devuelve el intento y reprograma, sin contarlo como rebote suyo', () => {
    const fn = ruta.slice(ruta.indexOf('async function devolverIntentoPorCorte'),
                          ruta.indexOf('// 35 personas se quedaron sin recibir NADA'))
    expect(fn).toContain('intentosSeguimiento: intentos')
    expect(fn).toContain('proximoSeguimiento:')
    /* Lo que NO debe hacer: cortarle los seguimientos. El número del lead está
       perfecto; la que está cortada es nuestra cuenta. */
    expect(fn).not.toContain('proximoSeguimiento: null')
    expect(fn).not.toContain('accionTrasThrottle')
  })

  it('los tres códigos de corte son los mismos que vigila la alerta', () => {
    /* Una lista para avisar y otra para no gastar intentos acabarían
       divergiendo: se leen del mismo sitio. */
    expect(ruta).toContain("Object.hasOwn(CODIGOS_CORTE, String(codigo))")
  })
})
