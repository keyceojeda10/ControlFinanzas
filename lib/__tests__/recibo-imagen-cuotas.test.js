...
describe('los tres viven en UN solo sitio', () => {
  ...
  it('la IMAGEN del comprobante los usa', () => {
    // Los tres, del sitio común. Sin fijar la lista entera: al fichero se le
    // añaden derivados nuevos (`saldoAntesDeEstePago`) y eso no es una regresión.
    const imp = imagen.match(/import \{([^}]*)\} from '@\/lib\/recibo-derivados'/)
    expect(imp, 'la imagen dejó de importar los derivados').not.toBeNull()
    for (const fn of ['numeroCuotaDe', 'porcentajeDe', 'cuotasRestantesDe']) {
      expect(imp[1]).toContain(fn)
    }
