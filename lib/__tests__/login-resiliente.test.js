...
describe('login · si signIn acertó, la persona está dentro', () => {
  it('no deja que la consulta del rol tumbe el login', () => {
    ...
    const bloque = login.slice(login.indexOf('let esSuperadmin'), login.indexOf('const url = esSuperadmin'))
    expect(bloque).toContain('try {')
    expect(bloque).toContain('catch')
    expect(login).toContain("const url = esSuperadmin ? '/admin/inicio' : '/dashboard'")
  })
