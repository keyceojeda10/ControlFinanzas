...
describe('el GPS no tiene esperando a un cobro', () => {
  ...
  it('y nunca lanza, ni sin pago ni sin GPS', async () => {
    // Contesta `false`: el comprobante solo dice «Ubicación registrada» con un `true`.
    await expect(geo.completarUbicacionDelPago(null)).resolves.toBe(false)
  })
})
...
describe('los caminos que mueven plata ya no esperan al GPS fuera de la pantalla', () => {
  ...
  it('crear préstamo, renovar y crear cliente también la usan', () => {
    ...
    // Lo que faltaba (22 sep 2026): el gasto y las dos correcciones del préstamo.
    expect(lee('components/gastos/ReportarGasto.jsx')).toContain("conPantalla('gasto'")
    expect(lee('components/prestamos/EditarPrestamo.jsx')).toContain("conPantalla('corregir'")
    expect(lee('components/prestamos/ModificarPlazo.jsx')).toContain("conPantalla('corregir'")
  })
})
...
describe('la pantalla de «estoy en eso»', () => {
  ...
  it('se queda lo que tarda en contarse —«es muy rápida, ni se ve»— y ni un «¡Listo!» de propina', async () => {
    // El dueño vio la versión de 0,7 s en su teléfono y no le dio tiempo a verla.
    const { MINIMO_MS, PASO_MS, GUIONES, minimoDe } = await import('@/lib/procesando')
    expect(MINIMO_MS).toBeGreaterThanOrEqual(1500)
    expect(MINIMO_MS).toBeLessThanOrEqual(2200)
    // Da tiempo a que salgan TODOS los estados del cobro antes de irse.
    expect((GUIONES.cobro.pasos.length - 1) * PASO_MS).toBeLessThan(MINIMO_MS)
    // Un «Guardando» suelto no tiene nada que apilar: se queda menos.
    expect(minimoDe('guardando')).toBeLessThan(minimoDe('cobro'))
  ...
  describe('los tiempos', () => {
    ...
    it('sale EN EL ACTO y se va en cuanto la operación termina', async () => {
      ...
      const promesa = conPantalla('cobro', () => new Promise((r) => setTimeout(() => r('ok'), 4000)))
      expect(pintado).toEqual(['cobro'])                       // sin esperar a nada
      await vi.advanceTimersByTimeAsync(4000)
      expect(pintado).toEqual(['cobro', 'saliendo'])           // 4 s > mínimo: ni un ms de más
      await vi.advanceTimersByTimeAsync(180)
      expect(pintado).toEqual(['cobro', 'saliendo', null])
      await expect(promesa).resolves.toBe('ok')
      expect(MINIMO_MS).toBeLessThan(4000)
    })
...
describe('«le hice un abono a interés y ni siquiera salió nada» — el dueño, 20 sep 2026', () => {
  // «Pagar los intereses» y «Cerrar anticipado» tienen su propia hoja en la ficha.
  // Cobraban bien y al terminar SOLO se cerraban: ni comprobante ni pantalla.
  const ficha = lee('app/(dashboard)/prestamos/[id]/page.jsx')
  const rp = lee('components/prestamos/RegistrarPago.jsx')

  it('pagar intereses acaba en el comprobante, no en una hoja que se cierra', () => {
    const fn = ficha.slice(ficha.indexOf('const pagarIntereses = async'), ficha.indexOf('const hayCierre'))
    expect(fn).toMatch(/conPantalla\(/)
    expect(fn).toMatch(/abrirReciboDe\(\{[\s\S]*tipo: 'intereses'/)
    expect(fn).toMatch(/completarUbicacionDelPago\(/)
  })

  it('cerrar anticipado cobrando también; con $0 no hay nada que comprobar', () => {
    const fn = ficha.slice(ficha.indexOf('async function confirmarLiquidacion'), ficha.indexOf('// Intent param desde rutas'))
    expect(fn).toMatch(/conPantalla\('cobro'/)
    expect(fn).toMatch(/if \(cobrado > 0\) \{\s*abrirReciboDe\(/)
  })

  it('el comprobante vive en un solo sitio: RegistrarPago lo abre con `reciboDe`', () => {
    expect(ficha).toContain('reciboDe={reciboDe}')
    expect(rp).toMatch(/if \(!open \|\| !reciboDe\?\.pago\) return/)
    expect(rp).toContain("tipo === 'liquidacion' ? 'Préstamo cerrado' :")
    // Va DESPUÉS del efecto que reinicia al abrir, o el reinicio le pisa el tipo.
    expect(rp.indexOf('if (!open || !reciboDe?.pago) return'))
      .toBeGreaterThan(rp.indexOf('}, [open, presetPago, cuotaDiaria, saldoPendiente, tabInicial, montoInicial])'))
  })

  it('al cerrar el comprobante se olvida el pago, o volvería a salir al abrir un cobro', () => {
    expect(ficha).toMatch(/setPresetPago\(null\)\s*setReciboDe\(null\)/)
  })
})
