...
describe('la caja se alcanza desde «Más»', () => {
  ...
  it('va pegada a «Capital»: son las dos preguntas de dinero', () => {
    ...
    // Dentro del menú del DUEÑO: desde el 19 sep 2026 el cobrador tiene el suyo
    // más arriba en el fichero, sin Capital, y ahí «Caja» va primera.
    const dueno = mas.slice(mas.indexOf('const herramientas = ['))
    const iPlata = dueno.indexOf("nombre: 'Capital'")
    const iCaja = dueno.indexOf("nombre: 'Caja'")
    const iNegocio = dueno.indexOf("nombre: '¿Cómo va el negocio?'")
  ...
  it('y su icono es EL DEL SISTEMA, no uno dibujado aquí', () => {
    // Desde el 20 sep 2026 «Más» no dibuja iconos: los pide por ruta a
    // `armazon/iconos.jsx`. Caja era un cajón aquí, una tarjeta en la barra
    // lateral y una caja 3D en el menú del +.
    expect(mas).toMatch(/caja: '\/caja'/)
    expect(mas).toContain("import { ICONO_DE_RUTA } from '@/components/armazon/iconos'")
    expect(mas).not.toMatch(/caja:\s+<>/)
  })
})
