...
describe('⚠ el modo del préstamo nuevo se puede elegir', () => {
  ...
  it('y la API lo acepta de verdad (no era una pantalla mintiendo)', () => {
    ...
    // Se guarda con `modoFinal`, que ES el modo elegido salvo al financiar (cuota fija).
    expect(api).toMatch(/modoInteres: modoFinal/)
...
describe('⚠ cambiar el modo no mueve un peso', () => {
  it('la hoja arranca con el CAPITAL puesto, no con el total a pagar', () => {
    ...
    /* ⚠ ACTUALIZADO EL 9 SEP 2026. Antes esto anclaba que la hoja usara
       `capitalRestante` a secas. Ese era justamente el fallo: el servidor NO usa
       esa regla —usa `minimoParaRenovar`, que arrastra el capital solo cuando
       hay tabla— y la hoja proponía una cifra que el servidor luego rechazaba o
       cambiaba. Ahora la cifra llega hecha y se comprueba lo que de verdad
       importa: que un Globo CON PLAZO siga arrancando en su capital. */
    expect(hoja).toMatch(/const saldo = minimoRenovacion != null \? Math\.max\(0, Number\(minimoRenovacion\)\) : saldoTotal/)
    expect(hoja).toMatch(/if \(soloModo && saldo > 0\) \{ setMontoTecleado\(null\); setMonto\(String\(Math\.round\(saldo\)\)\) \}/)
  })

  it('⚠ y el Globo de $60.000.000 sigue arrancando en su capital, no en los 206 millones', async () => {
    /* La prueba de verdad de lo anterior, con el préstamo real que motivó la
       regla: capital $60.000.000 y `totalAPagar` $206.880.000 por el interés
       futuro de 36 meses. Tiene tabla, así que `minimoParaRenovar` devuelve el
       capital y la hoja arranca ahí. */
    const { minimoParaRenovar, calcularSaldoPendiente } = await import('@/lib/calculos')
    const globoConPlazo = {
      montoPrestado: 60000000, totalAPagar: 206880000, totalPagado: 0,
      cuotaDiaria: 4080000, diasPlazo: 1080, frecuencia: 'mensual',
      modoInteres: 'solo_interes', pagos: [],
      cuotasAmortizacion: Array.from({ length: 36 }, (_, i) => ({
        numeroPeriodo: i + 1, capital: i === 35 ? 60000000 : 0, interes: 4080000,
        cuotaTotal: i === 35 ? 64080000 : 4080000, pagado: 0, interesPagado: 0,
      })),
    }
    expect(minimoParaRenovar(globoConPlazo)).toBe(60000000)
    expect(calcularSaldoPendiente(globoConPlazo), 'el total con el interés futuro').toBe(206880000)
  })
  ...
  it('avisa cuando el interés ya causado se vuelve capital', () => {
    ...
    /* La condición cambió el 9 sep: el aviso ya no se cuelga de que falte el
       capital, sino de que lo que se arrastra SEA el saldo entero — que es
       cuando de verdad hay interés causado volviéndose capital. */
    const i = hoja.indexOf('Math.round(saldo) === Math.round(saldoTotal) && saldoTotal > 0')
    expect(i, 'el aviso de que el interés se capitaliza desapareció').toBeGreaterThan(-1)
