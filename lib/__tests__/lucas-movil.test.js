...
describe('el chat se ancla a la ventana en móvil', () => {
  it('`fixed` en móvil y vuelve a fluir en escritorio', () => {
    // Desde el 20 sep 2026 la caja cubre la pantalla ENTERA en el teléfono (su
    // cabecera va dentro) en vez de colgar de abajo con la del armazón encima:
    // en iPhone esa cabecera se perdía al salir el teclado y Lucas no se podía
    // cerrar.
    expect(src).toMatch(/className="fixed inset-x-0 z-\[70\] lg:static/)
  })

  it('mide lo que de verdad SE VE (visualViewport), con `100dvh` de respaldo', () => {
    expect(src).toMatch(/height: visible\.alto \? `\$\{visible\.alto\}px` : '100dvh'/)
    expect(src).toMatch(/transform: visible\.arriba \? `translateY\(\$\{visible\.arriba\}px\)` : undefined/)
    const hook = require('fs').readFileSync('lib/alto-visible.js', 'utf8')
    expect(hook).toContain("vv.addEventListener('resize', medir)")
    expect(hook).toContain("vv.addEventListener('scroll', medir)")
  })

  it('la salida va DENTRO del chat y siempre se pinta', () => {
    expect(src).toMatch(/<AsistenteChat key=\{reinicio\} comoPagina onClose=\{volver\} \/>/)
    const chat = require('fs').readFileSync('components/asistente/AsistenteChat.jsx', 'utf8')
    // Sin condición: antes era `{onClose && (<CabeceraLucas …`.
    expect(chat).not.toMatch(/\{onClose && \(\s*<CabeceraLucas/)
    expect(chat).toMatch(/<CabeceraLucas\s+onEditar=\{nuevaConversacion\}\s+onCerrar=\{onClose\}\s+comoPagina=\{comoPagina\}/)
  })
