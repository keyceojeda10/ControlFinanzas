...
describe('el estado y su etiqueta', () => {
  ...
  it('acá se dice «de atraso», no «mora»', () => {
    // La lámina de esta pantalla escribe «de atraso» en los tres casos, y es
    // coherente: al cobrador en la puerta le importa cuántos días lleva sin
    // pagar, no la clasificación contable. El color ya dice la gravedad.
    expect(etiquetaAtraso(36)).toBe('36d de atraso')
    expect(etiquetaAtraso(1)).toBe('1d de atraso')
    expect(etiquetaAtraso(0)).toBe('Al día')
    expect(etiquetaAtraso(null)).toBe('Al día')
  })
})
