...
describe('el efectivo recibido llega limpio al endpoint', () => {
  it('la entrada se reduce a dígitos', () => {
    ...
    //
    // ⚠ Antes esto fijaba el `replace(/\D/g, '')` LITERAL, o sea la
    // implementación y no lo que garantiza. Ahora la limpieza la hace
    // `montoCrudo`, que es exactamente ese mismo `replace` (ver
    // `lib/adaptadores/pago.js`) y además aplica el modo abreviado. La prueba
    // comprueba que la entrada pasa por él.
    expect(cuadre).toMatch(/montoCrudo\(v\)/)
  })
