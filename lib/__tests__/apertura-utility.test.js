...
describe('la apertura', () => {
  ...
  it('la v2 dice lo mismo de usted y también se reconoce', () => {
    const v2 = textoAperturaUtility('Carlos', 'solicitud_recibida_v2')
    expect(v2).toBe('Hola Carlos, recibimos su solicitud en Control Finanzas. ¿Quiere que le contemos por aquí cómo funciona? Responda a este mensaje y le atendemos.')
    expect(v2).not.toMatch(/\b(?:tu|quieres|responde|te)\b/i)
    expect(esAperturaUtility(v2)).toBe(true)
    expect(esPrimerTurnoTrasApertura([{ rol: 'bot', texto: v2 }])).toBe(true)
  })
  it('un nombre de plantilla desconocido no deja el historial vacío', () => {
    expect(textoAperturaUtility('Ana', 'no_existe')).toBe(textoAperturaUtility('Ana'))
    expect(Object.keys(APERTURAS_UTILITY)).toEqual(['solicitud_recibida', 'solicitud_recibida_v2'])
  })
...
describe('el cableado', () => {
  it('el primer contacto sale de la variable, con contacto_v2 de reserva, y espera 4 h', () => {
    ...
    expect(B).toMatch(/Object\.keys\(APERTURAS_UTILITY\)\.map\(\(k\) => \[k, textoAperturaUtility\(nombre, k\)\]\)/)
    expect(B).toMatch(/esAperturaDeUtilidad\(templateName\) \? 4 \* 3600000 : 24 \* 3600000/)
  })
  ...
  it('el agente contesta con el pitch fijo solo al «sí» genérico, nunca a precio u objeción', () => {
    ...
    // Desde el 9 sep va en A/B: el largo aprobado contra uno de tres líneas.
    expect(A).toMatch(/mensaje: pitchTrasSolicitud\(\{ nombre: lead\.nombre, diasPrueba: EMPRESA\.diasPrueba, variante \}\)/)
    expect(A).toMatch(/const variante = varianteDePitch\(lead\.id\)/)
  })
  it('el rescate del 130472 cuenta la apertura del bridge para no mandarla dos veces', () => {
    ...
    expect(W).toMatch(/texto: `\$\{MARCA_RESCATE\} \$\{textoAperturaUtility\(nombre, plantillaRescate\)\}`/)
  })
})
