...
describe('lo que se abre dentro de la página, se va a enseñar', () => {
  ...
  it('quien dispara una acción sabe llevar, y es UNO solo', () => {
    /* ⚠ ESTO MIRABA DENTRO DE `QueNecesitas.jsx`, donde estaba escrito el
       `scrollIntoView`. Ahora lo llaman DOS —la caja de la sección y el botón
       final de la guía—, así que se mudó a `ejecutarAccion`, en el proveedor.
       Copiarlo en los dos sitios era garantizar que uno de ellos se quedara
       ejecutando sin llevar, que es el fallo original vuelto a nacer. */
    const proveedor = leer('components/acciones/AccionesProvider.jsx')
    expect(proveedor, 'el disparador dejó de saber a dónde lleva').toContain('llevarA')
    expect(proveedor).toMatch(/scrollIntoView/)

    for (const p of ['components/acciones/QueNecesitas.jsx', 'components/tutoriales/ModalGuia.jsx']) {
      const src = leer(p)
      expect(src, `${p} dispara la acción por su cuenta`).toMatch(/ejecutarAccion/)
      expect(src, `${p} se copió el scroll en vez de usar el compartido`).not.toMatch(/scrollIntoView/)
    }
  })
})
