...
describe('la pantalla vacía solo ofrece lo que Lucas tiene herramienta para hacer', () => {
  const chat = leer('components/asistente/AsistenteChat.jsx')
  const tools = leer('lib/asistente-tools.js')
  const i = chat.indexOf('const ACCIONES_BASE = [')
  const lista = chat.slice(i, chat.indexOf(']', i))

  it('no promete recordatorios ni reportes: no hay herramienta para eso', () => {
    expect(tools).not.toMatch(/name: '(send_reminder|send_whatsapp|build_report|create_report)'/)
    expect(lista).not.toMatch(/recordatorio|reporte del mes/i)
    expect(leer('components/pantallas/Lucas.jsx')).not.toMatch(/armar un reporte|mandar un recordatorio/)
  })

  it('y cada fila que ofrece tiene su herramienta', () => {
    expect(lista).toContain("texto: 'Regístrame un pago'")
    expect(tools).toContain("name: 'register_payment'")
    expect(lista).toContain("texto: 'Créame un cliente nuevo y su préstamo'")
    expect(tools).toContain("name: 'create_client'")
    expect(tools).toContain("name: 'create_loan'")
    expect(lista).toContain("texto: 'Anótame un gasto'")
    expect(tools).toContain("name: 'register_expense'")
  })

  it('y el prompt le dice lo que NO puede, con la pantalla donde sí se hace', () => {
    const lucas = leer('lib/asistente.js')
    expect(lucas).toContain('LO QUE NO PUEDES HACER')
    expect(lucas).toContain('NUNCA digas que lo hiciste')
  })
})
