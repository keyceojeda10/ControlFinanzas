...
describe('T07-03 · crear cliente a mano', () => {
  ...
  it('la cédula dice «opcional» EN EL CAMPO', () => {
    /* La ayuda de abajo la lee quien ya dudó; el placeholder lo lee todo el
       mundo.

       El rótulo ya NO es el literal «Cédula»: viene por prop porque el
       documento se llama distinto en cada país —DNI en Argentina, RUT en
       Chile— y `lib/countries.js` lo tenía definido para los dieciocho sin que
       nadie lo usara. Lo que esta prueba defiende no cambia. */
    expect(cliente).toMatch(/rotulo=\{rotuloDocumento\}[\s\S]{0,220}placeholder="opcional"/)
    expect(cliente, 'el rótulo volvió a estar escrito a fuego')
      .toMatch(/rotuloDocumento = 'Cédula'/)
  })
