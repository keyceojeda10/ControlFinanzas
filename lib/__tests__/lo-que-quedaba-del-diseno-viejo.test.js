...
describe('ni un enlace de YouTube en la app, salvo el del bot', () => {
   ...
   *   · `kb.js`          la constante, que es la única fuente
   *   · `sanitizador.js` la guarda que impide que se borre al limpiar el
   *                      mensaje. Necesita el literal para reconocerlo. */
  // Desde el 8 sep 2026 la constante vive en kb.js (BOT v3 · sprint 5); producto.js la re-exporta.
  /* `videos.js` es el catálogo de los 16 tutoriales, con la URL de cada uno
     verificada contra la playlist el 8 sep 2026. Es justo lo contrario de un
     enlace suelto: existe para que el modelo NUNCA escriba una URL de vídeo. */
  const PERMITIDOS = ['lib/bot-v2/kb.js', 'lib/bot-v2/sanitizador.js', 'lib/bot-v2/videos.js']
