...
describe('la fila que cobraría menos de lo prestado trae su arreglo', () => {
  /* Con el saldo del archivo IGUAL a la «cuota», eso era lo que faltaba por pagar
     y se corrige solo (ver importar-cuota-es-saldo.test.js). El ofrecimiento de
     más cuotas queda para el archivo que no trae saldo con qué distinguirlo. */
  it('diaria sin saldo: 30 cuotas de $12.000 no cubren $400.000; con el 20 % se ofrecen 40', () => {
    const r = validarFila({ ...DIARIA, saldoActual: '' }, 0, nadie)
    expect(r.estado).toBe('error')
    expect(r.correccion).toMatchObject({ numeroCuotas: 40, valorCuota: 12000, total: 480000, tasa: 20 })
  })

  it('semanal sin saldo: 4 cuotas de $60.000 no cubren $300.000; con el 20 % se ofrecen 6', () => {
    const r = validarFila({ ...SEMANAL, saldoActual: '' }, 1, nadie)
    expect(r.estado).toBe('error')
...
describe('el mismo archivo subido dos veces no crea nada', () => {
  ...
  it('el servidor tiene la misma guarda, contra una foto tomada ANTES de crear nada', () => {
    const ruta = src('lib/importar-cartera.js')
    expect(ruta).toMatch(/const yaLoTiene = \(clienteId, p\) => !!clienteId && !!huellasPrevias\.get\(clienteId\)\?\.has\(huellaPrestamo\(p\)\)/)
    // `prestamosRepetidosCliente` (I4, 24 sep): se cuenta en un local de la
    // iteración y solo se suma al total cuando la transacción del cliente
    // confirma — un commit fallido no puede contar trabajo que no quedó.
    expect(ruta).toMatch(/if \(yaLoTiene\(cedulaToId\.get\(cedula\), p\)\) \{ prestamosRepetidosCliente\+\+; continue \}/)
