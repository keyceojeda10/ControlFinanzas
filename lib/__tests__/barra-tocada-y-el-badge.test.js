...
describe('la barra tocada habla aunque hoy no toque cobrar', () => {
  ...
  it('y el dato viaja: el API lo manda y el adaptador lo pasa', () => {
    ...
    expect(bloque).toContain('cobros7d,')
    /* Y la FECHA de cada barra, para que el nombre del día lo sepa el servidor:
       sin ella el navegador lo deriva de su reloj en un efecto y el primer
       pintado sale con los días en blanco. Ése era el «parpadeo». */
    expect(bloque).toContain('dias7d,')
    const adaptador = leer('lib/adaptadores/panel.js')
    expect(adaptador).toContain('cobrosSemana:')
    expect(adaptador).toContain('fechasSemana:')
  })
})
