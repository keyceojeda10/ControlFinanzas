...
describe('El día que el negocio no cobra', () => {
  it('el inicio dice por qué la barra está en cero', () => {
    const v = adaptarPanel({ prestamos: { esperadoHoy: 0 }, cobros: { hoy: 2000 } }, { clientesHoy: 0 })
    expect(v.hero.sinCobrosHoy).toBe(true)
    expect(v.hero.meta).toBeNull()
  })

  it('y un día normal sin cobrar todavía NO lo dice', () => {
    const v = adaptarPanel({ prestamos: { esperadoHoy: 350000 }, cobros: { hoy: 0 } }, { clientesHoy: 3 })
    expect(v.hero.sinCobrosHoy).toBe(false)
  })

  it('la frase está en la pantalla, no solo en el adaptador', () => {
    const src = lee('components/pantallas/Panel.jsx')
    expect(src).toContain('>hoy no toca cobrarle a nadie</span>')
    expect(src).toContain('opacity: sinCobrosHoy ? 0.45 : 1')
  })
})
