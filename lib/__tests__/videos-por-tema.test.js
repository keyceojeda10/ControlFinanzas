...
import fs from 'fs'
import { VIDEOS, VIDEO_RETIRADO, elegirVideo, textoDeVideo, textoDeTodos, pideVer } from '@/lib/bot-v2/videos'
import { EMPRESA } from '@/lib/bot-v2/kb'

describe('el catálogo', () => {
  it('son 16 vídeos vivos, cada uno con id, url, duración y claves', () => {
    expect(VIDEOS).toHaveLength(16)
    for (const v of VIDEOS) {
      expect(v.id, v.titulo).toMatch(/^[\w-]{11}$/)
      expect(v.url).toBe(`https://youtu.be/${v.id}`)
    ...
    expect(new Set(VIDEOS.map((v) => v.id)).size).toBe(16)
  })
  it('el retirado por YouTube queda anotado y fuera del catálogo', () => {
    expect(VIDEO_RETIRADO.id).toBe('5AB_n3BU8OU')
    expect(VIDEOS.find((v) => v.id === VIDEO_RETIRADO.id)).toBeUndefined()
  })
})
