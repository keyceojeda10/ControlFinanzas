      ...
      async authorize(credentials) {
        ...
        if (esTelefono) {
          ...
          user = await prisma.user.findFirst({
            ...
            include: {
              organization: { select: { plan: true, activo: true, country: true, timezone: true, nombre: true } },
            },
        ...
        if (!user) {
          console.error(`[auth] user NOT FOUND: ${esTelefono ? 'phone' : 'email'} = ${rawInput}`)
          return null
        }
        ...
        if (!passwordOk) {
          console.error(`[auth] password FAIL for ${user.email} (input len=${credentials.password.length})`)
          return null
        }
