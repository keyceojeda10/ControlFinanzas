      ...
      async authorize(credentials) {
        ...
        if (esTelefono) {
          ...
          user = await prisma.user.findFirst({
            ...
            include: {
              organization: { select: { plan: true, activo: true, country: true, timezone: true, nombre: true, modoAbreviado: true, ocultarSaldoWA: true } },
            },
        ...
        return {
          ...
          ocultarCapitalCobradores: user.organization?.ocultarCapitalCobradores ?? false,
        }
      },
    }),
  ],
  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger }) {
      ...
      if (user) {
        ...
        token.ocultarCapitalCobradores = user.ocultarCapitalCobradores ?? false
        ...
        try {
          if (token.id && token.rol !== 'superadmin') {
            ...
            if (fresh && fresh.activo && (!fresh.organization || fresh.organization.activo)) {
              ...
              token.ocultarCapitalCobradores = fresh.organization?.ocultarCapitalCobradores ?? false
    ...
    async session({ session, token }) {
      if (token) {
        ...
        session.user.ocultarCapitalCobradores = token.ocultarCapitalCobradores ?? false
      }
      return session
    },
  },
