...
import { leerPase, VISTA_MS } from '@/lib/pase-de-vista'
import { copiarAlToken, volverSiVencio } from '@/lib/token-de-sesion'
...
export const authOptions = {
  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger }) {
      ...
      if (user) return copiarAlToken(token, user)

      // «Ver como»: a la hora, la vista vuelve sola a la cuenta del dueño (ver
      // lib/token-de-sesion.js). No toca nada si no hay vista o no ha vencido.
      token = await volverSiVencio(token)
