...
export async function recalcularSaldosCapital(tx, { organizationId }) {
  ...
  for (const mov of movimientos) {
    ...
    // Saldo por ruta: TODOS los movimientos con rutaId cuentan (incluido el
    // ajuste de arranque, que solo vive en la ruta).
    if (mov.rutaId) {
      const prev = saldoPorRuta.get(mov.rutaId) || 0
      saldoPorRuta.set(mov.rutaId, esIngreso ? prev + mov.monto : prev - mov.monto)
    }
  }
