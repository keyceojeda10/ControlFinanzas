...
/* ══ EL NOMBRE DEL ARCHIVO NO PUEDE ROMPER LA DESCARGA ═══════════════════════
 *
 * Una cabecera HTTP solo admite Latin-1. El pagaré lleva el nombre del cliente
 * en el archivo, y un nombre con emoji («🌟 María») o comillas «“ ”» reventaba
 * con «Cannot convert argument to a ByteString»: medido el 25 sep 2026, 209
 * clientes de 5 negocios no podían sacar su pagaré, y el 23 sep alguien lo
 * intentó 12 veces en 45 minutos.
 *
 * Va como manda el RFC 6266: `filename` con un respaldo solo ASCII (sin tildes,
 * sin comillas) y `filename*` con el nombre real en UTF-8, que es el que usan
 * los navegadores. Un emoji partido al recortar (media pareja suelta) se
 * quita: `encodeURIComponent` también revienta con eso. */
const sinMediasParejas = (s) => String(s ?? '').replace(/[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/g, '')

export function disposicionAdjunto(nombre) {
  const real = sinMediasParejas(nombre) || 'documento.pdf'
  const respaldo = real
  ...
  const utf8 = encodeURIComponent(real).replace(/['()*]/g, (c) => '%' + c.charCodeAt(0).toString(16).toUpperCase())
  return `attachment; filename="${respaldo}"; filename*=UTF-8''${utf8}`
}
...
export function respuestaPdf(buffer, nombre) {
  return new Response(buffer, {
    headers: {
      ...
      'Content-Disposition': disposicionAdjunto(nombre),
