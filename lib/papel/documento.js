 ...
 * cuántas hay.
 */
export function abrirDocumento({ pie = 'Control Finanzas' } = {}) {
  ...
  const escribir = (txt, x, y, opciones = {}) =>
    doc.text(String(txt ?? ''), x, y, { lineBreak: false, ...opciones })
  ...
  function cabecera({ negocio, titulo, subtitulo = '', meta = '' }) {
    doc.font(F.fuerte).fontSize(TIPO.titulo).fillColor(COLOR.ink)
    escribir(negocio || 'Mi negocio', L, HOJA.margen, { width: W * 0.66, ellipsis: true })
  ...
  function barras(datos, y, { alto = 96, titulo = null, formato = String } = {}) {
    ...
    const hueco = datos.length > 20 ? 1 : 3
    const ancho = (W - hueco * (datos.length - 1)) / datos.length
    const base = y + alto
    ...
    datos.forEach((d, i) => {
      ...
      if (esMax) {
        doc.font(F.cifra).fontSize(TIPO.pie).fillColor(COLOR.ink)
        escribir(formato(d.valor), x - 20, base - h - 10, { width: ancho + 40, align: 'center' })
      }
      if (i % cada === 0) {
        doc.font(F.texto).fontSize(TIPO.pie).fillColor(COLOR.ink4)
  ...
  function seccion(texto, y) {
    ...
    escribir(String(texto).toUpperCase(), L, y, { characterSpacing: 0.5 })
    doc.moveTo(L, y + 13).lineTo(R, y + 13).lineWidth(0.5).strokeColor(COLOR.border).stroke()
    return y + 22
  }
  ...
  function tabla({ columnas, filas = [], grupos = null }, y) {
    ...
    const altoDeFila = (fila) => {
      ...
      return renglones <= 1 ? ALTO_FILA : 8 + renglones * RENGLON
    }
      ...
      lista.forEach((fila, n) => {
        ...
        columnas.forEach((c, i) => {
          ...
          if (past) {
            ...
            doc.text(String(fila[c.clave] ?? ''), xs[i] + 5, yy + 5, {
  ...
  function nota(texto, y, { tono = 'neutro' } = {}) {
    ...
    doc.text(texto, L + 12, y + 9, { width: W - 24 })
    return y + alto + 14
  }
  ...
  async function cerrar() {
    ...
    for (let i = rango.start; i < rango.start + rango.count; i++) {
      ...
      doc.font(F.texto).fontSize(TIPO.pie).fillColor(COLOR.ink4)
      escribir(pie, L, HOJA.pieY, { width: W * 0.6 })
