...
const NAV_OWNER = [
  ...
  // Faltaban en el buscador global aunque si estan en el menu. Un destino que
  // el buscador no conoce es una funcion que el usuario pago y no encuentra.
  { id: 'nav-cobros-hoy', label: 'Cobrar hoy', sub: 'A quién le toca hoy', href: '/cobros-hoy', icon: I.caja, keywords: ['cobrar', 'cobro', 'cobros', 'hoy', 'pendientes', 'a quien', 'toca'] },
  { id: 'nav-migrador', label: 'Pasar mi cuaderno', sub: 'Subir tu cartera con fotos', href: '/migrador', icon: I.clientes, keywords: ['cuaderno', 'libreta', 'cartulina', 'pasar', 'subir', 'migrar', 'importar', 'cartera', 'foto'] },
  { id: 'nav-carga-masiva', label: 'Importar Excel', sub: 'Subir cartera desde un archivo', href: '/carga-masiva', icon: I.clientes, keywords: ['excel', 'importar', 'archivo', 'csv', 'masiva', 'cargar', 'subir'] },
  { id: 'nav-lineas', label: 'Líneas de crédito', sub: 'Cupos rotativos', href: '/lineas-credito', icon: I.prestamo, keywords: ['linea', 'lineas', 'credito', 'cupo', 'rotativo'] },
  { id: 'nav-socios', label: 'Socios', sub: 'Aportes y retiros', href: '/socios', icon: I.capital, keywords: ['socio', 'socios', 'aporte', 'aportes', 'retiro', 'inversionista'] },
  { id: 'nav-analiticas', label: 'Analíticas', sub: 'Cómo va el negocio', href: '/dashboard/analiticas', icon: I.reportes, keywords: ['analitica', 'analiticas', 'tendencia', 'crecimiento', 'comparar', 'meses', 'como va'] },
  { id: 'nav-simulador', label: 'Simulador de préstamo', sub: 'Calcular sin guardar', href: '/prestamos/simulador', icon: I.prestamo, keywords: ['simular', 'simulador', 'calcular', 'calculadora', 'cuanto', 'cuota', 'prueba'] },
  { id: 'nav-ranking', label: 'Ranking de cobradores', sub: 'Quién cobra mejor', href: '/cobradores/ranking', icon: I.cobradores, keywords: ['ranking', 'mejor', 'rendimiento', 'comparar', 'cobradores', 'top'] },
  ...
  { id: 'nav-capital', label: 'Capital', sub: 'Inversión y disponible', href: '/capital', icon: I.capital, keywords: ['capital', 'inversion', 'fondo', 'disponible', 'invertido', 'mi plata', 'plata'] }, // «mi plata»: como se llamó hasta el 2 sep 2026
  ...
  { id: 'nav-actividad', label: 'Historial', sub: 'Actividad reciente', href: '/actividad', icon: I.historial, keywords: ['historial', 'actividad', 'movimientos', 'registro', 'log', 'auditoria', 'quien hizo que'] }, // «quién hizo qué»: como se llamó hasta el 2 sep 2026
...
/* ══ LAS SECCIONES DE CONFIGURACION, DERIVADAS ═════════════════════
   Aqui habia seis entradas escritas a mano que apuntaban a `?tab=perfil`,
   `?tab=organizacion`, `?tab=suscripcion`... y la pantalla de configuracion usa
   `?s=`. O sea que llevaban tiempo cayendo en el indice: el buscador encontraba
   «Suscripcion», la abria, y te dejaba en la portada a buscarla otra vez.

   El dueno lo noto por el otro lado —«pongo seguridad y no sale nada»— porque
   ninguna de las secciones REALES estaba en el catalogo.

   Ahora se derivan de `seccionesConfig`, que es la misma lista que pinta la
   pantalla: la seccion que se anada manana entra sola, y una que se renombre no
   puede quedarse con el nombre viejo aqui. Lo unico escrito a mano son las
   palabras con las que la gente las busca, que si son cosa aparte del nombre.

   ⚠ `equipo` no entra: su seccion solo existe si hay cobradores, y ofrecer una
   puerta que a veces no abre es peor que no ofrecerla. Para eso esta
   «Cobradores» en la navegacion. */
const PALABRAS_DE_SECCION = {
  negocio:     ['negocio', 'organizacion', 'empresa', 'nombre del negocio', 'logo', 'moneda', 'pais', 'ajustes'],
  comoPrestas: ['como prestas', 'interes', 'tasa', 'modo', 'frecuencia', 'plazo', 'cuota', 'por defecto', 'preferencias'],
  plan:        ['plan', 'planes', 'pago', 'pagos', 'suscripcion', 'factura', 'facturacion', 'mensualidad', 'precio', 'renovar', 'upgrade'],
  portal:      ['portal', 'portal del cliente', 'consulta', 'enlace', 'link'],
  whatsapp:    ['whatsapp', 'aviso', 'avisos', 'mensaje', 'mensajes', 'recordatorio', 'plantilla', 'plantillas', 'wasap'],
  seguridad:   ['seguridad', 'restablecer', 'reiniciar', 'resetear', 'reset', 'borrar cuenta', 'eliminar cuenta',
                'empezar de cero', 'zona de peligro', 'peligro', 'limpiar', 'borrar todo'],
  datos:       ['datos', 'tus datos', 'mi perfil', 'perfil', 'mi cuenta', 'cuenta', 'contrasena', 'clave', 'password',
                'telefono', 'correo', 'email', 'nombre', 'foto'],
}

const ICONO_DE_SECCION = {
  negocio: I.config, comoPrestas: I.prestamo, plan: I.tarjeta,
  portal: I.clientes, whatsapp: I.bell, seguridad: I.config, datos: I.user,
}

function comandosDeConfiguracion(rol) {
  return seccionesConfig({ rol, cobradores: 0 }).map((sec) => ({
    id: `cfg-${sec.id}`,
    label: sec.nombre,
    sub: 'Configuracion',
    href: `/configuracion?s=${sec.id}`,
    icon: ICONO_DE_SECCION[sec.id] ?? I.config,
    keywords: PALABRAS_DE_SECCION[sec.id] ?? [],
  }))
}

/* El tema no es una seccion: vive en el indice de configuracion, junto a
   «Instalar la app». Se busca muchisimo y por eso tiene su propia entrada. */
const CONFIG_TEMA = {
  id: 'cfg-tema', label: 'Tema claro u oscuro', sub: 'Configuracion',
  href: '/configuracion', icon: I.tema,
  keywords: ['tema', 'apariencia', 'oscuro', 'claro', 'dark', 'light', 'modo', 'color', 'pantalla', 'noche'],
}

const CONFIG_PLAN = {
  id: 'cfg-plan', label: 'Cambiar de plan', sub: 'Ver planes disponibles',
  href: '/configuracion/plan', icon: I.tarjeta,
  keywords: ['plan', 'planes', 'upgrade', 'mejorar', 'subir plan', 'precio', 'suscripcion', 'pagar'],
}

/* ⚠ INSTALAR NO ES UN DESTINO: ES UNA ACCIÓN, y por eso lleva `evento` en vez
   de `href`. La guía ya existe y detecta el navegador, pero no tenía ninguna
   puerta enlazable — reportado tal cual: «si la gente pone "instalar
   aplicación" no le sale nada, toca enviarles capturas». */
export const ACCION_INSTALAR = {
  id: 'instalar-app',
  label: 'Instalar la app en tu celular',
  sub: 'Para abrirla más rápido y cobrar sin internet',
  evento: 'cf:abrir-instalacion',
  icon: 'descargar',
  keywords: ['instalar', 'instalar app', 'instalar aplicacion', 'descargar', 'descargar app',
    'bajar la app', 'poner en el celular', 'icono en la pantalla', 'pwa',
    'sin internet', 'offline', 'sin señal', 'sin datos'],
}
...
export function obtenerComandos({ esCobrador = false, permisos = {} } = {}) {
  ...
  if (esCobrador) {
    return [
      ...
      ACCION_INSTALAR,
      ...comandosDeConfiguracion('cobrador'),
      CONFIG_TEMA,
    ]
  }
  return [
