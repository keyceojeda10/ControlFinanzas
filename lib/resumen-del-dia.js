...
export function armarResumen(d, { nombre = '' } = {}) {
  ...
  return {
    ...
    promedioSemana: promedioDeLaSemana(semana),
...
/* EL PROMEDIO DE LA SEMANA, DE UNA SOLA FÓRMULA.
 *
 * El total entre los días que se enseñan, con los ceros dentro: es lo que hay
 * detrás del «69 % más que tu promedio ($722.795)» que ve el dueño, y cuadra
 * con el «en 7 días llevas $5.059.568» de la misma tarjeta.
 *
 * Vive aquí, y no en cada gráfica, porque el Inicio enseña la MISMA semana: con
 * dos fórmulas —por ejemplo, la media de los días con cobro— el mismo martes
 * sale «por encima» en una pantalla y «por debajo» en la otra. */
export function promedioDeLaSemana(montos = []) {
  if (!montos.length) return 0
  return Math.round(montos.reduce((a, b) => a + (Number(b) || 0), 0) / montos.length)
}
