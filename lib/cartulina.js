...
/* ══ EL RESPALDO DE OTRO PROVEEDOR ══════════════════════════════════════════
 *
 * Cuando Google no está —y el 31 ago 2026 no estuvo buena parte del día— no
 * sirve de nada tener otro modelo SUYO: hace falta otra casa.
 *
 * Medido ese mismo día contra la tabla real de un cliente, tres vueltas cada
 * uno y en el mismo momento:
 *
 *     gemini-2.5-flash               8,9 s de media · UNA de las tres falló
 *     deepseek-vision (sin razonar)  22,2 s        · ninguna falló
 *                                     (22,7 / 22,0 / 21,9 s · 20 de 21 montos)
 *
 * Es dos veces y media más lento y por eso NO es el primero. Pero es estable
 * como un reloj y cabe de sobra en los 180 s que aguanta la pantalla: veintidós
 * segundos son infinitamente mejores que «no pudimos leer la foto» cuando el
 * prestamista está pasando su cartera, que es el momento que decide si se queda.
 *
 * ⚠ SIN RAZONAMIENTO, Y ESA LÍNEA ES TODA LA DIFERENCIA. Por defecto este
 * modelo razona: la primera prueba se gastó los 12.288 tokens pensando, devolvió
 * el contenido VACÍO y tardó 233 segundos. Con `thinking: disabled` baja a 22 s
 * y acierta igual. Es exactamente el mismo fallo que tenía Gemini y que se
 * arregló con `thinkingBudget: 0`: estos papeles se transcriben, no se razonan.
 */
const DEEPSEEK_URL = 'https://api.deepseek.com/chat/completions'
const DEEPSEEK_VISION = 'deepseek-v4-flash-vision-exp'

async function llamarDeepSeekVision(base64, mimeType, prompt, maxTokens) {
  ...
  /* Igual que con Gemini: una respuesta cortada no se devuelve como buena. Una
     hoja a medias parece una hoja entera. */
  if (j.choices?.[0]?.finish_reason === 'length') return null
...
export async function llamarGemini(base64, mimeType, prompt, { maxTokens = 1024 } = {}) {
  ...
  for (const MODELO of MODELOS) {
   for (const key of GEMINI_KEYS) {
    ...
    /* ⚠ NI SIQUIERA UN ERROR RARO PUEDE SALTARSE EL RESPALDO.
     *
     * Esto era un `throw` y cortaba la cadena en seco: probando con una clave
     * rota a propósito, Gemini devolvió 400 y el lector se rindió a los 313 ms
     * SIN llegar a la otra casa, que estaba perfectamente. Solo se vio probando
     * la cadena entera; con las piezas por separado, todo parecía bien.
     *
     * El motivo no se pierde: viaja en `ultimoFallo` y sale al final si tampoco
     * hay respaldo. */
    if (!res.ok) {
      ultimoFallo = `Gemini ${res.status} en ${MODELO}: ${(await res.text()).slice(0, 120)}`
      continue
    }
  ...
  /* Google entero está fuera. Se prueba la otra casa antes de rendirse: es más
     lenta, pero la alternativa es que el prestamista se quede con la foto en la
     mano. */
  try {
    const otra = await llamarDeepSeekVision(base64, mimeType, prompt, maxTokens)
    if (otra) return otra
  } catch (e) {
    console.warn('[Cartulina] El respaldo de DeepSeek tampoco:', e.message)
  }
...
export {
  ...
  escalaDeLaHoja, TECHO_PRESTAMO, PISO_PESOS_COMPLETOS,
