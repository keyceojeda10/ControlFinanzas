...
export async function verificarLimitesPlan(organizationId) {
  ...
  return {
    plan: org.plan,
    clientes: { usado: clientes, limite: config.maxClientes, excede: clientes > config.maxClientes },
    ...
    excedeAlgo: clientes > config.maxClientes || usuarios > limiteUsuarios || rutas > limiteRutas,
  }
}
