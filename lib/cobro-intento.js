...
import { PLANES_CONFIG, admiteAdicionales } from '@/lib/planes'
...
/** Los adicionales que lleva un cobro de `plan`: ninguno si el plan no los admite. */
export function adicionalesDe(org, plan) {
  return admiteAdicionales(plan)
    ? { cobradores: org?.cobradoresAdicionales ?? 0, rutas: org?.rutasAdicionales ?? 0 }
    : { cobradores: 0, rutas: 0 }
}
...
const selectOrgCobro = {
  id: true, nombre: true, plan: true, planOriginal: true, ...selectPrecio,
...
/* El último plan PAGADO y lo que se pagó: de ahí sale qué plan se cobra y si
   ese pago está fuera de lista (lib/precio-plan.js#precioPorRevisar). */
export function ultimoPago(organizationId) {
  return prisma.suscripcion.findFirst({
    where: { organizationId, estado: { in: ['activa', 'vencida'] }, montoCOP: { gt: 0 } },
    orderBy: { fechaVencimiento: 'desc' },
    /* Con `montoAdicionales`: sin él, «por revisar» toma los adicionales pagados
       por plan y el cobro se queda corto (lib/precio-plan.js#precioPorRevisar). */
    select: selectPago,
  })
}

/* Un cobro que sale por debajo de la lista sin preferencial puesto: el panel ya
   lo avisa, y el log deja constancia de a quién y cuánto. */
function avisarPorRevisar(org, precio, origen) {
  if (!precio.porRevisar) return
  console.warn(`[cobro:${origen}] PRECIO POR REVISAR "${org.nombre}" (${org.id}): se cobra $${precio.monto} y la lista es $${precio.lista}. Fijar el precio preferencial o cobrar lista desde el panel.`)
}
...
export async function lanzarCobro({ org, plan, montoCOP, reiniciarFallos = false, origen = 'cobro' }) {
  ...
  /* La referencia dice con cuántos adicionales va este cobro: `montoDelCobro`
     los sumó de las mismas columnas (`org` trae `selectPrecio`). */
  const referencia = referenciaDeCobro(org.id, plan, 'mensual', adicionalesDe(org, plan), { automatico: true })
...
export async function reconciliar(org) {
  ...
  /* El cobro en vuelo es siempre del plan (lo escribe `lanzarCobro`). Una
     referencia de adicionales aquí sería un error nuestro: pasada a
     `activarPlanPagado` alargaría el plan. */
  if (!parsed || parsed.orgId !== org.id || parsed.periodo === 'adicionales') {
...
export async function cobrarAhoraSiToca({ orgId, planElegido = null, minutosEntreIntentos = 2, origen = 'manual' }) {
  ...
  /* ⚠ EL MONTO SALE DEL PRECIO, NO DEL ÚLTIMO PAGO (lib/precio-plan.js). Antes
     se repetía lo último pagado: un trimestre se cobraba entero cada mes y un
     precio de amigo no terminaba nunca. El plan sí: el elegido, o el último
     pagado, o el suyo. */
  const pagada = await ultimoPago(orgId)
  const suyo = org.planOriginal || org.plan
  const plan = planElegido && PLANES_CONFIG[planElegido] ? planElegido
    : pagada && PLANES_CONFIG[pagada.plan] ? pagada.plan
    : PLANES_CONFIG[suyo] ? suyo
    : null
  if (!plan) return { resultado: 'sin-plan' }
  const precio = montoDelCobro({ org, plan, pagada, ultima, ahora })
  if (!(precio.monto > 0)) return { resultado: 'sin-plan' }
  avisarPorRevisar(org, precio, origen)

  return lanzarCobro({ org, plan, montoCOP: precio.monto, reiniciarFallos: !vigente, origen })
}
