/* ══ CUÁNTO INTERÉS SE GANÓ EN UN TRAMO DE FECHAS ════════════════════════════
 *
 * Reportado por Miguel Ángel (Préstamos Rincón) el 12 ago 2026, con las dos
 * cifras copiadas de su pantalla:
 *
 *   «1) Dashboard: intereses cobrado del mes $135.417 […] 2) Cómo va mi
 *    negocio: ganancias del mes $215.019. En estas dos secciones solo la
 *    segunda es correcta.»
 *
 * Tenía razón en cuál, aunque no en el porqué —lo atribuyó a los gastos, y los
 * gastos estaban en cero—. La diferencia es otra:
 *
 *   · El DASHBOARD repartía cada pago por la PROPORCIÓN del préstamo entero,
 *     `(total − prestado) / total`. Sirve para los modos sin tabla.
 *   · ANALÍTICAS hace eso y además CORRIGE los préstamos con tabla de
 *     amortización con el interés que dice la tabla.
 *
 * Y en un préstamo francés («saldo») esa diferencia es enorme: las primeras
 * cuotas son casi todo interés, no la fracción plana del préstamo completo.
 * Reproducido al peso contra su base: $135.417 y $215.019, y los $79.602 de
 * diferencia salen de sus siete préstamos en modo `saldo`.
 *
 * ⚠ POR QUÉ VIVE AQUÍ Y NO EN CADA PANTALLA. La cuenta ya estaba escrita dos
 *   veces —una en analíticas y otra, incompleta, en el dashboard— y por eso la
 *   misma ganancia del mismo mes salía distinta según dónde la mirara. La
 *   fórmula de un solo sitio es lo que impide que vuelvan a separarse; que
 *   digan lo mismo lo fija `dos-pantallas-una-ganancia.test.js`, no la buena
 *   voluntad.
 *
 * Medido: 17 negocios tienen préstamos con tabla cobrando este mes, así que a
 * 17 se les movía la cifra.
 */

import { interesDelPagoSegunTabla } from '@/lib/calculos'
import { fraccionDe, totalDelReparto, capitalDelPrestamo, fraccionArrastrada, techoDeInteres, interesProporcionalDelPago } from '@/lib/dinero/reparto'
import { CAMPOS_DEL_REPARTO } from '@/lib/dinero/capital-base'

/** Los modos que llevan tabla de amortización y por tanto se corrigen. */
export const MODOS_CON_TABLA = ['lineal', 'solo_interes', 'lineal_dinamico', 'saldo']

/**
 * Interés ganado por UN préstamo con los pagos que caen dentro de la ventana.
 *
 * ⚠ Los pagos ANTERIORES a la ventana también hacen falta: el interés de un
 *   pago depende de cuánto se llevaba pagado antes (por dónde va la tabla). Si
 *   solo se pasan los del mes, el primero se calcula como si fuera el primero
 *   del préstamo y sale de más.
 *
 * @param {object} p
 * @param {object} p.prestamo   { montoPrestado, totalAPagar, modoInteres }
 * @param {Array}  [p.cuotas]   filas de la tabla, ordenadas por periodo. Sin
 *                              ellas se usa la proporción, que es lo correcto
 *                              para los modos sin tabla.
 * @param {Array}  p.pagos      TODOS sus pagos reales, ordenados por fecha
 * @param {Date}   [p.desde]    inclusive; sin fecha, cuenta todos
 * @param {Date}   [p.hasta]    inclusive
 * @returns {number} interés, sin redondear
 */
export function interesCobradoDelPrestamo({ prestamo, cuotas = null, pagos = [], desde = null, hasta = null }) {
  let interes = 0
  for (const p of interesPagoAPago({ prestamo, cuotas, pagos })) {
    const dentro = (!desde || (p.fecha && p.fecha >= desde)) && (!hasta || (p.fecha && p.fecha <= hasta))
    if (dentro) interes += p.interes
  }
  return interes
}

/**
 * Lo mismo, pero PAGO A PAGO.
 *
 * Hace falta para repartir por días o por meses: un total no se puede volver a
 * partir, porque el interés de cada pago depende de por dónde iba la tabla
 * cuando entró. Con el total del mes no hay forma de saber cuánto puso el día 3
 * y cuánto el día 20.
 *
 * ⚠ VIVE JUNTO AL TOTAL Y ESTE LO USA. Es la misma cuenta: si se escribiera
 *   aparte, el desglose por días podría no sumar el total del mes que se enseña
 *   encima — que es exactamente el error que este archivo existe para no repetir.
 *
 * @returns {Array<{fecha: Date|null, monto: number, interes: number, capital: number}>}
 */
export function interesPagoAPago({ prestamo, cuotas = null, pagos = [] }) {
  /* ⚠ EL PRÉSTAMO ABIERTO NO SE REPARTE: SU INTERÉS ESTÁ DEVENGADO.
   *
   * En un abierto el interés no vive en `totalAPagar` —nacen con el total igual
   * al capital—, vive en los devengos que se cierran período a período. Con la
   * proporción salía CERO interés, o casi.
   *
   * Medido en producción el 27 ago 2026: 3 abiertos en 2 negocios, $2.049.916
   * de interés cobrado que no aparecía en ninguna cifra de ganancia. Uno prestó
   * $4.000.000, le pagaron $2.800.000 —todo interés— y el informe daba por
   * devuelto $1.647.059 de capital que nadie devolvió.
   *
   * Va aquí ADEMÁS de en `repartirPagado` porque son dos caminos distintos:
   * aquél alimenta la ficha y las tarjetas; éste es el que corrige lo que puso
   * el SQL en analíticas, el informe del contador y el reparto a socios. Y el
   * SQL no sabe de devengos: por eso la corrección tiene que traer la
   * diferencia entera.
   *
   * El orden es el de `calcularCapitalRestante`: un pago declarado interés es
   * interés; un abono a capital no lo es; y un pago corriente cubre PRIMERO el
   * interés cerrado que falta por pagar. */
  if (prestamo?.sinPlazo && prestamo?.modoInteres === 'solo_interes') {
    const devengado = (prestamo.devengos ?? [])
      .reduce((a, d) => a + (Number(d.interes) || 0), 0)
    const declarado = pagos
      .filter((g) => g?.tipo === 'intereses')
      .reduce((a, g) => a + (Number(g?.montoPagado) || 0), 0)
    let sinCubrir = Math.max(0, devengado - declarado)
    /* Lo que pase del capital de verdad es el interés que arrastró de una
       renovación: el mismo tope que `repartoDeAbierto` (`Math.min(monto, …)`). */
    let capitalQueda = capitalDelPrestamo(prestamo)
    /* La MISMA forma de fila que la rama de abajo: `fecha`, `monto`, `tipo`,
       `interes` y `capital` por resta. Quien lee esto reparte por días y por
       meses y compara tipos; devolver otra forma habría dado `undefined` sin
       que nada reventara, que es como se rompen las cifras en este repo. */
    return pagos.map((g) => {
      const monto = Number(g?.montoPagado) || 0
      let interes = 0
      if (g?.tipo === 'intereses') interes = monto
      else if (g?.tipo !== 'capital') {
        interes = Math.min(monto, sinCubrir)
        sinCubrir -= interes
      }
      if (g?.tipo === 'capital' || g?.tipo === 'completo') {
        const aCapital = Math.min(monto - interes, capitalQueda)
        capitalQueda -= aCapital
        interes = monto - aCapital
      }
      interes = Math.round(interes)
      return {
        fecha: g?.fechaPago ? new Date(g.fechaPago) : null,
        monto,
        tipo: g?.tipo ?? null,
        interes,
        capital: monto - interes,
      }
    })
  }

  const usaTabla = Array.isArray(cuotas) && cuotas.length > 0
    && MODOS_CON_TABLA.includes(prestamo?.modoInteres)

  /* ── ⚠ LOS DOS TIPOS DECLARADOS NO SE REPARTEN ────────────────────────────
   *
   * Un ABONO A CAPITAL es 100% capital y un pago de SOLO INTERÉS es 100%
   * interés: lo dijo el prestamista al registrarlo, no hay nada que adivinar.
   * No es «inventar el reparto» de `06-ADENDA-modos-sin-tabla.md`: esa regla
   * prohíbe adivinar cuánto de un pago NORMAL fue interés.
   *
   * `repartirPagado` ya los sacaba —es el fallo del abono a capital en Globo,
   * donde la cascada se comía el abono en intereses y el capital casi no
   * bajaba— y esta función nunca se enteró. Por eso las dos pantallas partían
   * el mismo pago de dos formas distintas.
   *
   * Lo cazó Crediya el 25 ago 2026 haciendo la resta a mano: «en los intereses
   * ganados aparecen 1.586.706, en toda tu plata ¿no debería decir
   * 51.586.706?». Le faltaban $597.461, y $97.461 eran justo esto: sus
   * $171.000 de abonos a capital pasando por la tabla y saliendo con interés
   * encima.
   *
   * Medido en el espejo: 49 abonos a capital por $22.875.665 en 16 negocios, y
   * 61 pagos de solo interés por $13.721.200 en 19.
   *
   * ⚠ Y EL ACUMULADO NO AVANZA CON ELLOS. Marca por dónde va la tabla para los
   * pagos corrientes, y `repartirPagado` los deja fuera de `aRepartir` por lo
   * mismo: si avanzaran, empujarían a los siguientes a un tramo de la tabla que
   * no les toca. */
  const soloInteres = pagos
    .filter((g) => g?.tipo === 'intereses')
    .reduce((a, g) => a + (Number(g?.montoPagado) || 0), 0)

  /* La fracción mira el préstamo COMO SE PACTÓ. Sin tabla, cobrar interés
     aparte SUBE `totalAPagar`, y con el total inflado los pagos viejos se
     vuelven a repartir con una fracción que nunca existió. Es la misma resta
     que hacen `repartirPagado` y `repartoSql`. */
  const fraccion = usaTabla
    ? 0
    /* El total pactado (el de antes de renovar, si hubo) menos lo cobrado como solo
       interés, contra el capital de verdad. NO `fraccionInteres({ ...prestamo,
       totalAPagar: … })`: con el total pactado guardado, ese atajo se saltaba el
       total que se le pasa. Ver `totalDelReparto` en reparto.js. */
    : fraccionDe(totalDelReparto(prestamo) - soloInteres, capitalDelPrestamo(prestamo))

  /* ⚠ EL TECHO DEL PRÉSTAMO, REPARTIDO EN ORDEN. Nadie puede ganar más de lo
     que pactó cobrar por encima de lo prestado, y la tabla puede describir un
     calendario que ya no rige: una LIQUIDACIÓN ANTICIPADA baja `totalAPagar`
     —se le perdona el interés futuro— y no reescribe las cuotas.

     Crediya, 26 ago 2026: prestó $900.000 a Jose Bermejo, le perdonó TODO el
     interés al liquidar, el cliente devolvió $900.000 y el informe le contaba
     $283.479 de ganancia. Ver `techoDeInteres`.

     Se descuenta pago a pago y no al final: este desglose se usa para repartir
     por días y por meses, y un tope aplicado al total no se puede volver a
     partir. */
  let queda = Math.max(0, techoDeInteres(prestamo) - soloInteres)

  let acumulado = 0
  /* Con tabla, lo que la tabla llama capital lleva dentro el interés arrastrado de
     una renovación, en su proporción: la misma cuenta que `repartirPagado`. */
  const arrastrada = usaTabla ? fraccionArrastrada(prestamo) : 0
  const deLaTabla = (monto) => {
    const i = interesDelPagoSegunTabla(cuotas, acumulado, monto)
    return i + (monto - i) * arrastrada
  }
  const filas = []
  for (const g of pagos) {
    const monto = Number(g?.montoPagado) || 0
    const declarado = g?.tipo === 'capital' ? 0 : (g?.tipo === 'intereses' ? monto : null)
    let interes = declarado != null
      ? declarado
      : (usaTabla ? deLaTabla(monto) : monto * fraccion)
    /* El pago DECLARADO como interés no pasa por el techo: ya se restó de él
       arriba, y volver a toparlo lo dejaría en cero. */
    if (declarado == null) {
      interes = Math.min(interes, queda)
      queda -= interes
    }
    filas.push({
      fecha: g?.fechaPago ? new Date(g.fechaPago) : null,
      monto,
      /* El tipo viaja con la fila para que quien compare este reparto con el
         proporcional no tenga que volver a buscar el pago. Lo usa la corrección
         de analíticas, que enfrenta las dos formas de partir el mismo peso. */
      tipo: g?.tipo ?? null,
      interes,
      // El capital sale POR RESTA, igual que en `repartoSql`: así
      // `interés + capital = lo pagado` siempre, y la plata no se pierde de vista.
      capital: monto - interes,
    })
    // El acumulado avanza esté el pago dentro de la ventana o no, pero SOLO con
    // los pagos corrientes: los declarados no mueven la tabla.
    if (declarado == null) acumulado += monto
  }
  return filas
}

/**
 * EL REPARTO DE UN SOLO PAGO, el que va en el recibo.
 *
 * El papel que el cliente se guarda dice «de esto, tanto fue interés y tanto
 * capital». Esa frase no se puede derivar en el navegador: el reparto de un
 * pago depende de por dónde iba la tabla cuando entró, de los pagos declarados
 * que no mueven el acumulado y del techo de interés del préstamo. Restar dos
 * saldos en la pantalla daría otra cifra — y la pantalla y el papel dirían
 * cosas distintas sobre la misma plata.
 *
 * Por eso lo mide el servidor y viaja ATADO AL ID DEL PAGO, igual que
 * `saldoAntesDelPago`: al desplegar un cobro viejo de la lista, el recibo tiene
 * que poder comprobar que la cifra es de ESE pago y no del último.
 *
 * ⚠ `pagos` tiene que venir en orden CRONOLÓGICO ASCENDENTE y con `id`: el
 *   acumulado de `interesPagoAPago` avanza en el orden de la lista, y sin `id`
 *   no hay forma de saber cuál de las filas es la del pago que se imprime.
 *
 * @returns {{interes: number, capital: number}|null} null si no se puede afirmar
 */
export function repartoDeUnPago({ prestamo, cuotas = null, pagos = [], pagoId = null }) {
  if (!pagoId) return null
  const i = pagos.findIndex((p) => p?.id === pagoId)
  if (i < 0) return null
  /* Un recargo y un descuento no son plata que el cliente entregue repartida:
     el recargo sube la deuda y el descuento la baja. Partirlos en interés y
     capital sería poner por escrito algo que no ocurrió. */
  if (['recargo', 'descuento'].includes(pagos[i]?.tipo)) return null
  /* ⚠ EL PAPEL DEL CLIENTE HABLA DE SU DEUDA. Su cartulina y la tabla que firmó
     están hechas sobre `montoPrestado` entero, y el recibo tiene que cuadrar con
     ellas: el interés que arrastró de una renovación es, PARA ÉL, capital de este
     préstamo. Separarlo es cosa de la ganancia del prestamista (analíticas,
     contador, socios), no del recibo. El total pactado sí se queda: sin él, el
     recibo reimpreso de una cartulina renovada decía «interés $0». */
  const fila = interesPagoAPago({ prestamo: { ...prestamo, interesArrastrado: 0 }, cuotas, pagos })[i]
  if (!fila) return null
  const interes = Math.round(fila.interes)
  const capital = Math.round(fila.monto) - interes
  if (!Number.isFinite(interes) || !Number.isFinite(capital)) return null
  if (interes < 0 || capital < 0) return null
  if (interes === 0 && capital === 0) return null
  return { interes, capital }
}

/**
 * Lo mismo para una lista de préstamos ya cargados con sus cuotas y sus pagos.
 * @returns {number} interés redondeado
 */
export function interesCobradoDeLosPrestamos(prestamos = [], { desde = null, hasta = null } = {}) {
  let total = 0
  for (const p of prestamos) {
    total += interesCobradoDelPrestamo({
      prestamo: p,
      cuotas: p.cuotasAmortizacion ?? null,
      pagos: p.pagos ?? [],
      desde, hasta,
    })
  }
  return Math.round(total)
}

/** El `select` de Prisma que esto necesita. Que no se desvíen. */
export const SELECT_PARA_INTERES = {
  id: true,
  montoPrestado: true, ...CAMPOS_DEL_REPARTO,
  totalAPagar: true,
  modoInteres: true,
  /* ⚠ `sinPlazo` Y `devengos` NO SE PEDÍAN, y la rama del préstamo abierto de
     `interesPagoAPago` —la primera de la función— los lee. Sin ellos esa rama
     no se ejecutó NUNCA con datos de verdad: el abierto caía a la proporción y
     su interés salía de menos. El arreglo del 27 ago vivía en la función y el
     `select` que la alimenta no se enteró.

     Medido en el espejo el 20 sep 2026: 14 de 30 abiertos con pagos, en 5
     negocios, $5.568.063 de interés ya cobrado que no salía en la ganancia del
     Inicio, ni en el informe del contador, ni en el de ingresos. El mismo caso
     que cita el comentario de arriba seguía igual: $4.000.000 prestados,
     $2.800.000 cobrados de puro interés, y la cuenta decía $1.152.941. */
  sinPlazo: true,
  devengos: { select: { periodo: true, interes: true } },
  cuotasAmortizacion: {
    orderBy: { numeroPeriodo: 'asc' },
    select: { numeroPeriodo: true, cuotaTotal: true, interes: true },
  },
  pagos: {
    where: { tipo: { notIn: ['recargo', 'descuento'] } },
    orderBy: { fechaPago: 'asc' },
    /* ⚠ `tipo` NO SE PEDÍA, y sin él `interesPagoAPago` no puede distinguir un
       abono a capital de un cobro corriente: llega `undefined`, el predicado no
       acierta NUNCA y el reparto vuelve a ser el viejo — sin un error, sin un
       log y sin una prueba en rojo. Es la misma trampa que ya dejó cuatro días
       muerta la exclusión de reversos en la caja. */
    select: { montoPagado: true, fechaPago: true, tipo: true },
  },
}

/**
 * LA CORRECCIÓN DEL REPARTO PLANO, EN UN SOLO SITIO.
 *
 * Hay tres pantallas que reparten en SQL por velocidad y luego corrigen en
 * JavaScript los préstamos que el reparto plano no sabe partir: analíticas, su
 * PDF y el reparto a socios. Las tres llevaban **su propia copia** del mismo
 * bucle, y el 27 de agosto de 2026 se vio lo que eso cuesta: arreglé el reparto
 * bueno un día antes, corregí una sola de las copias y la misma organización
 * veía «ganancia $3.230.648» en una pantalla y «$2.500.993» en otra, con el
 * mismo recaudado. *«Aquí me sale un valor y en el reporte otro.»*
 *
 * ⚠ SON DOS CORRECCIONES, NO UNA, PORQUE HAY DOS BASES DISTINTAS:
 *
 *   · `porPago` — contra `repartoSql({ porFila: true })`, que va fila por fila
 *     y SÍ contesta los tipos declarados: a un abono a capital le da cero.
 *     La usan el desglose mensual y el reparto a socios.
 *   · `porVida` — contra `repartoSql({ porFila: false })`, que mira el préstamo
 *     entero (`totalPagado × fracción`) y no tiene `tipo` que preguntar.
 *     La usa la rentabilidad por ruta.
 *
 * Restar el mismo número a las dos deja una mal: encaja en la del mes y sobra
 * en la de ruta. Comprobado contra el SQL de verdad sobre los 91 préstamos con
 * pagos declarados de producción: base + corrección = el reparto bueno, en los
 * 91, con las dos bases.
 *
 * @returns {{porPago: Array<{fechaPago: *, delta: number}>, porVida: number}}
 */
export function correccionDelReparto(prestamo) {
  const cuotas = prestamo?.cuotasAmortizacion ?? []
  const pagos = prestamo?.pagos ?? []
  const filas = interesPagoAPago({ prestamo, cuotas, pagos })

  const porPago = pagos.map((pago, i) => ({
    fechaPago: pago?.fechaPago,
    delta: (filas[i]?.interes ?? 0) - interesProporcionalDelPago(prestamo, pago),
  }))

  const bueno = filas.reduce((a, f) => a + f.interes, 0)
  /* Lo que puso `porFila: false`: el préstamo entero como un solo pago sin
     tipo, que es exactamente como esa consulta lo mira. */
  const puestoPorElSql = interesProporcionalDelPago(prestamo,
    { montoPagado: Number(prestamo?.totalPagado) || 0 })

  return { porPago, porVida: bueno - puestoPorElSql }
}
