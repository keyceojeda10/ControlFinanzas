...
export function repartirPagado(prestamo) {
  ...
  if (tieneTablaAmortizacion(prestamo)) {
    ...
    const interes = Math.min(interesAcumuladoTabla(prestamo.cuotasAmortizacion, aRepartir), techo)
    return {
  ...
  const totalPactado = (Number(prestamo.totalAPagar) || 0) - soloInteres
  const interes = redondo(aRepartir * fraccionInteres({ ...prestamo, totalAPagar: totalPactado }))
  return {
...
export function interesProporcionalDelPago(prestamo, pago) {
  ...
  const total = Number(prestamo?.totalAPagar) || 0
  const prestado = Number(prestamo?.montoPrestado) || 0
...
export function repartoSql({ pago = 'p', prestamo = 'pr', monto = 'montoPagado', porFila = true } = {}) {
  ...
  const pactado = `(${prestamo}.totalAPagar - COALESCE((SELECT SUM(pi.montoPagado) FROM Pago pi`
  ...
  const frac = `GREATEST(0, LEAST(1, (${pactado} - ${prestamo}.montoPrestado) / ${pactado}))`
