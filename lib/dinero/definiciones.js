...
export const DEFINICIONES = Object.freeze({
  ...
  apertura: {
    ...
    universo: 'Es el saldo con el que cerró el día anterior. Sale del libro de movimientos: en la caja del negocio, el saldo grabado justo antes del primer movimiento de hoy; en la de un cobrador, el capital de sus rutas menos lo que se haya movido hoy. NO es lo que se contó al cuadrar anoche: eso es una foto de ese momento, y la plata sigue moviéndose después —una base que se repone, un retiro—. Cuando las dos cifras no coinciden, debajo se dice cuánto se movió.',
    formula: 'El saldo del libro al empezar el día.',
  ...
  /* Los tres de abajo son los asientos que movieron el saldo SIN ser plata que
     entrara o saliera. Antes se caían de la banda y dejaban el aviso «la cuenta
     no cierra» sin nada que lo explicara. Ver `clasificarSinEfecto` en
     lib/dinero/conciliacion.js. */
  perdonado: {
    rotulo: 'Deuda que le perdonaste',
    pregunta: '¿Cuánta deuda perdonaste hoy?',
    universo: 'Los descuentos y el interés perdonado al liquidar antes de tiempo. NO es plata que saliera de la caja: la pérdida ya la enseña la cartera. Sale aquí porque el libro sí bajó el saldo, y una cifra que mueve el total tiene que aparecer en la lista.',
    formula: 'Cuánto BAJÓ el saldo por los descuentos y el interés perdonado del día.',
    unidad: UNIDAD.DINERO,
    alcances: TODOS,
    filas: 'movimientos',
  },
  ...
  perdonDeshecho: {
    rotulo: 'Perdón que se deshizo',
    pregunta: '¿Por qué me devolvieron una deuda que había perdonado?',
    universo: 'Se anuló el pago o se borró el préstamo donde se había perdonado algo, así que el capital vuelve a su sitio. NO es plata que entrara hoy: nunca salió de la caja, solo estaba mal restada en el libro.',
    formula: 'Cuánto SUBIÓ el saldo por los reversos de descuento y de interés perdonado del día.',
    unidad: UNIDAD.DINERO,
    alcances: TODOS,
    filas: 'movimientos',
  },
  ...
  correcciones: {
    rotulo: 'Corrección de capital',
    pregunta: '¿Por qué cambió mi saldo sin que se moviera plata?',
    universo: 'Plata que el libro te había restado mal y se te devuelve. No entró ni salió un billete: se corrige el capital. El caso del 26 de agosto: perdonar deuda y perdonar interés bajaban el capital, y eso lo contaba dos veces.',
    formula: 'Cuánto movieron el saldo los asientos marcados como corrección.',
    unidad: UNIDAD.DINERO,
    alcances: TODOS,
    filas: 'movimientos',
  },
  ...
  renovaciones: {
    ...
    universo: 'Préstamos que reemplazan a uno anterior. El valor que se muestra es el EFECTIVO ENTREGADO, no el monto del préstamo: la parte que cubre el saldo viejo nunca sale de la caja. No cuenta los saldos financiados: esos van en su propia línea.',
  ...
  financiados: {
    rotulo: 'Saldos financiados',
    pregunta: '¿A cuántas cartulinas se les financió el saldo hoy, y cuánto interés se les cobró?',
    universo: 'Cartulinas a las que se les dio más tiempo con «Financiar el saldo»: la misma deuda con el interés que se pactó, desde hoy. No es un préstamo del día —no sale plata— y por eso no se suma a lo prestado. La cifra es el INTERÉS que se cobró por financiar, no el saldo: sube lo que el cliente debe, igual que un recargo.',
    formula: 'Suma del interés de las financiaciones del día.',
    unidad: UNIDAD.DINERO,
    alcances: TODOS,
    filas: 'prestamos',
  },
  ...
  capitalDisponible: {
    ...
    /* ⚠ EL PÁRRAFO CONTESTA UNA OBJECIÓN CONCRETA, y por eso está escrito así.
     *
     * Préstamos Rincón, 12 ago 2026: «al dinero disponible para préstamos suman
     * el capital cobrado + los intereses, y para este solo se debería tener en
     * cuenta el capital pagado». Comprobado contra su base: la cifra está bien.
     * Es efectivo, y el efectivo se presta venga de donde venga. Decirle que
     * solo tiene disponible el capital sería mentirle sobre plata que tiene en
     * la mano.
     *
     * Pero la pregunta de debajo sí era buena —cuánto de esto es ganancia— y la
     * contesta otra pantalla. Se la señala, en vez de dejarlo con la duda. */
    universo: 'El efectivo de la caja según el libro de movimientos. Incluye TODO lo cobrado, capital e intereses: esa plata está en la caja y se puede volver a prestar. NO incluye lo que está en la calle, que todavía no la tienes.',
    formula: 'El saldo actual del libro de capital.',
    nota: 'Para ver cuánto de lo cobrado fue ganancia y cuánto capital que vuelve, mira «De cada peso recaudado» en Cómo va mi negocio. Y si quieres sacar tu ganancia del negocio, regístralo en Capital como «Sacar dinero»: así deja de contar como disponible.',
    unidad: UNIDAD.DINERO,
    alcances: [ALCANCE.ORGANIZACION],
    filas: 'movimientos',
  },
  ...
  puntualidadCliente: {
    // El rotulo lo manda la lamina T03-03, que pinta «Cumple 31%». El
    // diccionario describe lo que la pantalla dice; no le cambia el nombre a
    // las cosas por gusto propio.
    rotulo: 'Cumple',
    ...
    filas: 'cuotas',
    notas: 'El plan de la auditoria daba por hecho que «Cumple %» eran dos KPI bajo un rotulo. Medido: NO lo son. Los dos sitios que lo calculan —/api/clientes y /api/rutas/[id]— usan la misma definicion y lo dicen en un comentario. Lo que si son dos cosas es esta y el avance de cobro de la ruta, que ya tienen rotulos distintos.',
  },
  ...
  avanceRuta: {
    ...
    filas: null,
    notas: 'Es otra pregunta que «Cumple»: aquella mira la puntualidad de UN cliente a lo largo de su plazo, esta mira cuanto lleva cobrado HOY una ruta. Alcance distinto, universo distinto, ventana distinta.',
  },
})
