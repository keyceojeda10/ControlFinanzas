 ...
 */
export function mensajeBorrarPrestamo(prestamos, id, pais = 'co', ahora = Date.now()) {
  const base = '¿Eliminar este préstamo y todos sus pagos? Esta acción no se puede deshacer.'
  const p = (prestamos || []).find((x) => x.id === id)
  if (!p) return base

  /* ⚠ SI EL DINERO SALIÓ OTRO DÍA, BORRAR NO LO TRAE DE VUELTA.
     PRESTA MIL, 21 ago 2026: borró un préstamo desembolsado el 24 de junio y el
     capital de la ruta subió $186.000 que no existían en ninguna parte. Ahora no
     sube — y hay que decirlo ANTES, porque «el capital no cambió» sorprende
     tanto como que cambiara de más. */
  /* Sin fecha no se avisa: un `undefined` daría «no es de hoy» y saldría el aviso
     en todos, que es peor que no sacarlo. La decisión de verdad la toma el API. */
  const viejo = !!p.createdAt && !esDelDiaAbierto(p.createdAt, pais, ahora)
  const avisoDia = viejo
    ? `\n\nEste préstamo se entregó otro día, así que esa plata ya salió de la caja y no vuelve al capital al borrarlo. Si el cliente devolvió el dinero, regístralo como un pago antes de eliminar.`
    : ''

  if (!p.renovadoDeId) return base + avisoDia
