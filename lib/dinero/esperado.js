...
// Imports RELATIVOS, no con el alias `@/`. Es la convencion de los modulos de
// calculo puro del proyecto (`calculos.js`, `comportamiento.js`) y no es
// estetica: el alias solo lo resuelve Next, asi que con `@/` este archivo no se
// puede correr desde un script suelto — y medir contra la cartera real es
// justamente como se comprueba que la funcion dice la verdad.
import {
  obtenerDiasPorPeriodo, inicioDiaLocal, calcularProximoCobro,
  tieneTablaAmortizacion, obtenerCuotaPeriodoActual,
} from '../calculos.js'
import { obtenerDiasSinCobro, esFestivo } from '../dias-sin-cobro.js'

const DIA_MS = 24 * 60 * 60 * 1000

/**
 * Cuanto vale UNA cuota de este prestamo ahora mismo.
 *
 * `cuotaDiaria` NO sirve en los modos con tabla: ahi es la PRIMERA cuota, que
 * en Decreciente es la mas alta de todas y en Globo es solo el interes — asi
 * que el mes del balloon salia enormemente subestimado.
 *
 * Estaba escrita DOS VECES, identica, en `app/api/caja/route.js:27` y dentro
 * del bucle de `app/api/rutas/[id]/route.js:390`. Vive aqui para que sea una.
 */
export function cuotaDelPeriodo(prestamo) {
  if (!prestamo) return 0
  return tieneTablaAmortizacion(prestamo)
    ? (obtenerCuotaPeriodoActual(prestamo) || 0)
    : (Number(prestamo.cuotaDiaria) || 0)
}
 ...
 */
export function inicioDia(valor, offsetHoras = -5) {
  if (valor == null) return inicioDiaLocal(Date.now(), offsetHoras)

  // ⚠ UN 'YYYY-MM-DD' ES UN DIA DEL CALENDARIO, NO UN INSTANTE.
  //
  // `new Date('2026-08-01')` se lee como medianoche UTC, que en Bogota son las
  // 7pm del 31 DE JULIO. Preguntar por el 1 contestaba por el 31.
  //
  // Me mordio en la primera medicion contra produccion: dos negocios de
  // cartera 100% mensual pasaron de $5.554.733 a $0 de esperado, y el cero
  // parecia hasta razonable —el dia 1 casi ningun mensual vence—. Solo al
  // cotejar prestamo por prestamo con la funcion vieja, que dio CERO
  // discrepancias, quedo claro que el predicado estaba bien y lo que fallaba
  // era el dia por el que preguntaba.
  //
  // Se ancla al MEDIODIA local: asi ningun corrimiento de zona horaria de unas
  // horas puede cambiar de dia. Es el mismo remedio que `normalizarFechaInicio`
  // en lib/calculos.js.
  if (typeof valor === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(valor)) {
    const pad = (n) => String(n).padStart(2, '0')
    return inicioDiaLocal(new Date(`${valor}T12:00:00-${pad(Math.abs(offsetHoras))}:00`), offsetHoras)
  }
 ...
 */
/* ⚠ EL PRIMER COBRO NO DEPENDE DEL DIA QUE SE PREGUNTE, y se recalculaba en
   cada pregunta.

   `tocaCobrarEn` llama a `calcularProximoCobro` en dos ramas —diario, y
   semanal/quincenal con ancla— y esa funcion, cuando hay dias sin cobro,
   avanza fecha a fecha hasta 5.000 veces. Preguntar por los 31 dias de un mes
   repetia ese recorrido 31 veces con el mismo resultado: la pantalla de
   analiticas del negocio mas grande —984 prestamos— tardaba 1,8 s.

   Se guarda el ULTIMO calculo por prestamo, y solo se reusa si se pregunta con
   los MISMOS `diasExcluidos` y los mismos festivos: quien recorre un mes
   pregunta siempre con los mismos, y si cambian se recalcula, asi que nunca
   devuelve el valor de otra configuracion. `WeakMap` para no retener los
   prestamos en memoria. */
const memoPrimerCobro = new WeakMap()

function primerCobroDe(prestamo, diasExcluidos, festivos) {
  const guardado = memoPrimerCobro.get(prestamo)
  if (guardado && guardado.dias === diasExcluidos && guardado.festivos === festivos) {
    return guardado.valor
  }
  const sinPagos = { ...prestamo, pagos: [], totalPagado: 0 }
  const valor = calcularProximoCobro(sinPagos, diasExcluidos, festivos)
  memoPrimerCobro.set(prestamo, { dias: diasExcluidos, festivos, valor })
  return valor
}
...
export function tocaCobrarEn(prestamo, fecha, diasExcluidos = [], festivos = []) {
  ...
  if (freq === 'diario') {
    ...
    // ⚠ NO BASTA CON «cualquier dia despues del desembolso».
    //
    // Escribi eso primero y la rejilla de equivalencia no lo caza hasta que se
    // le meten SEIS dias sin cobro — un cliente que solo cobra un dia a la
    // semana, que en la cartera real existe. Ahi la vieja dice que no toca y la
    // simplificacion decia que si: 331.000 pesos de diferencia en un solo
    // cobrador de PRESTA MIL.
    //
    // Se delega en `calcularProximoCobro`, que es lo que hacia la original:
    // sobre una copia SIN pagos devuelve la fecha del PRIMER cobro programado,
    // saltandose los dias muertos. Si esa fecha ya llego, el calendario esta en
    // marcha.
    const primerCobro = primerCobroDe(prestamo, diasExcluidos, festivos)
    if (!primerCobro) return false
    return inicioDia(primerCobro).getTime() <= dia.getTime()
  }

  /* ══ EL DIA CERRADO NO BORRA LA CUOTA: LA CORRE AL SIGUIENTE ═════════════
   *
   * Esto solo lo miraba la frecuencia DIARIA, aqui y en `tienePeriodoEsperadoHoy`.
   * Un semanal cuyo ciclo caia en un dia marcado sin cobro sumaba igual a la
   * meta del dia con el negocio cerrado, mientras la ficha de la ruta —que si
   * respeta el dia sin cobro— decia «0 cobros programados hoy».
   *
   ...
   * que `calcularProximoCobro` YA hace con los festivos en cualquier frecuencia.
   *
   *   · dia cerrado   → ese dia no vence nada (la cuota espera);
   *   · dia abierto   → vence lo suyo Y lo que cayo en los dias cerrados que
   *                     vienen justo antes.
   *
   * La cuota no se pierde de la meta: aparece el primer dia en que de verdad se
   * puede cobrar. Siete dias hacia atras es el tope: una racha mas larga seria
   * un negocio que no cobra nunca.
   *
   * ⚠ Las dos funciones tienen que cambiar A LA VEZ. `esperado-vs-calculos.test.js`
   * compara una contra otra justo para que no se separen. */
  if (esDiaMuerto(dia, diasExcluidos, festivos)) return false

  const candidatos = [dia]
  for (let i = 1; i <= 7; i++) {
    const antes = new Date(dia.getTime() - i * DIA_MS)
    if (!esDiaMuerto(antes, diasExcluidos, festivos)) break
    if (antes < inicio) break
    candidatos.push(antes)
  }

  return candidatos.some((d) => tocaPorCalendario(prestamo, d, inicio, freq, diasExcluidos, festivos))
}

/** ¿El calendario pedia un cobro EXACTAMENTE ese dia? Sin mirar dias cerrados:
 *  de correr la cuota al siguiente dia cobrable se encarga `tocaCobrarEn`. */
function tocaPorCalendario(prestamo, dia, inicio, freq, diasExcluidos, festivos) {
  ...
  if (prestamo.diaCobroSemana != null && (freq === 'semanal' || freq === 'quincenal')) {
    ...
    // ⚠ EL CICLO ARRANCA DONDE DIGA `calcularProximoCobro`, NO EN EL PRIMER
    // DIA ANCLA DESPUES DEL DESEMBOLSO.
    //
    // Escribi lo segundo —avanzar dia a dia hasta el ancla— y suena razonable.
    // La cartera real dice otra cosa: DOS prestamos semanales anclados en
    // sabado, desembolsados el lunes 27 de julio, que la version vieja NO
    // espera el sabado 1 de agosto porque solo han pasado cinco dias y no una
    // semana. $331.000 en un cobrador de PRESTA MIL, con los otros nueve
    // dando identico.
    //
    // Es la tercera vez en esta migracion que una simplificacion mia parece
    // mas correcta que el original. Puede que lo sea. Pero el trato es que NADA
    // cambia para hoy, asi que se delega en la misma funcion que usaba la
    // original y punto. Discutir la regla es otro trabajo, con su medicion.
    const primerCobro = primerCobroDe(prestamo, diasExcluidos, festivos)
    if (!primerCobro) return false
    const diff = Math.floor((dia.getTime() - inicioDia(primerCobro).getTime()) / DIA_MS)
