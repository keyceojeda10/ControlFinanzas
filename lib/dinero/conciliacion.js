...
/**
 * Cuánto movió el saldo este conjunto de asientos, sumando uno a uno.
 *
 * ⚠ UNA SOLA IMPLEMENTACIÓN, USADA EN LOS DOS SITIOS. La apertura del día se
 * deriva de esta resta, y la calculan tanto `resumirLibro` como el API de la
 * caja: hacerla dos veces —una en JS y otra con un `SUM` en la base— es cómo
 * se llega a que dos cifras de la misma pantalla difieran en un peso. Y aquí
 * importa: hay asientos con fracciones de céntimo, así que redondear cada
 * delta no da lo mismo que redondear la suma.
 */
export function loQueSeMovio(movimientos = []) {
  return movimientos.reduce(
    (a, m) => a + redondo((m.saldoNuevo ?? 0) - (m.saldoAnterior ?? 0)), 0,
  )
}

export function resumirLibro(movimientos = [], saldoPrevio = null, saldoActual = null) {
  const orden = [...movimientos].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt))
  const vivos = orden.filter(afectaCaja)

  /* ══ LO QUE SE MOVIÓ HOY, SUMANDO ASIENTO A ASIENTO ══════════════════════
   *
   * ⚠ CADA ASIENTO SABE LO QUE MOVIÓ ÉL; LA CADENA ENTERA PUEDE MENTIR. Las
   * dos fotos de una fila son fiables por separado —su resta es su efecto—
   * pero que la última de una fila enganche con la primera de la siguiente NO
   * está garantizado: basta con que alguien borre un movimiento del medio, o
   * con que uno entre con fecha de otro momento, para que la cadena se parta.
   *
   * Reportado el 27 de agosto de 2026 por un prestamista que llevaba días
   * peleándose con esto: «se me descuadra mucho la caja, cuando quiero retirar
   * un dinero se me descuadra enseguida todo». Su mañana, en el registro:
   * retiró $1.019.000 a las 09:01, lo deshizo a las 09:04, lo rehízo de otra
   * forma a las 09:12, borró los dos a las 09:43 y a las 09:44 metió un ajuste
   * de $2.038.000 para cuadrarlo a mano. El asiento del «deshecho» hereda la
   * fecha del original —a propósito, para no dejar descuadrado el día viejo—,
   * así que al borrar el original quedó ordenado DETRÁS del ajuste posterior.
   * Su tarjeta acabó enseñando TRES cifras distintas: las líneas sumaban
   * $1.519.000, el aviso decía que el saldo era $2.538.000 y el saldo en caja
   * decía $500.000. */
  const deltaTotal = loQueSeMovio(orden)
  ...
  /* ⚠ EL PUNTO DE LLEGADA ES EL SALDO DE VERDAD, NO LA ÚLTIMA FOTO.
   *
   * Con `saldoActual` —el `Capital.saldo` del negocio— el cierre es ése y la
   * apertura se DERIVA restándole lo que se movió hoy. Así la banda cuenta la
   * misma historia que la cifra grande de la pantalla, que siempre salió de
   * ahí, y una cadena rota deja de inventar un descuadre: los deltas siguen
   * siendo correctos aunque las fotos absolutas no lo sean.
   *
   * ⚠ NO SE PASA EN LA CAJA DE UN COBRADOR. Ahí los asientos vienen filtrados
   * por sus rutas y `Capital.saldo` es el de TODO el negocio: derivar la
   * apertura le achacaría a él lo que movieron las otras rutas. Sin
   * `saldoActual` se comporta como siempre, leyendo las fotos.
   *
   * Y esto NO apaga la alarma de verdad: `sinExplicar` enfrenta la suma de los
   * `monto` contra la suma de los deltas, y sigue intacta. Lo que se apaga es
   * el aviso que salía por una foto vieja, que no era un descuadre de plata. */
  const derivado = saldoActual != null && Number.isFinite(Number(saldoActual))

  const r = {
    apertura: derivado ? redondo(Number(saldoActual) - deltaTotal)
      : orden.length ? redondo(orden[0].saldoAnterior) : previo,
    ...
    cierre: derivado ? redondo(Number(saldoActual))
      : orden.length ? redondo(orden[orden.length - 1].saldoNuevo) : previo,
    // Lo que se movió hoy, sumando asiento a asiento. Se devuelve para que
    // quien quiera pueda comprobar la derivación sin rehacerla.
    deltaTotal,
