...
/** Una cartulina que cae en su sitio: un roce grave y una nota corta. */
export function sonidoCartulina() {
  const a = audio(); if (!a) return
  if (!ruido) {
    ruido = a.createBuffer(1, Math.round(a.sampleRate * 0.2), a.sampleRate)
    const d = ruido.getChannelData(0)
  ...
  const t = a.currentTime, fuente = a.createBufferSource(), paso = a.createBiquadFilter(), g = a.createGain()
  fuente.buffer = ruido; paso.type = 'lowpass'; paso.frequency.value = 1800
  g.gain.setValueAtTime(0.0001, t)
  g.gain.exponentialRampToValueAtTime(0.25, t + 0.015)
  g.gain.exponentialRampToValueAtTime(0.0001, t + 0.12)
  fuente.connect(paso).connect(g).connect(a.destination); fuente.start(t); fuente.stop(t + 0.15)
  nota(a, 1318.5, t, 0.28, 0.08, 'sine')
}

/** Una transferencia que llega: el aviso de mensaje del teléfono, mi–la, corto. */
export function sonidoNotificacion() {
  const a = audio(); if (!a) return
  const t = a.currentTime
  nota(a, 1318.5, t, 0.16, 0.09, 'sine'); nota(a, 1760, t + 0.09, 0.28, 0.09, 'sine')
}
