...
/**
 * Los meses del plazo, contados como `calcularPrestamo`: cuotas ÷ cuotas por
 * mes (`PERIODOS_POR_MES`, 4 semanas = 1 mes). 6 semanas son 1,5 meses.
 */
export function mesesDelPlazo({ periodos, frecuencia = 'diario' } = {}) {
  const n = Math.round(Number(periodos) || 0)
  return n > 0 ? n / (PERIODOS_POR_MES[frecuencia] || 30) : 1
}

/* ══ EL PORCENTAJE ES AL MES, COMO AL CREAR UN PRÉSTAMO ══════════════════════
 *
 * Inversiones Jk, 25 sep 2026: «el cliente me está debiendo 660, yo le presto
 * el 15 %, se lo voy a financiar a dos meses […] deberían de ser 858, y
 * solamente suma un solo mes, 760». La hoja cobraba el % UNA vez por todo el
 * plazo, mientras «Nuevo préstamo» con el mismo 15 % a 8 semanas da $859.200:
 * en `fijo` la tasa es mensual. Así hablan los prestamistas —«al 15 %» es al
 * mes— y el dueño lo decidió igual: el % de financiar es AL MES. A 30 días o 4
 * semanas no cambia nada. Una cifra en pesos sigue siendo por todo el plazo. */
export function interesPorPorcentajeMensual(deuda, porcentajeMensual, plazo) {
  const d = Number(deuda) || 0
  const p = Number(porcentajeMensual) || 0
  if (d <= 0 || p <= 0) return 0
  return Math.round((d * p * mesesDelPlazo(plazo)) / 100)
}
...
export function tasaDeFinanciar(deuda, interes, { periodos, frecuencia = 'diario' } = {}) {
  ...
  return (i / d / mesesDelPlazo({ periodos, frecuencia })) * 100
}
