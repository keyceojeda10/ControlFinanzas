...
/* Por qué falló la última lectura: 1 = el permiso está negado, 2 = el teléfono
   no la pudo dar (ubicación apagada, sin señal), 3 = tardó demasiado. Se guarda
   para poder DECÍRSELO al cobrador: ver `estadoUbicacion`. */
  ...
  return new Promise((resolve) => {
    ...
    try {
      navigator.geolocation.getCurrentPosition(
        (pos) => { ultimoFallo = null; listo({ latitud: pos.coords.latitude, longitud: pos.coords.longitude }) },
        (err) => { ultimoFallo = err?.code ?? 2; listo(null) },
...
/* ══ ¿ESTE TELÉFONO PUEDE MANDAR LA UBICACIÓN? ════════════════════════════════
 *
 * Medido en producción el 22 sep 2026: de los cobros de 14 días, el 70 % llega
 * con coordenadas, pero no está repartido: hay SEIS cobradores que no mandan
 * ninguna, ningún día. No es el sistema, es el permiso de su teléfono — y nadie
 * se lo dice, ni a ellos ni al dueño.
 *
 * Contesta con una palabra, para poder escribirla en pantalla:
 *   'ok'          → hay permiso (o ya hay una lectura buena)
 *   'sin-permiso' → el navegador lo tiene NEGADO: hay que dárselo a mano
 *   'apagada'     → hay permiso pero el teléfono no da la posición
 *   'sin-soporte' → el navegador no tiene geolocalización
 *   'preguntar'   → todavía no se le ha pedido
 *   null          → no se sabe (no preguntar dos veces por lo mismo)
 *
 * No pide permiso: solo mira. Pedirlo es `calentarCoords`, que ya lo hace al
 * abrir la hoja de cobro.
 */
export async function estadoUbicacion() {
  if (typeof navigator === 'undefined' || !navigator.geolocation) return 'sin-soporte'
  if (fresca()) return 'ok'
  try {
    const permiso = await navigator.permissions?.query?.({ name: 'geolocation' })
    if (permiso?.state === 'denied') return 'sin-permiso'
    if (permiso?.state === 'prompt') return ultimoFallo ? 'apagada' : 'preguntar'
    // 'granted', o un navegador que no sabe contestar: manda lo que pasó de verdad.
    if (permiso?.state === 'granted') return ultimoFallo ? 'apagada' : 'ok'
  } catch { /* Safari viejo no tiene `permissions`: se cae al último fallo */ }
  if (ultimoFallo === 1) return 'sin-permiso'
  if (ultimoFallo) return 'apagada'
  return ultima ? 'ok' : null
}
