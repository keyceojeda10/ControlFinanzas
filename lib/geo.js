...
export async function completarUbicacionDelPago(pagoId) {
  try {
    if (!pagoId || typeof navigator === 'undefined' || !navigator.geolocation) return
    const coords = fresca() ?? await (enCurso ?? leerGps(10_000))
    if (!coords) return
    await fetch(`/api/pagos/${pagoId}/ubicacion`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(coords),
      keepalive: true,
    })
  } catch { /* la ubicacion es un dato de apoyo: si no llega, no pasa nada */ }
}
