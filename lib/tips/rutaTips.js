...
export function generarTipRuta(clientes = []) {
  ...
  if (conCobroHoy === 0) {
    if (enMoraAlta >= 2) {
      return `Hoy no hay cobros programados, pero ${enMoraAlta} clientes llevan mas de 5 días en mora — buen momento para visitarlos.`
    }
    return 'Hoy no hay cobros programados en esta ruta. Aprovecha para ponerte al día o revisar clientes en mora.'
  }

  // CASO 2: ya cobro a todos los que tocaban hoy.
  if (porCobrarHoy === 0 && pagaronHoy > 0) {
    return `Ruta del día completada: cobraste a los ${pagaronHoy} clientes que tocaban hoy. Excelente trabajo.`
  }

  // CASO 3: mora alta es lo mas urgente, aunque haya pendientes normales.
  if (enMoraAlta >= 2) {
    return `${enMoraAlta} clientes de esta ruta llevan mas de 5 días en mora — priorizalos en tu recorrido de hoy.`
  }

  // CASO 4: progreso parcial del dia.
  if (pagaronHoy > 0 && porCobrarHoy > 0) {
    const pct = Math.round((pagaronHoy / conCobroHoy) * 100)
    return `Llevas ${pagaronHoy} de ${conCobroHoy} cobros del día (${pct}%) — te faltan ${porCobrarHoy} por visitar.`
  }
