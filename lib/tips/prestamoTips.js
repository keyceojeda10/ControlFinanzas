...
export function generarTipPrestamo(prestamo, pagos = []) {
  ...
  if ((prestamo.diasMora ?? 0) > 5) {
    ...
    if (ultimoPago) {
      ...
      return `Lleva ${prestamo.diasMora} dias sin pagar — ultimo pago fue el ${fecha}. Contactar pronto.`
    }
    return `Lleva ${prestamo.diasMora} dias en mora — priorizar cobro de este prestamo.`
  }
