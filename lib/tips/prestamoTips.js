...
export function generarTipPrestamo(prestamo, pagos = []) {
  ...
  if ((prestamo.diasMora ?? 0) > 5) {
    ...
    if (ultimoPago) {
      ...
      // «de atraso»: con «sin pagar» esta misma frase se contradecía cuando el
      // último pago era de ayer (un abono de solo interés deja el capital atrasado).
      return `Lleva ${prestamo.diasMora} días de atraso — último pago fue el ${fecha}. Contactar pronto.`
    }
