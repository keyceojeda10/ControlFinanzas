...
export function generarTipCliente(cliente, prestamosActivos = [], historial = {}) {
  ...
  if (maxMora > 7) {
    return {
      tono: 'aviso',
      etiqueta,
      // «de atraso», no «sin pagar»: `diasMora` cuenta desde la cuota más vieja
      // sin cubrir, y quien abonó solo el interés SÍ pagó. Ver ParadaDeCobro.
      titular: `Lleva ${maxMora} días de atraso.`,
  ...
  /* ── EL ATRASO CORTO: NI OFERTA NI ALARMA ────────────────────────────────
   *
   * ⚠ ENTRE 1 Y 7 DÍAS SE QUEDABA MUDO, y ese es el estado más común de todos.
   * La oferta pedía estar al día y el aviso arrancaba a los 7: un cliente con
   * 4 días de atraso —el caso real que me lo enseñó, ARIEL GIL con 3 préstamos
   * pagados y tope puesto— no recibía nada. El bloque desaparecía sin motivo
   * aparente, que es peor que decir poco.
   *
   * Aquí no se ofrece prestar —debe algo, aunque sea de ayer— pero sí se dice
   * qué pasa, que es lo que el dueño necesita para decidir si llamar.
   */
  if (maxMora > 0) {
    return {
      tono: 'info',
      etiqueta,
      titular: maxMora === 1 ? 'Se atrasó un día.' : `Se atrasó ${maxMora} días.`,
      porque: completados > 0 && incumplidos === 0
        ? `Suele pagar bien: lleva ${completados} préstamo${completados === 1 ? '' : 's'} completo${completados === 1 ? '' : 's'}.`
        : 'Todavía es poco, pero conviene no perderlo de vista.',
    }
  }
