...
export function generarTipCliente(cliente, prestamosActivos = []) {
  ...
  if (maxMora > 7) {
    return `Este cliente lleva ${maxMora} días en mora — considera contacto directo para evitar acumulacion.`
  }
  ...
  if (prestamosActivos.length > 1) {
    ...
    return `${prestamosActivos.length} préstamos activos por $${Math.round(total).toLocaleString('es-CO')} — monitorear capacidad de pago.`
  }
  ...
  if (prestamosActivos.length === 1) {
    ...
    if (pct >= 75) {
      return `Lleva ${pct}% pagado en su préstamo activo — cliente con buen historial de pago.`
    }
  }

  // Sin prestamos activos
  if (prestamosActivos.length === 0 && cliente.estado !== 'eliminado') {
    return 'Cliente sin préstamos activos — potencial para nuevo crédito si tiene buen historial.'
  }
