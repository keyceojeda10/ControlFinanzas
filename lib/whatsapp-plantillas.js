...
import { calcularProximoCobro, cuotaProximoCobro, obtenerCuotaPeriodoActual, calcularMontoEnMora, calcularDiasMora } from '@/lib/calculos'
...
/* LO QUE DEBE ATRASADO, PARA LAS PLANTILLAS QUE RESUMEN EL CRÉDITO.
 *
 * «En el recuadro le recuerda la cuota de la semana, mas no la cuota que tiene
 * atrasada.» Y en efecto: el resumen decía «Cuota semanal: $267.000» a un
 * cliente que llevaba $534.000 vencidos. La cifra que el cobrador va a pedir
 * de verdad no salía por ningún lado.
 *
 * ⚠ SE CALCULA AQUÍ, NO SE ESPERA DE QUIEN ABRA LA HOJA. `HojaWhatsApp` se
 * abre desde CINCO pantallas y cada una arma su préstamo por su cuenta; fiarse
 * de que todas traigan `montoEnMora` es como se arreglan las cosas a medias en
 * este repo. Si el campo viene, manda —es el mismo que pinta la ficha—; si no,
 * se calcula con la función de siempre. */
function loAtrasado(prestamo) {
  if (!prestamo) return 0
  /* ⚠ `null` NO ES CERO, ES «NO ME LO PASARON». `Number(null)` da 0 y es un
     número finito, así que la guarda ingenua se creía que el cliente no debe
     nada y no llegaba a calcularlo. Medido por los cinco caminos que abren esta
     hoja: uno mandaba `null` y perdía el renglón en las CUATRO plantillas. Un
     0 de verdad sí manda —significa al día— y por eso la comprobación es
     contra `null`, no contra la falsedad. */
  const delApi = prestamo.montoEnMora == null ? NaN : Number(prestamo.montoEnMora)
  if (Number.isFinite(delApi)) return Math.max(0, Math.round(delApi))
  try {
    if (calcularDiasMora(prestamo) <= 0) return 0
    return Math.max(0, Math.round(calcularMontoEnMora(prestamo) || 0))
  } catch { return 0 }
}

/* El renglón, o nada. Va como sección propia para que se pueda quitar de un
   toque: hay quien prefiere no nombrar la deuda en el mensaje de aprobación. */
function seccionAtraso(prestamo) {
  const mora = loAtrasado(prestamo)
  if (mora <= 0) return []
  return [{
    key: 'atraso',
    label: 'Lo que tiene atrasado',
    default: true,
    texto: `\n\n\u{26a0}\u{fe0f} *Tienes ${formatMoney(mora)} pendientes de cuotas anteriores.*`,
  }]
}

/* ⚠ NUNCA FIRMAR CON LA MARCA DEL SOFTWARE.
   Esto devolvía `_Control Finanzas_` cuando al negocio le faltaba el nombre, y
   la firma va al pie de TODAS las plantillas — incluida la de mora crítica, que
   dice «Última oportunidad antes de cobro jurídico… para evitar acciones
   legales, comunícate HOY mismo».
   O sea: una amenaza de cobro a un deudor, firmada con el nombre de la empresa
   de software. Es exactamente lo que no somos y lo que no hacemos.
   Sin nombre de negocio, el mensaje va sin firma. */
...
function excedenteDelPago(prestamo, pago) {
  ...
  if (!pagado) return 0
  /* ⚠ LA CUOTA DEL PERÍODO, NO LA QUE FALTA. Aquí se pregunta «de lo que
     acaba de entregar, cuánto sobró», y para eso hace falta lo que se le
     debía ANTES, no lo que queda después.

     `cuotaProximoCobro` empezó a descontar lo ya abonado el 24 ago —que es lo
     que pide el recordatorio— y por ahí este excedente se doblaba: pagó
     $190.000 sobre una cuota de $187.000, la fila siguiente se queda con los
     $3.000 encima, y con la cuota ya descontada ($184.000) el recibo diría
     que sobraron $6.000. Los mismos $3.000, contados dos veces. */
  const cuota = obtenerCuotaPeriodoActual(prestamo) || prestamo?.cuotaDiaria || 0
...
function resolverCampoWA(campo, cliente, prestamo, pago = null) {
  ...
  /* El único campo que puede no salir: sin cifra del servidor, o con una que es
     de OTRO pago, la línea desaparece en vez de decir «Antes debías: -». La
     regla entera vive en `saldoAntesDeEstePago`, compartida con el recibo. */
  if (campo === 'saldoAntes') {
  ...
  const map = {
    ...
    /* En un abierto `fechaFin` es el PRIMER corte de interés, no un
       vencimiento. Escribirlo en el recibo del cliente le promete un final que
       no existe: su préstamo dura hasta que abone el capital. */
    fechaVencimiento: (prestamo?.sinPlazo && prestamo?.modoInteres === 'solo_interes')
      ? 'sin vencimiento'
      : formatFechaCorta(prestamo?.fechaFin),
...
function secsPagoConfirmacion(ctx) {
  ...
  if (!ocultarSaldo) {
    /* LA RESTA, ANTES DEL RESULTADO. «Antes debías $480.000 / Saldo pendiente
       $430.000» se lee sola; con el saldo a secas el cliente tiene que fiarse.
       Solo cuando hay cifra: la mide el servidor en el momento del cobro y un
       mensaje armado desde la ficha del préstamo no la lleva. Y nunca si el
       dueño esconde el saldo, porque diría lo mismo por otra puerta. */
    const antes = saldoAntesDeEstePago(prestamo, pago)
    if (antes != null) estadoLines.push(`\u{1f4dd} Antes debías: ${formatMoney(antes)}`)
  ...
  const yaEnEstado = ocultarSaldo
    ? new Set(['totalPagado', 'saldoAntes'])
    : new Set(['totalPagado', 'saldoPendiente', 'progreso', 'saldoAntes'])
  const camposSinRepetir = (Array.isArray(camposRecibo) ? camposRecibo : [])
    .filter((c) => !(c?.tipo === 'dato' && yaEnEstado.has(c.campo)))

  // `null` es «este campo no tiene nada que decir hoy»: la línea no se pinta, y
  // si no queda ninguna, la sección tampoco (si no, un salto de línea suelto).
  const lineasExtra = camposSinRepetir.map(c => {
    const val = c.tipo === 'texto' ? c.valor : resolverCampoWA(c.campo, cliente, prestamo, pago)
    return val == null ? null : `\u{1f4cb} ${c.nombre}: ${val}`
  }).filter(Boolean)
...
function secsMora(nivel, ctx) {
  ...
  if (nivel === 'firme') {
    ...
    /* ⚠ LA CUOTA NO ES LO QUE DEBE. En un cliente con dos quincenas vencidas
       este bloque decía «Cuota semanal: $267.000» y lo atrasado eran $534.000:
       el mensaje le pedía la mitad de lo que el cobrador iba a cobrarle. */
    const vencido = loAtrasado(prestamo)
    if (vencido > 0) estadoLines.push(`\u{26a0}\u{fe0f} Vencido a hoy: ${formatMoney(vencido)}`)
    return [
      { key: 'saludo', label: 'Saludo', default: true, locked: true,
        texto: `Hola ${cliente?.nombre || ''} \u{1f44b}` },
...
/** El pago que se puede agradecer: el que se acaba de registrar, o uno de HOY
 *  (en el día del teléfono, que es el del cobrador). Sin eso, nada: un pago de
 *  la semana pasada no es «tu pago fue registrado», y la cuota no es un pago. */
export function pagoParaAgradecer({ prestamo, pago } = {}) {
  if (pago && Number(pago.montoPagado) > 0) return pago
  const pagos = Array.isArray(prestamo?.pagos) ? prestamo.pagos : []
  const hoy = new Date().toDateString()
  const deHoy = pagos
    .filter((p) => Number(p?.montoPagado) > 0 && !['recargo', 'descuento'].includes(p?.tipo))
    .filter((p) => p?.fechaPago && new Date(p.fechaPago).toDateString() === hoy)
  if (deHoy.length === 0) return null
  return deHoy.reduce((a, b) => new Date(a.fechaPago) > new Date(b.fechaPago) ? a : b)
}
...
export const PLANTILLAS = [
  ...
  /* ══ «GRACIAS POR TU PAGO» SOLO CUANDO HAY UN PAGO ═══════════════════════
     4 sep 2026. Un prestamista abrió WhatsApp desde un préstamo SIN pagos, la
     primera plantilla sugerida era esta, y el texto salió solo: «Tu pago de
     $150.000 fue registrado correctamente» — $150.000 era LA CUOTA, no un
     pago. El mensaje llegó al cliente, el pago nunca existió en el sistema, y
     al día siguiente el prestamista estaba seguro de haberlo registrado.

     Ya había pasado al crear un préstamo (ver `waSugerida` en la ficha) y se
     parchó solo ese camino. La regla va aquí, en la plantilla, para todos:
       · aplica solo con un pago DE HOY (o el que se acaba de registrar);
       · el monto es el de ese pago, nunca la cuota;
       · y dice la fecha, para que un mensaje viejo no se lea como de hoy. */
  {
    id: 'gracias_corto',
    label: 'Gracias por tu pago',
    desc: 'Confirmación corta sin saldo',
    icon: '\u{1f44d}', color: 'var(--color-success)',
    aplica: ({ prestamo, pago }) => !!prestamo && !!pagoParaAgradecer({ prestamo, pago }),
    getSecciones: null,
    generar: ({ cliente, prestamo, pago, orgNombre }) => {
      const p = pagoParaAgradecer({ prestamo, pago })
      const monto = p?.montoPagado || 0
      return `Hola ${cliente.nombre} \u{1f44b}\n\n✅ Tu pago de ${formatMoney(monto)} de hoy fue registrado correctamente.\n\n¡Gracias por tu puntualidad!\n\n${firma(orgNombre)} \u{1f4bc}`
    },
  },
  {
    id: 'credito_aprobado',
