 ...
 *
 * ⚠ ANTES SE LEÍA `prestamo.numeroCuota` A SECAS, y ese campo NO EXISTE en la
 * base: solo lo trae el préstamo cuando viene de un API que lo calcula. Desde
 * `RegistrarPago` llega el objeto crudo, así que el recibo decía «Cuota actual:
 * -». Reportado por un cliente nuevo, en pleno periodo de prueba: «este
 * comprobante no me muestra el número de cuotas pendientes y cuota cancelada».
 *
 * Se deriva de lo pagado, que es un dato que siempre está. Con tabla manda la
 * tabla; sin ella, cuántas cuotas cubre lo abonado.
 */
function numeroCuotaDe(prestamo) {
...
/* Cuánto lleva pagado, en porcentaje.
 *
 * Mismo caso: `prestamo.porcentajePagado` lo pone el API, y desde el recibo no
 * llega. El mensaje decía «Progreso: 0%» a quien acababa de pagar $140.000 de
 * $560.000 — un 25%. Se calcula de lo pagado sobre el total cuando no viene.
 */
function porcentajeDe(prestamo) {
...
function cuotasRestantesDe(prestamo) {
  const filas = prestamo?.cuotasAmortizacion
  if (Array.isArray(filas) && filas.length > 0) {
