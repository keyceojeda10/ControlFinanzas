...
function preciosConocidos(org, plan) {
  ...
  /* Con adicionales, el pago trae el plan MÁS ellos: $98.000 es Crecimiento
     con un cobrador, no un precio raro «por revisar». Los de sin adicionales
     se quedan: el panel puede apuntar solo el plan. */
...
export function montoDelCobro({ org, plan, pagada = null, ultima = null, ahora = new Date() }) {
  ...
  /* `monto` es lo que se cobra: el plan más sus adicionales. `montoPlan` es
     solo el plan, para enseñar el desglose. */
  const adicionales = adicionalesMensual(org, plan)
  ...
  const porRevisar = alMes < cobro.monto
  return { ...cobro, monto: porRevisar ? alMes : cobro.monto, porRevisar }
}
