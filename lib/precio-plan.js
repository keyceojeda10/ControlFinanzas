...
import { getPrecioPlan, PLANES_CONFIG } from '@/lib/planes'

export const MESES_PERIODO = { mensual: 1, trimestral: 3, anual: 12 }
 ...
 * cualquier periodo.
 */
function preciosConocidos(org, plan) {
  const country = org?.country ?? 'co'
  const lista = getPrecioPlan(plan, country)
  const precios = []
...
/** El precio conocido al que más se parece `monto`: dice qué compró. */
...
/** ¿`monto` es un precio que el sistema conoce para `plan`, salvo redondeo? */
export function pagoCuadra(org, plan, monto) {
  if (!(monto > 0) || !PLANES_CONFIG[plan]) return false
  return preciosConocidos(org, plan)
    .some(p => Math.abs(p.monto - monto) <= Math.max(1, p.monto * REDONDEO))
}
...
export function precioPorRevisar(org, pagada) {
  ...
  if (pagoCuadra(org, pagada.plan, pagada.montoCOP)) return false
  if (pagada.montoCOP > precioMasParecido(org, pagada.plan, pagada.montoCOP).monto) return false
 ...
 * @param {object} [p.pagada] el último pago `{plan, montoCOP}`
...
export function montoDelCobro({ org, plan, pagada = null, ultima = null, ahora = new Date() }) {
  ...
  if (!precioPorRevisar(org, pagada) || pagada.plan !== plan) return { ...mes, porRevisar: false, inicio }
  /* Lo que pagó, al mes: $90.000 de un trimestre de Inicial son $30.000. Solo
     cuenta si baja el cobro; si no, no hay nada que decidir. */
  const alMes = Math.round(pagada.montoCOP / precioMasParecido(org, plan, pagada.montoCOP).meses)
  const porRevisar = alMes < mes.monto
  return { ...mes, monto: porRevisar ? alMes : mes.monto, porRevisar, inicio }
}
  ...
  return {
    ...
    proximoCobro: {
      ...
      /* Pagando a mano no se mira el último pago: sale lo que cobra el checkout
         (`precioPeriodo`), la lista o el preferencial vigente. El inicio decía
         «a mano, $39.000» y debajo «$25.000 · paga a mano». */
      aMano: precioPeriodo(org, plan, 'mensual', cobro.inicio).total,
    },
  }
}
