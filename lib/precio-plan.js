...
/**
 * Lo que un pago apuntado A MANO (el panel) trae de adicionales. Si el monto
 * es un precio del plan, nada; si el monto MENOS los adicionales de hoy por
 * los meses del periodo lo es, eso. Si no cuadra de ninguna forma, nada: lo
 * decide el admin con «por revisar».
 *
 * ⚠ Sin esto, un trimestre de Crecimiento con 2 cobradores apuntado a mano
 * ($327.300) dejaba el cobro automático en $92.550 al mes (revisión del 24
 * sep 2026): «por revisar» lo tomaba entero por plan.
 */
export function adicionalesDelMonto(org, plan, periodo, monto) {
  const extras = adicionalesMensual(org, plan) * (MESES_PERIODO[periodo] ?? 1)
  if (!(extras > 0) || !(monto > extras)) return 0
  const precios = preciosConocidos(org, plan)
  if (cuadraCon(precios, monto)) return 0
  return cuadraCon(precios, monto - extras) ? extras : 0
}
...
export function resumenPrecio(org, { pagada = null, ultima = null, ahora = new Date() } = {}) {
  ...
  return {
    ...
    proximoCobro: {
      fecha: cobro.inicio,
      monto: cobro.monto,
      /* Solo el plan: el preferencial es precio del PLAN (los adicionales van
         aparte, siempre a lista). */
      montoPlan: cobro.montoPlan,
      lista: cobro.lista,
