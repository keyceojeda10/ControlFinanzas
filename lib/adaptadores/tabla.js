...
/**
 * La primera cuota que todavía no está cubierta. Es la que lleva la pastilla
 * «SIGUIENTE» y el anillo dorado.
 *
 * Se mira `pagado` Y `interesPagado`: en los modos de solo interés un pago de
 * tipo 'intereses' escribe `interesPagado` y deja `pagado` en 0, así que
 * mirando solo el primero la cuota seguiría contando como pendiente. Es el mismo
 * criterio que usa `calcularCuotasPendientes`, para que la tabla y el conteo de
 * la ficha no se contradigan.
 */
function indiceSiguiente(cuotas = []) {
  return cuotas.findIndex((c) => {
    if ((c.pagado || 0) >= c.cuotaTotal) return false
    if (c.interes > 0) return (c.interesPagado || 0) < c.interes
    return true
  })
}
...
export function adaptarTabla(p, pais) {
  ...
  const siguiente = indiceSiguiente(cuotasCrudas)
  ...
  const cuotas = cuotasCrudas.map((c, i) => ({
    ...
    pagada: (c.pagado || 0) >= (c.cuotaTotal || 0),
    ...
    faltanteNum: Math.max(0, Number(c.cuotaTotal ?? 0) - Number(c.pagado ?? 0)),
  }))
