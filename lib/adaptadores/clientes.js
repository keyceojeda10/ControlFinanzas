...
export function cifrasCliente(c, pais) {
  ...
  if (prox) cifras.push({ etiqueta: 'Cobra el', valor: prox, tono: prox === 'vencido' ? 'contra' : undefined })
...
export function proximoCobroTexto(c) {
  ...
  return sinPreposicion(d.toLocaleDateString('es-CO', { day: 'numeric', month: 'short' }).replace('.', ''))
}
