...
/**
 * "3 préstamos · Bolivariana · Cl 8 # 31-05" — cuántos debe, ruta y dónde vive.
 * Se omite la parte que falte en vez de dejar el separador colgando.
 *
 * EL CONTEO VA PRIMERO Y SOLO SI SON VARIOS. La tarjeta enseña "Deuda total",
 * que con tres préstamos es una suma: sin el número, un cliente con tres
 * créditos abiertos y uno con uno se ven idénticos, y son dos riesgos muy
 * distintos. Con uno solo no se escribe: un "1 préstamo" repetido en cada
 * tarjeta es el mismo ruido que el "0d" que ya se omite.
 */
export function contextoDe(cliente) {
  const n = Number(cliente?.prestamosActivos ?? 0)
  const cuantos = n > 1 ? `${n} préstamos` : null
  return [cuantos, cliente?.rutaNombre, cliente?.referencia].filter(Boolean).join(' · ') || null
}
