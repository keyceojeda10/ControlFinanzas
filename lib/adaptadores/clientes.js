...
export function adaptarClientes(clientes = [], pais) {
  return (clientes || []).map((c) => {
    ...
    return {
      ...
      /* ⚠ UN CLIENTE CON SU PRÉSTAMO PERDIDO SALÍA «AL DÍA · CUMPLE 100 %». El API
         excluye los clavos de la mora —ya se decidió que no se cobran—, así que
         a quien SOLO tiene clavos no le queda nada atrasado y la lista lo pintaba
         como su mejor cliente; la ruta, en cambio, decía «Clavo». `tieneClavo`
         venía del API desde siempre y este adaptador lo tiraba. Cazado el 19 sep
         2026. Con clavo sale la pastilla roja, y si es lo único que tiene, no
         se afirma además que va al día. */
      etiquetaEstado: c.soloClavos ? null : etiquetaDe(estado, c.diasMoraMax),
      clavo: !!c.tieneClavo,
      ...
      /* La estrella de cómo paga. La calcula el API con el historial cerrado;
         aquí solo se pasa, con su frase para el título — que es lo que se lee al
         mantener pulsado y lo que la hace entendible sin leer un manual. */
      calificacion: c.calificacion ?? null,
      tituloCalificacion: c.calificacion
        ? `${TEXTO_CALIFICACION[c.calificacion.nivel]} · ${c.calificacion.numero} préstamo${c.calificacion.numero === 1 ? '' : 's'} terminado${c.calificacion.numero === 1 ? '' : 's'}`
        : undefined,
      monto: formatMoney(saldo, pais),
      // «debe en 3 préstamos», bajo el monto. Era la mitad de la línea de
      // contexto; el turno 03 lo baja aquí y deja esa línea para identificar a
      // la persona. Con «debe» delante porque la cifra de arriba va sin rótulo y
      // en la lista de préstamos esa misma esquina enseña lo PAGADO.
      detalle: cuantosPrestamos(c) ? `debe en ${cuantosPrestamos(c)}` : null,
...
export function cifrasCliente(c, pais) {
  ...
  /* ⚠ SIN NADA ATRASADO DECÍA «LE FALTA $0», pegado a una deuda de $540.000
     (19 sep 2026). «Le falta» es también como la ficha llama a la DEUDA —«le
     falta pagar»—, así que la tira se leía «no debe nada». Con cero no hay que
     elegir palabra para lo que falta: no falta nada. Se dice «ATRASO · Nada»,
     en gris, igual que la ficha del préstamo. La columna no desaparece: que
     aparezca y desaparezca obliga a releer la tira en cada tarjeta. */
    if (!(mora > 0)) cifras.push({ clave: 'atraso', etiqueta: 'Atraso', valor: 'Nada', tono: 'apagado' })
    else cifras.push({ clave: 'atraso', etiqueta: Number(c?.diasMoraMax ?? 0) > 0 ? 'Atraso' : 'Le falta',
      valor: formatMoney(mora, pais), tono: 'contra' })
  }
