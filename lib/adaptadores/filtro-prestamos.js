...
export const CHIPS_DERIVADOS = ['mora', 'renovar', 'clavo', 'nuevos',
  'venceHoy', 'venceManana', 'vence2', 'vence3', 'vence5', 'vence7',
  'vence10', 'vence15', 'vence30']
 ...
 * @returns {Array}
 */
export function filtrarPrestamosGuardados(lista = [], { est = '', inicioHoy = null, desde = '', hasta = '' } = {}) {
  const hoy = inicioHoy ?? new Date(new Date().setHours(0, 0, 0, 0))

  /* ── EL RANGO A MANO, TAMBIÉN SIN CONEXIÓN ─────────────────────────────
   * Manda sobre `est` porque en la pantalla se apagan el uno al otro: si hay
   * fechas puestas, la ventana fija ya no lo está.
   *
   * ⚠ Sin esto, el cobrador sin señal escogía «del 3 al 17» y le salía la
   * cartera entera *aparentando que había filtrado*. Es el mismo fallo que ya
   * pasó con las cuatro ventanas nuevas y por el que existe este archivo. */
  if (desde || hasta) {
    /* ⚠ DÍAS DE CALENDARIO, NO HORAS DIVIDIDAS.
       Anclar la fecha al mediodía y dividir por 86.400.000 mete medio día de
       error, y el redondeo lo convierte en UNO: «desde mañana» se comía a los
       de mañana. Aquí se restan dos días del calendario y ya está — `hoy` es el
       arranque del día del país, así que su fecha en ISO es la de hoy allí. */
    const claveHoy = hoy.toISOString().slice(0, 10)
    const aDias = (f) => Math.round(
      (Date.parse(`${f}T00:00:00Z`) - Date.parse(`${claveHoy}T00:00:00Z`)) / 86400000)
    const d = desde ? aDias(desde) : 0
    const h = hasta ? aDias(hasta) : Infinity
    return lista
      .filter((p) => {
        if (p.estado !== 'activo' || (p.diasMora ?? 0) > 0 || !p.proximoCobro) return false
  ...
  if (CHIPS_DERIVADOS.includes(est)) {
    ...
    switch (est) {
      ...
      case 'vence30': {
        const [desde, hasta] = {
          venceHoy: [0, 0], venceManana: [1, 1], vence2: [2, 2], vence3: [3, 3],
          vence5: [0, 5], vence7: [0, 7], vence10: [0, 10], vence15: [0, 15],
          vence30: [0, 30],
        }[est]
        return vivos
          .filter((p) => {
