...
export function notaDelPanel(p, fmt = (n) => String(n)) {
  ...
  // Mora por encima de un tercio de la cartera de clientes.
  if (contexto.clientes > 0 && contexto.enMora / contexto.clientes > 0.33) {
    const p100 = Math.round((contexto.enMora / contexto.clientes) * 100)
    return `${contexto.enMora} de tus ${contexto.clientes} clientes están atrasados (${p100}%) y te deben ${fmt(contexto.saldoEnMora)}. Es a ellos a quienes hay que ir hoy.`
  }
