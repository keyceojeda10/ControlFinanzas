...
export function tramosDePlan(pais = 'co', formatear = String, cuantos = 3, perfil) {
  return escaleraPara(perfil)
    ...
    .map((t, i) => ({
      ...t,
      // DOS CRITERIOS, uno por pantalla, y los dos son correctos:
      //
      //   · Ya tiene plan (T10-01 «Plan Inicial · renueva el 11 ago»): el NOMBRE
      //     manda, porque es lo que compró. El techo baja a letra pequeña.
      //   · Todavía no tiene (T37-02, los precios son información): manda el
      //     TECHO, porque «Inicial» no le dice nada a quien aún no lo tiene —
      //     lo que compara es cuántos clientes le caben.
      //
      // Por eso salen los dos y la pantalla elige.
      texto: t.nombre,
      desbloquea: loQueDesbloquea(t.id),
      techo: `hasta ${t.limite.toLocaleString('es-CO')} clientes`,
      // «clientes» solo en el primero: repetirlo tres veces no aclara nada y
      // estrecha la columna del precio, que es lo que se está comparando.
      techoBreve: i === 0
