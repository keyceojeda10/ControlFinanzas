...
/* ── T45-04 · «Tu parte» ───────────────────────────────────────────────────
   La lámina dice para qué existe, y no es un adorno:
   ...
   Aquí NO se calcula ninguna ganancia: solo se resta lo que pusieron los socios
   de lo que hay en la calle. Es una cifra de capital, no de utilidad, y se dice
   así en la nota — confundir las dos es el error que ya infló las analíticas. */
export function tuParte({ capitalEnCalle, puestoPorSocios } = {}, formatear = String) {
  const enCalle = positivo(capitalEnCalle)
  const deSocios = positivo(puestoPorSocios)

  // Sin socios no hay nada que separar, y la tarjeta sobra.
  if (deSocios <= 0) return null

  // Puede pasar que los socios hayan puesto MÁS de lo que hay prestado ahora
  // mismo: parte de su plata está en caja, no en la calle. Decir «tu parte $0»
  // ahí sería mentir por redondeo, así que se dice lo que es.
  if (deSocios >= enCalle) {
    return {
      propio: formatear(0),
      enCalle: Math.round(enCalle),
      deSocios: Math.round(deSocios),
      nota: `Los socios pusieron ${formatear(Math.round(deSocios))} y hay ${formatear(Math.round(enCalle))} en la calle: todo lo prestado hoy sale de su capital. Lo que sobra está en caja.`,
    }
  }

  return {
    propio: formatear(Math.round(enCalle - deSocios)),
    enCalle: Math.round(enCalle),
    deSocios: Math.round(deSocios),
    nota: `Los socios pusieron ${formatear(Math.round(deSocios))} de los ${formatear(Math.round(enCalle))} que tienes en la calle. El resto es tuyo y su ganancia no se reparte.`,
  }
}
