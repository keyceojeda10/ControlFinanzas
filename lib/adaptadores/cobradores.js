...
export function agrupaCobradores(lista, formatear = (n) => String(n)) {
  ...
  return {
    ...
    // La frase entera va aqui, no partida entre adaptador y componente: con una
    // sola cuenta salia «1 cuenta sin ruta NO PUEDEN cobrar nada».
    aviso: sinRuta.length > 0
      ? {
        cuantas: sinRuta.length,
        texto: `${plural(sinRuta.length, 'cuenta')} sin ruta`,
        resto: sinRuta.length === 1
          ? 'no puede cobrar nada. Asígnale una o desactívala.'
          : 'no pueden cobrar nada. Asígnales una o desactívalas.',
      }
      : null,
  }
}
