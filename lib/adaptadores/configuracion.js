...
export function seccionesConfig({ rol = 'owner', cobradores = 0 } = {}) {
  ...
  return [
    ...
    // «Notificaciones» es de TODOS —el cobrador también recibe avisos— y va
    // aparte de WhatsApp: el interruptor de los avisos del teléfono vivía dentro
    // de «Avisos por WhatsApp», que es otra cosa, y casi nadie lo encontró.
    // SE LLAMA «NOTIFICACIONES». Se llamó «Qué te avisamos», y el dueño (20 sep
    // 2026): «no sé por qué manejamos esa terminología tan extraña… la nombramos
    // creyendo que se va a entender fácilmente pero realmente es mucho más
    // enredado». La palabra que todo el mundo busca en unos ajustes es ésta.
    { id: 'avisos',      nombre: 'Notificaciones',      visible: true },
...
export function valoresIndice({ org, uso, pais, cobradores = 0, diasParaRenovar } = {}) {
  ...
  if (uso?.clientes?.usado != null && uso?.clientes?.limite != null) {
    // «33 de 10000 clientes»: un número de cinco cifras sin punto se lee mal.
    const miles = (n) => Number(n).toLocaleString('es-CO')
    valores.clientesNota = `${miles(uso.clientes.usado)} de ${miles(uso.clientes.limite)} clientes`
  }
