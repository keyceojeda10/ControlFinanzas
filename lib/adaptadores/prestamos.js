...
import { DIAS_MORA, etiquetaDe, iniciales, esNuevo, proximoCobroTexto } from '@/lib/adaptadores/clientes'
...
export function cifrasDe(p, pais) {
  ...
  /* ── «COBRA EL», QUE ERA LA QUE FALTABA ──
     ...
     cada día. */
  const prox = proximoCobroTexto(p)
  if (prox) {
    cifras.push({
      etiqueta: 'Cobra el',
      valor: prox,
      tono: prox === 'vencido' ? 'contra' : prox === 'hoy' ? 'oro' : undefined,
    })
  }

  const fin = p?.fechaFin ? fechaCorta(p.fechaFin) : null
  /* Con las cuatro columnas puestas, «Vence» solo entra cuando NO hay próximo
     cobro que enseñar —un préstamo cerrado, o uno sin calendario— porque
     entonces la cuarta columna queda libre. Cinco columnas en 393px dan 74px
     cada una: ahí ya no cabe ni el rótulo. */
  if (fin && cifras.length < 4) cifras.push({ etiqueta: 'Vence', valor: fin })
