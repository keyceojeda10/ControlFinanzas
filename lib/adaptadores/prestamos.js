...
/**
 * «pagado de $1.200.000», bajo la cifra grande de la tarjeta.
 *
 * ⚠ DECÍA SOLO «de $1.200.000». La cifra de arriba es LO PAGADO —lo pidió el
 * dueño: un préstamo nuevo tiene que leerse «$0 de $1.800.000»— pero en la
 * lista de CLIENTES esa misma esquina enseña lo que DEBE. Dos listas, la misma
 * esquina, dos significados, y ninguna lo decía (19 sep 2026). Ahora cada una
 * lleva su palabra.
 */
export function totalDe(p, pais) {
  const total = Number(p?.totalAPagar ?? 0)
  return total > 0 ? `pagado de ${formatMoney(total, pais)}` : null
}
...
export function cifrasDe(p, pais) {
  ...
  /* ⚠ SIN NADA ATRASADO DECÍA «LE FALTA $0», pegado a una deuda de $540.000
    ...
    else cifras.push({ clave: 'atraso', etiqueta: Number(p?.diasMora ?? 0) > 0 ? 'Atraso' : 'Le falta',
