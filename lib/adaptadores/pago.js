 ...
 */
export function efectoSobreLaDeuda(tipo, monto, subeLaDeuda = false) {
  const m = Math.max(0, Number(monto) || 0)
  if (tipo === 'recargo') return +m
  // Pago de solo interés en un modo SIN tabla: entra la plata (baja la deuda) y
  // a la vez el interés es nuevo (la sube). Se anulan: el saldo NO se mueve.
  // Enseñarlo bajando sería mentir en la línea que el cobrador lee para decidir.
  if (tipo === 'intereses' && subeLaDeuda) return 0
  return -m
}
...
export function adaptarDespuesDelPago(p, entrada = {}, pais) {
  ...
  if (Number.isFinite(saldoAntes)) {
    // Lo decide quien llama: aquí no se puede mirar el modo del préstamo sin
    // arrastrar `lib/dinero/modos` a un adaptador que hoy no depende de nada.
    const subeLaDeuda = Boolean(entrada.interesSubeLaDeuda)
    const saldoDespues = Math.max(0, saldoAntes + efectoSobreLaDeuda(tipo, monto, subeLaDeuda))
    const sinMoverse = tipo === 'intereses' && subeLaDeuda && monto > 0
    filas.push({
      clave: 'saldo',
      ...
      //
      // Y cuando NO se mueve hay que decir por qué, o el cobrador ve el mismo
      // número dos veces y cree que la pantalla se colgó.
      nota: sinMoverse ? 'Igual: pagó interés, no capital'
        : saldoDespues === 0 && monto > 0 ? 'Queda saldado'
        : null,
    })
  }
...
/**
 * Los tres atajos de monto: «Cuota», «Al día», «Todo lo que debe».
 *
 * ⚠ ESTO SE APARTA DE LA LÁMINA A PROPÓSITO, Y ES LA ÚNICA VEZ.
 *
 * T02-04 dibuja «Cuota $12.000 · Mitad · Todo lo que debe». Los tres son suyos.
 * Pero el usuario —que es quien cobra— dijo que «hacen cosas muy locas» y pidió
 * cambiarlos por algo que funcione. Lleva razón, y se puede medir.
 *
 * Medido en un préstamo real —cuota $17.334, saldo $553.658—:
 *
 *  · «MITAD» SE VA. Era la mitad del SALDO: escribía $276.829. Eso no se cobra
 *    en ninguna visita de gota a gota. Un atajo que casi siempre produce una
 *    cifra absurda no ahorra tecleo — obliga a borrarlo. En su sitio va «AL
 *    DÍA», que responde la pregunta que sí se hace en la puerta cuando el
 *    cliente viene atrasado: cuánto para ponerse al corriente.
 *  · «TODO LO QUE DEBE» SE QUEDA, con su nombre de la lámina, pero deja de
 *    cobrar de más. Ponía el saldo entero, que lleva DENTRO todo el interés
 *    futuro; el sistema sabe liquidar anticipadamente y perdonarlo
 *    (`calcularLiquidacionAnticipada`) y el botón no lo usaba. Cuando hay
 *    interés que perdonar, el atajo pasa a llamarse «Cancelar hoy» y pone la
 *    cifra de la liquidación: el nombre cambia porque la cifra ya NO es todo lo
 *    que debe, es menos.
 *  · «CUOTA» estaba bien y no se toca.
 *
 * NINGUNO PASA DEL SALDO. Ofrecer «cuota $12.000» en un préstamo al que le
 * quedan $4.000 invita a cobrar de más, y la app tendría que rechazarlo después
 * de que el cliente ya entregó la plata.
 */
export function atajosDeMonto(p, pais) {
  ...
  // Lo que hay que cobrar para que deje de estar atrasado. Lo calcula el
  // servidor con los festivos de la organización; aquí solo se recibe.
  const alDia = Math.max(0, Math.round(Number(p?.montoAlDia ?? 0)))
  // Lo que paga HOY para cerrar, con el interés futuro ya perdonado. Viene de
  // /api/prestamos/[id]/liquidacion; si no llegó, el atajo no sale — es plata,
  // y un número inventado aquí es un cobro mal hecho.
  const cancelar = Math.max(0, Math.round(Number(p?.cancelarHoy ?? 0)))

  const atajos = []
  if (cuota > 0 && saldo > 0) {
    atajos.push({ id: 'cuota', etiqueta: 'Cuota', monto: Math.min(cuota, saldo) })
  }
  // Solo si de verdad debe MÁS que la cuota del día: si no, «Al día» y «Cuota»
  // serían el mismo botón con dos nombres.
  if (alDia > cuota && alDia <= saldo) {
    atajos.push({ id: 'aldia', etiqueta: 'Al día', monto: alDia })
  }
  // «Todo lo que debe» es el saldo. Si hay liquidación anticipada disponible
  // —o sea, si hay interés futuro que perdonar— se cobra ESA, y el botón se
  // llama distinto: poner «todo lo que debe» sobre una cifra MENOR que el saldo
  // sería mentir en el sitio donde más caro sale.
  if (cancelar > 0 && cancelar < saldo) {
    atajos.push({ id: 'cancelar', etiqueta: 'Cancelar hoy', monto: cancelar })
  } else if (saldo > 0) {
    atajos.push({ id: 'todo', etiqueta: 'Todo lo que debe', monto: saldo })
  }

  // Si dos atajos coinciden en el monto, se queda el primero: dos botones
  // idénticos con nombres distintos hacen dudar de cuál es el bueno.
...
export function mediosParaHoja(metodosPago = [], colorDe, limite = 4) {
  const cuentas = (Array.isArray(metodosPago) ? metodosPago : [])
    ...
    .map((m) => ({
      ...
      // Sigue siendo el RESPALDO: la cuenta que no tenga logo de marca dibujado
      // se queda con su letra de color, que es mejor que un icono genérico
      // repetido cuatro veces.
      inicial: String(m.nombre).trim().charAt(0).toUpperCase(),
      color: colorDe?.(m.nombre) || null,
      // El NOMBRE de la marca, para que quien pinta decida si tiene logo. Aquí
      // no se importa el componente del logo a propósito: este archivo compone
      // datos y no debe arrastrar UI (por eso `colorDe` se inyecta).
      plataforma: m.nombre,
      transferencia: true,
    }))
...
// ── EL MODO ABREVIADO ──────────────────────────────────────────────────────
//
// Con `modoAbreviado` el cobrador escribe en MILES: teclea «40» y se guardan
// $40.000. Es una preferencia del negocio (`Organization.modoAbreviado`) y para
// quien cobra todo el día en billetes de mil se ahorra tres teclas por cobro.
//
// ⚠ La hoja de cobro nueva NO lo aplicaba. `MoneyInput` —el campo viejo— sí lo
// hace desde siempre, pero el rediseño reemplazó ese campo por un `<input>`
// propio (el de 38px) y con él se perdió la conversión, en silencio: el modo
// seguía ENCENDIDO en configuración y no hacía nada. Un cobrador lo reportó
// creyendo que se le había desactivado solo.
//
// Va aquí y no en el componente para que la hoja de cobro, la de recargo y la
// de descuento lo hereden de un solo sitio.

/** Lo que se ESCRIBE → lo que se guarda. En abreviado, «40» son 40.000. */
export function montoCrudoConModo(texto, abreviado) {
  const crudo = montoCrudo(texto)
  if (!abreviado || !crudo) return crudo
  return String(Number(crudo) * 1000)
}

/** Lo que se guarda → lo que se VE en el campo. En abreviado, 40.000 son «40». */
export function montoParaMostrarConModo(texto, abreviado, pais) {
  if (!abreviado) return montoParaMostrar(texto, pais)
  const crudo = montoCrudo(texto)
  if (!crudo) return ''
  // Sin `Math.round`: 40.500 en abreviado se vería «41» y al guardar volvería
  // 41.000, cambiando la plata a espaldas de quien la escribió. Se deja el
  // valor tal cual cuando no es múltiplo de mil.
  const n = Number(crudo)
  if (!Number.isFinite(n) || n % 1000 !== 0) return montoParaMostrar(texto, pais)
  return montoParaMostrar(String(n / 1000), pais)
}
