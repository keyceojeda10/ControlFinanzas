...
export function fechaCortaDe(iso) {
  ...
  const d = new Date(iso)
  if (Number.isNaN(d.getTime())) return null
  return d.toLocaleDateString('es-CO', { day: 'numeric', month: 'short', timeZone: 'UTC' }).replace('.', '')
}
...
export function filaDeCobro(c, { pais, coords } = {}) {
  ...
  cobrada: !!c.pagoHoy && !c.cobroPendienteHoy,
  ...
  abonoHoy: c.pagoHoy && c.cobroPendienteHoy
  ...
  cifras: (c.pagoHoy && !c.cobroPendienteHoy) ? undefined : cifrasCobro(c, pais),
