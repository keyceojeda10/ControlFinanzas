...
function cifrasCobro(c, p) {
  const out = []
  /* ── AQUÍ EL NÚMERO NO ES EL ATRASO, Y POR ESO NO SE LLAMA ASÍ ─────────
     `montoParaPonerseAlDia` es, en sus propias palabras, «lo atrasado MÁS el
     cobro esperado al corte de hoy». O sea: lo que hay que cobrarle hoy para
     dejarlo al día. Es el número correcto para esta pantalla —es la pantalla
     de cobrar— pero llamarlo «Atraso» ponía «AL DÍA» y «ATRASO $29.000» en la
     misma ficha, siendo esos $29.000 la cuota de HOY.

     Y el rojo se reserva a quien va atrasado de verdad. Un cliente al día en su
     día de cobro no es una alarma: es el trabajo de la jornada. */
  const debeHoy = Number(c?.montoParaPonerseAlDia ?? 0)
  if (c?.montoParaPonerseAlDia != null) {
    out.push({ clave: 'atraso', etiqueta: 'Debe hoy', valor: formatMoney(debeHoy, p),
      tono: debeHoy > 0 && Number(c?.diasMora ?? 0) > 0 ? 'contra' : undefined })
  }
...
export function filaDeCobro(c, { pais, coords } = {}) {
  ...
  /* CÓMO HA PAGADO ANTES, al lado de cómo está hoy. La pastilla dice el
     préstamo de ahora; esto dice el historial, y son dos preguntas distintas:
     alguien puede llevar tres días de atraso y ser de los que siempre terminan.
     `null` en quien no ha cerrado ninguno — sin historial no hay marca. */
  ...
  /* ── EL AVISO DE MORA, EN UNA FRASE (E07) ──
     «Lleva 28 días de atraso. Debe $960.000 en total.»
     ...
     pero la frase sigue aportando: dice de una vez cuánto lleva de atraso y
