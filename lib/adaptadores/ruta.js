...
export function loDeHoy(ruta = {}, formatear = String) {
  ...
  /* ── PUERTAS, NO PAGOS ────────────────────────────────────────────────────
     «¿Cuántas me faltan?» es la pregunta que el cobrador se hace cada media
     hora, y hasta ahora la contestaba una línea gris de 12px perdida entre el
     efectivo y el digital: «1 de 5 cobros».

     Y contaba PAGOS, no visitas. Quien anotó «no estaba» ya no es una puerta
     por tocar —el botón «Empezar recorrido · N» lo descuenta desde que existe
     el cierre de visita—, pero este contador lo seguía contando como pendiente
     para siempre. Dos cifras del mismo día que no coincidían, que es el fallo
     clásico de esta casa.

     Así que el número de aquí sale del MISMO sitio que el del botón: los
     visitados son los que tocaban menos los que quedan por hacer. Si nadie
     manda `clientesVisitadosHoy` se cae a los que pagaron, que es lo que
     había. */
  const visitados = Math.min(
    cobros,
    ruta.clientesVisitadosHoy != null ? positivo(ruta.clientesVisitadosHoy) : pagaron,
  )
  ...
  // El resumen se queda SOLO con el desglose del medio de pago: el conteo del
  // día subió a su propio renglón, con su rótulo. Decirlo en los dos sitios es
  // cómo nacen las dos cifras que no coinciden.
  const partes = []
  if (hayDesglose) {
  ...
  return {
    ...
    resumen: partes.join(' · '),
    /* El contador del día. `total` a cero es un dato, no un hueco: un día sin
       cobros programados tiene que decirlo, no desaparecer. */
    visita: {
      hechas: visitados,
      total: cobros,
      texto: cobros > 0 ? `${visitados} de ${cobros}` : 'ninguna programada',
      completo: cobros > 0 && visitados >= cobros,
    },
    numeros: {
      ...
      visitados,
      porVisitar: Math.max(0, cobros - visitados),
    },
  }
}
...
/* Los cinco motivos de `VisitaReagendada`, en cristiano. Son los mismos que
   ofrece el «¿qué te dijo?» al cerrar una visita sin cobrar. */
const MOTIVO_CIERRE = {
  no_estaba: 'no estaba',
  negocio_cerrado: 'negocio cerrado',
  no_tenia_dinero: 'no tenía dinero',
  pidio_plazo: 'pidió plazo',
  pago_parcial: 'abonó lo que pudo',
  otro: 'otro motivo',
}
...
export function adaptarRecorrido(clientes = [], formatear = String, fmtHora) {
  return clientes
    ...
    .map((c, i) => {
      ...
      // Hecha pero sin plata: lo dice con las mismas palabras que el cierre del
      // día (`loQuePasoHoy`), motivo entre comillas incluido. Poner «cobrado»
      // aquí sería decir que entró dinero que no entró.
      const cerradaSinPago = Boolean(c.visitaCerrada) && !(positivo(c.montoCobrado) > 0)
      const abajo = cerradaSinPago
        ? ['no pagó', c.motivoCierre ? `“${MOTIVO_CIERRE[c.motivoCierre] ?? c.motivoCierre}”` : sitio]
        : cobrado
...
export function partirRecorrido(clientes = [], formatear = String, fmtHora) {
  ...
  /* LA VISITA CERRADA SIN PAGO ESTÁ HECHA, PERO NO PUSO UN PESO.
     Va con las hechas —no me falta pasar por ahí— y NO suma al total del grupo:
     ese total es el fajo de la noche, y sumarle la cuota de quien dijo «hoy no
     tengo» lo infla con dinero que nadie entregó. */
  const sinPagar = cobrados.filter((c) => c.visitaCerrada && !(positivo(c.montoCobrado) > 0))
  const total = cobrados
    .filter((c) => !sinPagar.includes(c))
    .reduce((t, c) => t + positivo(c.montoCobrado ?? c.montoACobrar ?? c.cuota), 0)

  return {
    actual,
    faltan: adaptarRecorrido(faltan, formatear, fmtHora),
    cobrados: adaptarRecorrido(cobrados, formatear, fmtHora),
    // El rótulo dice la verdad del grupo: «cobrados» solo cuando todos pagaron.
    cobradosTitulo: cobrados.length
      ? `${sinPagar.length > 0 ? 'Ya hechos' : 'Ya cobrados'} · ${cobrados.length}`
      : null,
    cobradosTotal: cobrados.length ? formatear(Math.round(total)) : null,
    /* «parada 7» — dónde vas en el orden. El CUÁNTAS LLEVAS lo dice el contador
       de la banda, justo debajo: cuando aquí ponía «parada 7 de 19» y la banda
       «6 de 19», eran dos cifras distintas del mismo día a 20px una de otra, que
       es de donde salen las preguntas. */
    posicion: actual
      ? `parada ${actual.orden ?? cobrados.length + 1}`
      : 'ruta terminada',
  }
}
...
/**
 * El largo de la RUTA ENTERA, para la cabecera — y solo si es creíble.
 *
 * Con un cliente fijado por error en otra ciudad la cabecera decía «9 clientes ·
 * 4497,1 km» (19 sep 2026): una ruta que se cobra a pie o en moto no mide eso, y
 * un número así le quita crédito a todos los demás. Por encima de 300 km se
 * calla. El dato no se pierde —el mapa enseña dónde está el punto raro—; lo que
 * no se hace es presentarlo como el tamaño de la ruta.
 */
export const RUTA_KM_CREIBLES = 300
export function kmDeLaRuta(metros) {
  const m = Number(metros)
  if (!Number.isFinite(m) || m > RUTA_KM_CREIBLES * 1000) return null
  return formatearKm(m)
}
...
/* ══ LAS PARADAS DE LA RUTA (Adenda 5 · E07, E08, E09) ═══════════════════════
   ...
   ── ⚠ EL REPARTO EN DOS ZONAS, REVOCADO POR EL DUEÑO ──

   Aquí decía: «arriba las VISITAS, que son las únicas que llevan número; abajo
   el resto», con la regla de la lámina detrás — «un contador que incluye
   paradas que no se hacen es peor que no tener contador», porque el cobrador
   lee 16, hace 10 y se cree atrasado yendo al día.

   El dueño lo rebatió con la pantalla delante, y tiene razón sobre lo que él
   ve todos los días:

     «en esa lista de cobros salían absolutamente todos los clientes
      enumerados. Así el primero fuera uno que estuviese con clavo, así en el
      dos estuviese un cliente que no tenía préstamo, no importa, porque eso
      era relevante para el cobrador o para el administrador. […] Ahora están
      saliendo hasta abajo, sin ninguna numeración, sin ningún dato de sus
      préstamos, sin ningún contexto, nada.»

   Y el número que él quiere NO es el que la lámina discute. La lámina habla
   del CONTADOR —«te quedan 16 cobros»—; él habla de la POSICIÓN EN LA RUTA,
   que es por dónde va caminando y con la que se orienta entre 142 clientes. Son
   dos cosas distintas y las dos pueden ser ciertas a la vez:

     · la fila lleva su POSICIÓN, todos, sin excepción  → lo de aquí
     · el contador de «Empezar recorrido · 67» sigue contando SOLO los cobros
       pendientes de hoy → intacto, y por eso la regla de la lámina se respeta
       donde de verdad importaba

   Así que ya no hay dos zonas: hay UNA lista con todos, cada uno con su
   tarjeta completa y su acento. Agrupar sigue existiendo, pero como VISTA
   —el segundo icono—, no como un fondo de saco. */
...
/* Por debajo de esto, «le prestas hasta X» no informa: informa de un dato
   roto. Salió «le prestas hasta $1» en una ruta real. */
const TECHO_MINIMO = 1000

export function paradasDeRuta(clientes = [], { pais, formatear = String } = {}) {
  ...
  const filas = lista.map((c, i) => {
    ...
    // «¿Me falta pasar por aquí hoy?» es la única pregunta que decide si es
    // visita. Quien YA pagó sigue siéndolo: el recorrido se camina en orden y
    ...
    const esHoy = pendiente || Boolean(c.pagoHoy)
    ...
    const zona = esHoy ? 'hoy' : zonaDe(c)
    const fila = filaDeCobro(crudo, { pais })

    /* ── ⚠ EL CLAVO NO SE DICE TRES VECES ──
       Salió en el espejo, no en las medidas: la tarjeta de un clavo llevaba la
       pastilla roja «Clavo», el aviso rojo «Tiene un préstamo dado por perdido»
       Y la frase «Préstamo dado por perdido…». Tres portadores del mismo dato
       en una tarjeta que ya tiene el anillo del avatar en rojo — que es
       exactamente lo que E10 le reprocha al riel: «el cuarto sitio donde se
       dice lo mismo».

       El aviso se queda donde SÍ aporta: en el cliente que tiene préstamos
       vivos Y además un clavo. Ahí la pastilla dice «Al día» y sin el aviso el
       cobrador no se entera. */
    const avisos = zona === 'clavo'
      ? fila.avisos.filter((a) => !/dado por perdido/i.test(a.texto))
      : fila.avisos
    ...
    return {
      ...
      avisos,
      zona,
      // ⚠ LA POSICIÓN ES LA DE LA RUTA, la misma con la que se camina, y la
      // llevan TODOS. Antes era el índice entre las visitas, así que el número
      // 7 de hoy podía ser el 23 de la ruta y el cobrador no encontraba a nadie
      // por número.
      orden: i + 1,
      ultima: i === lista.length - 1,
      // Lo propio de quien hoy no tiene cobro. `null` en las visitas.
      contexto: esHoy ? null : contextoZona(crudo, { pais, formatear }),
    }
  })

  // `visitas` y `tambien` siguen saliendo: los contadores del día —«Empezar
  // recorrido · 67», «paradas por hacer», el cierre— cuentan COBROS, no
  // clientes, y ahí la regla de la lámina sigue en pie.
  return {
    filas,
    visitas: filas.filter((f) => f.zona === 'hoy'),
    tambien: filas.filter((f) => f.zona !== 'hoy'),
  }
}

/* ══ LO QUE DICE LA TARJETA DE QUIEN HOY NO TIENE COBRO ══════════════════════

   El dueño: «no que si está al día ponga solamente "un préstamo, cobrar antes",
   no. Que salga que está el préstamo, cuánto es el valor del préstamo, y
   normal, como está la tarjeta de los que se cobran ese día».

   Así que la tarjeta es LA MISMA —`FilaCobro`, con su tira de cifras, sus
   préstamos plegados y sus acciones— y esto es lo único que cambia: la pastilla
   que dice en qué situación está, la frase que explica por qué hoy no le toca,
   y qué hace el botón grande, que no puede decir «Cobrar» en los cuatro casos.

   Las cifras se rehacen en los tres estados SIN préstamo vivo: ahí «Atraso $0 ·
   Cumple — · Últ. pago» son cuatro columnas que no dicen nada de alguien que no
   debe. Se sustituyen por lo que sí se decide con esa persona. */
export function contextoZona(c = {}, { pais, formatear = String } = {}) {
  const zona = zonaDe(c)
  const plata = (n) => formatear(Math.round(positivo(n)))

  if (zona === 'aldia') {
    // Los vivos, no todos: contar el clavo diría «2 préstamos» de alguien que
    // solo tiene uno que se le pueda cobrar.
    ...
    /* Sin fecha no se inventa una: se dice lo que se sabe. Una fecha de cobro
       ...
       Y la FECHA manda sobre los días: «cobra en 13d» deja al cobrador contando
       con los dedos; lo que se le dice al cliente en la puerta es «el 19 de
       agosto». Los días van al lado, nunca en lugar de ella. */
    const cuando = !fecha
    ...
    return {
      zona,
      pastilla: { texto: 'Al día', tono: 'aldia' },
      nota: cuando,
      // Las de siempre: al día SÍ tiene préstamo, así que el atraso, el
      // cumplimiento y la cuota son ciertos, y son los que deciden si conviene
      // renovarle. `null` = las que ya trae la fila.
      cifras: null,
      // Y el monto también: su cuota arriba y «debe $149.000» debajo, igual que
      // una visita de hoy. Es literalmente lo que se pidió.
      monto: 'defecto',
      // «Cobrar antes» va secundario a propósito: cobrarle hoy es un adelanto,
      // no la cuota del día.
      accion: { texto: 'Cobrar antes', tono: 'apagado' },
      cuantos,
    }
  }

  if (zona === 'clavo') {
    const ult = fechaLargaDe(c.ultimoPagoAt)
    return {
      zona,
      pastilla: { texto: 'Clavo', tono: 'mora' },
      nota: 'Préstamo dado por perdido. Lo que entre es recuperación.',
      cifras: [
        { etiqueta: 'Debe', valor: plata(c.saldoTotal), tono: positivo(c.saldoTotal) > 0 ? 'contra' : undefined },
        { etiqueta: 'Últ. pago', valor: ult ?? 'nunca', tono: ult ? undefined : 'contra' },
      ],
      /* La cuota de un clavo es $0 —ya no se amortiza—, así que el hueco de la
         derecha lleva el SALDO. Un «$0» grande sobre alguien que debe
         $900.000 es la peor cifra que puede tener esa tarjeta. */
      monto: { cifra: plata(c.saldoTotal), pie: 'perdido' },
      // Sigue siendo cobrable: un clavo se da por perdido en la contabilidad,
      // no en la puerta. Pero no en dorado — el dorado es la cuota del día.
      accion: { texto: 'Cobrar', tono: 'apagado' },
    }
  }
  ...
  if (zona === 'sindeuda') {
    ...
    return {
      zona,
      pastilla: { texto: 'Sin préstamo', tono: 'destacado' },
      // El singular entero, no solo la «s»: «pagó completos sus 1 préstamo» es
      // lo que salía, y se leyó en la captura antes que en el código.
      nota: cuantos === 1
        ? `Terminó de pagar el ${fechaLargaDe(c.terminoDePagar)}. Pagó completo su préstamo.`
        : `Terminó de pagar el ${fechaLargaDe(c.terminoDePagar)}. Pagó completos sus ${cuantos} préstamos.`,
      cifras: [
        { etiqueta: 'Pagados', valor: String(cuantos), tono: cuantos > 0 ? 'favor' : undefined },
        /* El techo solo si se sabe Y si dice algo. «Le puedes prestar hasta
           $0» es peor que callarse —el cliente cuyo único préstamo cerrado no
           tiene monto—, y con `> 0` a secas se colaba «le prestas hasta $1»,
           que salió en la captura de la RUTA #2 y se lee como una avería.
           Mil pesos es el suelo por debajo del cual la cifra no informa. */
        ...(techo >= TECHO_MINIMO ? [{ etiqueta: 'Le prestas hasta', valor: plata(techo), tono: 'oro' }] : []),
      ],
      // Nada a la derecha: no debe nada, y un «$0» se lee como una cifra
      // averiada, no como «está limpio».
      monto: 'ninguno',
      // DORADO: es la única de las cuatro que gana dinero. «Se puede retirar»
      // —lo que decía la pantalla vieja— es el botón contrario.
      accion: { texto: 'Prestarle', tono: 'oro' },
    }
  }
  ...
  return {
    zona: 'inactivo',
    pastilla: { texto: 'Sin préstamo', tono: 'neutro' },
    nota: desde ? `Sin préstamos desde el ${desde}.` : 'Nunca ha tenido un préstamo.',
    cifras: positivo(c.prestamosCompletados) > 0
      ? [{ etiqueta: 'Pagados', valor: String(positivo(c.prestamosCompletados)) }]
      : null,
    monto: 'ninguno',
    // SACARLO DE LA RUTA, no «retirar al cliente»: no se va él, se limpia la
    // ruta. Y solo aquí — al que acaba de pagar hay que prestarle.
    accion: { texto: 'Sacar de la ruta', tono: 'apagado' },
  }
}

/* Las secciones de la vista agrupada, en el orden del día del cobrador.
   Se calculan sobre las filas YA numeradas: agrupar no renumera, porque el
   número es la posición en la ruta y esa no cambia por mirarla de otra forma. */
export const GRUPOS_RUTA = [
  { clave: 'hoy',      titulo: 'Por cobrar hoy',     color: 'var(--cf-gold-dark)' },
  { clave: 'hechas',   titulo: 'Visitas hechas hoy', color: 'var(--cf-green-dark)' },
  { clave: 'aldia',    titulo: 'Al día',             color: 'var(--cf-green-dark)' },
  { clave: 'clavo',    titulo: 'Préstamos perdidos', color: 'var(--cf-red-dark)' },
  { clave: 'sindeuda', titulo: 'Listos para prestarles' },
  { clave: 'inactivo', titulo: 'Sin préstamo' },
]

export function gruposDeRuta(filas = []) {
  const de = (f) => (f.zona === 'hoy' ? (f.cobrada ? 'hechas' : 'hoy') : f.zona)
  return GRUPOS_RUTA
    .map((g) => ({ ...g, filas: filas.filter((f) => de(f) === g.clave) }))
...
export function zonaDe(c = {}) {
  ...
  const activos = c.prestamosActivos ?? []
  const vivos = activos.filter((p) => !p?.esClavo)
  if (vivos.length > 0) return 'aldia'

  /* ── EL CLAVO, CON NOMBRE PROPIO ──
     Antes caía en `sindeuda` o en `inactivo` según la fecha, o sea que un
     préstamo dado por perdido se leía como «terminó de pagar, préstale más» o
     como «esto es una ruta vieja, sácalo». Las dos son falsas y la primera es
     cara: es justo al cliente al que NO hay que volver a prestarle.

     El dueño lo pidió por su nombre: «el que esté clavo, que se distinga que es
     clavo». */
  if (activos.some((p) => p?.esClavo) || c.tieneClavo) return 'clavo'
...
/* ⚠ AQUÍ VIVÍA `filaZonaDe`, y se fue con `FilaFueraDeParada`.
   Componía las frases de las tarjetas reducidas del fondo de la lista. Ahora
   esas tarjetas son la tarjeta completa y las frases las compone
   `contextoZona`, arriba. Dejarla habría sido tener las mismas cadenas escritas
   dos veces para que se separaran sin que nadie lo notara — que es el fallo del
   comprobante, reportado dos días seguidos. */
