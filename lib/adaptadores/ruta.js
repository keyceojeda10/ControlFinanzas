...
export function zonaDe(c = {}) {
  if ((c.prestamosActivos ?? []).length > 0) return 'aldia'
  const dias = aNumero(c.diasDesdeUltimoPago)
  if (!Number.isNaN(dias) && dias > DIAS_INACTIVO) return 'inactivo'
  // Sin préstamo activo y sin fecha de nada no hay historia que contar, así que
  // es una fila vieja y no una oportunidad: ofrecerle plata a alguien de quien
  // no se sabe nada es lo contrario de lo que la lámina pide.
...
export function filaZonaDe(c = {}, { pais, formatear = String } = {}) {
  ...
  if (estado === 'aldia') {
    const cuantos = (c.prestamosActivos ?? []).length
