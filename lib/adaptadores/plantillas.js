...
export function enlaceWhatsApp(telefono, mensaje, country) {
  /* ⚠ ESTO PEGABA UN «57» A MANO —el indicativo de Colombia— y encima con una
     regla propia: `digitos.length > 10 ? digitos : '57' + digitos`.
     A un prestamista de Argentina le salía «+573625325911 no es un número de
     teléfono válido», y con el indicativo correcto le habría salido igual: al
     móvil argentino le falta además el 9. Ver `telefonoParaWhatsApp`.

     El país por defecto NO es Colombia: es el de la organización, que el panel
     deja puesto al arrancar. Medido contra los 5.841 teléfonos de producción:
     los 5.721 que armaban enlace lo siguen armando, cero pérdidas. */
  const numero = telefonoParaWhatsApp(telefono, country)
