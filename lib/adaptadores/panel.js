...
export function filasAtencion(alertas) {
  ...
  if (mora30 > 0) {
    filas.push({
      ...
      monto: alertas?.mora30Monto ?? null,
      tono: 'mora',
  ...
  if (sinPagos > 0) {
    filas.push({
      ...
      monto: alertas?.sinPagosMonto ?? null,
      tono: 'atraso',
  ...
  if (renovar > 0) {
    filas.push({
      ...
      monto: alertas?.renovarMonto ?? null,
      tono: 'ok',
...
export function adaptarPanel(datos, { nombre = '', pais, hora, clientesHoy = 0 } = {}) {
  ...
  // CLIENTES que tocaban hoy y ya pagaron, no PAGOS del día: el denominador son
  // clientes, y «3 de 15» con tres pagos de los que uno no tocaba hoy no cuadraba
  // con el «2 de 15» que suman las rutas. `cantidadHoy` queda de respaldo para
  // una respuesta vieja en caché.
  const cobrados = Number(p?.clientesCobradosHoy ?? c?.cantidadHoy ?? 0)
  ...
  return {
    ...
    hero: {
      ...
      cobrados,
      // Hoy no le toca a nadie: o el negocio cierra hoy (domingo, festivo) o no
      // hay un solo préstamo con cuota hoy. Sirve para decir por qué la barra
      // está en cero en vez de dejar un 0 % que se lee como «vas fatal».
      sinCobrosHoy: esperado === 0 && clientesHoy === 0,
      ...
      // LAS SIETE BARRAS DE LA SEMANA (T02-07). El endpoint ya las calculaba
      // —`sparkline7d`, de mas viejo a mas reciente, hoy el ultimo— y no las
      // pintaba nadie. Contestan la pregunta que la cifra sola no contesta:
      // «¿hoy es un buen dia o es un dia normal?».
      semana: Array.isArray(c?.sparkline7d) && c.sparkline7d.some((n) => n > 0)
        ? c.sparkline7d.map((n) => Number(n) || 0)
        : null,
      // Y cuántos cobros fueron, para la barra que se toca.
      cobrosSemana: Array.isArray(c?.cobros7d) ? c.cobros7d.map((n) => Number(n) || 0) : null,
      // La fecha de cada barra: el nombre del día lo sabe el servidor, así no
      // hay un primer pintado con los días en blanco.
      fechasSemana: Array.isArray(c?.dias7d) ? c.dias7d : null,

      /* ── LO QUE PIDE LA ADENDA 4 ──────────────────────────────────────
         La tira de cifras y la línea de la gráfica. Todo sale de datos que
         ya venían; lo único que faltaba era calcularlos y pasarlos. */

      // «Te faltan». La adenda es explícita: $378.167 = $626.167 − $248.000,
      // «la cifra tiene que cuadrar». Por eso se resta aquí, con los números,
      // y no se reconstruye luego desde los textos ya formateados.
      faltan: esperado > recaudado ? formatMoney(esperado - recaudado, pais) : null,

      // En mora ya se calculaba para la tarjeta blanca de al lado; en 1440 la
      // tira tiene sitio para una quinta columna y esta es la que falta para
      // que $248.000 sea un dato juzgable.
      enMora: cl?.enMora ?? 0,

      // El promedio de los siete días, para comparar el día de hoy contra lo
      // normal del negocio y no contra la nada.
      promedio7d: (() => {
        const s = Array.isArray(c?.sparkline7d) ? c.sparkline7d.map((n) => Number(n) || 0) : []
        if (!s.length) return null
        return formatMoney(Math.round(s.reduce((a, b) => a + b, 0) / s.length), pais)
      })(),

      /* La línea punteada de la gráfica: lo que toca cobrar. Va EN CRUDO —sin
         formatear— porque la gráfica tiene que compararla contra cada barra
         para saber cuáles la pasan.

         ⚠ Y ES LA DE HOY APLICADA A LOS SIETE DÍAS. El endpoint manda
         `esperadoHoy`, no el esperado de cada día de la semana pasada, así que
         la línea asume que lo que toca cobrar es parecido cada día. En una
         cartera diaria lo es casi siempre, pero NO es un dato exacto: si algún
         día se ve raro, la causa es ésta y no la gráfica. */
      esperadoCrudo: esperado > 0 ? esperado : null,
    },
    ...
    // En negativo «para prestar ahora» no dice nada: ver `Panel`.
    cajaNegativa: f ? Number(f.cajaDisponible) < 0 : false,
    mora: {
      cuantos: cl?.enMora ?? 0,
