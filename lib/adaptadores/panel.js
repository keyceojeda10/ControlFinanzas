...
export function filasAtencion(alertas, clientesEnMora = 0) {
  ...
  if (clientesEnMora > 0) {
    filas.push({
      titulo: `${clientesEnMora} cliente${clientesEnMora === 1 ? '' : 's'} en mora`,
      nota: 'Llevan días sin pagar',
      tono: 'mal',
  ...
  if (sinPagos > 0) {
    filas.push({
      titulo: `${sinPagos} sin pagar hace rato`,
      nota: 'Más de 15 días sin un abono',
      tono: 'mal',
      destino: '/prestamos?sinPagosDias=15',
    })
  }
  ...
  if (sinRuta > 0) {
    filas.push({
      ...
      titulo: `${sinRuta} cliente${sinRuta === 1 ? '' : 's'} sin ruta`,
      nota: 'Nadie los tiene asignados',
      tono: 'ambar',
