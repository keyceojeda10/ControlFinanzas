...
export function adaptarPlazo(p, cuotasQueFaltan, pais) {
  ...
  // faltaban.
  const finAntes = p?.fechaFin ? new Date(p.fechaFin) : null
...
export function adaptarAplazar(p, fechaNueva, cobrasHoyAntes, cobrasHoyDespues, pais) {
  const actual = p?.proximoCobro ? new Date(p.proximoCobro) : null
  const nueva = fechaNueva ? new Date(fechaNueva) : null
  if (!nueva || Number.isNaN(nueva.getTime())) return null
...
export function diasDeCobro(p, diasSinCobroCrudo) {
  const apagados = new Set(
    String(diasSinCobroCrudo ?? '')
      .split(',')
      // El `filter` de vacíos va ANTES del `Number`: `Number('')` es 0, así que una
      // cadena vacía apagaba el domingo en todas las organizaciones que no tienen
      // días sin cobro configurados.
      .map((x) => String(x).trim())
      .filter(Boolean)
      .map(aNumero)
...
export function adaptarCorregir(p, pais) {
  ...
  const recalcula = pagos > 0
    ? `Recalcula ${pagos} ${pagos === 1 ? 'pago' : 'pagos'}`
  ...
  return {
    aviso: pagos > 0
      ? `Ya hay ${pagos} ${pagos === 1 ? 'pago' : 'pagos'} registrados: cambiar el monto o el interés recalcula todo hacia atrás.`
      : 'Todavía no hay pagos registrados, así que corregir es seguro.',
    peligrosos: [
      ...
      Number.isFinite(tasa)
        ...
        : null,
      p?.fechaInicio
        ? { clave: 'inicio', etiqueta: 'Fecha de inicio', valor: fechaLargaConAno(p.fechaInicio), consecuencia: 'Mueve las fechas', texto: true }
        : null,
    ].filter(Boolean),
