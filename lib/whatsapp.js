...
function etiquetasFrecuencia(freq) {
  ...
  if (f === 'semanal')   return { cuota: 'Cuota semanal',   pago: 'pago semanal',   plazo: 'semanas' }
  if (f === 'quincenal') return { cuota: 'Cuota quincenal', pago: 'pago quincenal', plazo: 'quincenas' }
  if (f === 'mensual')   return { cuota: 'Cuota mensual',   pago: 'pago mensual',   plazo: 'meses' }
  return { cuota: 'Cuota diaria', pago: 'pago diario', plazo: 'dias' }
}
...
// Alias retrocompatible — asume Colombia. Codigo nuevo debe usar formatearTelefonoIntl de lib/i18n.js.
...
📅 ${etq.cuota}: ${formatCOP(prestamo.cuotaDiaria)}
...
⏳ Saldo pendiente: ${formatCOP(saldoPendiente)}
📆 ${etq.cuota}: ${formatCOP(cuotaDiaria)}
...
Llevamos ${prestamo.diasMora} días sin recibir tu ${etq.pago} de ${formatCOP(prestamo.cuotaDiaria)}.
...
export function generarTextoRecordatorio(cliente, prestamo) {
  const etq = etiquetasFrecuencia(prestamo?.frecuencia)
  const saldo = Number(prestamo?.saldoPendiente ?? 0)
  return `Hola ${cliente.nombre} 👋

Te recordamos amablemente tu próximo ${etq.pago} de ${formatCOP(prestamo?.cuotaDiaria || 0)}.

📋 *Resumen:*
💰 Saldo pendiente: ${formatCOP(saldo)}
📊 Cuotas pendientes: ${calcularCuotasPendientes(prestamo)}
...
export function generarTextoFelicitacion(cliente, prestamo) {
  const cuotasPagadas = calcularCuotasPagadas(prestamo)
  return `¡Hola ${cliente.nombre}! 🎉
...
export function generarTextoRenovacion(cliente) {
  return `Hola ${cliente.nombre} 👋
...
export function generarTextoVisita(cliente) {
  return `Hola ${cliente.nombre} 👋
...
export function generarTextoComprobantePedido(cliente) {
  return `Hola ${cliente.nombre} 👋
