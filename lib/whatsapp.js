 ...
 */
export function generarTextoComprobante(cliente, prestamo, pago, { orgNombre } = {}) {
 ...
 */
export function generarTextoHistorialCredito(cliente, prestamo, { orgNombre } = {}) {
 ...
 */
export function generarEnlaceMora(cliente, prestamo, { orgNombre } = {}) {
  const tel = formatearTelefono(cliente?.telefono)
  if (!tel) return null
...
export function generarTextoRecordatorio(cliente, prestamo, { orgNombre } = {}) {
  const etq = etiquetasFrecuencia(prestamo?.frecuencia)
