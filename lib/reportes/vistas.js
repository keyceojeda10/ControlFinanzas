...
/* ── LOS VOLCADOS, QUE YA NO SON UN APARTE ───────────────────────────────────
 *
 * «Los reportes de bajar hay que unificarlos en los nuevos: cada reporte con su
 * pantalla individual, con sus filtros y con sus dos formatos.» Eran cinco
 * Excel que se bajaban a ciegas desde `/reportes/bajar`.
 *
 * ⚠ ESTA VISTA NO DESCRIBE NADA: pasa lo que llega. Las columnas vienen del API
 *   —que las toma de `lib/reportes/columnas-crudas.js`, el mismo sitio del que
 *   salen las hojas del Excel— porque describirlas otra vez aquí sería el
 *   segundo sitio donde vive la lista, y con eso basta para que la pantalla
 *   enseñe una columna menos que el archivo.
 *
 * La cifra de arriba es el conteo, que es lo que de verdad se quiere saber
 * antes de bajar un volcado: cuántas filas van a salir. */
function vistaVolcado(rotuloConteo) {
  return (d) => {
    if (!d?.columnas) return vacia
    const cifras = [{ etiqueta: rotuloConteo, valor: d.total ?? (d.filas?.length ?? 0), tipo: 'numero' }]
    /* Solo «pagos» se filtra por fechas. Decirlo evita que alguien crea que
       acotó una foto que no se acota. */
    const nota = d.periodo
      ? null
      : 'Es la foto de hoy: este volcado no se filtra por fechas.'
    return { cifras, tabla: { columnas: d.columnas, filas: d.filas ?? [] }, nota }
  }
}
...
const VISTAS = {
  cartera: vistaVolcado('Préstamos en la foto'),
  'volcado-clientes': vistaVolcado('Clientes'),
  'volcado-pagos': vistaVolcado('Pagos en el período'),
  'volcado-cobradores': vistaVolcado('Cobradores'),
  entro: vistaEntro,
  calle: vistaCalle,
