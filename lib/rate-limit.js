...
/* ⚠ DOS LIMITES PARA EL REGISTRO, Y NO ES UN CAPRICHO.
 *
 * Habia uno solo —3 por IP y hora— y se consumia en CADA intento, antes de
 * validar nada. O sea que quien escribia mal el celular dos veces y la
 * contraseña una quedaba fuera UNA HORA sin haber creado ninguna cuenta. Me
 * pasó recorriendo el asistente: segundo intento, 429.
 *
 * Y en este mercado mucha gente sale por la IP del operador, asi que tres
 * vecinos registrandose la misma tarde bastan para bloquear al cuarto.
 *
 * Lo que hay que frenar es CREAR CUENTAS EN MASA, no equivocarse escribiendo:
 *
 *   · `registroLimiter`  — 3 cuentas CREADAS por IP y hora. Se consume solo
 *                          cuando la cuenta existe de verdad.
 *   · `registroIntentos` — 30 golpes al endpoint por IP y hora. Frena el
 *                          martilleo sin castigar al que se equivoca.
 */
export const registroLimiter = rateLimit('registro', 3, 60 * 60 * 1000)   // 3 CUENTAS CREADAS / hora
export const registroIntentos = rateLimit('registro-intentos', 30, 60 * 60 * 1000) // 30 golpes / hora
