...
function getColombiaDate() { return getLocalDate('co') }
...
export async function buildContexto(orgId) {
  ...
  for (const p of prestamosActivos) {
    ...
    if (calcularDiasMora(p, diasExcluidos, festivos) > 0) clientesMoraSet.add(p.clienteId)
  ...
  const moraUrgenteLista = moraUrgente
    .map(p => {
      const diasExcluidos = obtenerDiasSinCobro(p.cliente, p.cliente?.ruta, org)
      const dias = calcularDiasMora(p, diasExcluidos, festivos)
  ...
  for (const p of pagos7d) {
    const fechaCO = new Date(new Date(p.fechaPago).getTime() - Math.abs(getUtcOffset('co')) * 60 * 60 * 1000)
...
export function buildSystemPrompt(ctx) {
  ...
  const mora = moraUrgente.length > 0
    ? moraUrgente.map(m => {
        let linea = `  - ${m.nombre}: ${m.diasMora} días mora, debe ${fmt(m.saldo)}`
...
- Tendencia 7 días: ${tendenciaTexto}
...
export async function buildContextoCobrador(orgId, rutaIds, userId) {
  ...
  const [org, festivos, rutas, clientes, pagosHoy] = await Promise.all([
    prisma.organization.findUnique({
      where: { id: orgId },
    ...
    prisma.cliente.findMany({
      ...
      select: {
        ...
        prestamos: {
          where: { estado: 'activo' },
          select: {
            estado: true, totalAPagar: true, montoPrestado: true, cuotaDiaria: true,
            fechaInicio: true, diasPlazo: true, frecuencia: true, proximoCobroManual: true,
            pagos: { select: { montoPagado: true, tipo: true } },
          },
        },
      },
  ...
  for (const c of clientes) {
    ...
    for (const p of c.prestamos) {
      ...
      if (dias > 0) {
        clientesMora++
        const saldo = calcularSaldoPendiente(p)
        moraUrgente.push({ nombre: c.nombre, saldo, diasMora: dias })
      }
    }
  }
