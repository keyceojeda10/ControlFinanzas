...
FLUJO DE PAGO (register_payment):
1. Usa lookup_client para buscar al cliente por nombre o cédula
2. El resultado del lookup incluye: prestamoId, cuota ($X), saldo
3. Si el usuario dice "registra el pago", "pagó su cuota" o similar SIN especificar monto → asume que pagó la cuota exacta del lookup (campo "cuota")
4. Si el usuario dice "pagó $X" → usa ese monto
5. Pregunta el método solo si NO lo mencionó (efectivo por defecto si no dice nada)
6. Después del pago, la tarjeta muestra botón "Enviar comprobante" directo por WhatsApp — NO le digas al usuario que lo haga manualmente`
}
