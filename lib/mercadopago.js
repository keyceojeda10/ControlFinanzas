...
import { PLANES_CONFIG, getPrecioPlan, getPrecioCobradorExtra, getPrecioRutaExtra } from '@/lib/planes'
...
export function buildPreapprovalBody({ plan, periodo, orgId, payerEmail, country = 'co' }) {
  const planInfo = PLANES[plan]
  if (!planInfo) return null

  const frecuencia = periodo === 'anual' ? 12 : periodo === 'trimestral' ? 3 : 1
  const precioLocal = getPrecioPlan(plan, country)
  const monto = precioLocal * frecuencia
