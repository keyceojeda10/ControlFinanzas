...
import { marcadorOffline, esFalloReintentable } from '@/lib/pagos-sin-senal'
import { estaEnSoloLectura, errorEnVista } from '@/lib/modo-vista'
...
export async function guardarPagoPendiente(pago) {
  if (estaEnSoloLectura()) throw errorEnVista()
...
/* ⚠ «PERMANENTE» YA NO ES PARA SIEMPRE. Un cobro rechazado por sesión cerrada,
   plan vencido, caja cerrada o «duplicado» se guarda con `reintentable: true`:
   vuelve a la cola solo (con espera) y desde el botón «Reintentar». Antes el
   único botón era «Descartar», y descartar es borrar plata cobrada. Ver
   `lib/pagos-sin-senal.js`. */
async function marcarPagoFallidoPermanente(id, errorMsg, { reintentable = false, status = null } = {}) {
  const db = await openDB()
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_PAGOS, 'readwrite')
    const store = tx.objectStore(STORE_PAGOS)
    ...
    req.onsuccess = () => {
      ...
      if (pago) {
        ...
        pago.reintentable = !!reintentable
        pago.status = status
        pago.ultimoIntentoAt = Date.now()
        store.put(pago)
      }
      tx.oncomplete = () => resolve()
    }
...
/** Devuelve un cobro fallido a la cola. Es lo que hace «Reintentar». */
export async function reencolarPagoFallido(id) {
  const db = await openDB()
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_PAGOS, 'readwrite')
    const store = tx.objectStore(STORE_PAGOS)
...
/** Los fallidos reintentables vuelven a la cola: todos si `forzar` (el botón
 *  «Sincronizar ahora»), o solo los que llevan más de `minutos` esperando. */
export async function reencolarFallidosReintentables({ forzar = false, minutos = 10 } = {}) {
  const fallidos = await obtenerPagosFallidos()
  const ahora = Date.now()
  let n = 0
  for (const p of fallidos) {
    if (!p.reintentable) continue
    if (!forzar && ahora - (p.ultimoIntentoAt || 0) < minutos * 60 * 1000) continue
    await reencolarPagoFallido(p.id); n++
  }
  return n
}
...
export async function invalidarCachePorPrefijo(prefijo) {
  try {
    const db = await openDB()
    return new Promise((resolve) => {
      ...
      req.onsuccess = (e) => {
        ...
        if (!cursor) return
        if (typeof cursor.key === 'string' && cursor.key.startsWith(prefijo)) {
          cursor.delete()
        }
        cursor.continue()
      }
...
export async function sincronizarPagos({ forzarFallidos = false } = {}) {
  if (estaEnSoloLectura()) return { synced: 0, failed: 0 }
  ...
  // Los fallidos por sesión, plan, caja o «duplicado» vuelven a intentarse.
  try { await reencolarFallidosReintentables({ forzar: forzarFallidos }) } catch {}
  ...
  for (const pago of pendientes) {
    // Si «Ver como» se activó A MITAD de esta subida (red lenta, 20s por
    // ítem), el resto se queda pendiente en vez de salir con la cookie del
    // cobrador y volver 403 del portero.
    if (estaEnSoloLectura()) break
    try {
      const res = await fetchConTimeout(`/api/prestamos/${pago.prestamoId}/pagos`, {
        ...
        body: JSON.stringify({
          ...
          tipo:        pago.tipo,
          nota:        pago.nota ? `${pago.nota} ${marcadorOffline(pago.createdAt)}` : marcadorOffline(pago.createdAt),
          diasAbonados: pago.diasAbonados,
          metodoPago:  pago.metodoPago,
          plataforma:  pago.plataforma,
          /* ⚠ LO QUE FALTABA, y que costaba plata en el día equivocado:
             la FECHA REAL del cobro (se registraba con la hora en que hubo
             señal), la CUENTA por la que entró (Nequi/Bancolombia se perdía
             y el cobro caía como transferencia sin dueño) y el MARCADOR con
             el que el servidor reconoce el mismo cobro si llega dos veces y
             no lo confunde con otro cobro igual de otro día. */
          fechaPago:   pago.createdAt,
          metodoPagoId: pago.metodoPagoId ?? null,
          offlineId:   pago.createdAt,
          // Coords capturadas al momento del cobro (MVP geo). null si no las hay.
      ...
      if (res.ok) {
        ...
        // Sesión, plan, caja cerrada o «duplicado» son pasajeros: se guardan
        // como reintentables. Préstamo inexistente o inactivo, no.
        await marcarPagoFallidoPermanente(pago.id, errorMsg, { reintentable: esFalloReintentable(res.status), status: res.status })
        failed++
      } else {
        // 5xx — reintento temporal
        await incrementarIntentoFallido(pago.id)
        if ((pago.intentos || 0) + 1 >= MAX_INTENTOS) {
          await marcarPagoFallidoPermanente(pago.id, `Fallaron ${MAX_INTENTOS} intentos`, { reintentable: true, status: res.status })
        }
        failed++
      }
    } catch {
...
export async function sincronizarOrdenes() {
  ...
  for (const entry of entries) {
    // Si «Ver como» se activó a mitad de esta subida, lo que falta se queda
    // pendiente en vez de salir con la cookie del cobrador.
    if (estaEnSoloLectura()) break
    try {
...
async function aplicarMutacionOptimista(tipo, entityId, payload) {
  try {
    if (tipo === 'cliente.update') {
      ...
      let patch = tipo === 'prestamo.cerrar'
        ? { estado: payload.estado || 'completado' }
        : payload

      // Si es modificacion de plazo (extender/corregir), calcular localmente
      // los campos derivados para que el optimistic update refleje la UI.
      if (tipo === 'prestamo.update' && prestamo && (payload.modo === 'extender' || payload.modo === 'corregir') && payload.fechaFin) {
        const fechaInicio = new Date(prestamo.fechaInicio)
        const nuevaFechaFin = new Date(payload.fechaFin)
        const nuevoDiasPlazo = Math.round((nuevaFechaFin - fechaInicio) / (1000 * 60 * 60 * 24))
        const diasPorPeriodo = { diario: 1, semanal: 7, quincenal: 15, mensual: 30 }[prestamo.frecuencia] || 1
        const totalPeriodos = Math.max(1, Math.ceil(nuevoDiasPlazo / diasPorPeriodo))
        patch = { diasPlazo: nuevoDiasPlazo, fechaFin: payload.fechaFin }
        if (payload.modo === 'extender') {
          const cuotaBase = (prestamo.totalAPagar || 0) / totalPeriodos
          const nuevaCuota = Math.max(50, Math.round(cuotaBase / 50) * 50)
          patch.cuotaDiaria = nuevaCuota
          patch.totalAPagar = nuevaCuota * totalPeriodos
        }
      }
      // Corregir fecha de inicio: mueve inicio y desplaza fin igual (mantiene plazo).
      if (tipo === 'prestamo.update' && prestamo && payload.modo === 'corregirInicio' && payload.fechaInicio) {
        const inicioAnterior = new Date(prestamo.fechaInicio)
        const nuevoInicio = new Date(payload.fechaInicio)
        const deltaMs = nuevoInicio.getTime() - inicioAnterior.getTime()
        const nuevaFin = new Date(new Date(prestamo.fechaFin).getTime() + deltaMs)
        patch = { fechaInicio: payload.fechaInicio, fechaFin: nuevaFin.toISOString() }
      }
      if (prestamo) {
...
export async function obtenerMutacionesFallidas() {
  ...
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_MUTACIONES, 'readonly')
    const req = tx.objectStore(STORE_MUTACIONES).getAll()
    // Separamos conflictos del resto: los conflictos tienen UI propia.
    req.onsuccess = () => resolve((req.result || []).filter(m => m.failedPermanent && !m.conflict))
    req.onerror   = () => reject(req.error)
  })
}

export async function obtenerMutacionesConflicto() {
  const db = await openDB()
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_MUTACIONES, 'readonly')
    const req = tx.objectStore(STORE_MUTACIONES).getAll()
    req.onsuccess = () => resolve((req.result || []).filter(m => m.conflict))
    req.onerror   = () => reject(req.error)
  })
}
...
export async function reintentarMutacion(id, { forzar = false } = {}) {
  const db = await openDB()
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_MUTACIONES, 'readwrite')
    const store = tx.objectStore(STORE_MUTACIONES)
    ...
    req.onsuccess = () => {
      ...
      if (m) {
        ...
        delete m.error
        if (forzar) {
          // "Usar mis cambios": descartamos baseUpdatedAt y snapshot del
          // servidor para que el reenvio pise el estado remoto sin 412.
          delete m.baseUpdatedAt
          delete m.conflict
          delete m.servidorSnapshot
        }
        store.put(m)
      }
      tx.oncomplete = () => resolve()
...
export async function sincronizarMutaciones() {
  ...
  try {
    ...
    for (const m of pendientes) {
      // Si «Ver como» se activó a mitad de esta subida, lo que falta se
      // queda pendiente en vez de salir con la cookie del cobrador — aquí
      // importa más que en pagos: una mutación 4xx queda SIN reintento
      // automático.
      if (estaEnSoloLectura()) break
      try {
        ...
        if (m.tipo === 'cliente.update') {
          ...
          url = '/api/gastos'
          method = 'POST'
        } else if (m.tipo === 'cobrador.update') {
          url = `/api/cobradores/${realId}`
        } else if (m.tipo === 'soporte.create') {
          url = '/api/soporte'
          method = 'POST'
        } else {
        ...
        if (res.ok) {
          ...
          // que el usuario comenzo a editar offline. Hacemos GET para traer
          // el estado actual y marcar con conflict:true para que la UI lo
          // muestre con opciones de resolver (mantener local / usar servidor).
          let servidorSnapshot = null
          try {
            const getRes = await fetchConTimeout(url, { method: 'GET' })
            if (getRes.ok) servidorSnapshot = await getRes.json()
          } catch {}
...
export async function sincronizarCreaciones() {
  ...
  try {
    ...
    for (const c of clientesPend) {
      // Si «Ver como» se activó a mitad de esta subida, lo que falta se
      // queda pendiente en vez de salir con la cookie del cobrador — aquí
      // importa más que en pagos: una creación 4xx queda SIN reintento
      // automático.
      if (estaEnSoloLectura()) break
      try {
    ...
    for (const p of prestamosPend) {
      // Misma guarda que en el loop de clientes, arriba.
      if (estaEnSoloLectura()) break
      try {
...
export async function sincronizarTodo(onProgress) {
  if (estaEnSoloLectura()) return { clientes: 0, prestamos: 0, rutas: 0, syncedAt: null }
  ...
  // Store grupos de cobro (para formulario de crear cliente offline)
  if (data.grupos) {
    promises.push(guardarEnCache('sync:grupos', data.grupos))
  }

  // Store cobradores (para lista offline)
  if (data.cobradores) {
    promises.push(guardarEnCache('sync:cobradores', data.cobradores))
  }
  ...
  onProgress?.({ step: 'caching', message: 'Preparando páginas...' })
  try {
    const pagesToCache = [
      '/dashboard',
      '/clientes',
...
export async function obtenerCobradoresOffline() {
  return (await leerDeCache('sync:cobradores')) || []
}

export async function obtenerGruposOffline() {
  return (await leerDeCache('sync:grupos')) || []
}
