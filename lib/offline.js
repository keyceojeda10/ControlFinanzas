...
export async function sincronizarOrdenes() {
  ...
  for (const entry of entries) {
    try {
      const res = await fetch(`/api/rutas/${entry.rutaId}/reordenar`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    if (prestamo) {
      ...
      if (tipo === 'recargo') {
        ...
        if (tipo === 'capital' && prestamo.tasaInteres > 0 && prestamo.diasPlazo > 0) {
          ...
          const ahorro = Math.round(monto * (prestamo.tasaInteres / 100) * (diasRest / 30))
