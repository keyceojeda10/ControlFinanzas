 ...
 */
export function dibujarSimulacion({
  tabla, frecuencia = 'mensual', orgNombre = '', resumen = [], cuotaTexto = '', cuotaPie = '',
  /* El mismo papel sirve para la tabla de un préstamo DE VERDAD, que es la
     misma pregunta con otro sujeto. Solo cambian el rótulo y el pie: llamarle
     «simulación» a una deuda que el cliente ya tiene sería mentirle. */
  rotulo = 'SIMULACIÓN DE CRÉDITO',
  pie = 'Simulación · no es un compromiso ni una deuda registrada',
  encabezadoCifra = 'Le cobras',
} = {}) {
  ...
  ctx.font = `13px ${SANS}`
  ctx.fillText(encabezadoCifra, L + 22, y + 30)
  ...
  if (filas.length) {
    y += 10
    const rotuloPeriodo = ROTULO_PERIODO[frecuencia] || 'Cobro'
    ...
    ctx.fillText(rotuloPeriodo.toUpperCase(), xNum, y + 12)
    ctx.textAlign = 'right'
    ...
    filas.forEach((f, i) => {
      ...
      ctx.textAlign = 'left'
      /* ⚠ LA PAGADA SE APAGA, NO SE TIÑE DE VERDE. Es la regla de la tabla en
         pantalla: en verde compite por la atención con la que toca cobrar. */
      ctx.fillStyle = f.pagada ? TINTA.ink4 : TINTA.ink
      ctx.font = `600 12px ${SANS}`
      ctx.fillText(`${rotuloPeriodo} ${f.numeroPeriodo}`, xNum, y + 5)
      ...
      ctx.textAlign = 'right'
      ctx.fillStyle = f.pagada ? TINTA.ink4 : TINTA.ink2
      ctx.font = `12px ${MONO}`
      ctx.fillText(formatMoney(Math.round(f.capital || 0)), xCapital, y + 12)
      ctx.fillStyle = f.pagada ? TINTA.ink4 : TINTA.goldInk
      ctx.fillText(formatMoney(Math.round(f.interes || 0)), xInteres, y + 12)
      ctx.fillStyle = f.pagada ? TINTA.ink3 : TINTA.ink
...
/** La tabla de un préstamo REAL. Mismo papel, otro rótulo y otro pie. */
export function compartirTablaImagen(datos) {
  let canvas
  try {
    canvas = dibujarSimulacion({
      ...datos,
      rotulo: 'TABLA DEL PRÉSTAMO',
      encabezadoCifra: 'Cada cobro',
      /* ⚠ AQUÍ NO SE DICE «SIMULACIÓN». Esta tabla es de una deuda que el
         cliente YA tiene; el pie recuerda que las pagadas van apagadas, que es
         lo que va a preguntar al recibirla. */
      pie: 'Las cuotas en gris ya están pagadas',
    })
  } catch { return false }
  return compartirLienzo(canvas, {
    nombre: 'Tabla-del-prestamo.png',
    titulo: 'Tabla del préstamo',
    texto: datos?.orgNombre ? `Tabla del préstamo - ${datos.orgNombre}` : 'Tabla del préstamo',
  })
}
