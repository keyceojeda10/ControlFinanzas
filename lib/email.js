...
import { PLAN_NAMES } from './planes.js'

/* ⚠ EL CLIENTE SE CREA AL ENVIAR, NO AL CARGAR EL MÓDULO.
   `new Resend(undefined)` lanza «Missing API key», y al estar arriba del todo
   tumbaba CUALQUIER ruta que importara este fichero —doce—, no solo el correo.
   Lo destapó el espejo el 21 sep 2026, que corre sin clave a propósito para no
   escribirle a nadie: asignar un plan desde el superadmin daba 500 antes de
   hacer nada. Sin clave, ahora no se manda ese correo y la acción sigue. */
let clienteResend = null
function resend() {
  if (!process.env.RESEND_API_KEY) return null
  return (clienteResend ??= new Resend(process.env.RESEND_API_KEY))
}
...
/* ⚠ AQUÍ ESTABA `YT`, la lista de los ocho videos de YouTube que iban en los
   correos. Se fue entera: los ocho son de marzo y enseñan la interfaz de antes
   del rediseño de julio. No queda ni la lista de reproducción — mandar a una
   lista de videos viejos es lo mismo, repartido.

   Si algún día se regraban, esto vuelve; pero entonces con la fecha al lado,
   que es lo que faltó para darse cuenta a tiempo. */

// ─── Utilidad para enviar emails ────────────────────────────────
export async function enviarEmail({ to, subject, html }) {
  const cliente = resend()
  if (!cliente) {
    console.warn('[Email] Sin RESEND_API_KEY: no se envía a', to)
...
// ─── Helpers de UI ──────────────────────────────────────────────
import { formatMoney as formatMoneyI18n } from './i18n.js'
const formatCOP = (n, country = 'co') => formatMoneyI18n(n, country)

const formatPlanNombre = (plan) => {
  if (!plan) return 'Plan'
  const raw = String(plan).trim()
  const key = raw.toLowerCase()
  return PLAN_NAMES[key] ?? raw
}
...
/* ── ⚠ PASOS ESCRITOS EN VEZ DEL VIDEO DEL PAGO ────────────────────────────
 *
 * Los tres correos de cobro —pago fallido, aviso de vencimiento y renovación—
 * llevaban el mismo video, «Cómo elegir y pagar tu plan». Ese video es de marzo
 * y el flujo de pago CAMBIÓ en julio, cuando entró la pasarela integrada: antes
 * había que salir de la app.
 *
 * O sea que en el peor sitio posible —el correo que pide que paguen, con la
 * persona a punto de hacerlo— le enseñábamos un camino que ya no existe. El bot
 * tiene desde hace tiempo la regla de no mandar estos videos a los interesados;
 * a los que pagan se los seguíamos mandando.
 *
 * Tres renglones con el flujo de verdad pesan menos, no envejecen con un
 * rediseño de pantalla y se leen sin salir del correo.
 */
const pasos = (titulo, lista) => `
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#fafafa;border:1px solid #f4f4f5;border-radius:12px;margin:24px 0 8px;">
  <tr><td style="padding:20px;">
    <p style="margin:0 0 12px;color:#71717a;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">${titulo}</p>
    ${lista.map((t, i) => `
      <p style="margin:0 0 8px;color:#3f3f46;font-size:14px;line-height:1.5;">
        <strong style="color:#0a0a0a;">${i + 1}.</strong> ${t}
      </p>
    `).join('')}
  </td></tr>
</table>`

/** Los pasos reales del pago, en un solo sitio: los tres correos los repiten. */
const PASOS_PAGO = [
  'Entra a <strong>Configuración → Mi plan</strong> (o toca el botón de arriba).',
  'Toca <strong>Renovar mi plan</strong>, o elige otro plan y toca <strong>Pagar</strong>.',
  'Paga con <strong>PSE, tarjeta o Nequi</strong> sin salir de la app. Tu plan queda activo enseguida.',
]
...
/* ── ⚠ AQUÍ VIVÍA `videoCard`, Y SE FUE ────────────────────────────────────
 *
 * Enseñaba la miniatura de un video de YouTube y llevaba a él. Los cinco que
 * usaba —referidos, primeros pasos, crear ruta, registrar cliente, registrar
 * cobro— son de marzo y muestran la interfaz de antes del rediseño de julio.
 *
 * O sea que los correos de bienvenida, que son los que tratan de que la
 * persona ARRANQUE, le enseñaban una app distinta de la que iba a abrir. Y en
 * los tres correos de cobro pasaba lo mismo con el video del pago, que además
 * es el flujo que más cambió (pasarela integrada).
 *
 * Lo reemplaza una captura de verdad, sacada del mismo juego que los tutoriales
 * y rehecha por `scripts/capturar-tutoriales.mjs` en cada rediseño. Ya no
 * envejece sola.
 *
 * ⚠ La imagen va por URL ABSOLUTA: en un correo no hay página que resuelva las
 * rutas relativas. Y sale de `public/tutoriales/`, que se sirve sin sesión —
 * aquí eso es lo que se quiere, y las capturas son de un negocio de mentira.
 */
const imagenCard = ({ imagen, titulo: tit, descripcion, url }) => `
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:24px 0;background:#fafafa;border:1px solid #f4f4f5;border-radius:14px;overflow:hidden;">
  <tr><td>
    <a href="${url || APP_URL}" style="display:block;text-decoration:none;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr><td style="background:#f4f4f5;text-align:center;padding:0;">
          <img src="${APP_URL}/tutoriales/${imagen}" alt="${tit}" width="516" style="display:block;width:100%;height:auto;border:0;" />
        </td></tr>
        <tr><td style="padding:20px;">
          <p style="margin:0 0 6px;color:#a16207;font-size:11px:font-weight:700;text-transform:uppercase;letter-spacing:0.08em;">Así se ve</p>
...
// 0. Verificación de email (codigo OTP de 6 digitos)
export function emailVerificacion({ nombre, codigo }) {
  return {
    subject: `${codigo} — Tu código de verificación · Control Finanzas`,
    html: layout(`
      ${titulo(`Hola ${nombre}, verifica tu correo`)}
      ${parrafo('Ingresa este código en la app para verificar tu cuenta:')}
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:24px 0;">
        <tr><td align="center">
          <div style="display:inline-block;background:#fafafa;border:2px solid #f4f4f5;border-radius:16px;padding:24px 40px;letter-spacing:12px;font-size:36px;font-weight:800;color:#0a0a0a;font-family:'SF Mono',SFMono-Regular,Consolas,'Liberation Mono',Menlo,monospace;">${codigo}</div>
        </td></tr>
      </table>
      ${parrafo('El codigo expira en <strong>30 minutos</strong>. Si no lo solicitaste, ignora este mensaje.')}
    `),
  }
}
...
export function emailPagoAprobado({ nombre, plan, monto, fechaVencimiento }) {
  const fechaFmt = new Date(fechaVencimiento).toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })
  const planNombre = formatPlanNombre(plan)
  return {
    ...
    html: layout(`
      ...
      ${card(dataTable(
        dataRow('Plan', planNombre, { bold: true }) +
...
export function emailPagoFallido({ nombre, plan, monto }) {
  ...
  return {
    ...
    html: layout(`
      ...
      ${pasos('Cómo activar tu plan', PASOS_PAGO)}
    `),
  }
}
...
export function emailAvisoVencimiento({ nombre, plan, diasRestantes, fechaVencimiento }) {
  ...
  return {
    ...
    html: layout(`
      ...
      ${parrafo(`Hola ${nombre}, tu plan <strong>${planNombre}</strong> vence el <strong>${fechaFmt}</strong>. Renueva ahora para no perder acceso a tu cartera.`)}
      ...
      ${pasos('Cómo renovar, en tres pasos', PASOS_PAGO)}
    `),
  }
}
...
export function emailSuscripcionVencida({ nombre, plan }) {
  ...
  return {
    ...
    html: layout(`
      ...
      ${parrafo(`Hola ${nombre}, tu plan <strong>${planNombre}</strong> expiró y tu acceso a la plataforma está suspendido. Tus datos están seguros — renueva para recuperar el acceso completo.`)}
...
export function emailReferidoExitoso({ nombre, nombreReferido, userId }) {
  return {
    ...
    html: layout(`
      ...
      ${imagenCard({
        imagen: '16_configuracion.png',
        titulo: 'Tu enlace está en Configuración',
        descripcion: 'Entra a Configuración y busca Referidos. Copia el enlace y mándalo por WhatsApp.',
...
export function emailOnboardingDia1({ nombre, userId }) {
  return {
    ...
    html: layout(`
      ...
      ${beneficio({
        badge: '3',
        titular: 'Registra cada cobro desde el celular',
        texto: 'En la calle, sin internet, con un toque. El sistema guarda la ubicación GPS del cobro automáticamente, y tu cliente recibe el comprobante por WhatsApp al instante.',
      })}
      ...
      ${imagenCard({
        imagen: '02_dashboard.png',
        titulo: 'Tu panel, apenas entres',
        descripcion: 'Arriba, lo que toca cobrar hoy y lo que llevas. Abajo, lo que necesita tu atención.',
  ...
  return {
    ...
    html: layout(
      ...
      ${beneficio({
        ...
        texto: 'Agrupa tus clientes por zona o cobrador. El día del cobro, cada cobrador entra a "Mis cobros de hoy" y ve exactamente quién le toca — sin coordinar por WhatsApp ni llamadas.',
      })}

      ${beneficio({
        badge: '→',
        titular: 'Asigna un cobrador',
        texto: 'Tu cobrador entra desde su celular, cobra con un toque y el GPS registra dónde fue cada cobro. Tú ves el reporte en tiempo real, sin excusas ni papel.',
      })}
      ...
      ${imagenCard({
        imagen: '09_rutas_lista.png',
        titulo: 'Así se ven tus rutas',
        descripcion: 'Cada ruta con su cobrador, lo recogido y lo que falta. El botón + de arriba crea una nueva.',
        url: APP_URL + '/rutas',
      })}
    ...
    `
      ...
      ${imagenCard({
        imagen: '04_cliente_nuevo.png',
        titulo: 'Solo el nombre es obligatorio',
        descripcion: 'La cédula la puedes dejar vacía y ponerla después. Un cliente son 30 segundos.',
        url: APP_URL + '/clientes/nuevo',
      })}
...
export function emailOnboardingDia7({ nombre, userId }) {
  return {
    ...
    html: layout(`
      ...
      ${beneficio({
        badge: '✦',
        titular: 'Lucas IA: tu asistente de cartera',
        texto: 'Pregúntale a Lucas cuánto lleva pagado un cliente, qué préstamos están en mora o cuánto recaudaste esta semana — en lenguaje natural, sin buscar en la app. Lo encuentras en el menú como "Lucas IA".',
      })}
      ...
      ${imagenCard({
        imagen: '19_cobros_hoy.png',
        titulo: 'Cobrar es un toque por cliente',
        descripcion: 'Todos los cobros del día en una lista, cada uno con su botón. Y el comprobante sale por WhatsApp.',
        url: APP_URL + '/cobros-hoy',
      })}
...
export function emailOnboardingDia12({ nombre, userId }) {
  return {
    ...
    html: layout(`
      ...
      ${card(dataTable(
        dataRow('Inicial', '$39.000/mes', { bold: true }) +
  ...
  return {
    ...
    html: layout(`
      ...
      ${beneficio({
        ...
        texto: 'Tu cartera, tus clientes, tus rutas y todo el historial se mantienen. Pagas desde $39.000/mes, cancelas cuando quieras.',
      })}
...
export function emailRecuperacionDia30({ nombre, clientesCreados, prestamosCreados, userId }) {
  ...
  return {
    ...
    html: layout(`
      ...
      ${parrafo(`Si activas cualquier plan antes de que eliminemos tu cuenta, <strong>todo se conserva intacto</strong> y puedes retomar donde te quedaste. Desde $39.000/mes, cancelas cuando quieras.`)}
