...
/* ⚠ Y AL REVÉS, CUANDO EL ARCHIVO DA LOS DÍAS Y NO LAS CUOTAS.
 *
 * Un export de septiembre traía «Dias credito» (30, 15, 21…) y ninguna columna
 * de cuotas. Sin las cuotas no se puede convertir la tasa plana a mensual, así
 * que las 124 filas se descartaban con «no pude convertir la tasa». Los días
 * partidos por lo que dura un periodo SON las cuotas: 30 días cobrando cada
 * quince son dos. */
export function cuotasDesdeDias(frecuencia, dias) {
  const d = DIAS_POR_PERIODO[frecuencia]
  const n = Number(dias)
  if (!d || !Number.isFinite(n) || n <= 0) return null
...
export function aCargaMasiva(filas = [], { semilla = '', hoy = null } = {}) {
  ...
  filas.forEach((f, i) => {
    /* El plazo que trae el archivo manda; si no lo trae, se deduce de la
       frecuencia y el número de cuotas, que es el caso del export de julio. */
    const diasPlazo = f.diasPlazo > 0 ? Math.round(f.diasPlazo) : diasDePlazo(f.frecuencia, f.nCuotas)
    ...
    /* Sin frecuencia no hay nada que hacer, y el motivo tiene que decirlo con
       esas palabras: antes caía en «no pude convertir la tasa», que no le dice
       a nadie qué le falta a su archivo. */
    if (!f.frecuencia) {
      descartadas.push({ nombre: f.nombre, motivo: 'no dice cada cuánto se cobra' })
      return
    }
    ...
    const cuotas = f.nCuotas > 0 ? f.nCuotas : cuotasDesdeDias(f.frecuencia, diasPlazo)
    const tasa = tasaMensualEquivalente(f.interes ?? 0, f.frecuencia, cuotas)
    if (tasa == null) {
