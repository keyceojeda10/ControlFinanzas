...
export const PLANES_CONFIG = {
  ...
  // aprieta todavía.
  //
  // Nivel 1: el resumen, los ingresos, el reporte del día y —desde ago 2026—
  // el EXCEL EN CRUDO de toda la cuenta. Ese último estaba en nivel 3
  // (Empresarial) y lo alcanzaban cinco negocios de 457; bajarlo es lo que le
  // da a Básico algo que Inicial no tiene y que se pide de verdad.
  //
  // Lo de nivel 2 (cartera por ruta, cobradores, seguros) sigue siendo de
  // Profesional en adelante — si Básico se llevara todo, no quedaría motivo
  // para subir de ahí.
...
/* ══ LOS CUPOS: PLAN + REGALADOS + ADICIONALES ══════════════════════════════
 *
 * El cupo de usuarios y rutas se calculaba en SIETE sitios con la misma suma
 * escrita a mano (`maxUsuarios + cobradoresExtra`). Al añadir los adicionales
 * pagados, un sitio que se quedara sin sumarlos le cobraría al negocio un
 * cobrador que la pantalla de «Nuevo cobrador» no le deja crear. Por eso la
 * suma vive aquí y los siete la importan.
 *
 * · `cobradoresExtra` / `rutasExtra` / `clientesExtra`: los REGALA el panel.
 * · `cobradoresAdicionales` / `rutasAdicionales`: los PAGA el negocio con su
 *   plan. Solo cuentan en un plan que los admite (Crecimiento en adelante): es
 *   la misma regla que decide si se cobran (`precioAdicionales`), así que nunca
 *   se cobra uno que no se pueda usar ni se usa uno que no se cobre. */

/** ¿El plan admite cobradores y rutas adicionales pagados? */
export function admiteAdicionales(plan) {
  return (PLANES_CONFIG[plan]?.cobradorExtra ?? 0) > 0
}

/** Los campos de `Organization` que necesita `cuposDe`. */
export const selectCupos = {
  plan: true,
  cobradoresExtra: true,
  rutasExtra: true,
  clientesExtra: true,
...
/**
 * El cupo de un negocio. `plan` es el que manda los límites (el de la
 * suscripción o `org.plan`); si no se da, `org.plan`.
 *
 * @returns {{ usuarios: number, rutas: number, clientes: number }}
 */
export function cuposDe(org, plan = org?.plan) {
  const config = PLANES_CONFIG[plan] || PLANES_CONFIG.starter
  const pagados = admiteAdicionales(plan)
  return {
    usuarios: config.maxUsuarios + (org?.cobradoresExtra ?? 0) + (pagados ? (org?.cobradoresAdicionales ?? 0) : 0),
    rutas:    config.maxRutas    + (org?.rutasExtra ?? 0)      + (pagados ? (org?.rutasAdicionales ?? 0) : 0),
    clientes: config.maxClientes + (org?.clientesExtra ?? 0),
  }
}

/**
 * Lo que va en cada escritura que CAMBIA el plan de un negocio: pasar a uno
 * que no admite adicionales los pone en cero. Si no, quedaban en la columna y
 * volver a subir de plan meses después los traía al cobro sin que nadie los
 * pidiera (revisión del 24 sep 2026). Se extiende en el `data` del update.
 */
export function adicionalesAlCambiarA(plan) {
  return admiteAdicionales(plan) ? {} : { cobradoresAdicionales: 0, rutasAdicionales: 0 }
}
