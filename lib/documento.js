...
// El país se omite a propósito en las llamadas: `lib/i18n` guarda el de la
// organización al arrancar el panel y `getCountryConfig` lo recibe por el mismo
// camino que `formatMoney`. Así estas doce fichas no tienen que cablear un hook
// cada una — varias ni siquiera son componentes de React.
import { getCountryConfig } from '@/lib/countries'
import { paisDeLaApp } from '@/lib/i18n'

/**
 * La abreviatura del documento, la que va delante del número en las fichas:
 * «CC 1.034.887», «DNI 12345678», «RUT 12345678-9».
 */
export function abreviaturaDocumento(country = paisDeLaApp()) {
  return getCountryConfig(country).documentAbbr
}

/** El nombre largo, para rótulos de formulario: «Cédula», «DNI», «RUT». */
export function nombreDocumento(country = paisDeLaApp()) {
