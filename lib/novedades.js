...
export const NOVEDADES_VERSION = 21
...
export const NOVEDADES = [
    ...
    items: [
      {
        ...
        texto: 'Ahora puedes ver en tiempo real donde esta tu cobrador mientras hace la ruta. Entra a cualquier ruta con cobrador asignado y el mapa se abre automaticamente mostrando su ubicacion con un punto dorado pulsante. Se actualiza cada 20 segundos.',
      },
      {
        icon: 'route',
        titulo: 'Recorrido del dia',
        texto: 'Ademas de la ubicacion en vivo, el mapa muestra el recorrido completo que ha hecho el cobrador durante el dia como una linea dorada. Asi puedes verificar que paso por todos los puntos de la ruta.',
      },
      {
        icon: 'document',
        titulo: 'Resumen mensual en PDF',
        texto: 'Descarga un informe completo de tu negocio en PDF desde la seccion de Reportes. Incluye recaudo, capital, mora, prestamos activos y detalle por cobrador. Ideal para tus registros o para compartir con socios.',
      },
    ],
  },
  {
    ...
    items: [
      {
        icon: 'calculator',
        titulo: 'Cuotas extra en prestamos globo',
        texto: 'Ahora puedes programar abonos a capital en cuotas especificas de un prestamo globo (por ejemplo, en los meses donde el cliente recibe primas, bonos o ingresos extra). Toca cualquier cuota intermedia en la tabla al crear el prestamo para agregar el monto. El interes de las cuotas siguientes baja automaticamente y el capital final se reduce.',
      },
      {
        icon: 'sparkles',
        titulo: 'Modulo de socios completo',
        texto: 'Registra socios que aportan capital a tu negocio. Cada aporte y retiro mueve la caja de capital automaticamente. Puedes ver el ROI, la liquidacion por ano y vincular prestamos a un socio especifico desde el detalle del prestamo.',
      },
      {
        icon: 'pen',
        titulo: 'Indicador de prestamo perdido',
        texto: 'Los prestamos marcados como "clavo" ahora muestran una etiqueta "Perdido" en la lista de clientes, en la ruta y en cobros del dia, para identificarlos rapidamente sin entrar al detalle.',
      },
      {
        icon: 'calculator',
        titulo: 'Correccion de capital adeudado',
        texto: 'El capital adeudado ahora se calcula correctamente en todos los casos, incluyendo prestamos con acuerdos de pago o tablas de amortizacion. Tambien se corrigio un error de redondeo que podia mostrar $2 de mas en el capital.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        texto: 'Ahora puedes configurar un interes por mora para los clientes que llevan varios dias sin pagar. Ve a Configuracion → Mi organizacion → "Intereses moratorios" y define la tasa mensual y los dias de gracia. El sistema calcula el monto automaticamente, pero tu decides si aplicarlo o no desde el detalle de cada prestamo.',
      },
      {
        icon: 'sparkles',
        titulo: 'Cuotas correctas por tipo de prestamo',
        texto: 'Los cobros de ruta y cobros del dia ahora muestran la cuota real de cada periodo para prestamos con tabla de amortizacion (lineal y globo), en lugar de la cuota fija. Tambien se muestra el numero de cuota y una alerta cuando toca la cuota globo (capital + interes).',
      },
      {
        icon: 'pen',
        titulo: 'Tabla de amortizacion interactiva',
        texto: 'La tabla de amortizacion ahora resalta la proxima cuota a pagar, muestra cuotas vencidas en rojo y permite registrar el pago directamente desde cada fila. Cada pago registrado queda vinculado al numero de cuota que cubre.',
      },
    ],
  },
  {
    ...
    items: [
      {
        icon: 'pen',
        titulo: 'Cedula opcional al crear clientes',
        texto: 'Ahora puedes crear clientes sin numero de cedula. Marca la casilla "No tengo la cedula" al crear un cliente o en el migrador express. Util cuando estas migrando tu cartera y no tienes el documento a la mano. Puedes agregarlo despues editando el cliente.',
      },
      {
        icon: 'calculator',
        titulo: 'Nuevo modo: Solo interes (globo)',
        texto: 'Nuevo modo de interes para prestamos donde el cliente paga solo el interes cada periodo y devuelve el capital completo al final en una sola cuota. Ideal para clientes que manejan flujo de caja propio. Tambien permite abonos a capital en cualquier momento, lo que baja el interes de las cuotas futuras automaticamente.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        texto: 'Ahora puedes editar monto, tasa, frecuencia, plazo y modo de interes directamente desde el resumen del prestamo, sin tener que ir atras en el wizard. Toca cualquier campo para cambiarlo y la cuota se recalcula al instante.',
      },
      {
        icon: 'sparkles',
        titulo: 'Migrador express mas completo',
        texto: 'El migrador express ahora incluye el selector de dias sin cobro (dias de la semana en que no se cobra al cliente). Ademas, el resumen del migrador tambien es editable.',
      },
      {
        icon: 'calculator',
        titulo: 'Ver todos los clientes en ruta',
        texto: 'En "Trabajo del dia" ahora hay un boton "Ver todos" que muestra todos los clientes en una lista plana, sin agrupar por estado. Asi nadie queda escondido en secciones colapsadas.',
      },
      {
        icon: 'camera',
        titulo: 'Foto del cliente con camara',
        texto: 'En Android, al subir la foto del cliente ahora aparecen ambas opciones: tomar foto con la camara o elegir desde la galeria.',
      },
      {
        icon: 'upload',
  ...
  },
  {
    ...
    items: [
      ...
      {
        icon: 'pen',
        titulo: 'Cambiar fecha de proximo cobro',
        texto: 'Los administradores ahora pueden cambiar manualmente la fecha del proximo cobro de un prestamo desde el boton "Proximo cobro" en gestion del prestamo. Al registrar un pago, la fecha vuelve a calcularse automaticamente.',
      },
      {
        icon: 'calculator',
        titulo: 'Ruta sin perder la posicion',
        texto: 'Al entrar a un cliente desde la ruta y volver, la pantalla se mantiene en la misma posicion donde estabas. Ya no se sale de la ruta ni se pierde el orden.',
      },
    ],
  },
  {
