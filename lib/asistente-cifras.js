...
import { CAMPOS_DEL_REPARTO, capitalDelPrestamo } from '@/lib/dinero/capital-base'
...
export function cifrasDeLaCartera({ prestamos = [], org = null, festivos = [], pagosDeHoy = [], topeMora = TOPE_MORA }) {
  ...
  for (const p of prestamos) {
    ...
    // Contra el capital de verdad: `yaCobrado` ya cuenta el interés arrastrado de una renovación.
    const interesPactado = Math.max(0, (p.totalAPagar ?? 0) - capitalDelPrestamo(p))
