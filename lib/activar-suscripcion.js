...
/* ⚠ PAGAR UN PLAN SIN ADICIONALES LOS PONE EN CERO. Bajar de Crecimiento con
   dos cobradores adicionales a Básico los deja sin contar ni cobrar, pero en la
   columna. Sin esto, volver a Crecimiento meses después los traía de vuelta al
   cobro sin que nadie los pidiera. */
const sinAdicionalesSi = (plan) => (admiteAdicionales(plan) ? {} : { cobradoresAdicionales: 0, rutasAdicionales: 0 })
    ...
    subFinal = await prisma.$transaction(async (tx) => {
      ...
      if (sub) {
        ...
        await tx.organization.update({
          ...
          data: { plan, activo: true, waChurnSent: false, waPreVencSent: false, ...COBRO_AL_DIA, ...sinAdicionalesSi(plan) },
        })
        return actualizada
      }
