...
export async function dispararSiCruzoUmbral({ organizationId, antes, despues }) {
  ...
  try {
    ...
    for (const umbral of cruzados) {
      /* ⚠ UNA VEZ POR NEGOCIO. `antes = despues - creados` vuelve a cruzar el
         umbral cuando se borra un cliente y se crea otro: DyA mandó dos
         `Activacion21` con 21 clientes, a las 18:49 y a las 19:04. Meta los
         cuenta los dos. La marca vive en PushLog (tipo 'capi'): es por negocio,
         no la lee nadie más (celebraciones filtra por su tipo) y no hace falta
         columna nueva. */
      const evento = EVENTO_POR_UMBRAL[umbral]
      const yaEnviado = await prisma.pushLog.findFirst({
        where: { organizationId, tipo: 'capi', clave: evento },
        select: { id: true },
      })
      if (yaEnviado) continue
      await prisma.pushLog.create({
        data: { organizationId, tipo: 'capi', clave: evento, titulo: evento, cuerpo: `clientes=${despues}` },
      })
