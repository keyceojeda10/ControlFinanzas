...
// Una letra NO identifica un mes: hay dos M, dos J y dos A. La barra se rotula
// con tres letras y las frases dicen el nombre entero.
...
export function mesesDe(hasta) {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    salida.push({
      ...
      nombre: NOMBRES[d.getUTCMonth()],
      corta: CORTOS[d.getUTCMonth()],
      desde: d,
...
export function diasCobrablesEnMes({ desde, hasta }, prestamo, diasSinCobro = []) {
  ...
  /* ⚠ LOS DÍAS SIN COBRO LLEGAN COMO NÚMEROS, Y SE TIRABAN TODOS.
     `obtenerDiasSinCobro` devuelve `[0]` (domingo) y esto solo entendía NOMBRES
     (`'domingo'`): `DIA_A_INDICE['0']` es `undefined`, así que el filtro los
     descartaba en silencio y a un préstamo diario se le pedían los SIETE días.
     En un negocio que no cobra domingos, quien paga todas sus cuotas salía
     «cumplió 86 %» —barra dorada, «pagó parte»— sin haber fallado ni una.
     Se aceptan las dos formas. */
  const excluidos = new Set(
    (diasSinCobro || [])
      .map((d) => (Number.isInteger(d) && d >= 0 && d <= 6 ? d
        : /^[0-6]$/.test(String(d)) ? Number(d)
        : DIA_A_INDICE[String(d).toLowerCase()]))
...
export function lecturaDe(meses = [], nombresMes = INICIALES) {
  ...
  /* ⚠ DECÍA «DESDE ABRIL» CUANDO ERA AGOSTO. El nombre salía de un mapa por la
     INICIAL, y tres letras se repiten: M (marzo/mayo), J (junio/julio) y A
     (abril/agosto). Mayo, julio y agosto ni estaban. Ahora cada mes trae su
     nombre; el mapa se queda solo para datos viejos sin él. */
  const nombre = desde
    ? (desde.nombre ?? { E: 'enero', F: 'febrero', S: 'septiembre', O: 'octubre', N: 'noviembre', D: 'diciembre' }[desde.etiqueta] ?? null)
    : null
...
/**
 * LO QUE SE DICE CON PALABRAS Y CON PLATA, encima de las barras.
 *
 * El dueño, 20 sep 2026: «ese "cómo paga" no se entiende una mierda… sería súper
 * útil si se entendiera el gráfico y hubiese algo más textual». Doce barras sin
 * escala ni leyenda y una frase suelta («Viene fallando.») no dicen de qué se
 * habla. Esto saca lo que se puede afirmar con los mismos datos:
 *
 *   · un VEREDICTO de una palabra, con su tono;
 *   · la cuenta de los últimos tres meses con dato, en plata: pagó X de Y;
 *   · cuántos meses cerró completos de los que tuvo préstamo.
 *
 * Misma advertencia que arriba: es una aproximación para leer la TENDENCIA.
 */
export function resumenDe(meses = []) {
  const conDato = meses.filter((m) => m.cumplio !== null && m.cumplio !== undefined)
  if (conDato.length === 0) return { veredicto: 'sin', titulo: 'Todavía no hay historial', conDato: 0 }

  const ultimos = conDato.slice(-3)
  const pagado = ultimos.reduce((n, m) => n + (m.pagado || 0), 0)
  const esperado = ultimos.reduce((n, m) => n + (m.esperado || 0), 0)
  const pct = esperado > 0 ? Math.min(100, Math.round((pagado / esperado) * 100)) : null
  const completos = conDato.filter((m) => m.cumplio >= 90).length

  const veredicto = pct === null ? 'sin' : pct >= 90 ? 'bien' : pct >= 60 ? 'regular' : 'mal'
  const titulo = { bien: 'Paga bien', regular: 'Paga, pero no completo', mal: 'Viene fallando', sin: 'Todavía no hay historial' }[veredicto]

  /* LA TENDENCIA, Y SOLO LA TENDENCIA. La tarjeta enseñaba el título de aquí y,
     debajo, la frase de `lecturaDe`: dos reglas distintas que se contradecían en
     la misma tarjeta —«Paga bien · 100 %» y «rara vez cierra el mes completo»—.
     El veredicto lo da UNA regla (la plata de los últimos tres meses); esto solo
     añade si antes pagaba distinto, que es lo que la plata sola no dice. */
  const previos = conDato.slice(0, -3)
  const media = (xs) => xs.reduce((n, m) => n + m.cumplio, 0) / xs.length
  let tendencia = null
  if (previos.length > 0) {
    const salto = media(ultimos) - media(previos)
    // Desde CUÁNDO: el primer mes de la mala racha que llega hasta hoy, no el
    // primero de los últimos tres. Con junio al 39 % y julio al 4 % decía «bajó
    // desde julio», y la barra de junio estaba ahí al lado, en rojo.
    let k = conDato.length - 1
    while (k > 0 && conDato[k].cumplio < 90 && conDato[k - 1].cumplio < 90) k--
    const torcido = conDato[k].cumplio < 90 ? conDato[k] : ultimos.find((m) => m.cumplio < 90)
    if (salto <= -20) tendencia = torcido?.nombre ? `Antes pagaba mejor: bajó desde ${torcido.nombre}.` : 'Antes pagaba mejor.'
    else if (salto >= 20) tendencia = 'Antes pagaba peor: últimamente ha mejorado.'
  }

  return {
    veredicto, titulo, pct, tendencia, pagado: Math.round(pagado), esperado: Math.round(esperado),
    cuantos: ultimos.length,                 // de cuántos meses es esa cuenta (1 a 3)
    desde: ultimos[0]?.nombre ?? null, hasta: ultimos[ultimos.length - 1]?.nombre ?? null,
    completos, conDato: conDato.length,
  }
}
