...
export async function sendConversionEvent({ eventName, email, phone, leadId, eventSourceUrl, customData }) {
  ...
  if (leadId) {
    // Vincula el evento al lead exacto del formulario de Meta (Lead Ads),
    // requerido por la "Conversions API for CRM" para alimentar la calidad
    // del lead en el algoritmo de optimizacion.
    eventData.custom_data = { lead_event_source: 'Control Finanzas CRM', lead_id: leadId, ...customData }
  } else if (customData) {
